...
- `-ih`, `--imghost HOST`: Select image host.
  - Choices: `imgbb`, `imgbox`, `pixhost`, `lensdump`, `ptscreens`, `onlyimage`, `dalexni`, `zipline`, `midnightscene`, `passtheimage`, `seedpool_cdn`, `sharex`, `utppm`, `lostimg`. See [image-hosts.md](image-hosts.md) for API/authentication details and provider status notes.
