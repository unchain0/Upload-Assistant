...
def _tracker_contract_groups() -> list[tuple[str, ...]]:
    ...
    effect_nodes = [f"{TRACKER_EFFECT_PREFIX}[{tracker}]" for tracker in trackers]
    private_nodes = [f"{TRACKER_PRIVATE_PREFIX}[{tracker}]" for tracker in trackers]
    return [(TRACKER_DETERMINISTIC_NODE,), *_chunks(effect_nodes, TRACKER_CONTRACT_SHARD_SIZE), *_chunks(private_nodes, TRACKER_CONTRACT_SHARD_SIZE)]
...
def _regular_groups(files: list[Path], excluded: set[Path]) -> list[tuple[str, ...]]:
...
def _contract_groups(files: list[Path], tracker_contract_path: Path) -> list[tuple[str, ...]]:
    contract_files = [path for path in files if "contract" in path.stem and path != tracker_contract_path]
...
def discover_shards() -> list[TestShard]:
    ...
    excluded = {tracker_contract_path, *(path for path in files if "contract" in path.stem)}
    ...
    return [TestShard(index=index, targets=group) for index, group in enumerate(groups)]
...
def run_shard(shard: TestShard) -> None:
    ...
    print(f"\n== coverage shard {shard.index:03d}: {', '.join(shard.targets)} ==", flush=True)
    command = [
        "uv",
        "run",
        "coverage",
    ...
    completed = subprocess.run(command, cwd=ROOT, env=os.environ.copy(), check=False)  # noqa: S603 -- deterministic internal command
...
def combine_and_report(*, fail_under: float) -> float:
    ...
    if len(parts) != expected:
        raise RuntimeError(f"Expected {expected} coverage shards, found {len(parts)}")
    coverage = Coverage(data_file=str(COMBINED_DATA), config_file=str(ROOT / "pyproject.toml"))
    ...
    if round(total, 2) < fail_under:
        raise SystemExit(f"Combined line coverage {total:.2f}% is below the required {fail_under:.2f}%")
    return total


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--list", action="store_true", help="List deterministic shards and exit")
    parser.add_argument("--shard", type=int, help="Run only one shard index")
    parser.add_argument("--combine-only", action="store_true", help="Combine existing shard data without running tests")
    parser.add_argument("--prepare", action="store_true", help="Reset coverage parts before a sharded local run")
    parser.add_argument("--fail-under", type=float, default=100.0, help="Required combined line coverage percentage")
...
def _run_requested_shard(index: int, shards: list[TestShard]) -> int:
    if index < 0 or index >= len(shards):
        print(f"Unknown shard {index}; valid range is 0..{len(shards) - 1}", file=sys.stderr)
        return 2
