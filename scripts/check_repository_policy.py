...
def _runtime_dependency_violations(
    ...
    return [
        PolicyViolation(
            "UV-02",
            "pyproject.toml",
            "project.dependencies must contain runtime dependencies",
        )
    ]
...
def _failed_checks(
    ...
    return [
        PolicyViolation(rule, "pyproject.toml", detail)
        for valid, rule, detail in checks
        if not valid
    ]


def _coverage_policy_violations(
    data: dict[str, object],
) -> list[PolicyViolation]:
    ...
    checks = (
        (
            float(report_dict.get("fail_under", 0)) == 100.0,
            "COV-01",
            "tool.coverage.report.fail_under must equal 100",
        ),
        (
            run_dict.get("source") == ["src"],
            "COV-02",
            "coverage source must be exactly ['src']",
        ),
        (
            not run_dict.get("omit"),
            "COV-03",
            "coverage omit rules are forbidden",
        ),
    )
...
def _has_pinned_radon(data: dict[str, object]) -> bool:
    return any(
        str(item).startswith("radon==") for item in _dev_dependencies(data)
    )
...
def _complexity_policy_violations(
    data: dict[str, object],
) -> list[PolicyViolation]:
    checks = (
        (
            _has_pinned_radon(data),
            "CC-01",
            "Radon must be pinned in dependency-groups.dev",
        ),
        (
            _radon_no_assert(data),
            "CC-06",
            "tool.radon.no_assert must be true for semantic block complexity",
        ),
        (
            _quality_max_is_a(data),
            "CC-02",
            "maximum cyclomatic complexity must be 5 (Radon rank A)",
        ),
        (
            _has_complexity_paths(data),
            "CC-03",
            "complexity_paths must define repository Python roots",
        ),
    )
    return _failed_checks(checks)
...
def _required_quality_files(root: Path) -> list[PolicyViolation]:
    required = (
        ...
        (
            "scripts/check_radon_complexity.py",
            "CC-04",
            "mandatory Radon complexity enforcement is missing",
        ),
        (
            ".githooks/pre-commit",
            "CC-05",
    ...
    return [
        PolicyViolation(rule, path, detail)
        for path, rule, detail in required
...
def _validate_pyproject(root: Path) -> list[PolicyViolation]:
    ...
    if not path.is_file():
        return [
            PolicyViolation(
                "UV-01", "pyproject.toml", "missing project manifest"
            )
        ]
...
def _production_python_paths(root: Path) -> list[Path]:
    ...
    for item in PRODUCTION_ROOTS:
        ...
        paths.extend(
            resolved.rglob("*.py")
            if resolved.is_dir()
            else ([resolved] if resolved.is_file() else [])
        )
    return sorted(paths)


def _coverage_source_violation(
    path: Path, root: Path
    ...
    return PolicyViolation(
        ...
        "production coverage exclusions are forbidden",
    )
...
def _validate_sources(root: Path) -> list[PolicyViolation]:
    return [
        ...
        if (violation := _coverage_source_violation(path, root)) is not None
    ]
...
def _obsolete_path_violations(root: Path) -> list[PolicyViolation]:
    return [
        PolicyViolation(
            "REPO-01", path, "obsolete tool or non-uv dependency file exists"
        )
...
def _frontend_path_violations(root: Path) -> list[PolicyViolation]:
    paths = [
        ...
        for path in _frontend_paths(root / prefix)
    ]
    return [
        PolicyViolation(
            "CLI-01",
            str(path.relative_to(root)),
            "frontend/HTTP delivery is forbidden in the CLI-only project",
        )
        for path in paths
    ]
