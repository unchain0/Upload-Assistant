...
from src.integrations.media.zentag import prepare_zenith_audiobook, prepare_zenith_ebook, should_prepare_zenith_audiobook, should_prepare_zenith_ebook
...
async def process_trackers(
    ...
    async def process_single_tracker(tracker: str, shared_meta: Meta = meta) -> None:
        ...
        if argument_parser_factory is None:
            ...
            meta = await prepare_tracker_meta(shared_meta, tracker, config, argument_parser_factory)
        ...
        if tracker in api_trackers:
            ...
            if upload_status:
                ...
                try:
                    if not await check_bandwidth_and_dupes(tracker, tracker_class):
                        ...
                        status.setdefault("status_message", "Skipped due to new dupe found after bandwidth wait")
            ...
            if meta.unattended:
                ...
                try:
                    ...
                    raise OperationAbortedError("Manual tracker upload confirmation was cancelled by the user.") from None
            if do_manual:
