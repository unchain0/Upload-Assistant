...
def _apply_tvdb_series_metadata(
    ...
    if isinstance(episodes_data, dict):
        ...
        if isinstance(series_year, (str, int)) and re.fullmatch(
            r"19\d\d|20[0-3]\d", str(series_year)
        ):
...
async def all_ids(
    ...
    all_tasks: list[Awaitable[Any]] = [
        ...
        tmdb_manager.tmdb_other_meta(
            ...
            or (isinstance(meta.mal_manual, str) and meta.mal_manual.isdigit())
            else None,
            aka=meta.aka,
    ...
    if (
        ...
        if (
            ...
            all_tasks.append(
                tvmaze_manager.get_tvmaze_episode_data(
                    tvmaze_id, season_int, episode_int
                )
            )
        # TMDb last
        ...
        if tmdb_id is not None and season_int is not None:
            tmdb_task_idx = len(all_tasks)
            all_tasks.append(
                tmdb_manager.get_season_details(tmdb_id, season_int)
            )
    ...
    try:
        ...
        logger.error(
            f"[red]Error occurred while processing core metadata: {e}[/red]"
        )
    ...
    if meta.category == "TV":
        ...
        if tvdb_episode_data and not isinstance(tvdb_episode_data, Exception):
            ...
            if episodes_data is not None:
                ...
                logger.info(
                    f"[yellow]Unexpected TVDb data format: {tvdb_episode_data!r}[/yellow]"
                )
        elif isinstance(tvdb_episode_data, Exception):
            logger.info(
                f"[yellow]TVDb episode data retrieval failed: {tvdb_episode_data}[/yellow]"
            )
    ...
    if meta.category == "TV" and not meta.tv_pack and meta.episode_int != 0:
        ...
        if tvmaze_task_idx is not None:
            ...
            if (
                not isinstance(tvmaze_episode_data, Exception)
                and tvmaze_episode_data
            ):
                meta.tvmaze_episode_data = tvmaze_episode_data
                meta.we_asked_tvmaze = True
            elif isinstance(tvmaze_episode_data, Exception):
                logger.info(
                    f"[yellow]TVMaze episode data retrieval failed: {tvmaze_episode_data}"
                )
        ...
        if tmdb_task_idx is not None:
            ...
            if (
                not isinstance(tmdb_episode_data, Exception)
                and tmdb_episode_data
            ):
                meta.tmdb_episode_data = tmdb_episode_data
                meta.we_checked_tmdb = True
            elif isinstance(tmdb_episode_data, Exception):
                logger.info(
                    f"[yellow]TMDb episode data retrieval failed: {tmdb_episode_data}"
                )
        ...
        if tmdb_task_idx is not None:
            ...
            if (
                not isinstance(tmdb_season_data, Exception)
                and tmdb_season_data
            ):
                meta.tmdb_season_data = tmdb_season_data
                meta.we_checked_tmdb = True
            elif isinstance(tmdb_season_data, Exception):
                logger.info(
                    f"[yellow]TMDb season data retrieval failed: {tmdb_season_data}[/yellow]"
                )
...
async def imdb_tmdb_tvdb(
    ...
    if len(results) > result_index:
        ...
        if not isinstance(tmdb_metadata, Exception) and tmdb_metadata:
            ...
            logger.info(
                f"[yellow]TMDb metadata retrieval failed: {tmdb_metadata}[/yellow]"
            )
    ...
    if meta.category == "TV" and len(results) > result_index:
        ...
        if isinstance(tvmaze_id, int):
            ...
            logger.info(
                f"[yellow]TVMaze ID retrieval failed: {tvmaze_id}[/yellow]"
            )
            meta.tvmaze_id = 0
    ...
    if meta.category == "TV":
        if len(results) > result_index:
            ...
            if tvdb_episode_data and not isinstance(
                tvdb_episode_data, Exception
            ):
                # tvdb_episode_data is a tuple: (episodes_list, series_name)
                episodes_data: Any = None
                series_name: Any = None
                if isinstance(tvdb_episode_data, tuple):
                    tvdb_episode_tuple = cast(
                        tuple[Any, Any], tvdb_episode_data
                    )
                    if len(tvdb_episode_tuple) == 2:
                        episodes_data, series_name = tvdb_episode_tuple
                if episodes_data is not None:
                    meta.tvdb_episode_data = episodes_data
                    _apply_tvdb_series_metadata(
                        meta, episodes_data, series_name
                    )
                    meta.we_checked_tvdb = True
                else:
                    logger.info(
        ...
        if (
            ...
            if (
                ...
                logger.info(
                    f"[yellow]TMDb episode data retrieval failed: {tmdb_episode_data}[/yellow]"
                )
    ...
    if meta.category == "TV":
        if len(results) > 2:
            ...
            if tvmaze_id is not None:
                ...
                if (
                    not meta.tvdb_id
                    and isinstance(tvdb_id, int)
                    and tvdb_id > 0
                ):
                    meta.tvdb_id = tvdb_id
                    logger.debug(
                        f"[green]Set TVDb ID from TVMaze: {tvdb_id}[/green]"
                    )
            elif isinstance(tvmaze_result, int):
                meta.tvmaze_id = tvmaze_result
            elif isinstance(tvmaze_result, Exception):
                logger.info(
                    f"[red]TVMaze API call failed: {tvmaze_result}[/red]"
                )
                meta.tvmaze_id = (
                    0  # Set default value if an exception occurred
                )
            else:
                logger.info(
                    f"[yellow]Unexpected TVMaze result type: {tvmaze_result!r}[/yellow]"
                )
                meta.tvmaze_id = 0
        ...
        if not meta.tv_pack:
            if len(results) > 3:
                ...
                if isinstance(episode_details_result, dict):
                    ...
                    logger.info(
                        f"[red]TMDb episode details API call failed: {episode_details_result}[/red]"
                    )
        else:
            if "season_int" in meta and len(results) > 3:
                ...
                if isinstance(season_details_result, dict):
                    ...
                    logger.info(
                        f"[red]TMDb season details API call failed: {season_details_result}[/red]"
                    )
...
async def get_tvmaze_tvdb(
    ...
    if isinstance(tvmaze_result, tuple):
        ...
        logger.info(
            f"[yellow]Unexpected TVMaze result type: {type(tvmaze_result)}[/yellow]"
        )
        tvmaze = 0
    ...
    if (imdb and imdb != 0) or (tmdb and tmdb != 0):
        ...
        if isinstance(tvdb_result, Exception):
            ...
            if tvdb_name:
                logger.debug(
                    f"[green]Got TVDb series name: {tvdb_name}[/green]"
                )
            ...
            if tvdb_result is not None:
                logger.info(
                    f"[yellow]Unexpected TVDb lookup result type: {type(tvdb_result)}[/yellow]"
                )
            tvdb = 0
    elif len(results) > 1:
        ...
        if tvdb_result and not isinstance(tvdb_result, Exception):
            ...
            if isinstance(tvdb_result, tuple):
                ...
                series_id_int = (
                    _coerce_int(series_id) if series_id is not None else None
                )
                if series_id_int is not None:
                    tvdb = series_id_int
                    logger.debug(
                        f"[green]Got TVDb series ID: {series_id}[/green]"
                    )
                if series_results:
                    tvdb_data = series_results
            else:
                logger.info(
                    f"[yellow]Unexpected TVDb result format: {tvdb_result}[/yellow]"
                )
        elif isinstance(tvdb_result, Exception):
            logger.info(
                f"[yellow]TVDb series data retrieval failed: {tvdb_result}[/yellow]"
            )

    tmdb_only_identity = bool(tmdb and tmdb != 0) and not bool(
        imdb and imdb != 0
    )
    ...
    if (
        not tvdb
        and tvmaze
        and isinstance(tvmaze_result, tuple)
        and not tmdb_only_identity
    ):
        tvmaze_tuple = cast(tuple[Any, Any, Any], tvmaze_result)
        if len(tvmaze_tuple) == 3:
...
async def get_tv_data(
    ...
    if not meta.tv_pack and meta.episode_int != 0:
        if (
            (not meta.we_checked_tvdb and not meta.we_asked_tvmaze)
            and meta.tvmaze_id != 0
            and meta.tvdb_id != 0
            and meta.tmdb_id != 0
            and not meta.anime
        ):
            ...
            (
                tvdb_episode_data,
                tvdb_name,
            ) = await tvdb_handler.get_tvdb_episodes(
                meta.tvdb_id,
                meta.base_dir,
                season=meta.season_int,
                episode=meta.episode_int,
        ...
        if meta.tvdb_episode_data and meta.tvdb_id:
            try:
                ...
                logger.error(
                    f"[red]Error fetching TVDb episode data: {e}[/red]"
                )
        ...
        if (
            ...
            and tvdb_season_int != meta.season_int
            and not meta.season
            and not meta.no_season
            and not meta.manual_date
        ):
        ...
        if (
            tvdb_episode_int is not None
            and tvdb_episode_int != meta.episode_int
            and not meta.episode
            and not meta.manual_date
        ):
        ...
        if (
            "tvmaze_episode_data" not in meta
            or meta.tvmaze_episode_data is None
        ):
            ...
            if (
                ...
                tvmaze_episode_data = (
                    await tvmaze_manager.get_tvmaze_episode_data(
                        tvmaze_id, season_int, episode_int, meta
                    )
                )
                if tvmaze_episode_data:
                    meta.tvmaze_episode_data = tvmaze_episode_data
        if meta.auto_episode_title is None or meta.overview_meta is None:
            tvmaze_name = meta.tvmaze_episode_data.get(
                "episode_name"
            ) or meta.tvmaze_episode_data.get("name")
            if meta.auto_episode_title is None and isinstance(
                tvmaze_name, str
            ):
                tvmaze_name_lc = tvmaze_name.lower()
                if (
                    tvmaze_name_lc.startswith("episode")
                    or "tba" in tvmaze_name_lc
                ):
                    meta.auto_episode_title = None
                else:
        ...
        if (
            ...
            if not meta.tmdb_episode_data:
                ...
                if (
                    ...
                    episode_details_result = (
                        await tmdb_manager.get_episode_details(
            ...
            if meta.auto_episode_title is None and isinstance(
                ...
                if (
                    episode_name_lc.startswith("episode")
                    or "tba" in episode_name_lc
                ):
                    meta.auto_episode_title = None
                else:
                    meta.auto_episode_title = episode_name
            if (
                not meta.overview_meta
                and episode_details.get("overview") is not None
            ):
    ...
    if tvdb_episode_id is not None:
        meta.tvdb_imdb_id = (
            await tvdb_handler.get_imdb_id_from_tvdb_episode_id(
                tvdb_episode_id
            )
        )
...
async def get_tvdb_tvmaze_tmdb_episode_data(
    ...
    if "tvmaze" in task_map:
        ...
        if tvmaze_episode_data and not isinstance(
            tvmaze_episode_data, Exception
        ):
            meta.tvmaze_episode_data = tvmaze_episode_data
            meta.we_asked_tvmaze = True
            ...
            logger.info(
                f"[yellow]TVMaze episode data retrieval failed: {tvmaze_episode_data}[/yellow]"
            )

    # Process TVDB results
    if "tvdb" in task_map:
        tvdb_episodes_result = results[task_map["tvdb"]]
        if tvdb_episodes_result and not isinstance(
            tvdb_episodes_result, Exception
        ):
            ...
            if tvdb_episode_data is not None:
                ...
                if meta.debug and isinstance(tvdb_episode_data, list):
                    ...
                    logger.info(
                        f"[green]TVDb episodes list retrieved with {len(tvdb_episode_list)} episodes[/green]"
                    )
            else:
                logger.info(
                    f"[yellow]Unexpected TVDb episodes result format: {tvdb_episodes_result}[/yellow]"
                )
            ...
            logger.info(
                f"[yellow]TVDb episode data retrieval failed: {tvdb_episodes_result}[/yellow]"
            )
...
async def get_douban_id(meta: Meta) -> int:
    ...
    try:
        ...
        completed_response = cast(httpx.Response, response)
        soup = BeautifulSoup(completed_response.text, "html.parser")
        ...
        await cache.set(
            "douban", "imdb_lookup", str(imdb_id), {"id": 0}, negative=True
        )
        return douban_id

    except Exception as e:
        logger.info(
            f"[bold yellow]Failed to fetch Douban ID for IMDb ID {imdb_id}: {e}[/bold yellow]"
        )
        return douban_id
