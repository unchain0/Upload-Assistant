...
def _extract_embedded_artwork(
    ...
    for audio_path in audio_paths:
        try:
            audio = mutagen.File(audio_path)  # pyright: ignore[reportPrivateImportUsage]
        ...
        if data:
            destination = (
                output_dir / f"MUSIC_COVER{_image_suffix(data, mime)}"
            )
...
def _apply_music_cli_overrides(meta: Meta, release: Any) -> None:
    ...
    if artist:
        artists = [
            part.strip()
            for part in re.split(r"\s+&\s+", artist)
            if part.strip()
        ]
    ...
    release_year = _music_override_year(
        meta.music_release_year, "--music-release-year"
    )
    if release_year:
        set_user("release_year", release_year)
    edition_year = _music_override_year(
        meta.music_edition_year, "--music-edition-year"
    )
    if edition_year:
    ...
    release_type = release_type_map.get(
    ...
    genres = [
        item.strip()
        for item in str(meta.music_genres or "").split(",")
        if item.strip()
    ]
    if genres:
    ...
    if isinstance(edition, list):
        edition = " ".join(
            str(item).strip() for item in edition if str(item).strip()
        )
...
def _discogs_ids(meta: Meta, release: Any) -> tuple[str, str]:
    ...
    if release_id:
        ...
        release.set_field(
            "discogs_release", release_id, MetadataSource.USER, 1.0, force=True
        )
    if master_id:
        release.external_ids["discogs_master"] = master_id
        release.set_field(
            "discogs_master", master_id, MetadataSource.USER, 1.0, force=True
        )
...
async def _find_discogs_release(meta: Meta, release: Any, token: str) -> str:
    ...
    filtered_matches = DiscogsEnricher.filter_releases_by_media(
        matches, release.get("media", "")
    )
    if len(filtered_matches) != len(matches):
        logger.info(
            f"[cyan]MUSIC: filtered Discogs matches by {release.get('media')} media ({len(matches)} -> {len(filtered_matches)}).[/cyan]"
        )
    matches = filtered_matches
    catalogue = release.get("directory_catalogue_number", "") or release.get(
    ...
    if len(catalogue_matches) != len(matches):
        logger.info(
            f"[cyan]MUSIC: filtered Discogs matches by catalogue number ({len(matches)} -> {len(catalogue_matches)}).[/cyan]"
        )
    ...
    for index, candidate in enumerate(matches, 1):
        ...
        logger.info(
            f"[cyan]{index}.[/cyan] {candidate.get('title', '')} {f'({details})' if details else ''} [dim]ID: {candidate.get('id', '')}[/dim]"
        )
    while True:
        try:
            choice = (
...
async def gather_music_prep(meta: Meta, config: dict[str, Any]) -> None:
    ...
    enrichment_enabled = (
        meta.music_enrichment
        if meta.music_enrichment is not None
        else default_config.get("music_enrichment_enabled", False)
    )
...
async def enrich_music_from_discogs(
    ...
    if (
        meta.category != "MUSIC"
        or not meta.music_discogs_enabled
        or not isinstance(meta.music_release, dict)
    ):
        return False
    ...
    existing_release = DiscogsEnricher.parse_reference(
        release.external_ids.get("discogs_release", ""), "release"
    )
    existing_master = DiscogsEnricher.parse_reference(
        release.external_ids.get("discogs_master", ""), "master"
    )
    if (
        not release_id
        and existing_release
        and existing_release[0] == "release"
    ):
...
def _sync_release_to_meta(meta: Meta, release: MusicRelease) -> None:
    ...
    meta.year = (
        int(release.get("year"))
        if str(release.get("year", "")).isdigit()
        else meta.year
    )
    ...
    meta.genres = (
        list(release.get("genres", meta.genres))
        if isinstance(release.get("genres", meta.genres), list)
        else meta.genres
    )
...
async def enrich_music_from_orpheus(
    ...
    _set_tracker_field(
        release, "album", str(group.get("name", "")).strip(), 0.93
    )
    if artists:
    ...
    _set_tracker_field(
        release, "year", str(group.get("year", "")).strip(), 0.84
    )
    _set_tracker_field(
        release,
        "release_type",
        str(group.get("releaseTypeName", "")).strip(),
        0.88,
    )
    _set_tracker_field(release, "genres", group.get("tags", []), 0.78)
    _set_tracker_field(
        release, "media", str(torrent.get("media", "")).strip(), 0.9
    )
    _set_tracker_field(
        release,
        "release_year",
        str(torrent.get("remasterYear", "")).strip(),
        0.89,
    )
    ...
    _set_tracker_field(
        release,
        "release_label",
        str(torrent.get("remasterRecordLabel", "")).strip(),
        0.91,
    )
    _set_tracker_field(
        release,
        "release_catalogue_number",
        str(torrent.get("remasterCatalogueNumber", "")).strip(),
        0.91,
    )
    _set_tracker_field(
        release,
        "orpheus_encoding",
        str(torrent.get("encoding", "")).strip(),
        0.9,
    )
    ...
    if group_id:
        ...
        release.external_ids.setdefault(
            "orpheus_url",
            f"{orpheus.base_url}/torrents.php?id={group_id}&torrentid={identifier}",
        )
...
def _music_name(release: Any) -> str:
    ...
    media, format_name = (
        str(release.get("media", "")),
        str(release.get("format", "")),
    )
