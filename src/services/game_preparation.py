...
async def detect_platform_from_files(
    ...
    if archives and base_dir:
        ...
        if binary_path:
            ...
            for archive in archives:
                ...
                archive_contents = await list_archive_contents_with_7z(
                    archive, binary_path
                )
    ...
    if any("/code/app.xml" in f for f in files_lower) or (
        any(b == "title.tmd" for b in basenames_lower)
        and any(b == "title.tik" for b in basenames_lower)
    ):
        return "WIIU"
    ...
    # PlayStation 3 (PS3)
    if any("ps3_game" in f for f in files_lower) or any(
        b == "param.sfo" and "ps3_game" in f
        for b, f in zip(basenames_lower, files_lower, strict=False)
    ):
        return "PS3"
    ...
    if any(b.endswith(".cso") for b in basenames_lower) or any(
        b == "eboot.pbp" for b in basenames_lower
    ):
        return "PSP"
    ...
    if pkg_files:
        ...
        if any(
            re.search(
                r"(?:^|[^a-z0-9])(?:np|bl|bc)[a-z]{2}\d{5}(?:[^a-z0-9]|$)", pkg
            )
    ...
    # Xbox 360
    if any(b.endswith(".xex") for b in basenames_lower) or any(
        b == "default.xex" for b in basenames_lower
    ):
        return "X360"
    ...
    # Xbox (Original)
    if any(b.endswith(".xbe") for b in basenames_lower) or any(
        b == "default.xbe" for b in basenames_lower
    ):
        return "XBOX"
    ...
    if any(
        "binaries/win64" in f
        or "binaries/win32" in f
        or "engine/binaries" in f
...
def resolve_game_filelist(
    ...
    if Path(videoloc).is_dir():
        ...
        archive_files = [
            f for f in filelist if f.lower().endswith(archive_exts)
        ]
        ...
        if exe_files:
            ...
            videopath = sorted(
                archive_files, key=os.path.getsize, reverse=True
            )[0]
        else:
    ...
    if videopath in filelist:
        filelist = [
            videopath,
            *(item for item in filelist if item != videopath),
        ]
...
async def gather_game_prep(
    ...
    title_source = str(
        meta.path or videopath or meta.filename or meta.title or ""
    )
    ...
    if platform:
        ...
        meta.console_game = any(
            word in platform_lower for word in console_words
        )
    else:
    ...
    if meta.game_version:
        ...
        if not version and path_to_check:
            search_dir = (
                ...
                else str(Path(path_to_check).parent)
            )
            if search_dir:
        ...
        if not version:
            nfo_files = [
                f for f in meta.filelist if f.lower().endswith(".nfo")
            ]
            if path_to_check:
                search_dir = (
                    path_to_check
                    ...
                    with contextlib.suppress(Exception):
                        for f in (p.name for p in Path(search_dir).iterdir()):
                            if f.lower().endswith(".nfo"):
                                abs_f = str(
                                    Path(Path(search_dir) / f).resolve()
                                )
    ...
    if config and "DEFAULT" in config:
        ...
        client_secret = (
            config["DEFAULT"].get("twitch_client_secret", "").strip()
        )
    ...
    title_query = clean_game_title(
        str(meta.path or videopath or meta.filename or meta.title or "")
    )
        ...
        for nfo_path in nfo_files:
            try:
                ...
                try:
                    ...
                    async with aiofiles.open(
                        nfo_path, encoding="latin-1"
                    ) as f:
                        content = await f.read()

                # Search for Steam App URL/ID
                match = re.search(
                    r"store\.steampowered\.com/app/(\d+)", content
                )
                if match:
                    detected_steam_id = match.group(1)
                    logger.debug(
                        f"[green]Auto-detected Steam ID {detected_steam_id} from NFO file.[/green]"
                    )
                    break
            except Exception as e:
                logger.debug(
                    f"[yellow]Debug: Error reading NFO {nfo_path}: {e}[/yellow]"
                )
    ...
    if not selected_game:
        ...
        if len(results) == 1 or meta.unattended:
            ...
            try:
                ...
                if (
                    choice != "Skip - Don't use any of these matches"
                    and choice != "Skip - Don't select any match"
                ):
    ...
    # Genres
    genres = [
        g.get("name") for g in selected_game.get("genres", []) if g.get("name")
    ]
    if genres:
        meta.genres = genres
    ...
    if not cli_overrides["platform"]:
        raw_platforms = [
            p.get("name")
            for p in selected_game.get("platforms", [])
            if p.get("name")
        ]
        ...
        if meta.platform and meta.platform.upper() in platforms:
            ...
            if raw_platforms:
                ...
                for idx, p_name in enumerate(raw_platforms):
                    ...
                    for alias in aliases:
                        if re.search(
                            rf"\b{re.escape(alias)}\b", normalized_basename
                        ):
            ...
            if detected_platform:
                ...
                logger.info(
                    f"[green]Game platform auto-detected from folder/file name: {detected_platform}[/green]"
                )
            elif len(platforms) == 1:
                meta.platform = platforms[0]
                logger.debug(
                    f"[green]Game platform set to: {platforms[0]}[/green]"
                )
        ...
        for ext in selected_game.get("external_games", []):
            if ext.get("external_game_source") == 1:
                ...
                if not steam_url and ext.get("uid"):
                    steam_url = (
                        f"https://store.steampowered.com/app/{ext.get('uid')}"
                    )
                break
    if steam_url:
    ...
    if steam_id:
        ...
        try:
            ...
            if (
                ...
                and res_data
                and steam_id in res_data
                and res_data[steam_id].get("success")
            ):
            ...
            logger.info(
                f"[yellow]Steam: Error fetching app details: {e}[/yellow]"
            )
    ...
    if igdb_screenshots:
        ...
        if image_list:
            ...
            try:
                ...
                logger.info(
                    f"[yellow]IGDB: Failed to save screenshots to image_data.json: {e}[/yellow]"
                )
