...
from src.integrations.observability.runtime_support import logger, suppress_cli_progress
...
async def _run_early_artifact_task(factory: Callable[[], Awaitable[None]]) -> None:
...
def start_early_artifact_tasks(meta: Meta, client: Clients, config: Mapping[str, Any]) -> tuple[asyncio.Task[None], asyncio.Task[None]]:
    ...
    if tasks is None:
        tasks = (
            asyncio.create_task(_run_early_artifact_task(lambda: create_base_torrents_early(meta, client))),
            asyncio.create_task(_run_early_artifact_task(lambda: prepare_usenet_archive_early(meta, config))),
        )
...
async def create_base_torrents_early(meta: Meta, client: Clients) -> None:
    ...
    try:
        ...
        if reuse_torrent and Path(reuse_torrent).exists():
            ...
            created_path = await TorrentCreator.create_base_from_existing_torrent(reuse_torrent, str(meta.base_dir), release_id)
