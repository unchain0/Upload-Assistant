...
def _ebook_source(meta: Meta) -> Path | None:
    ...
    if source.is_file() and source.suffix.lower() in EBOOK_SUFFIXES:
        return source
    candidates = [Path(str(path)).expanduser().resolve() for path in meta.filelist if Path(str(path)).suffix.lower() in EBOOK_SUFFIXES]
...
def should_prepare_zenith_ebook(meta: Meta, config: dict[str, Any]) -> bool:
    ...
    if not config.get("DEFAULT", {}).get("auto_zentag", True):
        return False
    return meta.category == "BOOK" and not meta.audiobook and not (meta.comic or meta.manga or meta.magazine or meta.newspaper) and _ebook_source(meta) is not None
...
def _written_output(stdout: str, output_root: Path) -> Path | None:
    ...
    try:
        ...
        output = Path(json.loads(raw_output) if raw_output.startswith('"') else raw_output).resolve()
...
async def prepare_zenith_ebook(meta: Meta, base_dir: str, config: dict[str, Any]) -> str | None:
    ...
    try:
        ...
        logger.info(f"[cyan]ZENITH: preparing a compliant ebook copy with zentag: {source.name}[/cyan]")
