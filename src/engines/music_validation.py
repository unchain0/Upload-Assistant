...
class MusicValidator:
    ...
    @staticmethod
    def _required_metadata_issues(release: MusicRelease) -> list[ValidationIssue]:
        return [
    ...
    def _format_issues(release: MusicRelease) -> list[ValidationIssue]:
        ...
        return [ValidationIssue(ValidationLevel.ERROR, "mixed_formats", f"Release mixes formats: {formats}.")]
    ...
    def _album_conflict_issue(release: MusicRelease) -> ValidationIssue | None:
        ...
        return ValidationIssue(level, "inconsistent_album", "Audio tags do not agree on the album title.")

    @staticmethod
    def _artist_conflict_issue(release: MusicRelease) -> ValidationIssue | None:
        if not release.conflicts.get("artist"):
            return None
        if str(release.get("artist", "")).casefold() == "various artists":
            return None
        return ValidationIssue(ValidationLevel.WARNING, "inconsistent_artist", "Audio tags contain multiple album artists.")

    @classmethod
    def _track_sequence_issues(cls, release: MusicRelease) -> list[ValidationIssue]:
    ...
    @staticmethod
    def _collect_track_number(track: object, tracks_by_disc: dict[int, list[int]]) -> ValidationIssue | None:
    ...
    @staticmethod
    def _disc_sequence_issues(tracks_by_disc: dict[int, list[int]]) -> list[ValidationIssue]:
        issues: list[ValidationIssue] = []
...
class OrpheusMusicValidator(MusicValidator):
    ...
    @staticmethod
    def _orpheus_required_metadata_issues(release: MusicRelease) -> list[ValidationIssue]:
        return [
            ValidationIssue(
    ...
    @classmethod
    def _unsupported_format_issue(cls, track: object) -> ValidationIssue | None:
        track_format = str(getattr(track, "format", ""))
        if track_format in cls.ALLOWED_FORMATS:
            return None
        path = str(getattr(track, "relative_path", ""))
        return ValidationIssue(ValidationLevel.ERROR, "unsupported_format", f"{path}: {track_format} is not an allowed Orpheus music format.")

    @staticmethod
    def _invalid_flac_container_issue(track: object) -> ValidationIssue | None:
        if getattr(track, "format", "") != "FLAC" or Path(str(getattr(track, "path", ""))).suffix.lower() == ".flac":
            return None
        path = str(getattr(track, "relative_path", ""))
        return ValidationIssue(ValidationLevel.ERROR, "invalid_container", f"{path}: FLAC must use the .flac container.")
    ...
    def _invalid_aac_container_issue(track: object) -> ValidationIssue | None:
        ...
        if getattr(track, "format", "") != "AAC" or suffix in {".m4a", ".aac"}:
            return None
        path = str(getattr(track, "relative_path", ""))
        return ValidationIssue(ValidationLevel.ERROR, "invalid_container", f"{path}: AAC must use the .m4a or .aac container.")
    ...
    def _flac_bit_depth_issue(track: object) -> ValidationIssue | None:
        ...
        if not bit_depth or bit_depth <= 24:
            return None
        path = str(getattr(track, "relative_path", ""))
        return ValidationIssue(ValidationLevel.ERROR, "bit_depth", f"{path}: FLAC depth exceeds 24-bit.")
    ...
    def _flac_sample_rate_issue(cls, track: object) -> ValidationIssue | None:
        ...
        if not sample_rate or sample_rate in cls.ALLOWED_SAMPLE_RATES:
            return None
        path = str(getattr(track, "relative_path", ""))
        return ValidationIssue(ValidationLevel.ERROR, "sample_rate", f"{path}: sample rate is not allowed by Orpheus.")
    ...
    def _flac_16bit_rate_issue(track: object) -> ValidationIssue | None:
        ...
        if bit_depth != 16 or not sample_rate or sample_rate <= 48000:
            return None
        path = str(getattr(track, "relative_path", ""))
        return ValidationIssue(ValidationLevel.ERROR, "16bit_high_rate", f"{path}: 16-bit FLAC is limited to 44.1/48 kHz.")
    ...
    def _mp3_bitrate_issue(track: object) -> ValidationIssue | None:
        ...
        if not bitrate or bitrate <= 320_000 or getattr(track, "bitrate_mode", None) != "CBR":
            return None
        return ValidationIssue(
            ValidationLevel.ERROR,
            "mp3_cbr_limit",
    ...
    @staticmethod
    def _hybrid_technical_issue(release: MusicRelease) -> ValidationIssue | None:
    ...
    @classmethod
    def _orpheus_release_issues(cls, release: MusicRelease) -> list[ValidationIssue]:
        issues = (
    ...
    def _single_track_issue(release: MusicRelease) -> ValidationIssue | None:
        if len(release.tracks) != 1 or str(release.get("release_type", "")) == "Single":
            return None
        return ValidationIssue(ValidationLevel.ERROR, "single_track", "A one-track upload must be an officially released single.")
    ...
    def _missing_log_issue(release: MusicRelease) -> ValidationIssue | None:
        ...
        if not physical or not release.is_lossless or release.auxiliary.logs:
            return None
        return ValidationIssue(ValidationLevel.WARNING, "missing_log", "A lossless physical-media rip has no rip log; it may be trumpable or require review.")

    @staticmethod
    def _possible_unsplit_issue(release: MusicRelease) -> ValidationIssue | None:
        ambiguous = len(release.tracks) == 1 and release.is_lossless and not release.auxiliary.cues
        if not ambiguous or str(release.get("release_type", "")) == "Single":
            return None
        return ValidationIssue(ValidationLevel.WARNING, "possible_unsplit", "A one-file FLAC without a cue requires confirmation that it is an official single.")

    @staticmethod
    def _unknown_media_issue(release: MusicRelease) -> ValidationIssue | None:
        if release.get("media"):
            return None
        return ValidationIssue(ValidationLevel.WARNING, "unknown_media", "Source media is unknown; do not guess. WEB is appropriate only for verified digital downloads.")

    @staticmethod
    def _missing_lineage_issue(release: MusicRelease) -> ValidationIssue | None:
        media = str(release.get("media", "")).upper()
        if media not in {"SACD", "BD", "CASSETTE"} or release.auxiliary.lineage:
            return None
        return ValidationIssue(ValidationLevel.WARNING, "missing_lineage", "This source type requires or strongly benefits from lineage information.")
