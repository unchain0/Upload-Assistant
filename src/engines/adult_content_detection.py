...
def _normalized_existing_keywords(existing_keywords: Iterable[str] | str | None) -> list[str]:
    values = existing_keywords.split(",") if isinstance(existing_keywords, str) else (existing_keywords or ())
    return [text for keyword in values if (text := str(keyword).strip())]


def _candidate_keyword_matches(tokens: list[str]) -> list[tuple[int, int, str]]:
...
def _matches_at_position(tokens: list[str], start: int) -> list[tuple[int, int, str]]:
    matches: list[tuple[int, int, str]] = []
...
def _select_non_overlapping_matches(matches: list[tuple[int, int, str]]) -> list[tuple[int, str]]:
...
def _append_unseen_keywords(existing: list[str], matched: list[tuple[int, str]]) -> list[str]:
