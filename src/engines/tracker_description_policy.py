...
def resolve_description_mode(
    ...
    if not isinstance(configured_mode, str):
        raise ValueError(
            "DEFAULT.tracker_description_mode must be one of: ids, images, text, text_and_images"
        )
    try:
...
class DescriptionCandidate:
    ...
    def audit_record(self) -> dict[str, Any]:
        ...
        record["raw_sha256"] = hashlib.sha256(
            self.raw_description.encode("utf-8")
        ).hexdigest()
        record["cleaned_sha256"] = hashlib.sha256(
            self.cleaned_description.encode("utf-8")
        ).hexdigest()
...
def description_fingerprint(meta: Any, tracker: str) -> str:
    ...
    payload = {
        ...
        "images": [
            image.get("raw_url", image.get("img_url", ""))
            for image in images
...
def score_release_name(
    search_term: object, release_name: object, *, explicit_id: bool = False
) -> int:
...
def _normalized_release_names(
    search_term: object, release_name: object
) -> tuple[str, str] | None:
...
def _token_overlap(expected: str, actual: str) -> float:
    ...
    return len(expected_tokens & actual_tokens) / len(
        expected_tokens | actual_tokens
    )


def add_candidate(
    meta: Any, candidate: DescriptionCandidate, *, selected: bool
) -> None:
