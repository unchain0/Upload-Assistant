...
class QbittorrentClientMixin:
            ...
            if proxy_url:
                ...
                try:
                    ...
                    torrent_properties = await self.retry_qbt_operation(
                        lambda direct_client=direct_client: asyncio.to_thread(direct_client.torrents_properties, torrent_hash=info_hash_v1),
    ...
    async def search_qbit_for_torrent(
        ...
        torrent_storage_dir = client.get("torrent_storage_dir")
        if not str(meta.base_dir or "").strip() or not str(meta.uuid or "").strip():
        ...
        try:
            ...
            try:
                if proxy_url:
                    ...
                    direct_client = cast(qbittorrentapi.Client, qbt_client)
                    torrents = await self.retry_qbt_operation(lambda: asyncio.to_thread(direct_client.torrents_info), "Get torrents list", initial_timeout=14.0)
            except TimeoutError:
                logger.info("[bold red]Getting torrents list timed out after retries")
                return None
            except Exception as e:
            ...
            for torrent in torrents:
                ...
                if not comment:
                    try:
                        if proxy_url:
                            qbt_session = cast(httpx.AsyncClient, qbt_session)
                            response = await qbt_session.get(f"{proxy_url.rstrip('/')}/api/v2/torrents/properties", params={"hash": torrent_hash_value})
                            if response.status_code == 200:
                                comment = str(response.json().get("comment", "") or "")
                        elif qbt_client is not None:
                            properties = await self.retry_qbt_operation(
                                lambda qbt_client=qbt_client, torrent_hash=torrent_hash_value: asyncio.to_thread(qbt_client.torrents_properties, torrent_hash=torrent_hash),
                                f"Get properties for torrent {torrent.name}",
                            )
                            comment = str(properties.get("comment", "") or "")
                    except Exception as error:
                        logger.debug(f"[yellow]Could not inspect torrent comment for {torrent_hash_value}: {error}[/yellow]")
                ...
                matching_torrents.append({"hash": torrent_hash_value, "name": torrent.name})
            ...
            for matching_torrent in matching_torrents:
                ...
                try:
                    ...
                    valid, torrent_path = await self.is_valid_torrent(meta, str(torrent_file_path), torrent_hash, "qbit", client)
    ...
    async def qbittorrent(
        ...
        if use_symlink or use_hardlink:
            ...
            tracker_dir = tracker_directory(cast(str | Path, link_target), link_dir_name, tracker)
        ...
        save_path = str(cast(str | Path, tracker_dir)) if use_symlink or use_hardlink else str(path)
            ...
            try:
                if proxy_url:
                    ...
                    torrents_info = await self.retry_qbt_operation(
                        lambda direct_client=direct_client: asyncio.to_thread(direct_client.torrents_info, torrent_hashes=torrent.infohash),
        ...
        if not cross:
            try:
                if proxy_url:
                    ...
                    direct_client = cast(qbittorrentapi.Client, qbt_client)
                    await self.retry_qbt_operation(lambda direct_client=direct_client: asyncio.to_thread(direct_client.torrents_resume, torrent.infohash), "Resume torrent")
            except TimeoutError:
        ...
        if tracker in client.get("super_seed_trackers", []) and not cross:
            try:
                ...
                if proxy_url:
                    ...
                    direct_client = cast(qbittorrentapi.Client, qbt_client)
                    await self.retry_qbt_operation(
                        lambda direct_client=direct_client: asyncio.to_thread(direct_client.torrents_set_super_seeding, torrent_hashes=torrent.infohash),
                        "Set super-seed mode",
                        initial_timeout=10.0,
                    )
            except TimeoutError:
                ...
                if valid:
                    ...
                    if meta.subtitle_files and not self._torrent_includes_all_local_subtitles(validated_torrent_path, meta):
                        ...
                        try:
                            ...
                            is_better_match = bool(piece_limit and piece_size <= 16777216)
            ...
            if not found_valid_torrent or (use_piece_preference and not piece_size_best_match):
                ...
                for torrent_match in matching_torrents[1:]:  # Skip the first one since we already tried it
                    ...
                    try:
                        ...
                        logger.info(f"[bold red]Error preparing alternative torrent {alt_torrent_hash}: {e}")
                        if alt_torrent_file_path and Path(alt_torrent_file_path).exists() and alt_torrent_file_path.startswith(extracted_torrent_dir):
...
async def create_cross_seed_links(meta: Meta, torrent: Torrent, tracker_dir: str | Path, use_hardlink: bool) -> bool:
...
async def async_link_directory(src: str, dst: str | Path, use_hardlink: bool = True) -> bool:
    try:
        ...
        # Handle directory linking
            ...
            if use_hardlink:
                ...
                all_items = await asyncio.to_thread(_collect_files, src, str(dst))
