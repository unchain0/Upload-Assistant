...
class RtorrentClientMixin:
    ...
    def rtorrent(self, path: str, torrent_path: str, torrent: Torrent, meta: Meta, local_path: str, remote_path: str, client: dict[str, Any], tracker: str) -> None:
        ...
        if use_symlink or use_hardlink:
            ...
            tracker_dir = tracker_directory(cast(str, link_target), link_dir_name, tracker)
        ...
        save_path = str(tracker_dir) if use_symlink or use_hardlink else str(path)
        ...
        try:
            ...
            resume_path = str(dst) if (use_symlink or use_hardlink) and Path(dst).exists() else path
            logger.debug(f"[cyan]Using resume path: {resume_path}")
            fast_resume = self.add_fast_resume(metainfo, str(resume_path), torrent)
        ...
        # Use dst path if linking was successful, otherwise use original path
        path = str(dst) if (use_symlink or use_hardlink) and Path(dst).exists() else path
