...
from src.integrations.torrent_clients import DelugeClientMixin, QbittorrentClientMixin, RtorrentClientMixin, TransmissionClientMixin
from src.integrations.torrent_clients.path_utils import coerce_str_list, is_path_under
...
class Clients(QbittorrentClientMixin, RtorrentClientMixin, DelugeClientMixin, TransmissionClientMixin):
    ...
    def _get_tracker_comment_hosts(self) -> dict[str, tuple[str, ...]]:
        if self._tracker_comment_hosts is None:
            from src.integrations.trackers.registry import get_tracker_comment_hosts
            ...
            # Otherwise we match either only videos (no subtitles) OR videos + subtitles (if subtitles are present)
                ...
                for cand in candidates:
                    ...
                    if len(torrent.files) == len(cand) == 1:
                        if Path(torrent.files[0]).name == Path(cand[0]).name and "length" in torrent.metainfo["info"]:
                            valid = True
                            break
                        logger.debug(f"Single file match status: valid={valid}")
            ...
            if Path(torrent_path).exists():
                try:
                    ...
                    if reuse_torrent.pieces >= 5000 and reuse_torrent.piece_size < 4294304 and (max_piece_size is None or max_piece_size >= 4):
                        ...
                        valid = False
                    elif max_piece_size is None and reuse_torrent.pieces >= 12000:
                        ...
                        valid = False
                    elif max_piece_size is None and torrent_file_size_kib > 250:
