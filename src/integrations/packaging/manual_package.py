...
class ManualPackageManager:
    ...
    async def package(self, meta: Meta) -> str | bool:
        ...
        async with aiofiles.open(
            ...
            poster_img = str(
                artwork_dir(meta.base_dir, meta.uuid) / "POSTER.png"
            )
            if (
                meta.artwork_url not in ["", None]
                and not Path(poster_img).exists()
            ):
                if meta.rehosted_artwork_url is None:
                    ...
                    if response.status_code == 200:
                        ...
                        await asyncio.to_thread(
                            Path(poster_img).write_bytes, response.content
                        )
                        if not meta.skip_imghost_upload:
                            ...
                            poster = poster[0]
                            await generic.write(
                                f"TMDB Cover: {poster.get('raw_url', poster.get('img_url'))}\n"
                            )
                            meta.rehosted_artwork_url = poster.get(
                        ...
                        logger.info(
                            "[bold yellow]Cover could not be retrieved"
                        )
            elif (
                Path(poster_img).exists()
                and meta.rehosted_artwork_url is not None
            ):
                await generic.write(
                    f"TMDB Cover: {meta.rehosted_artwork_url}\n"
                )
        ...
        torrent_files = [
            f.name
            for f in (Path(meta.base_dir) / "tmp" / meta.uuid).glob(
                "*.torrent"
            )
        ]
        if len(torrent_files) > 1:
            for each in torrent_files:
                if not each.startswith(("BASE", "[RAND")):
                    Path(
                        f"{meta.base_dir}{'/' + 'tmp' + '/'}{meta.uuid}/{each}"
                    ).resolve().unlink()
        try:
            if Path(
            ...
            manual_tracker_cfg: dict[str, Any] = (
                cast(dict[str, Any], manual_tracker_raw)
                if isinstance(manual_tracker_raw, dict)
                else {}
            )
            manual_filebrowser = manual_tracker_cfg.get("filebrowser")
            filebrowser = (
                manual_filebrowser
                if isinstance(manual_filebrowser, str)
                else None
            )
            shutil.make_archive(
                archive,
