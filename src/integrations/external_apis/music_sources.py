...
def _music_cache_path(
    base_dir: str, provider: str, kind: str, key: str
) -> Path | None:
...
class MusicBrainzEnricher:
    ...
    def __init__(
        self,
        user_agent: str = "Upload-Assistant/2.x (+https://github.com/wastaken7/Upload-Assistant)",
        base_dir: str = "",
    ) -> None:
    ...
    async def enrich(self, release: MusicRelease) -> None:
        ...
        result = await self._find_release(
            artist, album, len(release.tracks), media, catalogue_number
        )
    ...
    def _release_identity(
        ...
        catalogue = (
            release.get("directory_catalogue_number", "")
            or release.get("release_catalogue_number", "")
            or release.get("catalogue_number", "")
        )
        return artist, album, media, str(catalogue).strip()

    @classmethod
    def _apply_result(
        cls, release: MusicRelease, result: dict[str, Any]
    ) -> None:
        ...
        _set_external_release_type(
            release, cls._release_type(result), 0.72, "MusicBrainz"
        )
        release.set_field(
            "year",
            str(release_group.get("first-release-date", ""))[:4],
            MetadataSource.EXTERNAL,
            0.7,
        )
        release.set_field(
        ...
        release.set_field(
            "release_label", cls._label(result), MetadataSource.EXTERNAL, 0.78
        )
        release.set_field(
            "release_catalogue_number",
            cls._catalogue_number(result),
            MetadataSource.EXTERNAL,
            0.78,
        )
        ...
        release.set_field(
            "artist", " & ".join(artists), MetadataSource.EXTERNAL, 0.8
        )
    ...
    async def _find_release(
        ...
        key = (
            ...
            str(track_count),
            media.casefold(),
            catalogue_number.casefold(),
        )
        cache_path = _music_cache_path(
            self.base_dir,
            "musicbrainz",
            "release_search",
            "\x1f".join(map(str, key)),
        )
        ...
        async with self._lock:
            ...
            return (
                result
                if hit
                else await self._request_release(
                    key,
                    cache_path,
                    artist,
                    album,
    ...
    async def _cached_release(
        cls, key: tuple[str, str, str, str, str], cache_path: Path | None
    ...
    async def _request_release(
        ...
        try:
            result = await self._search_remote(
                artist, album, track_count, media, catalogue_number
            )
        ...
        if request_succeeded:
            await _write_music_cache(
                cache_path,
                result if result is not None else {"not_found": True},
            )
        return result
    ...
    async def _search_remote(
        ...
        query = (
            f"barcode:{barcode}"
            if barcode
            else f'artist:"{artist}" AND release:"{album}"'
        )
        async with httpx.AsyncClient(
            timeout=httpx.Timeout(5.0), headers={"User-Agent": self.user_agent}
        ) as client:
        ...
        releases = (
            payload.get("releases", []) if isinstance(payload, dict) else []
        )
        return self._select_release(
            releases, album, track_count, media, catalogue_number
        )
    ...
    def _select_release(
        ...
        return (
            max(candidates, key=cls._candidate_score) if candidates else None
        )

    @classmethod
    def _title_candidates(
        cls, releases: list[Any], album: str
    ) -> list[dict[str, Any]]:
        ...
        return [
            ...
            if cls._normalise_title(item.get("title", "")) == expected
        ]

    @classmethod
    def _track_candidates(
        cls, releases: list[dict[str, Any]], track_count: int
    ) -> list[dict[str, Any]]:
        return (
            releases
            if not track_count
                ...
                if cls._track_count(item) == track_count
            ]
        )

    @classmethod
    def _media_candidates(
        cls, releases: list[dict[str, Any]], media: str
    ) -> list[dict[str, Any]]:
        return (
            releases
            if not media
                ...
                item
                for item in releases
                if cls._has_compatible_media(item, media)
            ]
        )

    @classmethod
    def _catalogue_candidates(
        cls, releases: list[dict[str, Any]], catalogue_number: str
    ) -> list[dict[str, Any]]:
        return (
            releases
            if not catalogue_number
                ...
                item
                for item in releases
                if cls._matches_catalogue_or_barcode(item, catalogue_number)
            ]
        )
    ...
    @staticmethod
    def _matches_catalogue_or_barcode(
        result: dict[str, Any], value: str
    ) -> bool:
        expected = str(value).strip().casefold()
        barcode = MusicBrainzEnricher._barcode(value)
        if (
            barcode
            and MusicBrainzEnricher._barcode(result.get("barcode", ""))
            == barcode
        ):
            return True
        return any(
            isinstance(info, dict)
            and str(info.get("catalog-number", "")).strip().casefold()
            == expected
    ...
    def _has_compatible_media(cls, result: dict[str, Any], media: str) -> bool:
        expected = {"web": "digital media"}.get(
            str(media).strip().casefold(), str(media).strip().casefold()
        )
        return any(
            cls._media_format_matches(expected, candidate)
            for candidate in cls._media_formats(result)
        )
    ...
    def _media_formats(result: dict[str, Any]) -> list[str]:
        ...
        if not isinstance(values, list):
            return []
        return [
            str(item.get("format", "")).strip().casefold()
            for item in values
    ...
    def _media_track_counts(result: dict[str, Any]) -> list[int]:
        ...
        if not isinstance(media, list):
            return []
        return [
            _safe_int(item.get("track-count", 0))
            for item in media
            if isinstance(item, dict)
        ]
    ...
    def _artists(cls, result: dict[str, Any]) -> list[str]:
        ...
        return _unique_nonempty_strings(
            [cls._artist_credit_name(credit) for credit in credits]
        )
...
class DiscogsEnricher:
    ...
    @classmethod
    def parse_reference(
        cls, value: Any, default_kind: str = "release"
    ) -> tuple[str, str] | None:
        ...
        if not text:
            return None
        return (
            cls._path_reference(text)
            or cls._named_reference(text)
            or cls._bare_reference(text, default_kind)
        )

    @staticmethod
    def _path_reference(text: str) -> tuple[str, str] | None:
        match = re.search(
            r"(?:discogs\.com/|^)(release|master)[/:](\d+)(?:[-/?#]|$)",
            text,
            re.I,
        )
    ...
    async def enrich(
        ...
        *,
        ...
        release_data, inferred_master = await self._release_enrichment(
            release, release_id
        )
        await self._master_enrichment(
            release, master_id or inferred_master, release_data
        )

    async def _release_enrichment(
        self, release: MusicRelease, release_id: str
        ...
        if release_data is None:
            logger.warning(
                f"[yellow]MUSIC: Discogs release {release_id} was not found or could not be read.[/yellow]"
            )
            return None, ""
        self._apply_release(release, release_data)
        return release_data, str(release_data.get("master_id", ""))

    async def _master_enrichment(
        self,
        release: MusicRelease,
        master_id: str,
        ...
        if master_data is None:
            logger.warning(
                f"[yellow]MUSIC: Discogs master {master_id} was not found or could not be read.[/yellow]"
            )
            return
        self._apply_master(release, master_data)
        logger.info(
            f"[cyan]MUSIC: corroborated original release data from Discogs master {master_id}.[/cyan]"
        )
        await self._enrich_master_main_release(
            release, master_id, master_data, release_data
        )
    ...
    async def _enrich_master_main_release(
        ...
        if concrete_release is not None:
            ...
            logger.info(
                f"[cyan]MUSIC: enriched concrete release from Discogs master {master_id}.[/cyan]"
            )

    async def find_exact_releases(
        ...
        if not self.token:
            logger.warning(
                "[yellow]MUSIC: Discogs exact-match search skipped because no Discogs token is configured.[/yellow]"
            )
            return []
        key = ("search", f"{artist.casefold()}\x1f{album.casefold()}")
        cache_path = _music_cache_path(
            self.base_dir, "discogs", "release_search", key[1]
        )
        ...
        async with self._lock:
            ...
            return (
                matches
                if hit
                else await self._request_exact_releases(
                    key, cache_path, artist, album
                )
            )
    ...
    @classmethod
    async def _cached_exact_releases(
        cls, key: tuple[str, str], cache_path: Path | None
    ...
    async def _search_exact_releases_remote(
        self, artist: str, album: str
    ) -> list[dict[str, Any]]:
        headers = {
        ...
        return [
            item
            for item in self._search_result_mappings(payload)
            if self._is_exact_release(item, artist, album)
        ]
    ...
    @classmethod
    def _is_exact_release(
        cls, result: dict[str, Any], artist: str, album: str
    ) -> bool:
        title = str(result.get("title", "")).strip()
        if " - " not in title:
            return False
        candidate_artist, candidate_album = title.split(" - ", 1)
        return cls._normalise_match(candidate_artist) == cls._normalise_match(
            artist
        ) and cls._normalise_match(candidate_album) == cls._normalise_match(
            album
        )

    @classmethod
    def filter_releases_by_media(
        cls, releases: list[dict[str, Any]], media: Any
    ) -> list[dict[str, Any]]:
        ...
        return [
            ...
            if cls._release_matches_media(release, target)
        ]

    @classmethod
    def _release_matches_media(
        cls, release: dict[str, Any], target: frozenset[str]
    ) -> bool:
    ...
    @classmethod
    def filter_releases_by_catalogue(
        cls, releases: list[dict[str, Any]], catalogue: Any
    ) -> list[dict[str, Any]]:
        ...
        matches = [
            release
            for release in releases
            if cls._normalise_catalogue(release.get("catno", "")) == wanted
        ]
    ...
    async def _get(
        ...
        cache_path = _music_cache_path(
            self.base_dir, "discogs", resource, identifier
        )
        ...
        async with self._lock:
            ...
            return (
                result
                if hit
                else await self._request_resource(
                    key, cache_path, resource, identifier
                )
            )

    @classmethod
    async def _cached_resource(
        cls, key: tuple[str, str], cache_path: Path | None
    ) -> tuple[bool, dict[str, Any] | None]:
        if key in cls._cache:
            return True, cls._cache[key]
    ...
    async def _fetch_resource(
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                f"https://api.discogs.com/{resource}/{identifier}"
            )
            response.raise_for_status()
    ...
    def _apply_release(
        ...
        cls._set_external_if_present(
            release,
            "retail_date",
            str(result.get("released", "")).strip(),
            0.90,
        )
        cls._set_external_if_present(
            release, "release_year", cls._year(result.get("year")), 0.92
        )
        cls._set_external_if_present(
            release,
            "release_country",
        ...
        cls._set_external_if_present(
            release, "media", cls._media(result.get("formats")), 0.84
        )
        _set_external_release_type(
            release, cls._release_type(result.get("formats")), 0.86, "Discogs"
        )

    @classmethod
    def _apply_release_ids(
        cls, release: MusicRelease, result: dict[str, Any]
    ) -> None:
        ...
        if identifier:
            release.set_field(
                "discogs_release", identifier, MetadataSource.EXTERNAL, 0.9
            )
        cls._store_external_id(
            release, "discogs_release_url", str(result.get("uri", "")).strip()
        )
        cls._store_external_id(
            release, "discogs_master", str(result.get("master_id", "")).strip()
        )

    @staticmethod
    def _store_external_id(
        release: MusicRelease, name: str, value: str
    ) -> None:
        if value:
            release.external_ids[name] = value

    @classmethod
    def _apply_release_label(
        cls, release: MusicRelease, label: tuple[str, str] | None
    ) -> None:
        if label is None:
            return
        cls._set_external_if_present(release, "release_label", label[0], 0.94)
        cls._set_external_if_present(
            release, "release_catalogue_number", label[1], 0.94
        )

    @classmethod
    def _set_external_if_present(
        cls, release: MusicRelease, name: str, value: Any, confidence: float
    ) -> None:
        if value:
    ...
    def _apply_master(
        ...
        if identifier:
            ...
            release.set_field(
                "discogs_master", identifier, MetadataSource.EXTERNAL, 0.9
            )
    ...
    def _apply_common(
        ...
        *,
        ...
        cls._set_external_if_present(
            release, "artist", " & ".join(artists), 0.92
        )
        cls._set_external_if_present(
            release, "album", cls._title(result, artists), 0.92
        )
        cls._set_external_if_present(
            release, "genres", cls._genres(result), 0.86
        )
        if release_record:
            cls._set_external_if_present(
                release,
                "discogs_notes",
                str(result.get("notes", "")).strip(),
                0.5,
            )
    ...
    def _set_external(
        ...
        release.set_field(
            name, value, MetadataSource.EXTERNAL, effective_confidence
        )
    ...
    def _artists(cls, result: dict[str, Any]) -> list[str]:
        ...
        if not isinstance(values, list):
            return []
        return _unique_nonempty_strings(
            [cls._artist_name(value) for value in values]
        )
    ...
    def _media(cls, formats: Any) -> str:
        mapped = {
            _DISCOGS_MEDIA_MAPPING[name]
            for name in cls._format_names(formats)
            if name in _DISCOGS_MEDIA_MAPPING
        }
    ...
    def _release_type(cls, formats: Any) -> str:
        ...
        return next(
            (
                name
                for name, needle in _DISCOGS_RELEASE_TYPES
    ...
    def _format_descriptions(formats: Any) -> set[str]:
        ...
        for item in formats:
            if isinstance(item, dict):
                descriptions.update(
                    str(value).casefold()
                    for value in item.get("descriptions", [])
                )
        return descriptions


def _set_external_release_type(
    release: MusicRelease, value: str, confidence: float, provider: str
) -> None:
    ...
    if conflict is not None:
        ...
        return
    release.set_field(
        "release_type", release_type, MetadataSource.EXTERNAL, confidence
    )


def _external_single_conflict(
    release: MusicRelease, release_type: str
...
def _record_external_type_conflict(
    release: MusicRelease, provider: str, conflict: tuple[int, float]
) -> None:
