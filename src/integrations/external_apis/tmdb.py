...
from src.integrations.external_apis.tmdb_authentication import build_tmdb_authentication
...
def _apply_config(config: dict[str, Any]) -> None:
    ...
    raw_credential = default_cfg.get("tmdb_access_token") or default_cfg.get("tmdb_api")
...
def _tmdb_client(**kwargs: Any) -> httpx.AsyncClient:
    ...
    if _tmdb_credential is None:
        raise TmdbCredentialMissingError("TMDb API credential is missing. Configure DEFAULT.tmdb_api or DEFAULT.tmdb_access_token in the active runtime config.")
...
async def get_tmdb_id(
    ...
    category_str = str(category_preference or new_category or category_value or "MOVIE").upper()
        ...
        async with _tmdb_client() as client:
            try:
                ...
                if category == "MOVIE":
                    ...
                    params = {"query": filename, "language": "en-US", "include_adult": "true"}
                    ...
                    if len(limited_results) > 1:
                        ...
                        while True:
                            ...
                            try:
                                ...
                                raise OperationAbortedError("TMDb selection was cancelled by the user.") from None
    ...
    while True:
        try:
            ...
            await cleanup_manager.cleanup()
            cleanup_manager.reset_terminal()
            raise OperationAbortedError("TMDb lookup was cancelled because no metadata selection was accepted.") from None

        try:
            parsed_category, parsed_id = parse_tmdb_id(tmdb_input or "", category)
            ...
            logger.info("[bold red]Invalid TMDb ID. Use tv/12345 or movie/12345.[/bold red]")
            continue
        if tmdb_id <= 0:
            logger.info("[bold red]Invalid TMDb ID. Use tv/12345 or movie/12345.[/bold red]")
            continue
...
async def tmdb_other_meta(
    ...
    if isinstance(config, dict):
        ...
        if isinstance(configured_default, dict):
            ...
            raw_credential = configured_default.get("tmdb_access_token") or configured_default.get("tmdb_api")
        ...
        try:
            ...
            if tmdb_id == 0:
                if mode == "cli":
                    ...
                    raise OperationAbortedError("TMDb metadata selection was cancelled by the user.") from None
    ...
    async with _tmdb_client() as client:
        ...
        if category == "MOVIE":
            ...
            year = datetime.strptime(media_data["release_date"], "%Y-%m-%d").replace(tzinfo=UTC).year if media_data["release_date"] else (search_year or 0)
        ...
        if runtime_default_config.get("add_logo", False):
            endpoints.append(client.get(f"{TMDB_BASE_URL}/{('movie' if category == 'MOVIE' else 'tv')}/{tmdb_id}/images", params={}))
        ...
        if runtime_default_config.get("add_logo", False) and logo_data and not isinstance(logo_data, Exception):
            try:
    ...
    original_language = manual_language or original_language_from_tmdb or requested_original_language
...
async def get_genres(response_data: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if response_data is not None:
        ...
        if tmdb_genres:
            ...
            valid_genres = [genre for genre in tmdb_genres if isinstance(genre, dict) and genre.get("name")]
            genre_names = [str(genre["name"]).replace(",", " ") for genre in valid_genres]
            genre_ids = [str(genre["id"]) for genre in valid_genres if genre.get("id") is not None]
...
async def get_tmdb_imdb_from_mediainfo(
    ...
    first_track = tracks[0] if isinstance(tracks, list) and tracks and isinstance(tracks[0], dict) else None
...
async def daily_to_tmdb_season_episode(tmdbid: int, date: str | datetime) -> tuple[int, int]:
    ...
    async with _tmdb_client() as client:
        ...
        try:
            response = await client.get(f"{TMDB_BASE_URL}/tv/{tmdbid}", params={})
        except Exception as error:
            logger.info(f"[bold red]Failed to fetch TV data: {error}[/bold red]")
            return 0, 0
        try:
            response.raise_for_status()
        ...
        try:
            season_response = await client.get(f"{TMDB_BASE_URL}/tv/{tmdbid}/season/{season}", params={})
        except Exception as error:
            logger.info(f"[bold red]Failed to fetch season data: {error}[/bold red]")
            return 0, 0
        try:
...
async def get_season_details(
    ...
    async with _tmdb_client() as client:
        try:
            # Get season details
            response = await client.get(f"{TMDB_BASE_URL}/tv/{tmdb_id}/season/{season_number}", params={"append_to_response": "images,credits"})
            try:
                response.raise_for_status()
    ...
    try:
        ...
        if logo_json:
            ...
            async with _tmdb_client(params=explicit_params) as client:
                ...
                image_response = await client.get(f"{resolved_base_url}/{endpoint}/{tmdb_id}/images")
                try:
        ...
        for language in logo_languages:
            matching_logo = next((logo for logo in logos if logo.get("iso_639_1") == language and logo.get("file_path")), None)
        ...
        if not logo_path:
            null_language_logo = next(
                (logo for logo in logos if (logo.get("iso_639_1") is None or logo.get("iso_639_1") == "") and logo.get("file_path")),
                None,
            )
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if attempt < max_attempts:
                    ...
                    raise RuntimeError(f"Unable to resolve TMDb metadata for ID {meta.tmdb_id}: {e!s}") from e
