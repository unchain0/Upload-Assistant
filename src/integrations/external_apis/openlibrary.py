...
class OpenLibraryManager:
    async def get_author_name(
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if name:
                    await cache.set(
                        "openlibrary", "author", author_id, {"name": name}
                    )
                else:
                    await cache.set(
                        "openlibrary", "author", author_id, {}, negative=True
                    )
                return name
            ...
            logger.debug(
                f"[yellow]Warning: Error fetching author name for {author_id}: {e}[/yellow]"
            )
        return ""
    ...
    async def search_by_work_id(
        ...
        if not is_cache_miss(cached_data) and isinstance(cached_data, dict):
            if cached_data.get("not_found"):
                logger.info(
                    f"{openlibrary_color_str}: Work match not found (cached): {work_id}"
                )
                return None
            logger.info(
                f"{openlibrary_color_str}: Work match found (cached): {work_id}"
            )
            return cached_data
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if title:
                        ...
                        await cache.set(
                            "openlibrary", "work", work_id, metadata
                        )
                        return metadata
                    ...
                    logger.info(
                        f"{openlibrary_color_str}: API returned error status code {resp.status_code} for Work ID: {work_id}"
                    )
                    if resp.status_code == 404:
                        await cache.set(
                            "openlibrary",
            ...
            logger.info(
                f"{openlibrary_color_str}: Network or query error for Work ID {work_id}: {e}"
            )
    ...
    async def search_by_isbn(
        ...
        if not is_cache_miss(cached_data) and isinstance(cached_data, dict):
            if cached_data.get("not_found"):
                logger.info(
                    f"{openlibrary_color_str}: ISBN match not found (cached): {clean_isbn}"
                )
                return None
            logger.info(
                f"{openlibrary_color_str}: ISBN match found (cached): {clean_isbn}"
            )
            return cached_data
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True) as client:
                ...
                if resp.status_code == 200:
                        ...
                        if work_key:
                            ...
                            if metadata:
                                ...
                                if (
                                    ...
                                    and not metadata.get("publisher")
                                ):
                                    metadata["publisher"] = ", ".join(
                                        publishers
                                    )
                                self._add_year(
                                    metadata, details.get("publish_date")
                                )
                                metadata["isbn"] = clean_isbn
                                await cache.set(
                                    "openlibrary", "isbn", clean_isbn, metadata
                                )
                                return metadata
                    ...
                    logger.info(
                        f"{openlibrary_color_str}: API returned error status code {resp.status_code} for ISBN: {clean_isbn}"
                    )
                    if resp.status_code == 404:
                        await cache.set(
                            "openlibrary",
            ...
            logger.info(
                f"{openlibrary_color_str}: Network or query error for ISBN {clean_isbn}: {e}"
            )
