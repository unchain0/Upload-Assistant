...
class MyAnonamouseManager:
    def _parse_torrent_info(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        publisher_value = next(
            (item.get(field) for field in ("publisher", "publisher_info", "publisher_name", "publishers", "pubname") if item.get(field)),
            None,
        )
        ...
        publication_value = next(
            (item.get(field) for field in ("year", "release_year", "publication_year", "published", "publish_date", "publication_date", "released") if item.get(field)),
            None,
        )
