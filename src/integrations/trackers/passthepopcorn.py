    ...
    async def get_tags(self, check_against: Any) -> list[str]:
        ...
        for item in check_against_list:
            if isinstance(item, list):
                normalized_check_against.extend(normalize_tag(x) for x in item if isinstance(x, str) and x.strip())
            elif isinstance(item, str) and item.strip():
                normalized_check_against.extend(normalize_tag(x) for x in item.split(",") if x.strip())
