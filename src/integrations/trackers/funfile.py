...
class FunFile:
    ...
    async def login(self, meta: Meta) -> None:
        ...
        logger.info(
            f"{self.tracker}: Trying to login...", extra={"markup": False}
        )
        response = await self.session.post(
            "https://www.funfile.org/takelogin.php", data=self._login_payload()
        )
        if response.status_code != 302:
            logger.info(
                f"{self.tracker}: Login failed. Status code: {response.status_code}",
                extra={"markup": False},
            )
            return
        logger.info(
            f"{self.tracker}: Login Successful!", extra={"markup": False}
        )
        await self._write_cookie_file(cookie_file)
        logger.info(
            f"{self.tracker}: Saving the cookie file...",
            extra={"markup": False},
        )
    ...
    async def search_existing(self, meta: Meta) -> list[str]:
        ...
        response = await self._authenticated_search_response(
            meta, search_url, response
        )
        if response is None:
            return []
    ...
    async def _authenticated_search_response(
        self, meta: Meta, search_url: str, response: httpx.Response
    ) -> httpx.Response | None:
    ...
    def _requires_login(response: httpx.Response) -> bool:
        return (
            ...
            or "login" in str(response.url).casefold()
            or "login" in response.text.casefold()
        )
    ...
    def _request_checks_enabled(self) -> bool:
        return bool(
            self.config["TRACKERS"][self.tracker].get("check_requests", False)
        )
    ...
    async def _request_search_text(self, meta: Meta) -> str:
        ...
        urls = [
            f"{self.requests_url}?filter=open&category={category}&search={query}"
            for query in queries
        ]
        responses = await asyncio.gather(
            *(self.session.get(url) for url in urls)
        )
    ...
    def _parse_requests(cls, html: str) -> list[dict[str, str]]:
        ...
        return [
            result
            for row in soup.select("td.mf_content table tr")
            if (result := cls._request_row(row)) is not None
        ]
    ...
    def _anime_audio_codec(audio: str) -> str:
        ...
        return next(
            (value for key, value in options if key in audio_desc), "0"
        )
            ...
            data.update(
                {
                    ...
                    "anime_a_codec[]": [
                        "0",
                        *languages.get("anime_a_codec", []),
                    ],
                    "anime_a_ch[]": ["0", *languages.get("anime_a_ch", [])],
                    "anime_a_lang[]": [
                        "0",
                        *languages.get("anime_a_lang", []),
                    ],
                    "anime_s_format[]": [
                        "0",
                        *languages.get("anime_s_format", []),
                    ],
                    "anime_s_type[]": [
                        "0",
                        *languages.get("anime_s_type", []),
                    ],
                    "anime_s_lang[]": [
                        "0",
                        *languages.get("anime_s_lang", []),
                    ],
                }
            )
