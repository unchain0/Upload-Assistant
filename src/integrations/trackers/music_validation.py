...
class MusicValidator:
    ...
    def validate(self, release: MusicRelease) -> list[ValidationIssue]:
        ...
        if (
            release.conflicts.get("artist")
            and str(release.get("artist", "")).casefold() != "various artists"
        ):
            issues.append(
                ValidationIssue(
                    ValidationLevel.WARNING,
    ...
    def validate(self, release: MusicRelease) -> list[ValidationIssue]:
        ...
        for track in release.tracks:
            ...
            if track.format == "FLAC":
                ...
                if (
                    track.sample_rate
                    and track.sample_rate not in self.ALLOWED_SAMPLE_RATES
                ):
                    issues.append(
                        ValidationIssue(
                            ValidationLevel.ERROR,
            ...
            if (
                track.format == "MP3"
                and track.bitrate
                and track.bitrate > 320_000
                and track.bitrate_mode == "CBR"
            ):
                issues.append(
                    ValidationIssue(
                        ValidationLevel.ERROR,
        ...
        if (
            ...
            and str(release.get("release_type", "")) != "Single"
        ):
            issues.append(
                ValidationIssue(
                    ValidationLevel.ERROR,
        ...
        if (
            release.get("media") in {"CD", "SACD", "BD"}
            and release.is_lossless
            and not release.auxiliary.logs
        ):
            issues.append(
                ValidationIssue(
                    ValidationLevel.WARNING,
        ...
        if (
            str(release.get("media", "")).upper() in {"SACD", "BD", "CASSETTE"}
            and not release.auxiliary.lineage
        ):
            issues.append(
                ValidationIssue(
                    ValidationLevel.WARNING,
