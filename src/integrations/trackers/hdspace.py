...
class HDSpace:
    ...
    async def get_additional_checks(self, meta: Meta) -> bool:
        ...
        if not filelist:
            logger.info(
                f"{self.tracker}: No payload files found, skipping upload."
            )
            return False
    ...
    def _codec_policy_passes(self, meta: Meta) -> bool:
        values = (
            str(meta.video_codec or "").casefold(),
            str(meta.video_encode or "").casefold(),
        )
        if not self._contains_forbidden_codec(values):
            return True
        logger.info(
            f"{self.tracker}: xvid/divx codecs are not allowed, skipping the upload..."
        )
        return False
    ...
    def _archive_policy_passes(self, filelist: list[Any]) -> bool:
        ...
        if not archive:
            return True
        logger.info(
            f"{self.tracker}: RAR files are not allowed: {archive}. Skipping upload."
        )
        return False
    ...
    def _screenshot_policy_passes(self, meta: Meta) -> bool:
        ...
        if meta.category in {"MOVIE", "TV"} and len(screenshots) < 3:
            logger.info(
                f"{self.tracker}: HD-Space requires at least 3 valid screenshots for Movie/TV uploads."
            )
            return False
        return all(
            self._screenshot_url_policy_passes(url) for url in screenshots
        )

    def _screenshot_url_policy_passes(self, image_url: str) -> bool:
        if not image_url.lower().startswith(("http://", "https://")):
            logger.info(
                f"{self.tracker}: Screenshot links must use direct HTTP(S) URLs. Found invalid entry: {image_url}."
            )
            return False
        if self._is_png_screenshot(image_url):
            return True
        logger.info(
            f"{self.tracker}: HD-Space requires .png screenshots only, skipping upload."
        )
        return False
    ...
    async def search_existing(self, meta: Meta) -> list[dict[str, str | None]]:
        ...
        if imdb_id == "0":
            logger.info(
                f"{self.tracker}: IMDb ID not found, cannot search for duplicates on {self.tracker}."
            )
            return []
        return await self._search_pages(meta, imdb_id)

    async def _search_pages(
        self, meta: Meta, imdb_id: str
    ...
    async def _search_page_response(
        self, imdb_id: str, current_page: int
    ) -> httpx.Response:
    ...
    async def _search_login_failure(
    ...
    def _is_login_response(response: httpx.Response) -> bool:
        text = response.text
        return (
            "Recover password" in text
            or "page=login" in str(response.url)
            or "page=login" in text
        )

    def _parse_search_page(
        self, html: str, current_page: int
    ) -> tuple[list[dict[str, str | None]], bool] | None:
        parts = html.split("Show/Hide Categories", 1)
        if len(parts) < 2:
            logger.info(
                f"{self.tracker}: [bold yellow]Unexpected page structure on page {current_page}, stopping search[/bold yellow]"
            )
            return None
        ...
        entries = [
            entry
            for row in rows
            if (entry := self._search_row(row)) is not None
        ]
    ...
    def _search_size(row: Any) -> str | None:
        ...
        return next(
            (
                ...
                and re.search(r"([0-9.]+)\s+(GB|MB|KB|B)", text, re.I)
            ),
            None,
        )
    ...
    def _has_next_page(soup: BeautifulSoup, current_page: int) -> bool:
        for link in soup.find_all("a", href=re.compile(r"pages=")):
            if isinstance(link, Tag) and re.search(
                r"Next|>>", link.get_text(" ", strip=True), re.I
            ):
                return True
        return (
            soup.find("a", href=re.compile(rf"pages={current_page + 1}"))
            is not None
        )
    ...
    def _category_group(cls, meta: Meta) -> str:
        ...
        if meta.anime:
            return "ANIME"
        return (
            str(meta.category) if meta.category in {"MOVIE", "TV"} else "OTHER"
        )
    ...
    def _request_entries(cls, html: str) -> list[dict[str, str | None]]:
        ...
        return [
            ...
            for row in soup.select(
                'form[action="index.php?page=takedelreq"] table.lista tr'
            )
            if (entry := cls._request_row(row)) is not None
        ]
    ...
    async def get_nfo(self, meta: Meta) -> dict[str, tuple[str, bytes, str]]:
        ...
        if nfo_files:
            ...
            return {
                "nfo": (nfo_path.name, nfo_bytes, "application/octet-stream")
            }
        return {}
    ...
    def _processed_screenshot(cls, image: Any) -> tuple[str, str] | None:
        ...
        failed_local = (
            local_file
            if local_file and cls._invalid_local_screenshot(local_file)
            else ""
        )
    ...
    def _invalid_local_screenshot(cls, local_file: str) -> bool:
        ...
        return width is not None and not cls._is_allowed_screenshot_width(
            width
        )
    ...
    def _log_dimension_failures(cls, failures: list[str]) -> None:
        if not failures:
            return
        logger.info(
            f"{cls.tracker}: found screenshot(s) with invalid dimensions (must be 1280x*, 1920x*, or 3840x*). Offending: {', '.join(failures)}"
        )
