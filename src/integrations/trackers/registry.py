...
class TrackerSetup:
    ...
    def filter_unsupported_trackers(self, meta: Meta) -> None:
        ...
        meta.trackers = [
            ...
            if self._tracker_is_supported(meta, str(tracker), str(category))
        ]

    def _tracker_is_supported(
        self, meta: Meta, tracker_name: str, category: str
    ) -> bool:
        ...
        if supported is None:
            ...
            logger.info(
                f"{tracker_name}: [bold red]Error: Tracker does not have"
                " 'supported_categories' defined. Removing from queue.[/bold"
                " red]",
                extra={"markup": False},
            )
            return False
        if self._category_supported(category, supported):
            return True
        logger.info(
            f"{tracker_name}: [bold red]category '{category}' is not"
            " supported. Removing from queue.[/bold red]"
        )
        self._mark_tracker_skipped(meta, tracker_name)
        return False
    ...
    def _missing_required_tracker_config(self, tracker_name: str) -> list[str]:
        ...
        example_tracker = example_config.get("TRACKERS", {}).get(
            tracker_name, {}
        )
        ...
        return [
            ...
            if key in example_tracker and not tracker_config.get(key)
        ]

    @staticmethod
    def _log_missing_tracker_config(
        tracker_name: str, missing: list[str]
    ) -> None:
        for label in missing:
            logger.info(
                f"{tracker_name}: [bold red]Tracker is missing an {label} and"
                " will be ignored.[/bold red]"
            )
    ...
    def _tracker_config(self, tracker_name: str) -> dict[str, Any]:
        ...
        value = tracker_map.get(tracker_name, {})
        return cast(dict[str, Any], value) if isinstance(value, dict) else {}

    @staticmethod
    def _category_supported(category: str, supported: Any) -> bool:
        values = (
            cast(tuple[Any, ...] | list[Any], supported)
            if isinstance(supported, tuple | list)
            else []
        )
    ...
    def trackers_enabled(self, meta: Meta) -> list[str]:
        ...
        valid = [
            tracker for tracker in active if self._valid_tracker_name(tracker)
        ]
    ...
    def _normalized_tracker_names(self, meta: Meta) -> list[str]:
        value = (
            meta.trackers
            if meta.trackers is not None
            else self.config["TRACKERS"]["default_trackers"]
        )
    ...
    def _warn_removed_trackers(active: list[str], valid: list[str]) -> None:
        for tracker in set(active) - set(valid):
            logger.warning(
                f"Warning: Tracker '{tracker}' is not recognized and will be"
                " ignored.",
                extra={"markup": False},
            )

    async def get_banned_groups(
        ...
        if url is None:
            return None
        return await self._refresh_banned_groups(
            tracker, instance, url, file_path
        )
    ...
    async def _luminarr_banned_file(
        self, tracker: str, file_path: Path
    ) -> Path | None:
    ...
    async def _fetch_banned_groups(
        self, tracker: str, instance: Any, url: str
    ) -> list[Any] | None:
    ...
    async def _banned_page(
        ...
        try:
            ...
            logger.info(
                f"[red]HTTP Request failed for tracker '{tracker}':"
                f" {error}[/red]"
            )
            return None
    ...
    @staticmethod
    def _banned_params(
        api_key: str, auth_mode: str, cursor: str | None
    ) -> JsonDict:
    ...
    def _parse_banned_page(
        self, response: Any, tracker: str, instance: Any
    ) -> tuple[list[Any], str | None, bool] | None:
    ...
    def _parse_banned_mapping(
        ...
        if not isinstance(meta_info, dict):
            logger.info(
                f"[red]Unexpected 'meta' format: {type(meta_info)}[/red]"
            )
            return None
        meta_map = cast(JsonDict, meta_info)
        cursor = cast(str | None, meta_map.get("next_cursor")) or None
    ...
    def _log_banned_status(status_code: int, tracker: str) -> None:
        if status_code == 404:
            logger.info(
                f"Error: Tracker '{tracker}' returned 404 for the banned"
                " groups API."
            )
        else:
            logger.info(
                f"Error: Received status code {status_code} for tracker"
                f" '{tracker}'."
            )

    async def write_banned_groups_to_file(
        self, file_path: str | Path, json_data: list[Any]
    ) -> None:
        try:
            path = Path(file_path)
            ...
            logger.debug(
                f"File '{file_path}' updated successfully with {len(names)}"
                " groups."
            )
        except Exception as error:
    ...
    @staticmethod
    def _banned_group_file_content(
        names: list[str], raw: list[Any]
    ) -> dict[str, Any]:
        return {
    ...
    async def sync_trash_groups(self, file_path: str | Path) -> None:
        ...
        if not groups:
            logger.debug(
                "[yellow]No groups extracted from TRaSH data.[/yellow]"
            )
            return
        await self.write_banned_groups_to_file(
            file_path, [{"name": group} for group in groups]
        )
    ...
    async def _trash_payload(self) -> JsonDict | None:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    "[red]Failed to fetch TRaSH groups: HTTP"
    ...
    def _trash_group_names(cls, value: Any) -> list[str]:
        specs = cast(list[Any], value) if isinstance(value, list) else []
        groups: list[str] = []
        for spec in specs:
    ...
    def _trash_spec_groups(cls, spec: Any) -> list[str]:
        try:
            ...
            logger.debug(
                "[yellow]Skipped invalid TRaSH specification:"
                f" {error}[/yellow]"
            )
            return []
    ...
    def _release_group_spec_fields(spec: Any) -> JsonDict:
        if not isinstance(spec, dict):
            return {}
        spec_map = cast(JsonDict, spec)
        if spec_map.get("implementation") != "ReleaseGroupSpecification":
            return {}
        fields: Any = spec_map.get("fields") or {}
    ...
    async def check_banned_group(
        self, tracker: str, banned_group_list: list[Any], meta: Meta
    ) -> bool:
        ...
        matched = self._banned_group_match(
            group, banned_group_list, tracker, meta
        )
        if not matched:
            return False
    ...
    async def _dynamic_banned_groups(
        self, tracker: str, meta: Meta
    ) -> list[Any] | None:
        if tracker.upper() not in {
            "AITHER",
            "CAPYBARABR",
            "LST",
            "LUMINARR",
        ...
        if not file_path:
            logger.info(
                f"[bold red]Failed to load banned groups for '{tracker}'."
            )
            return []
        return await self._banned_groups_from_file(tracker, Path(file_path))

    async def _banned_groups_from_file(
        self, tracker: str, file_path: Path
    ) -> list[str]:
        try:
            content = await asyncio.to_thread(self._read_file, file_path)
            ...
            payload_map = (
                cast(JsonDict, payload) if isinstance(payload, dict) else {}
            )
            value = payload_map.get("banned_groups", "")
            return str(value).split(", ") if value else []
        except FileNotFoundError:
            logger.info(
                f"[bold red]Banned group file for '{tracker}' not found."
            )
            return []
        except json.JSONDecodeError:
            logger.info(
                f"[bold red]Failed to parse banned group file for '{tracker}'."
            )
            return []

    @classmethod
    def _banned_group_match(
        cls, group: str, values: list[Any], tracker: str, meta: Meta
    ) -> bool:
        for value in values:
    ...
    def _banned_group_entry(value: Any) -> tuple[str, str]:
        if isinstance(value, list):
            ...
            return (
                (items[0], items[1] if len(items) > 1 else "")
                if items
                else ("", "")
            )
        return str(value), ""
    ...
    def _log_banned_group_match(tracker: str, meta: Meta, note: str) -> None:
        logger.info(
            f"[bold yellow]{str(meta.tag or '')[1:]}[/bold yellow][bold red]"
            " was found on"
            f" [bold yellow]{tracker}'s[/bold yellow] list of banned groups."
        )
        if note:
    ...
    async def _banned_group_decision(self, meta: Meta) -> bool:
        ...
        try:
            ...
            logger.info(
                "\n[yellow]Prompt ended; keeping the banned-group safeguard"
                " active.[/yellow]"
            )
    ...
    async def write_internal_claims_to_file(
        self, file_path: str | Path, data: list[JsonDict]
    ) -> None:
        try:
            path = Path(file_path)
            path.parent.mkdir(parents=True, exist_ok=True)
            extracted = [
                claim
                for item in data
                if (claim := self._claim_record(item)) is not None
            ]
            if not extracted:
            ...
            await asyncio.to_thread(self._write_file, path, content)
            logger.debug(
                f"File '{file_path}' updated successfully with"
    ...
    def _claim_record(item: JsonDict) -> JsonDict | None:
        ...
        if not isinstance(attributes, dict):
            ...
            return None
        attribute_map = cast(JsonDict, attributes)
        return {
            "title": attribute_map.get("title", "Unknown"),
            "season": attribute_map.get("season", "Unknown"),
            "tmdb_id": attribute_map.get("tmdb_id", "Unknown"),
            "resolutions": attribute_map.get("resolutions", []),
            "types": attribute_map.get("types", []),
        }

    @staticmethod
    def _claims_file_content(
        extracted: list[JsonDict], raw: list[JsonDict]
    ) -> JsonDict:
        return {
            "last_updated": datetime.now(UTC).strftime("%Y-%m-%d"),
            "titles_csv": ", ".join(
                str(entry.get("title", "")) for entry in extracted
            ),
    ...
    async def get_torrent_claims(
        ...
        file_path = (
            ...
            / "data"
            / "banned"
            / f"{tracker}_claimed_releases.json"
        )
        instance = self._create_tracker_instance(tracker)
        if instance is None:
            return None
    ...
    @classmethod
    def _parse_claim_page(
        cls, response: Any
    ) -> tuple[list[JsonDict], str | None] | None:
        if response.status_code != 200:
            logger.error(
                "[red]Error: Received status code"
                f" {response.status_code}[/red]"
            )
            return None
        payload = response.json()
    ...
    def _claim_payload_page(
        ...
        if not isinstance(payload, dict):
            ...
            return None
        payload_map = cast(JsonDict, payload)
        page = cls._claim_data_list(payload_map.get("data", []))
        meta_info = cls._claim_meta_mapping(payload_map.get("meta", {}))
    ...
    async def check_tracker_claims(
        self, meta: Meta, tracker: str | list[str]
    ) -> bool:
        trackers = self._tracker_names(tracker)
        results = await asyncio.gather(
            *(
                self._check_single_tracker_claim(meta, name)
    ...
    async def _check_single_tracker_claim(
        self, meta: Meta, tracker_name: str
    ) -> bool:
        try:
            ...
            if instance is None:
                logger.info(
                    f"[red]Tracker {tracker_name} is not registered in"
                    " tracker_class_map[/red]"
                )
                return False
            ...
            logger.error(
                f"[red]Error processing tracker {tracker_name}: {error}[/red]"
            )
            return False

    @staticmethod
    async def _claim_mapping_ids(
        instance: Any, meta: Meta
    ) -> dict[str, list[Any]]:
        type_mapping = cast(
            JsonDict, await instance.get_type_id(meta, mapping_only=True)
        )
        resolution_mapping = cast(
            JsonDict, await instance.get_resolution_id(meta, mapping_only=True)
        )
        return {
            "types": [type_mapping.get(meta.type)] if meta.type else [],
            "resolutions": (
                [resolution_mapping.get(meta.resolution)]
                if meta.resolution
                else []
            ),
    ...
    async def _claim_file_records(
        ...
        path = (
            ...
            / "data"
            / "banned"
            / f"{tracker_name}_claimed_releases.json"
        )
        if not path.exists():
            logger.info(
                f"[red]No claim data file found for {tracker_name}[/red]"
            )
            return []
        ...
        records = payload_map.get("extracted_data", [])
        return (
            TrackerSetup._mapping_items(records)
    ...
    def _matching_claim(
        ...
        for claim in claims:
            if cls._claim_matches(meta, claim, ids):
                logger.info(
                    f"[green]Claimed match found at [cyan]{tracker}:"
                    f" [yellow]{claim.get('title')}, Season:"
                    f" {claim.get('season')}, TMDB ID:"
                    f" {claim.get('tmdb_id')}[/green]"
                )
                return True
        return False
    ...
    @classmethod
    def _claim_matches(
        cls, meta: Meta, claim: JsonDict, ids: dict[str, list[Any]]
    ) -> bool:
        checks = (
            cls._claim_tmdb_matches(claim, ids),
            cls._claim_season_matches(meta, claim),
            cls._claim_values_match(
                claim.get("resolutions", []), ids["resolutions"]
            ),
    ...
    @staticmethod
    def _claim_tmdb_matches(
        claim: JsonDict, ids: dict[str, list[Any]]
    ) -> bool:
        return claim.get("tmdb_id") in ids["tmdb"]

    @staticmethod
    def _claim_season_matches(meta: Meta, claim: JsonDict) -> bool:
        return meta.category == "MOVIE" or claim.get("season") == (
            meta.season_int or 0
        )

    @staticmethod
    def _claim_values_match(value: Any, expected: list[Any]) -> bool:
        return isinstance(value, list) and all(
            item in value for item in expected
        )

    async def get_tracker_requests(
        self, meta: Meta, tracker: str, url: str
    ) -> list[JsonDict]:
        logger.debug(
            f"[bold green]Searching for existing requests on {tracker}[/bold"
            " green]"
        )
        ...
        payload = await self._request_payload(
            "GET", url, tracker, headers=headers, params=params
        )
    ...
    async def _request_payload(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                f"[bold red]Unable to search for existing torrents: {error}"
            )
            return None
        except Exception as error:
    ...
    @staticmethod
    def _response_payload_or_none(
        response: Any, tracker: str
    ) -> JsonDict | None:
        if response.status_code != 200:
            logger.info(
                f"[bold red]Failed to search torrents on {tracker}. HTTP"
        ...
        if not isinstance(payload, dict):
            logger.info(
                f"[bold red]Unexpected response format: {type(payload)}[/bold"
                " red]"
            )
            return None
    ...
    @classmethod
    def _unit3d_request_results(
        cls, tracker: str, payload: JsonDict | None
    ) -> list[JsonDict]:
        return [
            cls._unit3d_request_result(tracker, item)
            for item in cls._request_result_items(payload)
        ]

    @staticmethod
    def _unit3d_request_result(tracker: str, item: JsonDict) -> JsonDict:
        attributes = (
            item.get("attributes", item) if tracker == "HAWKEUNO" else item
        )
        attrs = (
            cast(JsonDict, attributes) if isinstance(attributes, dict) else {}
        )
        return {
    ...
    async def bhd_request_check(
        self, meta: Meta, tracker: str, url: str
    ) -> list[JsonDict]:
        if not self._beyondhd_configured():
            logger.info(
                "[red]BEYONDHD API key not configured. Skipping BEYONDHD"
                " request check.[/red]"
            )
            return []
        logger.debug(
        ...
        payload = await self._request_payload(
            "POST", url, tracker, params=params
        )
        return [
            self._beyondhd_request_result(item)
            for item in self._request_result_items(payload)
        ]
    ...
    def _beyondhd_configured(self) -> bool:
        ...
        if not isinstance(trackers, dict):
            return False
        tracker_map = cast(dict[str, Any], trackers)
        value = tracker_map.get("BEYONDHD")
        return isinstance(value, dict) and bool(
            cast(JsonDict, value).get("api_key")
        )
    ...
    async def tracker_request(
        self, meta: Meta, tracker: str | list[str]
    ) -> bool:
        trackers = self._tracker_names(tracker)
        results = await asyncio.gather(
            *(self._process_tracker_request(meta, name) for name in trackers)
        )
        return any(results)

    async def _process_tracker_request(
        self, meta: Meta, tracker_name: str
    ) -> bool | list[JsonDict]:
        instance = self._create_tracker_instance(tracker_name)
        if instance is None:
            ...
            return False
        custom = await self._custom_tracker_request(
            meta, tracker_name, instance
        )
        ...
        request_data, existing_uuids, log_path = await self._load_request_log(
            meta, tracker_name
        )
        self._process_request_entries(
            meta,
            tracker_name,
            requests,
            url,
    ...
    async def _custom_tracker_request(
        self, meta: Meta, tracker_name: str, instance: Any
    ) -> bool | None:
        custom_trackers = {
            "AMIGOSSHARE",
            "BJSHARE",
    ...
    async def _request_context(
        ...
        if tracker_name == "BEYONDHD":
            if not isinstance(url, str):
                return None
            return (
                await self.bhd_request_check(meta, tracker_name, url),
                url,
                {},
            )
        if not isinstance(url, str):
    ...
    @classmethod
    async def _request_mapping_ids(
        cls, meta: Meta, instance: Any
    ) -> dict[str, list[Any]]:
        return {
            "types": await cls._mapped_id(
                instance.get_type_id, meta, meta.type, "Type"
            ),
            "resolutions": await cls._mapped_id(
                instance.get_resolution_id, meta, meta.resolution, "Resolution"
            ),
            "categories": await cls._mapped_id(
                instance.get_category_id, meta, meta.category, "Category"
            ),
        }

    @staticmethod
    async def _mapped_id(
        method: Any, meta: Meta, name: Any, label: str
    ) -> list[Any]:
        mapping = cast(JsonDict, await method(meta, mapping_only=True))
        values = [mapping.get(name)] if name else []
        if None in values:
            logger.warning(
                f"[yellow]Warning: {label} in meta not found in tracker"
                " mapping.[/yellow]"
            )
        return values

    async def _load_request_log(
        self, meta: Meta, tracker_name: str
    ) -> tuple[list[JsonDict], set[str], Path]:
        log_path = (
            Path(meta.base_dir)
            / "tmp"
            / f"{tracker_name}_request_results.json"
        )
    ...
    async def _save_request_log(
        log_path: Path, request_data: list[JsonDict]
    ) -> None:
        if not request_data:
            return
    ...
    def _process_request_entries(
        ...
        for request in requests:
            if tracker_name == "BEYONDHD":
                self._process_beyondhd_request(
                    meta, tracker_name, request, request_data, existing_uuids
                )
            else:
                self._process_unit3d_request(
                    meta,
                    tracker_name,
                    request,
                    url,
    ...
    def _process_unit3d_request(
        ...
        request_url = re.sub(
            r"/api/requests/filter$", f"/requests/{request.get('id')}", url
        )
        self._log_unit3d_request(
            meta, tracker_name, request, request_url, match
        )
        if not request.get("claimed"):
            self._append_request_log(
                meta, request_data, existing_uuids, request, request_url, match
            )

    @staticmethod
    def _request_category_matches(
        request: JsonDict, mappings: dict[str, list[Any]]
    ) -> bool:
        expected = {str(value) for value in mappings.get("categories", [])}
        return str(request.get("category")) in expected

    @classmethod
    def _unit3d_request_match(
        cls, meta: Meta, request: JsonDict, mappings: dict[str, list[Any]]
    ) -> str:
        type_match, type_any = cls._mapped_or_any(
            request.get("type"), mappings.get("types", [])
        )
        resolution_match, resolution_any = cls._mapped_or_any(
            request.get("resolution"), mappings.get("resolutions", [])
        )
        exact = cls._unit3d_exact_request(
            meta, request, type_match, resolution_match
        )
        if exact:
    ...
    def _episode_request_matches(meta: Meta, request: JsonDict) -> bool:
        ...
        return bool(
            season
            and episode
            and season == meta.season_int
            and episode == meta.episode_int
        )
    ...
    @classmethod
    def _log_unit3d_request(
        cls, meta: Meta, tracker: str, request: JsonDict, url: str, match: str
    ) -> None:
        exact = match in {"exact", "double_check"}
        label = "exact request match" if exact else "request"
        logger.info(
            f"[bold blue]Found {label} on [bold yellow]{tracker}[/bold yellow]"
            f" with bounty [bold yellow]{request.get('bounty')}[/bold yellow]"
            f" and with status [bold yellow]{request.get('status')}[/bold"
            " yellow][/bold blue]"
        )
        logger.info(
            "[bold blue]Claimed status:[/bold blue] [bold"
            f" yellow]{request.get('claimed')}[/bold yellow]"
        )
        cls._log_request_name(meta, request, url)
        if match == "double_check":
            logger.info(
                "[bold red]Type and/or resolution was set to ANY, double check"
                " any description requirements:[/bold red]"
            )
        if match != "exact":
            logger.info(
                "[bold green]Request desc:"
                f" {str(request.get('description') or '')[:100]}[/bold green]"
            )
    ...
    def _log_request_name(meta: Meta, request: JsonDict, url: str) -> None:
        ...
        episode = TrackerSetup._safe_request_int(request.get("episode"))
        logger.info(
            f"[bold yellow]{name}[/bold yellow] - [bold yellow]S{season:02d}"
            f" E{episode:02d}:[/bold yellow] {url}"
        )
    ...
    @staticmethod
    def _request_log_appendable(
        uuid_value: str, existing_uuids: set[str]
    ) -> bool:
        return bool(uuid_value and uuid_value not in existing_uuids)

    @staticmethod
    def _request_log_entry(
        meta: Meta, request: JsonDict, url: str, match: str
    ) -> JsonDict:
        entry: JsonDict = {
    ...
    def _process_beyondhd_request(
        ...
        if match in {"exact", "hdr_mismatch"}:
            self._append_beyondhd_request_log(
                meta, request_data, existing_uuids, request, state
            )

    @classmethod
    def _beyondhd_request_state(
        cls, meta: Meta, request: JsonDict
    ) -> JsonDict:
        resolution = str(request.get("resolution") or "")
        return {
            "unclaimed": request.get("status") == 1,
            "internal": request.get("internal") == 1,
            "claimed_status": cls._beyondhd_claimed_status(
                request.get("status")
            ),
            "season": cls._beyondhd_season_matches(
                meta, str(request.get("name") or "")
            ),
            ...
            "type": cls._beyondhd_type_matches(
                meta, str(request.get("type") or ""), resolution
            ),
    ...
    @classmethod
    def _beyondhd_resolution_matches(
        cls, meta: Meta, request_resolution: str
    ) -> bool:
    ...
    def _beyondhd_remux_resolution(meta: Meta, lowered: str) -> bool:
        if meta.type != "REMUX":
            return False
        return ("uhd" in lowered and meta.resolution == "2160p") or (
            "uhd" not in lowered and meta.resolution == "1080p"
        )
    ...
    @classmethod
    def _beyondhd_type_matches(
        cls, meta: Meta, request_type: str, request_resolution: str
    ) -> bool:
        if cls._beyondhd_remux_type_matches(meta, request_resolution):
            return True
        return cls._beyondhd_source_type_matches(
            str(meta.type or ""), request_type
        )

    @staticmethod
    def _beyondhd_remux_type_matches(
        meta: Meta, request_resolution: str
    ) -> bool:
        return "remux" in request_resolution.lower() and meta.type == "REMUX"

    @staticmethod
    def _beyondhd_source_type_matches(
        meta_type: str, request_type: str
    ) -> bool:
    ...
    def _beyondhd_request_eligible(cls, meta: Meta, state: JsonDict) -> bool:
        return cls._beyondhd_request_fields_ok(
            state
        ) and cls._beyondhd_request_category_ok(meta, state)
    ...
    def _log_beyondhd_request(
        ...
        logger.info(
            cls._beyondhd_request_log_line(tracker, request, state, match)
        )
        cls._log_internal_request(state)
        cls._log_beyondhd_request_name(meta, request)

    @staticmethod
    def _beyondhd_request_log_line(
        tracker: str, request: JsonDict, state: JsonDict, match: str
    ) -> str:
        bounty = request.get("bounty")
        status = state["claimed_status"]
        if match == "exact":
            return (
                "[bold blue]Found exact request match on [bold"
                f" yellow]{tracker}[/bold yellow] with bounty [bold"
                f" yellow]{bounty}[/bold yellow] and with status [bold"
                f" yellow]{status}[/bold yellow][/bold blue]"
            )
        if match == "hdr_mismatch":
            return (
                "[bold blue]Found request match on [bold"
                f" yellow]{tracker}[/bold yellow] with bounty [bold"
                f" yellow]{bounty}[/bold yellow] with mismatched HDR or"
                " DV[/bold blue]"
            )
        return (
            f"[bold blue]Found request on [bold yellow]{tracker}[/bold yellow]"
            f" with bounty [bold yellow]{bounty}[/bold yellow] and with status"
            f" [bold yellow]{status}[/bold yellow][/bold blue]"
        )
    ...
    def _log_beyondhd_request_name(meta: Meta, request: JsonDict) -> None:
        ...
        logger.info(
            f"[bold yellow]{name}[/bold yellow]{suffix}: {request.get('url')}"
        )
    ...
    async def process_trumpables(self, meta: Meta, tracker: str) -> bool:
        ...
        if tracker == "LST":
            logger.debug(
                "[bold green]LST does not support searching existing trump"
                " reports[/bold green]"
            )
            return True
        if not await self._existing_trump_reports_allow_upload(
            meta, tracker, url, reported_torrent_id
        ):
            return False
        return await self._collect_trump_comparisons(meta, tracker)

    def _trumpable_context(
        self, meta: Meta, tracker: str
    ) -> tuple[str, str] | None:
        instance = self._create_tracker_instance(tracker)
        if instance is None:
            logger.info(
                f"[red]Tracker {tracker} is not registered in"
                " tracker_class_map[/red]"
            )
            return None
        ...
        if not isinstance(url, str):
            logger.info(
                f"[red]Tracker {tracker} does not support trumping"
                " reports.[/red]"
            )
            return None
        ...
        if not reported:
            logger.info(
                "[red]No reported torrent ID found in meta for trumpable"
                f" processing on {tracker}[/red]"
            )
            return None
        return url, reported
    ...
    def _reported_torrent_id(cls, meta: Meta, tracker: str) -> str:
        ...
        return (
            matched
            if matched
            else cls._first_matched_episode_id(meta, tracker)
        )
    ...
    def _first_matched_episode_id(meta: Meta, tracker: str) -> str:
        ...
        if not isinstance(episodes, list) or not episodes:
            return ""
        values = cast(list[Any], episodes)
        first = values[0]
        return (
            str(cast(JsonDict, first).get("id", ""))
            if isinstance(first, dict)
            else ""
        )

    async def _existing_trump_reports_allow_upload(
        self, meta: Meta, tracker: str, url: str, reported_id: str
    ) -> bool:
        self._ensure_skip_upload_trackers(meta)
        reports, status = await self.get_tracker_trumps(
            tracker, url, reported_id
        )
        if status != 200:
        ...
        if not reports:
            logger.debug(
                "[bold green]Will make a trumpable report for this upload at"
                f" {tracker}[/bold green]"
            )
            return True
    ...
    @classmethod
    def _mark_trump_api_failure(
        cls, meta: Meta, tracker: str, status: int | None
    ) -> None:
        logger.info(
            f"[bold red]Failed to retrieve trumping reports from {tracker}."
            f" HTTP Status: {status}[/bold red]"
        )
        logger.info(
            f"[bold red]Marking {tracker} to be skipped due to API"
            " failure[/bold red]"
        )
    ...
    def _log_trump_reports(cls, tracker: str, reports: list[JsonDict]) -> None:
        logger.info(
            f"[bold yellow]Found {len(reports)} existing trumping report/s on"
            f" {tracker} for this release[/bold yellow]"
        )
    ...
    def _log_trump_report(report: JsonDict) -> None:
        logger.info(
            f"  [cyan]Report ID:[/cyan] {report.get('id')} -"
            f" [cyan]Title:[/cyan] {report.get('title')}"
        )
        torrents = report.get("trumping_torrent", [])
        if not isinstance(torrents, list) or not torrents:
            logger.info(
                "  [yellow]The trumping torrent for this report seems to be in"
                " modq.....[/yellow]"
            )
            return
        for torrent in cast(list[Any], torrents):
            if isinstance(torrent, dict):
                torrent_map = cast(JsonDict, torrent)
                logger.info(
                    "  [bold green]Already being trumped by:[/bold green]"
                    f" {torrent_map.get('name', 'Unknown')} (ID:"
                    f" {torrent_map.get('id', 'N/A')})"
                )
    ...
    def _confirm_existing_trump_upload(cls, meta: Meta, tracker: str) -> bool:
        try:
            ...
            logger.info(
                "[yellow]Prompt cancelled; treating as 'no' for"
                " safety.[/yellow]"
            )
            confirmed = False
        if confirmed:
            logger.info(
                "[bold green]Proceeding with upload despite existing trumping"
                f" reports on {tracker}[/bold green]"
            )
            return True
    ...
    async def _collect_trump_comparisons(
        self, meta: Meta, tracker: str
    ) -> bool:
        if meta.tv_pack:
            logger.debug(
                "[bold green]TV pack upload detected, skipping comparison"
                f" images for trump report on {tracker}[/bold green]"
            )
            return True
        logger.info(
            f"[yellow]{tracker} requires comparisons to be provided for trump"
            " reports.\nAre the comparison images in the description or are"
            " you adding links?"
        )
        ...
        logger.info(
            "[yellow]Skipping trump report creation as no comparison method"
            " provided.[/yellow]"
        )
        return False
    ...
    def _comparison_mode() -> str | None:
        try:
            value = cli_ui.ask_string(
                "Enter 'd' if in description, 'L' if you want to paste links,"
                " or press Enter to skip trumping:",
                default="",
            )
        except EOFError, KeyboardInterrupt:
            logger.info(
                "[yellow]Prompt cancelled; skipping trump report"
                " creation.[/yellow]"
            )
            return None
    ...
    def _collect_comparison_links(cls, meta: Meta) -> bool:
        ...
        if not reported_links or not trumping_links:
            logger.info(
                "[yellow]No valid screenshot links provided. Skipping trump"
                " report creation.[/yellow]"
            )
            return False
    ...
    def _comparison_link_prompts(cls) -> tuple[str, str] | None:
        ...
        logger.info(
            "[yellow]No screenshot links provided. Skipping trump report"
            " creation.[/yellow]"
        )
        return None
    ...
    def _prompt_comparison_link_pair() -> tuple[str, str] | None:
        try:
            reported = str(
                cli_ui.ask_string(
                    "Paste screenshot links for the reported torrent"
                    " (comma-separated):",
                    default="",
                )
                or ""
            ...
            trumping = str(
                cli_ui.ask_string(
                    "Paste screenshot links for the trumping torrent"
                    " (comma-separated):",
                    default="",
                )
                or ""
    ...
    async def get_tracker_trumps(
        self, tracker: str, url: str, reported_torrent_id: str
    ) -> tuple[list[JsonDict], int | None]:
        logger.debug(
            f"[bold green]Searching for trumps on {tracker}[/bold green]"
        )
        headers = self._trump_headers(tracker)
        params: JsonDict = {"reported_torrent_id": str(reported_torrent_id)}
        data, status = await self._fetch_trump_pages(
            tracker, url, headers, params
        )
    ...
    async def _fetch_trump_pages(
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                return await self._trump_page_loop(
                    client, tracker, url, headers, params
                )
        except httpx.TimeoutException:
    ...
    async def _trump_page_loop(
        ...
        while True:
            page = await self._trump_page(
                client, tracker, url, headers, params, cursor
            )
            if page is None:
                return data, status
            ...
            if not cursor:
                return data, status
            logger.info(
                f"[cyan]Fetched {len(page_data)} trumping reports, waiting 1"
                " second before next page...[/cyan]"
            )
    ...
    async def _trump_page(
        ...
        if response.status_code != 200:
            logger.info(
                f"[bold red]Failed to search trumps on {tracker}. HTTP Status:"
                f" {response.status_code} - {response.text}[/bold red]"
            )
            return [], None, response.status_code
        return self._parse_trump_page(response)

    @classmethod
    def _parse_trump_page(
        cls, response: Any
    ) -> tuple[list[JsonDict], str | None, int] | None:
        payload = response.json()
        if not isinstance(payload, dict):
            ...
            return None
        payload_map = cast(JsonDict, payload)
        data = cls._request_result_items(payload_map)
        meta_info = payload_map.get("meta", {})
        if not isinstance(meta_info, dict):
            logger.info(
                f"[bold red]Unexpected 'meta' format: {type(meta_info)}[/bold"
                " red]"
            )
    ...
    def _trump_result(cls, entry: JsonDict) -> JsonDict:
        return {
            ...
            "trumping_torrent": cls._normalized_trumping_torrents(
                entry.get("trumping_torrent")
            ),
        }
    ...
    async def make_trumpable_report(self, meta: Meta, tracker: str) -> bool:
        ...
        logger.debug(
            f"[bold green]Creating trump report on {tracker}[/bold green]"
        )
        context = self._trump_report_context(meta, tracker)
        if context is None:
            return False
        create_url, reported_id, trumping_id = context
        payload = self._trump_report_payload(
            meta, tracker, reported_id, trumping_id
        )
        if payload is None:
            return False
        if meta.debug:
    ...
    def _trump_report_context(
        ...
        return self._validated_trump_context(
            meta, create_url, reported_id, trumping_id
        )
    ...
    def _stored_reported_torrent_id(meta: Meta, tracker: str) -> str:
        ...
        if not value:
            logger.info(
                "[red]No reported torrent ID found in meta for trump report"
                f" creation on {tracker}[/red]"
            )
        return value

    @staticmethod
    def _validated_trump_context(
        meta: Meta, create_url: str, reported_id: str, trumping_id: Any
    ) -> tuple[str, str, Any] | None:
    ...
    @staticmethod
    def _trump_create_url(
        tracker: str, base_url: str, reported_id: str
    ) -> str | None:
        if tracker != "LST":
        ...
        if not value.isdigit():
            logger.info(
                "[red]Invalid or missing reported torrent ID for LST:"
                f" {reported_id}[/red]"
            )
            return None
    ...
    def _trumping_torrent_id(meta: Meta, tracker: str) -> Any | None:
        ...
        status_map = (
            cast(dict[str, Any], status) if isinstance(status, dict) else {}
        )
        tracker_status = status_map.get(tracker, {})
        if isinstance(tracker_status, dict) and "torrent_id" in tracker_status:
            return cast(JsonDict, tracker_status)["torrent_id"]
        logger.info(
            "[red]No torrent ID found in meta for trumping torrent on"
            f" {tracker}[/red]"
        )
        logger.info("[red]Either the upload failed, or you're in debug[/red]")
        return None

    def _trump_report_payload(
        self, meta: Meta, tracker: str, reported_id: str, trumping_id: Any
    ) -> JsonDict | None:
        message = self._trump_message(meta)
        if tracker == "LST":
            return self._lst_trump_payload(meta, message, trumping_id)
        return self._standard_trump_payload(
            meta, reported_id, trumping_id, message
        )
    ...
    def _trump_message(meta: Meta) -> str:
        ...
        return (
            f"{meta.ua_name} is trumping this torrent for reasons"
            f" {meta.ua_name} has not correctly caught. User selected yes at a"
            " prompt."
        )

    @staticmethod
    def _standard_trump_payload(
        meta: Meta, reported_id: str, trumping_id: Any, message: str
    ) -> JsonDict:
        payload: JsonDict = {
            "reported_torrent_id": reported_id,
            "trumping_torrent_id": trumping_id,
            "message": message,
        }
    ...
    def _append_trump_screenshots(payload: JsonDict, meta: Meta) -> None:
        if "screenshots_reported_torrent" in meta:
            payload["screenshots_reported_torrent"] = ",".join(
                cast(list[str], meta.screenshots_reported_torrent)
            )
        if "screenshots_trumping_torrent" in meta:
            payload["screenshots_trumping_torrent"] = ",".join(
                cast(list[str], meta.screenshots_trumping_torrent)
            )
        if (
            "screenshots_in_description" in meta
            and meta.screenshots_in_description
        ):
            payload["message"] = (
                f"{payload.get('message', '')} - User says comparison"
                " screenshots are in description."
            )

    @staticmethod
    def _lst_trump_payload(
        meta: Meta, message: str, trumping_id: Any
    ) -> JsonDict | None:
        if not meta.tv_pack:
            user_message = TrackerSetup._lst_user_message()
            message = (
                f"{message}:"
                f" {user_message or 'No additional message provided by user'}"
            )
    ...
    def _lst_user_message() -> str | None:
        try:
            value = cli_ui.ask_string(
                "Enter a reason for the trump report on LST:"
            )
            return str(value) if value else None
        except EOFError, KeyboardInterrupt:
            logger.info(
                "[yellow]Prompt cancelled; no additional message"
                " provided.[/yellow]"
            )
            return None
    ...
    async def _post_trump_report(
        self, tracker: str, create_url: str, payload: JsonDict
    ) -> bool:
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.post(
                    url=create_url,
                    headers=self._trump_report_headers(tracker),
                    json=payload,
                )
            return self._trump_post_success(tracker, response)
        except httpx.TimeoutException:
            logger.info(
                "[bold red]Request timed out after 10 seconds[/bold red]"
            )
            return False
        except httpx.RequestError as error:
            logger.info(f"[bold red]HTTP Request failed: {error}[/bold red]")
    ...
    def _trump_post_success(tracker: str, response: Any) -> bool:
        if response.status_code in {200, 201}:
            logger.info(
                "[bold green]Successfully created trump report on"
                f" {tracker}[/bold green]"
            )
            return True
        logger.info(
            "[bold red]Failed to create trump report. HTTP Status:"
    ...
    def _log_debug_trump_report(create_url: str, payload: JsonDict) -> None:
        logger.info(
            "[bold yellow]Debug mode enabled, skipping actual trump report"
            " creation.[/bold yellow]"
        )
...
def get_tracker_comment_hosts(
    ...
    for tracker_name, tracker_class in tracker_class_map.items():
        domains = _comment_hosts_for_tracker(
            tracker_name, tracker_class, tracker_config
        )
        if domains:
...
def _comment_hosts_for_tracker(
    tracker_name: str, tracker_class: Any, tracker_config: dict[str, Any]
) -> tuple[str, ...]:
    values = [getattr(tracker_class, "base_url", "")]
    values.extend(_tracker_class_comment_values(tracker_class))
    values.extend(
        _configured_comment_values(tracker_config.get(tracker_name, {}))
    )
...
def _tracker_class_comment_values(tracker_class: Any) -> list[Any]:
    ...
    for attribute_name in ("comment_hosts", "tracker_urls"):
        values.extend(
            _as_comment_values(getattr(tracker_class, attribute_name, ()))
        )
    return values
...
def _configured_comment_values(value: Any) -> list[Any]:
    ...
    return [mapping.get("base_url", ""), mapping.get("announce_url", "")]
...
api_trackers: set[str] = {
    ...
    if getattr(cls, "auth_type", None) == "unit3d_api"
}
other_api_trackers: set[str] = {
    name
    for name, cls in tracker_class_map.items()
    if getattr(cls, "auth_type", None) == "other_api"
}
http_trackers: set[str] = {
    name
    for name, cls in tracker_class_map.items()
    if getattr(cls, "auth_type", None) == "cookies"
}
