...
class TrackerSetup:
    ...
    def _log_missing_tracker_config(
        ...
        for label in missing:
            logger.info(
                f"{tracker_name}: [bold red]Tracker is missing an {label} and will be ignored.[/bold red]"
            )
    ...
    def _warn_removed_trackers(active: list[str], valid: list[str]) -> None:
        for tracker in set(active) - set(valid):
            logger.warning(
                f"Warning: Tracker '{tracker}' is not recognized and will be ignored.",
                extra={"markup": False},
            )
    ...
    async def _banned_page(
        ...
        try:
            ...
            logger.info(
                f"[red]HTTP Request failed for tracker '{tracker}': {error}[/red]"
            )
            return None
    ...
    def _log_banned_status(status_code: int, tracker: str) -> None:
        if status_code == 404:
            logger.info(
                f"Error: Tracker '{tracker}' returned 404 for the banned groups API."
            )
        else:
            logger.info(
                f"Error: Received status code {status_code} for tracker '{tracker}'."
            )
    ...
    async def write_banned_groups_to_file(
        ...
        try:
            ...
            logger.debug(
                f"File '{file_path}' updated successfully with {len(names)} groups."
            )
        except Exception as error:
    ...
    async def _trash_payload(self) -> JsonDict | None:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"[red]Failed to fetch TRaSH groups: HTTP {response.status_code}[/red]"
                )
                return None
            payload = response.json()
    ...
    def _trash_spec_groups(cls, spec: Any) -> list[str]:
        try:
            ...
            logger.debug(
                f"[yellow]Skipped invalid TRaSH specification: {error}[/yellow]"
            )
            return []
    ...
    def _log_banned_group_match(tracker: str, meta: Meta, note: str) -> None:
        logger.info(
            f"[bold yellow]{str(meta.tag or '')[1:]}[/bold yellow][bold red] was found on [bold yellow]{tracker}'s[/bold yellow] list of banned groups."
        )
        if note:
    ...
    async def write_internal_claims_to_file(
        ...
        try:
            ...
            await asyncio.to_thread(self._write_file, path, content)
            logger.debug(
                f"File '{file_path}' updated successfully with {len(extracted)} claims."
            )
        except Exception as error:
            logger.info(f"An error occurred: {error}")
    ...
    async def _check_single_tracker_claim(
        ...
        try:
            ...
            if instance is None:
                logger.info(
                    f"[red]Tracker {tracker_name} is not registered in tracker_class_map[/red]"
                )
                return False
    ...
    async def get_tracker_requests(
        ...
        logger.debug(
            f"[bold green]Searching for existing requests on {tracker}[/bold green]"
        )
    ...
    def _response_payload_or_none(
        ...
        if response.status_code != 200:
            logger.info(
                f"[bold red]Failed to search torrents on {tracker}. HTTP Status: {response.status_code}"
            )
            return None
        payload = response.json()
        if not isinstance(payload, dict):
            logger.info(
                f"[bold red]Unexpected response format: {type(payload)}[/bold red]"
            )
            return None
    ...
    async def bhd_request_check(
        ...
        if not self._beyondhd_configured():
            logger.info(
                "[red]BEYONDHD API key not configured. Skipping BEYONDHD request check.[/red]"
            )
            return []
        logger.debug(
            f"[bold green]Searching for existing requests on {tracker}[/bold green]"
    ...
    async def _mapped_id(
        ...
        if None in values:
            logger.warning(
                f"[yellow]Warning: {label} in meta not found in tracker mapping.[/yellow]"
            )
        return values
    ...
    def _log_unit3d_request(
        ...
        logger.info(
            f"[bold blue]Claimed status:[/bold blue] [bold yellow]{request.get('claimed')}[/bold yellow]"
        )
        cls._log_request_name(meta, request, url)
        if match == "double_check":
            logger.info(
                "[bold red]Type and/or resolution was set to ANY, double check any description requirements:[/bold red]"
            )
        if match != "exact":
            logger.info(
                f"[bold green]Request desc: {str(request.get('description') or '')[:100]}[/bold green]"
            )
    ...
    def _log_request_name(meta: Meta, request: JsonDict, url: str) -> None:
        ...
        episode = TrackerSetup._safe_request_int(request.get("episode"))
        logger.info(
            f"[bold yellow]{name}[/bold yellow] - [bold yellow]S{season:02d} E{episode:02d}:[/bold yellow] {url}"
        )
    ...
    async def process_trumpables(self, meta: Meta, tracker: str) -> bool:
        ...
        if tracker == "LST":
            logger.debug(
                "[bold green]LST does not support searching existing trump reports[/bold green]"
            )
            return True
    ...
    def _trumpable_context(
        ...
        if instance is None:
            logger.info(
                f"[red]Tracker {tracker} is not registered in tracker_class_map[/red]"
            )
            return None
        url = getattr(instance, "trumping_url", None)
        if not isinstance(url, str):
            logger.info(
                f"[red]Tracker {tracker} does not support trumping reports.[/red]"
            )
            return None
        reported = self._reported_torrent_id(meta, tracker)
        if not reported:
            logger.info(
                f"[red]No reported torrent ID found in meta for trumpable processing on {tracker}[/red]"
            )
            return None
        return url, reported
    ...
    async def _existing_trump_reports_allow_upload(
        ...
        if not reports:
            logger.debug(
                f"[bold green]Will make a trumpable report for this upload at {tracker}[/bold green]"
            )
            return True
    ...
    def _mark_trump_api_failure(
        ...
        logger.info(
            f"[bold red]Failed to retrieve trumping reports from {tracker}. HTTP Status: {status}[/bold red]"
        )
        logger.info(
            f"[bold red]Marking {tracker} to be skipped due to API failure[/bold red]"
        )
    ...
    def _log_trump_reports(cls, tracker: str, reports: list[JsonDict]) -> None:
        logger.info(
            f"[bold yellow]Found {len(reports)} existing trumping report/s on {tracker} for this release[/bold yellow]"
        )
    ...
    def _log_trump_report(report: JsonDict) -> None:
        logger.info(
            f"  [cyan]Report ID:[/cyan] {report.get('id')} - [cyan]Title:[/cyan] {report.get('title')}"
        )
        torrents = report.get("trumping_torrent", [])
        if not isinstance(torrents, list) or not torrents:
            logger.info(
                "  [yellow]The trumping torrent for this report seems to be in modq.....[/yellow]"
            )
            return
        for torrent in cast(list[Any], torrents):
            if isinstance(torrent, dict):
                ...
                logger.info(
                    f"  [bold green]Already being trumped by:[/bold green] {torrent_map.get('name', 'Unknown')} (ID: {torrent_map.get('id', 'N/A')})"
                )
    ...
    def _confirm_existing_trump_upload(cls, meta: Meta, tracker: str) -> bool:
        try:
            ...
            logger.info(
                "[yellow]Prompt cancelled; treating as 'no' for safety.[/yellow]"
            )
            confirmed = False
        if confirmed:
            logger.info(
                f"[bold green]Proceeding with upload despite existing trumping reports on {tracker}[/bold green]"
            )
            return True
    ...
    async def _collect_trump_comparisons(
        ...
        if meta.tv_pack:
            logger.debug(
                f"[bold green]TV pack upload detected, skipping comparison images for trump report on {tracker}[/bold green]"
            )
            return True
        logger.info(
            f"[yellow]{tracker} requires comparisons to be provided for trump reports.\nAre the comparison images in the description or are you adding links?"
        )
        ...
        logger.info(
            "[yellow]Skipping trump report creation as no comparison method provided.[/yellow]"
        )
        return False
    ...
    def _comparison_mode() -> str | None:
        try:
            ...
            logger.info(
                "[yellow]Prompt cancelled; skipping trump report creation.[/yellow]"
            )
            return None
    ...
    def _collect_comparison_links(cls, meta: Meta) -> bool:
        ...
        if not reported_links or not trumping_links:
            logger.info(
                "[yellow]No valid screenshot links provided. Skipping trump report creation.[/yellow]"
            )
            return False
    ...
    def _comparison_link_prompts(cls) -> tuple[str, str] | None:
        ...
        logger.info(
            "[yellow]No screenshot links provided. Skipping trump report creation.[/yellow]"
        )
        return None
    ...
    async def _trump_page_loop(
        ...
        while True:
            ...
            if not cursor:
                return data, status
            logger.info(
                f"[cyan]Fetched {len(page_data)} trumping reports, waiting 1 second before next page...[/cyan]"
            )
    ...
    async def _trump_page(
        ...
        if response.status_code != 200:
            logger.info(
                f"[bold red]Failed to search trumps on {tracker}. HTTP Status: {response.status_code} - {response.text}[/bold red]"
            )
    ...
    def _parse_trump_page(
        ...
        if not isinstance(meta_info, dict):
            logger.info(
                f"[bold red]Unexpected 'meta' format: {type(meta_info)}[/bold red]"
            )
    ...
    def _stored_reported_torrent_id(meta: Meta, tracker: str) -> str:
        ...
        if not value:
            logger.info(
                f"[red]No reported torrent ID found in meta for trump report creation on {tracker}[/red]"
            )
        return value
    ...
    def _trump_create_url(
        ...
        if not value.isdigit():
            logger.info(
                f"[red]Invalid or missing reported torrent ID for LST: {reported_id}[/red]"
            )
            return None
    ...
    def _trumping_torrent_id(meta: Meta, tracker: str) -> Any | None:
        ...
        logger.info(
            f"[red]No torrent ID found in meta for trumping torrent on {tracker}[/red]"
        )
    ...
    def _append_trump_screenshots(payload: JsonDict, meta: Meta) -> None:
        ...
        if (
            ...
            payload["message"] = (
                f"{payload.get('message', '')} - User says comparison screenshots are in description."
            )
    ...
    def _lst_user_message() -> str | None:
        try:
            ...
            logger.info(
                "[yellow]Prompt cancelled; no additional message provided.[/yellow]"
            )
            return None
    ...
    def _trump_post_success(tracker: str, response: Any) -> bool:
        if response.status_code in {200, 201}:
            logger.info(
                f"[bold green]Successfully created trump report on {tracker}[/bold green]"
            )
            return True
        logger.info(
            f"[bold red]Failed to create trump report. HTTP Status: {response.status_code}[/bold red]"
        )
        return False
    ...
    def _log_debug_trump_report(create_url: str, payload: JsonDict) -> None:
        logger.info(
            "[bold yellow]Debug mode enabled, skipping actual trump report creation.[/bold yellow]"
        )
