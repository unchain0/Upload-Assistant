...
class TrackerSetup:
    ...
    def _required_tracker_config_present(self, meta: Meta, tracker_name: str) -> bool:
        ...
        status["status_message"] = "Missing required tracker configuration: " + ", ".join(missing)
        return False
    ...
    def _category_supported(category: str, supported: Any) -> bool:
        values = cast(tuple[Any, ...] | list[Any], supported) if isinstance(supported, tuple | list) else []
    ...
    async def _banned_groups_from_file(self, tracker: str, file_path: Path) -> list[str]:
        try:
            ...
            payload_map = cast(JsonDict, payload) if isinstance(payload, dict) else {}
    ...
    def _log_banned_group_match(tracker: str, meta: Meta, note: str) -> None:
        logger.info(f"[bold yellow]{str(meta.tag or '')[1:]}[/bold yellow][bold red] was found on [bold yellow]{tracker}'s[/bold yellow] list of banned groups.")
        if note:
    ...
    def _beyondhd_configured(self) -> bool:
        ...
        return isinstance(value, dict) and bool(cast(JsonDict, value).get("api_key"))
    ...
    def _first_matched_episode_id(meta: Meta, tracker: str) -> str:
        ...
        first = values[0]
        return str(cast(JsonDict, first).get("id", "")) if isinstance(first, dict) else ""
    ...
    def _log_trump_report(report: JsonDict) -> None:
        ...
        for torrent in cast(list[Any], torrents):
            if isinstance(torrent, dict):
                ...
                logger.info(f"  [bold green]Already being trumped by:[/bold green] {torrent_map.get('name', 'Unknown')} (ID: {torrent_map.get('id', 'N/A')})")
