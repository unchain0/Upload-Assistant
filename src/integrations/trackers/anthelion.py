...
class Anthelion:
    ...
    def _edition_flags(meta: Meta) -> list[str]:
        ...
        values = ("Directors", "Extended", "Uncut", "Unrated", "4KRemaster", "IMAX")
        return [flag for flag in values if flag in edition]

    @staticmethod
    def _audio_flags(meta: Meta) -> list[str]:
        audio = str(meta.audio or "")
        return [flag.replace("-", "") for flag in ("Dual-Audio", "Atmos") if flag in audio]

    @classmethod
    def _feature_flags(cls, meta: Meta) -> list[str]:
        return [flag for flag, matched in cls._feature_flag_checks(meta) if matched]
    ...
    def _normalized_tags(value: Any) -> list[str]:
        if isinstance(value, str):
            ...
            return []
        return [str(item).replace(" ", ".").lower() for item in values if str(item).strip()]

    async def _notify_manual_tagging(self, meta: Meta, tags: list[str]) -> None:
    ...
    async def _prompt_type(self, meta: Meta) -> int:
        ...
        choice = await prompt_in_thread(cli_ui.ask_choice, "Select the proper type for ANTHELION", choices=choices)
        return {"Feature Film": 0, "Short Film": 1, "Miniseries": 2, "Other": 3}.get(choice, 0)
    ...
    async def _prepare_torrent(self, meta: Meta) -> str:
        ...
        await TorrentCreator.create_torrent(meta, str(Path(str(meta.path))), self.tracker, tracker_url=tracker_url)
        return self.tracker

    def _torrent_tracker_url(self, meta: Meta) -> str:
        if not meta.mkbrr:
            return ""
        return str(self.tracker_config.get("announce_url", "https://fake.tracker")).strip()

    async def _torrent_upload_file(self, meta: Meta) -> dict[str, tuple[str, bytes, str]]:
    ...
    async def _upload_data(self, meta: Meta, audioformat: str) -> dict[str, Any]:
        data: dict[str, Any] = {
    ...
    async def _apply_media_payload(self, meta: Meta, data: dict[str, Any]) -> None:
        if meta.is_disc == "BDMV":
            data["bdinfo"] = await self._read_text_payload(meta, "BD_SUMMARY_00.txt")
            data["container_type"] = "m2ts"
            return
        mediainfo = await self._read_text_payload(meta, "MEDIAINFO_CLEANPATH.txt")
    ...
    async def _apply_release_group(self, meta: Meta, data: dict[str, Any]) -> None:
    ...
    async def _apply_screenshot_policy(self, meta: Meta, data: dict[str, Any]) -> None:
    ...
    def _screenshot_text(meta: Meta) -> str:
        ...
        urls = [str(image.get("raw_url")) for image in images[:4] if isinstance(image, dict) and image.get("raw_url")]
        return "\n".join(urls)
    ...
    async def _apply_adult_screenshots(self, meta: Meta, data: dict[str, Any], screenshots: str) -> None:
        if meta.unattended and not meta.unattended_confirm:
            data["screenshots"] = ""
            return
        logger.info(f"{self.tracker}: [bold red]Adult content detected[/bold red]")
        safe = await prompt_in_thread(cli_ui.ask_yes_no, "Are the screenshots safe?", default=False)
        if not safe:
            data["screenshots"] = ""
            return
        data["screenshots"] = screenshots
    ...
    async def _debug_upload(self, meta: Meta, data: dict[str, Any]) -> bool:
        if "mediainfo" in data:
            path = release_temp_dir(meta.base_dir, meta.uuid) / f"{self.tracker}_MEDIAINFO.txt"
            async with aiofiles.open(path, "w", newline="", encoding="utf-8") as handle:
                await handle.write(str(data["mediainfo"]))
            logger.info(f"{self.tracker}: [green]Final MediaInfo payload written to {path}[/green]")
    ...
    async def _submit_upload(self, meta: Meta, files: dict[str, tuple[str, bytes, str]], data: dict[str, Any]) -> bool:
        try:
            response = await self._post_upload(files, data, {"User-Agent": self._user_agent(meta)})
    ...
    async def _post_upload(self, files: dict[str, tuple[str, bytes, str]], data: dict[str, Any], headers: dict[str, str]) -> httpx.Response:
        async with httpx.AsyncClient(timeout=40) as client:
            return await client.post(url=self.api_url, files=files, data=data, headers=headers)

    def _handle_upload_response(self, meta: Meta, response: httpx.Response) -> bool:
        try:
        ...
        if response.status_code not in {200, 201}:
            payload = {"error": f"ANTHELION returned status code: {response.status_code}", "response_content": response.text}
    ...
    def _response_is_success(payload: dict[str, Any]) -> bool:
        return "success" in payload or str(payload.get("status", "")).lower() == "success"

    def _log_unexpected_upload_error(self, meta: Meta, error: Exception) -> None:
        import traceback
    ...
    async def search_existing(self, meta: Meta) -> list[dict[str, Any]]:
        ...
        headers = {"X-API-Key": self.api_key, "User-Agent": self._user_agent(meta)}
    ...
    def _search_results(self, meta: Meta, payload: Any) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            if entry is not None:
                ...
                logger.debug(f"{self.tracker}: [green]Found potential dupe: {escape(str(entry['name']))} ({entry['size']} bytes)")
        return results
    ...
    @classmethod
    def _search_entry(cls, item: dict[str, Any], target_resolution: str) -> dict[str, Any] | None:
    ...
    @staticmethod
    def _resolution_matches(item: dict[str, Any], target_resolution: str) -> bool:
        if not target_resolution:
            return True
        return str(item.get("resolution") or "").lower() == target_resolution

    @classmethod
    def _search_entry_payload(cls, item: dict[str, Any], files: list[dict[str, Any]]) -> dict[str, Any]:
        return {
    ...
    def _largest_file_name(cls, files: list[dict[str, Any]]) -> str:
        if not files:
            return ""
        largest = max(files, key=lambda item: cls._safe_int(item.get("size", 0)))
    ...
    async def _safe_file_search(self, filename: str, api_key: str) -> list[dict[str, Any]]:
        try:
            ...
            logger.info(f"{self.tracker}: [bold red]Unable to search for existing torrents: {escape(str(error))}")
            return []
        except Exception as error:
            logger.error(f"{self.tracker}: [bold red]Unexpected error: {escape(str(error))}")
            return []
        return self._file_search_response_ids(filename, response)

    def _file_search_response_ids(self, filename: str, response: httpx.Response) -> list[dict[str, Any]]:
    ...
    async def _file_search_response(self, filename: str, api_key: str) -> httpx.Response:
        headers = {"X-API-Key": api_key, "User-Agent": f"Upload Assistant/2.4 ({platform.system()} {platform.release()})"}
        params = {"t": "search", "filename": filename, "o": "json"}
        async with httpx.AsyncClient(timeout=15.0) as client:
            return await client.get(url=self.api_url, params=params, headers=headers)

    def _file_search_ids(self, filename: str, response: httpx.Response) -> list[dict[str, Any]]:
        try:
            payload = response.json()
        except json.JSONDecodeError:
    ...
    def _matched_file_item(self, filename: str, payload: Any) -> dict[str, Any] | None:
    ...
    def _matched_multiple_file_item(self, filename: str, items: list[dict[str, Any]]) -> dict[str, Any] | None:
        if not items:
            return None
        matched = next((item for item in items if self._item_matches_filename(item, filename)), None)
        if matched is None:
            logger.debug(f"{self.tracker}: [yellow]Could not match filename, returning empty list")
        return matched

    @staticmethod
    def _single_file_item(items: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    @classmethod
    def _item_matches_filename(cls, item: dict[str, Any], filename: str) -> bool:
        return any(cls._filename_matches(filename, str(file.get("name", ""))) for file in cls._valid_files(item.get("files", [])))
