...
async def gen_desc(
    ...
    if meta.nfo:
        ...
        if nfo_files:
            ...
            nfo_content_utf8 = nfo_content.encode("utf-8", "ignore").decode(
                "utf-8"
            )
    ...
    if description_link:
        try:
            parsed: ParseResult = urllib.parse.urlparse(
                description_link.replace("/raw/", "/") or ""
            )
            split = os.path.split(parsed.path)
            raw = parsed._replace(
                path=f"{split[0]}/raw/{split[1]}"
                if split[0] != "/"
...
class DescriptionBuilder:
    ...
    def _get_tag_override(self, key: str, meta: Meta | None) -> str | None:
        ...
        for config_source in config_sources:
            ...
            for configured_tag, overrides in tag_overrides.items():
                if str(configured_tag).strip().lstrip(
                    "-"
                ).casefold() != tag or not isinstance(overrides, dict):
                    continue
    ...
    async def get_logo_section(self, meta: Meta) -> tuple[str, str]:
        ...
        try:
            ...
            if self.tracker in (
                ...
                if logo_resize_url:
                    if logo_resize_url.endswith(".svg"):
                        logo_resize_url = logo_resize_url.replace(
                            ".svg", ".png"
                        )
            ...
            logger.warning(
                f"[yellow]Warning: Error getting logo section: {e!s}[/yellow]"
            )
    ...
    async def get_tv_info(self, meta: Meta) -> tuple[str, str]:
        ...
        try:
            if (
                not self._get_bool_config("episode_overview", False)
                or meta.category != "TV"
            ):
            ...
            season_name = (
                tvmaze_episode_data.get("season_name", "")
                or meta.tvdb_season_name
            )
            season_number = meta.season
            episode_number = meta.episode
            overview = str(
                tvmaze_episode_data.get("overview", "")
                or meta.overview_meta
                or ""
            )
            ...
            episode_title = meta.auto_episode_title or (
                episode_name
                if (
            ...
            logger.warning(
                f"[yellow]Warning: Error getting TV info: {e!s}[/yellow]"
            )
    ...
    async def get_mediainfo_section(self, meta: Meta) -> str:
        ...
        if meta.is_disc == "BDMV" or meta.category in (
            "GAME",
            "BOOK",
            "MUSIC",
            "PODCAST",
        ...
        if file_exists and file_size > 0:
            with contextlib.suppress(Exception):
                async with aiofiles.open(
                    cache_file_path, encoding="utf-8"
                ) as f:
                    return await f.read()
        ...
        if meta.mediainfo:
            media_info_content = self.format_short_mediainfo_json(
            ...
            if media_info_content:
                with contextlib.suppress(Exception):
                    ...
                    async with aiofiles.open(
                        cache_file_path, mode="w", encoding="utf-8"
                    ) as f:
    ...
    def format_short_mediainfo_json(
        mediainfo: dict[str, Any] | None, video_file: str = ""
    ) -> str:
        ...
        def format_duration(seconds: str) -> str:
            try:
                ...
                return ""
            hours, remaining_milliseconds = divmod(milliseconds, 3_600_000)
            minutes, remaining_milliseconds = divmod(
                remaining_milliseconds, 60_000
            )
            whole_seconds, remaining_milliseconds = divmod(
                remaining_milliseconds, 1000
            )
            return f"{hours:02}:{minutes:02}:{whole_seconds:02}.{remaining_milliseconds:03}"
        ...
        def format_bitrate(bits_per_second: str) -> str:
            ...
            return (
                f"{int((bitrate / 1000).to_integral_value(rounding=ROUND_HALF_UP)):,}".replace(
                    ",", " "
                )
                + " kb/s"
            )
        ...
        general = next(
            (track for track in tracks if value(track, "@type") == "General"),
            None,
        )
        if general is None:
            return ""
        ...
        for video in (
            track for track in tracks if value(track, "@type") == "Video"
        ):
            codec = value(video, "Format")
            codec += (
                ", " + value(video, "Encoded_Library")
                if value(video, "Encoded_Library")
                else ""
            )
            codec += (
                ", " + value(video, "HDR_Format_String")
                if value(video, "HDR_Format_String")
                else ""
            )
            codec += (
                ", " + value(video, "transfer_characteristics")
                if value(video, "transfer_characteristics")
                else ""
            )
            output.extend(
                [
        ...
        for audio in (
            track for track in tracks if value(track, "@type") == "Audio"
        ):
        ...
        for index, text in enumerate(
            track for track in tracks if value(track, "@type") == "Text"
        ):
            if index == 0:
                output.append("---SUBTITLES---")
            title = value(text, "Title")
            output.append(
                f"Language.......: {language_name(value(text, 'Language'))}{f' ({title})' if title else ''}, {value(text, 'Format')}"
            )
    ...
    async def get_bdinfo_section(self, meta: Meta) -> str:
        ...
        try:
            ...
            logger.warning(
                f"[yellow]Warning: Error getting bdinfo section: {e!s}[/yellow]"
            )
    ...
    async def get_user_description(self, meta: Meta) -> str:
        ...
        try:
            ...
            logger.warning(
                f"[yellow]Warning: Error getting user description: {e!s}[/yellow]"
            )
    ...
    async def get_bluray_section(self, meta: Meta) -> tuple[str, str]:
        ...
        try:
            ...
            if (
                ...
                and bluray_link
                and meta.release_url
            ):
                release_url = meta.release_url

            cover_data = meta.hosted_artwork
            if not cover_data and await self.common.path_exists(
                f"{meta.base_dir}{'/' + 'tmp' + '/'}{meta.uuid}/covers.json"
            ):
                try:
                    async with aiofiles.open(
                        f"{meta.base_dir}{'/' + 'tmp' + '/'}{meta.uuid}/covers.json",
            ...
            use_bluray_images = self._get_bool_config(
                "use_bluray_images", False
            )
            if (
                meta.is_disc in ["BDMV", "DVD"]
                and use_bluray_images
                and cover_data
            ):
            ...
            logger.warning(
                f"[yellow]Warning: Error getting bluray section: {e!s}[/yellow]"
            )
    ...
    async def get_audio_spectrogram_section(self, meta: Meta) -> str:
        ...
        try:
            add_audio_spectrogram = self._get_bool_config(
                "add_audio_spectrogram", False
            )
            add_spec = (
                meta.audio_spectrogram
                or meta.audio_spectrogram_tracks
                or add_audio_spectrogram
            )
            if not add_spec:
                return ""

            spectrograms_images = get_tracker_image_collection(
                meta, self.tracker, "spectrograms_images"
            )
            if not spectrograms_images:
                return ""
            audio_spectrogram_header = self._get_str_config(
            ...
            desc_parts: list[str] = (
                [audio_spectrogram_header]
                if audio_spectrogram_header is not None
                else []
            )
            desc_parts.append("\n[center]")
                ...
                if isinstance(spec_img, dict):
                    ...
                    if web_url and raw_url:
                        ...
                        self._append_screenshot_row_separator(
                            desc_parts, img_index, screens_per_row
                        )
            desc_parts.append("[/center]\n")
            return "".join(desc_parts)
        except Exception as e:
            logger.warning(
                f"[yellow]Warning: Error getting audio spectrogram section: {e!s}[/yellow]"
            )
        return ""
    ...
    async def get_dynamic_hdr_plot_section(self, meta: Meta) -> str:
        ...
        if not (
            ...
            return ""
        plot_images = get_tracker_image_collection(
    ...
    def _build_book_desc_section(
        ...
        str_avg_bitrate = (
            "Average Bitrate" if not use_pt_br else "Bitrate Médio"
        )
        ...
        str_technical_details = (
            "Technical Details" if not use_pt_br else "Detalhes Técnicos"
        )
        ...
        if series:
            series_value = (
                f"{series} #{series_index}" if series_index else series
            )
        ...
        if table:
            ...
            if fields:
                final_book_parts.append(
                    f"{header}{str_technical_details}{header_end}"
                )
                table_lines = ["[table]"]
                for label, val in fields:
                    table_lines.append(
                        f"[tr][td][b]{label}[/b][/td][td]{val}[/td][/tr]"
                    )
            ...
            if meta.epubmeta_output:
                final_book_parts.append(
                    f"[spoiler=EPUB Metadata][code]{meta.epubmeta_output}[/code][/spoiler]"
                )

            if overview:
                final_book_parts.append(
                    f"{header}{str_overview}{header_end}\n{overview}"
                )

            return "\n\n".join(
                part for part in final_book_parts if part.strip()
            )
        book_parts = []
        ...
        if overview:
            final_book_parts.append(
                f"{header}{str_overview}{header_end}{overview}"
            )
    ...
    def _build_game_desc_section(
        ...
        str_system_requirements = (
            "System Requirements" if not use_pt_br else "Requisitos do Sistema"
        )
        str_minimum = "Minimum" if not use_pt_br else "Mínimo"
        str_recommended = "Recommended" if not use_pt_br else "Recomendado"
        str_official_supported_languages = (
            "Officially Supported Languages"
            if not use_pt_br
            else "Idiomas Oficialmente Suportados"
        )
        str_language = "Language" if not use_pt_br else "Idioma"
        str_support = "Support" if not use_pt_br else "Suporte"
        str_content_type = (
            "Content Type" if not use_pt_br else "Tipo de Conteúdo"
        )
        str_package = "Package" if not use_pt_br else "Pacote"
        str_release_group = (
            "Release Group" if not use_pt_br else "Grupo de Lançamento"
        )
        str_installation = (
            "Installation and Usage Instructions"
            if not use_pt_br
            else "Instruções de Instalação e Uso"
        )
        ...
        if meta.software and meta.software_notes:
            ...
            if notes:
                game_parts.append(
                    f"{header}{str_installation}{header_end}{notes}"
                )
        ...
        if req_min or req_rec:
            ...
            if req_rec:
                ...
                clean_rec = re.sub(
                    r"^\[b\](Recommended|Recomendado):\[/b\]\s*",
                    "",
                    clean_rec,
                    flags=re.IGNORECASE,
                )
            ...
            if table:
                ...
                table_lines = ["[table]"]
                table_lines.append(
                    f"[tr][td][b]{col_min_header}[/b][/td][td][b]{col_rec_header}[/b][/td][/tr]"
                )
                table_lines.append(
                    f"[tr][td]{clean_min}[/td][td]{clean_rec}[/td][/tr]"
                )
                table_lines.append("[/table]")
                game_parts.append("\n".join(table_lines))
            else:
        ...
        if languages and isinstance(languages, dict):
            if table:
                table_rows = []
                table_rows.append(
                    f"[tr][td][b]{str_language}[/b][/td][td][b]{str_support}[/b][/td][/tr]"
                )
                ...
                for lang, support in sorted(languages.items()):
                    ...
                    table_rows.append(
                        f"[tr][td]{lang}[/td][td]{support_str}[/td][/tr]"
                    )
                ...
                simple_section = (
                    f"{header}{str_official_supported_languages}{header_end}\n"
    ...
    def _build_music_desc_section(
        ...
        if meta.category != "MUSIC" or not isinstance(
            meta.music_release, dict
        ):
            return ""
        ...
        if self.tracker == "TORRENTLEECH" and not header_size:
            ...
            self.tracker in ("BJSHARE", "BRASILTRACKER", "SPEEDAPP")
            and not header_size
        ):
            header_size = 3
        ...
        def value(name: str, fallback: Any = "") -> Any:
            ...
            if isinstance(item, dict) and item.get("value") not in (
                None,
                "",
                [],
                {},
        ...
        text = {
            ...
            "bit_depth": "Bit Depth"
            if not use_pt_br
            else "Profundidade de Bits",
            "sample_rate": "Sample Rate"
            if not use_pt_br
            else "Taxa de Amostragem",
        ...
        def discogs_link(kind: str, identifier: Any) -> str:
            ...
            numeric_identifier = (
                ...
                if raw_identifier.isdecimal()
                else ""
            )
            return (
                f"[url=https://www.discogs.com/{kind}/{numeric_identifier}]{numeric_identifier}[/url]"
        ...
        external_id_links = [
            ...
            (
                "Discogs Release",
                discogs_link("release", external_ids.get("discogs_release")),
            ),
            (
                "Discogs Master",
                discogs_link("master", external_ids.get("discogs_master")),
            ),
        ]
        external_id_links = [
            f"{label}: {link}" for label, link in external_id_links if link
        ]

        music_fields = [
            (
                text["artist"],
                display(value("artists", value("artist", meta.artist))),
            ),
            ...
            (
                text["format"],
                display(value("format", technical_values("format"))),
            ),
            (text["codec"], technical_values("codec")),
            (
                text["bit_depth"],
                technical_values("bit_depth", lambda item: f"{item}-bit"),
            ),
            (
                text["sample_rate"],
                technical_values(
                    "sample_rate", lambda item: f"{int(item) / 1000:g} kHz"
                ),
            ),
            (
                text["channels"],
                technical_values(
                    "channels",
                    lambda item: {1: "Mono", 2: "Stereo"}.get(
                        int(item), f"{item} channels"
                    ),
                ),
            ),
            (
                text["bitrate"],
                technical_values(
                    "bitrate", lambda item: f"{round(int(item) / 1000)} kbps"
                ),
            ),
            (
                text["external_ids"],
        ...
        music_fields = [
            (label, field_value)
            for label, field_value in music_fields
            if field_value
        ]
        ...
        if table:
            table_lines = ["[table]"]
            table_lines.extend(
                f"[tr][td][b]{label}[/b][/td][td]{field_value}[/td][/tr]"
                for label, field_value in music_fields
            )
            table_lines.append("[/table]")
            body = "\n".join(table_lines)
        ...
        for track in tracks:
            if not isinstance(track, dict):
                continue
            try:
                disc_numbers.append(int(track.get("disc_number") or 1))
            except TypeError, ValueError:
                disc_numbers.append(1)
        disc_count = max(disc_numbers, default=1)
    ...
    async def general_description_generator(
        ...
        image_list = get_tracker_image_collection(
        ...
        if languages:
            try:
                ...
                if meta.audio_languages and meta.write_audio_languages:
                    desc_parts.append(
                        f"[code]Audio Language/s: {', '.join(meta.audio_languages)}[/code]"
                    )
                ...
                logger.warning(
                    f"[yellow]Warning: Error processing language: {e!s}[/yellow]"
                )
        ...
        if logo:
            ...
            if logo_url and logo_size:
                desc_parts.append(
                    f"[center][img={logo_size}]{logo_url}[/img][/center]\n"
                )
        ...
        if mediainfo:
            if self.tracker == "BJSHARE":
                ...
                if bd_info:
                    desc_parts.append(
                        f"[hide=BDInfo][pre]{bd_info}[/pre][/hide]"
                    )
            elif self.tracker == "DIGITALCORE":
                bd_info = await self.get_bdinfo_section(meta)
                if bd_info:
                    desc_parts.append(bd_info)
            elif self.tracker in (
                "FUNFILE",
                "HDSPACE",
                "IPTORRENTS",
                "IMMORTALSEED",
                ...
                if mediainfo_sec:
                    desc_parts.append(
                        f"[left][font=consolas]{mediainfo_sec}[/font][/left]"
                    )
                bd_info = await self.get_bdinfo_section(meta)
                if bd_info:
                    desc_parts.append(
                        f"[left][font=consolas]{bd_info}[/font][/left]"
                    )
        ...
        if nfo:
            ...
            if isinstance(nfo_content, str) and nfo_content:
                if self.tracker == "DIGITALCORE":
                    ...
                    desc_parts.append(
                        f"<div style='display: flex; justify-content: center;'><div style='background-color: #000000; color: #ffffff;'>{nfo_content}</div></div>"
                    )
                else:
        ...
        if user_description:
            ...
            if (
                not description
                or user_description_content.strip() != meta_description.strip()
            ):
    ...
    async def _check_saved_pack_image_links(
        ...
        if await self.common.path_exists(str(pack_images_file)):
            try:
                async with aiofiles.open(
                    ...
                    for key_name, key_data in pack_images_data.get(
                        "keys", {}
                    ).items():
                        ...
                        for img in key_data.get("images", []):
                            ...
                            try:
                                parsed_url: ParseResult = (
                                    urllib.parse.urlparse(raw_url or "")
                                )
                                ...
                                logger.debug(
                                    f"[yellow]Could not parse URL: {raw_url}[/yellow]"
                                )
                                continue
                    ...
                    pack_images_data["total_count"] = sum(
                        key_data["count"]
                        for key_data in pack_images_data.get(
                            "keys", {}
                        ).values()
                    )
                    ...
                    if pack_images_data.get("total_count", 0) < 3:
                        ...
                        logger.debug(
                            f"[green]Loaded previously uploaded images from {pack_images_file}"
                        )
                        logger.debug(
                ...
                logger.warning(
                    f"[yellow]Warning: Could not load pack image data: {e!s}[/yellow]"
                )
    ...
    async def _handle_discs_and_screenshots(
        ...
        if meta.category == "GAME":
            ...
            for img_index in range(
                len(images[: meta.screens if meta.screens is not None else 6])
            ):
        ...
        if len(discs) == 1:
            ...
            if each["type"] == "DVD":
                desc_parts.append("[center]")
                desc_parts.append(
                    f"[spoiler={Path(each['vob']).name}][code]{each['vob_mi']}[/code][/spoiler]\n\n"
                )
                desc_parts.append("[/center]")
            if screenheader is not None:
                desc_parts.append(screenheader + "\n")
                    ...
                    for i, key in enumerate(
                        ...
                        if meta.get(new_images_key):
                            ...
                            for img_index, img in enumerate(
                                ...
                                desc_parts.append(
                                    self.format_screenshot(
                                        web_url, raw_url, img_url, thumb_size
                                    )
                                )
                                self._append_screenshot_row_separator(
                                    desc_parts, img_index, screens_per_row
                            ...
                            if not new_screens:
                                logger.warning(
                                    f"[yellow]Missing prepared screenshots for PLAYLIST_{i}; skipping its images in the description.[/yellow]"
                                )
                            if new_screens and not meta.skip_imghost_upload:
                                ...
                                if (
                                    ...
                                    await self.common.save_image_links(
        ...
        # Handle multiple discs case
            ...
            if multi_screens != 0:
                ...
                logger.info(
                    f"[cyan]{total_discs_to_process} files (processLimit)[/cyan]"
                )
            ...
            for i, each in enumerate(discs):
                ...
                if i == 0:
                    ...
                    if each["type"] == "BDMV":
                        ...
                        desc_parts.append(
                            f"[spoiler={Path(each['ifo']).name}][code]{each['ifo_mi']}[/code][/spoiler]\n\n"
                        )
                    ...
                    if multi_screens != 0:
                        ...
                        if meta.get(new_images_key):
                            logger.debug(
                                f"[yellow]Found needed image URLs for {new_images_key}"
                            )
                            desc_parts.append("[center]")
                            if each["type"] == "BDMV":
                                desc_parts.append(
                            ...
                            if each["type"] == "BDMV":
                                ...
                                new_screens = [
                                    ...
                                    for f in manifest_files(
                                        meta.base_dir,
                                        meta.uuid,
                                        await self.takescreens_manager.sanitize_filename(
                                            meta.discs[i]["name"]
                                        ),
                                    )
                                ]
                            if not new_screens:
                                logger.warning(
                                    f"[yellow]Missing prepared screenshots for {new_images_key}; skipping its images in the description.[/yellow]"
                                )
        ...
        if len(filelist) == 1:
            if meta.comparison and meta.comparison_groups:
                ...
                if not isinstance(comparison_groups, dict):
                    comparison_groups = {
                        str(i): v for i, v in enumerate(comparison_groups)
                    }
        ...
        other_files_spoiler_open = (
            False  # Track if "Other files" spoiler has been opened
        )
        ...
        if multi_screens != 0 and total_files_to_process > 1:
            ...
            logger.info(
                f"[cyan]{total_files_to_process} files (processLimit)[/cyan]"
            )
                    ...
                    if new_images_key not in meta or not meta[new_images_key]:
                        ...
                        if not new_screens:
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Error during generic screenshot capture: {e}",
                                    extra={"markup": False},
                                )
        ...
        if len(filelist) > 1:
            for i, file in enumerate(filelist):
                ...
                filename = (
                    Path(file.strip()).stem.replace("[", "").replace("]", "")
                )
                ...
                if (
                    multi_screens != 0
                    and i >= file_limit
                    and not other_files_spoiler_open
                ):
                ...
                if multi_screens != 0:
                    if i > 0 and char_count < max_char_limit:
                        ...
                        formatted_bbcode = self.parser.format_bbcode(
                            parsed_mediainfo
                        )
                        desc_parts.append(
                ...
                if (
                    i == 0
                ):  # For the first file, use 'image_list' key and add screenheader if applicable
                    if images:
                        if screenheader is not None:
                            desc_parts.append(screenheader + "\n")
                            char_count += len(screenheader + "\n")
                        ...
                        for img_index in range(len(images)):
                            ...
                            raw_url = images[img_index]["raw_url"]
                            img_url = images[img_index].get("img_url", raw_url)
                            image_str = self.format_screenshot(
                                web_url, raw_url, img_url, thumb_size
                            )
                    ...
                    multi_screens != 0
                    and new_images_key in meta
                    and meta[new_images_key]
                ):
                    desc_parts.append("[center]")
                    char_count += len("[center]")
        ...
        if char_count >= 1 and meta.debug:
            logger.info(
                f"[yellow]Total characters written to description: {char_count}"
            )
        if total_files_to_process > 1:
            logger.info("")
            ...
            if meta.is_disc:
                ...
                if menu_images:
                    ...
                    for img_index, image in enumerate(menu_images):
                        ...
                        self._append_screenshot_row_separator(
                            menu_parts, img_index, screens_per_row
                        )
                    menu_parts.append("[/center]\n\n")
                    menu_image_section = "".join(menu_parts)
        except Exception as e:
            logger.warning(
                f"[yellow]Warning: Error processing disc menu section: {e!s}[/yellow]"
            )
    ...
    def tracker_specific_formats(self, tracker: str, description: str) -> str:
        ...
        if tracker == "BJSHARE":
            description = bbcode.convert_named_spoiler_to_named_hide(
                description
            )
        ...
        if tracker == "DIGITALCORE":
            description = description.replace("[user]", "").replace(
                "[/user]", ""
            )
        ...
        if tracker == "FUNFILE":
            ...
            description = description.replace("[ul]", "").replace("[/ul]", "")
            description = description.replace("[ol]", "").replace("[/ol]", "")
            description = description.replace("[hide]", "").replace(
                "[/hide]", ""
            )
            description = (
                description.replace("•", "-")
            ...
            description = bbcode.convert_comparison_to_centered(
                description, 1000
            )
        ...
        if tracker == "PTSKIT":
            ...
            description = description.replace("[sup]", "").replace(
                "[/sup]", ""
            )
            description = description.replace("[sub]", "").replace(
                "[/sub]", ""
            )
            description = description.replace("[alert]", "").replace(
                "[/alert]", ""
        ...
        if tracker == "TORRENTLEECH":
            ...
            description = re.sub(
                r"\[hr\]", "---", description, flags=re.IGNORECASE
            )
            description = re.sub(
                r'\[img=[\d"x]+\]', "[img]", description, flags=re.IGNORECASE
            )
            description = (
                description.replace("[*] ", "• ")
                .replace("[*]", "• ")
        ...
        if tracker == "IMMORTALSEED":
            ...
            description = re.sub(
                r"</(?:p|div|li|tr|h[1-6]|blockquote|pre)\s*>",
                "\n",
                description,
                flags=re.IGNORECASE,
            )
        ...
        if tracker in unit3d_trackers:
            ...
            description = description.replace("[ul]", "").replace("[/ul]", "")
            description = description.replace("[ol]", "").replace("[/ol]", "")
            description = bbcode.convert_comparison_to_collapse(
                description, 1000
            )
