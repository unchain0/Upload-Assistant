...
class TVChaosUK:
    ...
    async def get_tmdb_data(self, meta: Meta) -> dict[str, Any]:
        ...
        if meta.category == "TV":
            ...
            try:
                if not meta.tv_pack:
                    if "tmdb_episode_data" not in meta or not meta.tmdb_episode_data:
                        episode_info = await self.tmdb_manager.get_episode_details(
                            int(meta.tmdb),
                            int(meta.season_int),
                            int(meta.episode_int),
                        )
                    ...
                    if "tmdb_season_data" not in meta or not meta.tmdb_season_data:
                        season_info = await self.tmdb_manager.get_season_details(
                            int(meta.tmdb),
                            int(meta.season_int),
                        )
            ...
            except (httpx.RequestError, requests.exceptions.RequestException, KeyError, TypeError, ValueError, RuntimeError) as e:
