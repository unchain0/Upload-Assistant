...
class TVChaosUK:
    ...
    def _logo_block(self, meta: Meta) -> str:
        ...
        logo_size = default.get("logo_size", self.DEFAULT_LOGO_SIZE) if isinstance(default, dict) else self.DEFAULT_LOGO_SIZE
    ...
    def _screenshots_block(self, meta: Meta, image_list: list[dict[str, Any]]) -> str:
    ...
    def _episode_title_block(meta: Meta) -> str:
        ...
        return f"[center][b]Episode Title:[/b] {name}[/center]\n\n" if name else ""
    ...
    def _country_release_entries(cls, group: Any) -> list[str]:
        ...
        return [entry for release in values if (entry := cls._tv_release_entry(country, release))]
    ...
    async def upload(self, meta: Meta) -> bool | None:
        ...
        description = await self.edit_desc(meta, self.tracker, self.signature, image_list)
        name = await self._upload_name(meta, category_id, media_info)
        if not self._confirm_upload_name(meta, name):
            return None
        data = self._upload_data(meta, name, description, mediainfo_dump, bdinfo_dump, category_id, resolution_id)
        if meta.debug:
    ...
    async def _load_mediainfo_json(self, meta: Meta) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"{self.tracker}: [yellow]Warning: Could not load MediaInfo.json: {error}")
            return {}

    async def _upload_category(self, meta: Meta, media_info: dict[str, Any]) -> str:
        category_id = await self.get_cat_id(meta.genres) if meta.category == "TV" else "44"
    ...
    def _foreign_original_language(value: Any) -> bool:
        ...
        return bool(language) and not language.startswith("en") and language not in {"ga", "gd", "cy"}
    ...
    async def _upload_name(self, meta: Meta, category_id: str, media_info: dict[str, Any]) -> str:
    ...
    def _release_type(meta: Meta) -> str:
        ...
        if meta.type == "ENCODE" and any(marker in path for marker in ("bluray", "brrip", "bdrip")):
            return "BRRip"
    ...
    @classmethod
    def _episode_upload_name(cls, meta: Meta, release_type: str, year: str) -> str:
    ...
    def _format_date(value: str) -> str:
        try:
            return datetime.strptime(value, "%Y-%m-%d").replace(tzinfo=UTC).strftime("%d-%m-%Y")
        except ValueError, TypeError:
            return value
    ...
    def _localized_upload_name(meta: Meta, category_id: str, name: str) -> str:
        ...
        if not meta.original_title or meta.original_title == meta.title:
            return name
        return name.replace(str(meta.title), f"{meta.title} ({meta.original_title})")
    ...
    def _append_hevc_marker(meta: Meta, name: str) -> str:
        return name.replace("]", " HEVC]") if meta.video_codec == "HEVC" else name
    ...
    def _confirm_upload_name(meta: Meta, name: str) -> bool:
        if meta.unattended:
            return True
        if cli_ui.ask_yes_no(f"Upload to TVCHAOSUK with the name {name}?", default=False):
            return True
        new_name = cli_ui.ask_string("Please enter New Name:") or name
        return bool(cli_ui.ask_yes_no(f"Upload to TVCHAOSUK with the name {new_name}?", default=False))
    ...
    async def _submit_upload(self, meta: Meta, data: dict[str, str | int]) -> bool:
        ...
        try:
            ...
            meta.tracker_status[self.tracker]["status_message"] = self._request_error_message(error, response)
            return False
        except Exception as error:
            meta.tracker_status[self.tracker]["status_message"] = self._unexpected_error_message(error, response)
            return False

    async def _upload_response(self, meta: Meta, data: dict[str, str | int]) -> httpx.Response:
        ...
        params = {"api_token": str(self._tracker_config().get("api_key", "")).strip()}
        async with httpx.AsyncClient(timeout=30.0) as client:
            return await client.post(self.upload_url, files=files, data=data, headers={"User-Agent": "Mozilla/5.0"}, params=params)

    async def _handle_upload_response(self, meta: Meta, response: httpx.Response) -> bool:
        if response.status_code != 200:
            meta.tracker_status[self.tracker]["status_message"] = self._http_error_message(response)
            return False
    ...
    def _upload_json(response: httpx.Response) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise ValueError("Invalid TVCHAOSUK response: JSON payload must be an object")
    ...
    def _uploaded_torrent_id(payload: dict[str, Any]) -> str:
        ...
        if not isinstance(data, str):
            raise ValueError(f"Invalid TVCHAOSUK response: 'data' missing or not a string: {data}")
        segments = [segment for segment in urlparse(data).path.split("/") if segment]
        if not segments:
            raise ValueError(f"Invalid TVCHAOSUK response format: no path segments in {data}")
        return segments[-1]

    @staticmethod
    def _request_error_message(error: httpx.RequestError, response: httpx.Response | None) -> str:
        text = response.text if response is not None else "No response"
        return f"data error: Unable to upload. Error: {error}.\nResponse: {text}"

    @staticmethod
    def _unexpected_error_message(error: Exception, response: httpx.Response | None) -> str:
        text = response.text if response is not None else "No response"
        return f"data error: It may have uploaded, go check. Error: {error}.\nResponse: {text}"

    async def _debug_upload(self, meta: Meta, data: dict[str, str | int]) -> bool:
    ...
    def get_audio_languages(self, mi: dict[str, Any]) -> list[str]:
        languages = {language for track in self._media_tracks(mi, "Audio") if (language := self._normalized_audio_language(track))}
        return sorted(languages)

    @classmethod
    def _media_tracks(cls, media_info: Any, track_type: str) -> list[dict[str, Any]]:
    ...
    def _audio_language_value(track: dict[str, Any]) -> str:
        for key in ("Language/String", "Language/String1", "Language/String2", "Language"):
            value = track.get(key)
            if value:
    ...
    def _is_english_audio(language: str) -> bool:
        return language.lower() in {"en", "eng", "en-us", "en-gb", "en-ie", "en-au"}
    ...
    async def get_tmdb_data(self, meta: Meta) -> dict[str, Any]:
        ...
        except (httpx.RequestError, requests.exceptions.RequestException, KeyError, TypeError, ValueError, RuntimeError) as error:
    ...
    def _production_country_codes(value: Any) -> list[str]:
        ...
        return [str(country["iso_3166_1"]) for country in countries if isinstance(country, dict) and "iso_3166_1" in country]
    ...
    def _log_existing_movie_tmdb(self, meta: Meta) -> None:
        if meta.debug and meta.tmdb is not None:
            logger.info(f"{self.tracker}: [cyan]DEBUG: TMDb movie ID already resolved: {meta.tmdb}[/cyan]")
    ...
    def _season_episode_rows(cls, value: Any) -> list[dict[str, str]]:
        ...
        return [row for episode in episodes if (row := cls._season_episode_row(episode)) is not None]
    ...
    def _log_tmdb_error(self, meta: Meta, error: Exception) -> None:
        logger.info(f"{self.tracker}: [yellow]Expected error while fetching TV episode/season info: {error}")
    ...
    async def edit_desc(
        ...
        description = self._finalize_description("".join(parts), comparison, signature)
        await self._write_description_file(path, description)
        return description

    def _description_parts(self, meta: Meta, image_list: list[dict[str, Any]], base: str) -> list[str]:
        parts: list[str] = []
        if meta.discs:
    ...
    def _category_description(self, meta: Meta, image_list: list[dict[str, Any]]) -> str:
        if meta.category == "MOVIE":
    ...
    def _finalize_description(self, description: str, comparison: bool, signature: str) -> str:
    ...
    def get_links(self, meta: Meta) -> str:
        links = [link for key, url in self._metadata_links(meta) if (link := self._icon_link(key, url))]
        if not links:
            return ""
    ...
    def _tvdb_link(meta: Meta) -> str:
        ...
        return f"https://www.thetvdb.com/?id={tvdb_id}&tab=series" if tvdb_id else ""
    ...
    def _icon_link(self, image_key: str, url: str) -> str:
        ...
        image = str(images.get(image_key, "")) if isinstance(images, dict) else ""
    ...
    def _is_english_subtitle(language: str) -> bool:
        return language.lower() in {"en", "eng", "en-us", "en-gb", "en-ie", "en-au", "english"}
