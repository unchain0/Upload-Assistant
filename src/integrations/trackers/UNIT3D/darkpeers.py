...
class DarkPeers(UNIT3D):
    ...
    async def get_additional_checks(self, meta: Meta) -> bool:
        ...
        if not isinstance(filelist, (list, tuple, set)):
            logger.info(
                f"{self.tracker}: [bold red]File list metadata is invalid. Skipping upload.[/bold red]"
            )
            return False
        ...
        if category in {"MOVIE", "TV"}:
            if self._is_local_path_name(str(meta.name or "")):
                logger.info(
                    f"{self.tracker}: [bold red]Generated upload title contains a local file path. Skipping upload.[/bold red]"
                )
                return False
            ...
            if (
                meta.keep_folder
                and (
                    category == "MOVIE" or not self._is_single_tv_season(meta)
                )
                and not await self._confirm_or_skip(
        ...
        if group == "EVO" and release_type != "WEBDL":
            logger.info(
                f"{self.tracker}: [bold red]only allows EVO releases when they are WEB-DLs. Skipping upload."
            )
            return False

        if group == "HDT" and release_type != "REMUX":
            logger.info(
                f"{self.tracker}: [bold red]only allows HDT releases when they are Remuxes. Skipping upload."
            )
            return False

        if category in {"MOVIE", "TV"} and meta.hardcoded_subs:
            logger.info(
                f"{self.tracker}: [bold red]does not allow Movies or TV releases with hardcoded subtitles. Skipping upload."
            )
            return False
    ...
    def _min_webl_bitrate(
        ...
        base = (
            self._WEB_DL_MIN_VIDEO_BITRATE_KBPS
            if bitrate_type == "video"
            else self._WEB_DL_MIN_AUDIO_BITRATE_KBPS
        )
    ...
    async def validate_video_quality(self, meta: Meta) -> bool:
        ...
        if min_video_kbps is not None:
            ...
            if video_bitrate is None:
                logger.info(
                    f"{self.tracker}: [bold red]Could not determine video bitrate for this WEBDL upload."
                )
                return False
        ...
        if (
            min_audio_kbps is not None
            and audio_bitrate is not None
            and audio_bitrate < min_audio_kbps
        ):
            logger.info(
    ...
    async def validate_video_languages(self, meta: Meta) -> bool:
        ...
        valid = bool(audio & accepted) or (
            bool(original) and original in audio and bool(subtitles & accepted)
        )
        if not valid:
            logger.info(
                f"{self.tracker}: [bold red]requires English/Nordic audio, or original audio with English/Nordic subtitles. Skipping upload."
            )
        return valid
    ...
    def validate_video_files(self, meta: Meta) -> bool:
        ...
        archive = next(
            (
                ...
                if Path(str(item)).suffix.lower() in {".rar", ".zip", ".7z"}
            ),
            "",
        )
        if archive:
            logger.info(
                f"{self.tracker}: [bold red]does not permit archives in Movie/TV uploads: {archive}. Skipping upload."
            )
            return False
    ...
    def validate_video_content(self, meta: Meta) -> bool:
        ...
        if not paths:
            logger.info(
                f"{self.tracker}: [bold red]Movie/TV uploads require a payload of files. Skipping upload."
            )
            return False
        ...
        if not valid_video_files:
            logger.info(
                f"{self.tracker}: [bold red]Movie/TV uploads did not include a recognized video file extension. Skipping upload."
            )
            return False
        if invalid_files:
            logger.info(
    ...
    def validate_tv_scope(self, meta: Meta) -> bool:
        ...
        if re.search(
            r"\b(?:complete[ ._-]*series|all[ ._-]*seasons?|seasons?[ ._-]*\d+[ ._-]*(?:-|to)[ ._-]*\d+|s\d{1,2}[ ._-]*-[ ._-]*s?\d{1,2})\b",
            name,
        ):
            logger.info(
                f"{self.tracker}: [bold red]only individual seasons or episodes are allowed. Skipping multi-season/complete-series upload."
            )
            return False
        seasons = {
            match.casefold()
        ...
        if len(seasons) > 1:
            logger.info(
                f"{self.tracker}: [bold red]torrent contains files from multiple seasons. Skipping upload."
            )
            return False
        return True
    ...
    async def validate_book(self, meta: Meta) -> bool:
        ...
        if format_name not in allowed:
            logger.info(
                f"{self.tracker}: [bold red]does not support {format_name or 'an unspecified'} book format. Skipping upload."
            )
            return False
        collection = self._is_book_collection(meta)
        if meta.audiobook:
            validated_isbn = validate_isbn_checksum(
                str(meta.isbn or meta.book_isbn or "")
            )
            if not validated_isbn and not collection:
                logger.info(
                    f"{self.tracker}: [bold red]Audiobooks require a valid ISBN-10 or ISBN-13. Re-run with --isbn. Skipping upload.[/bold red]"
                )
                return False
            ...
            if not validated_isbn:
                logger.info(
                    f"{self.tracker}: [bold red]Individual eBooks require a valid ISBN-10 or ISBN-13 in the upload title. Re-run with --isbn. Skipping upload.[/bold red]"
                )
                return False
        ...
        if meta.audiobook and format_name in {"MP3", "AAC", "OPUS", "VORBIS"}:
            ...
            if int(meta.audiobook_bitrate) < 64:
                logger.info(
                    f"{self.tracker}: [bold red]Speech-only audiobooks require a bitrate of at least 64 kbps. Skipping upload.[/bold red]"
                )
                return False
        if meta.audiobook:
            details = (
            ...
            if meta.unattended:
                ...
                return False
            logger.info(
                f"{self.tracker}: [yellow]Verify that all audiobook edition fields describe the same recording. {details}[/yellow]"
            )
            if not await self.common.prompt_user_for_confirmation(
        ...
        if (
            ...
            and format_name == "PDF"
            and not bool(
                meta.get("page_count", None)
                or meta.get("book_page_count", None)
            )
        ):
            return await self._missing_required("PDF page count", meta)
        if not meta.audiobook:
            source = (
                str(meta.manual_source or meta.source or "").strip().upper()
            )
            ...
            if meta.ocr and source != "SCAN":
                logger.info(
                    f"{self.tracker}: [bold red]OCR cannot be combined with Retail. Use --source SCAN --ocr. Skipping upload.[/bold red]"
                )
                return False
    ...
    def _is_book_collection(meta: Meta) -> bool:
        ...
        book_files = [
            item
            for item in files
            if item.suffix.upper().lstrip(".")
            in DarkPeers._BOOK_FORMATS | DarkPeers._AUDIOBOOK_FORMATS
        ]
        label = " ".join(
            (
                str(meta.title or ""),
        ...
        marked = bool(
            re.search(
                r"\b(?:collection|complete|books?\s+\d+\s*-\s*\d+|series\s+pack)\b",
                label,
                re.IGNORECASE,
            )
        )
    ...
    def _validate_book_file_layout(self, meta: Meta, format_name: str) -> bool:
        ...
        if meta.audiobook and source_path.exists() and source_path.is_file():
            logger.info(
                f"{self.tracker}: [bold red]Audiobooks must be uploaded inside a single descriptive folder. Skipping upload.[/bold red]"
            )
            return False
        ...
        if meta.audiobook:
            audio_files = [
                item
                for item in files
                if item.suffix.upper().lstrip(".") in self._AUDIOBOOK_FORMATS
            ]
            if format_name == "M4B" and len(audio_files) == 1:
                ...
                if actual != expected:
                    logger.info(
                        f"{self.tracker}: [bold red]single-file M4B must be named 'Author - Title - Year.m4b': {audio_files[0].name}. Skipping upload.[/bold red]"
                    )
                    return False
            if len(audio_files) > 1:
                ...
                if invalid:
                    logger.info(
                        f"{self.tracker}: [bold red]Audiobook files must use logical chapter, disc, or part numbering: {invalid}. Skipping upload.[/bold red]"
                    )
                    return False
            return True
        ...
        if source_path.exists():
            author_tokens = set(
                re.findall(
                    r"[a-z0-9]+",
                    str(meta.author or meta.book_author or "").casefold(),
                )
            )
            title_tokens = set(
                re.findall(
                    r"[a-z0-9]+",
                    str(meta.title or meta.book_title or "").casefold(),
                )
            )
            ebook_files = [
                item
                for item in files
                if item.suffix.upper().lstrip(".") in self._BOOK_FORMATS
            ]
            invalid = next(
                (
                    item.name
                    for item in ebook_files
                    if not author_tokens.issubset(
                        set(re.findall(r"[a-z0-9]+", item.stem.casefold()))
                    )
                    or not title_tokens.issubset(
                        set(re.findall(r"[a-z0-9]+", item.stem.casefold()))
                    )
                ),
                "",
            ...
            if invalid:
                logger.info(
                    f"{self.tracker}: [bold red]eBook filename must include the author and title: {invalid}. Skipping upload.[/bold red]"
                )
                return False
        return True
    ...
    def _book_format(meta: Meta) -> str:
        ...
        if not meta.audiobook or format_name not in {"M4A", "OGG", "WAV"}:
            return format_name
        media = (
            meta.mediainfo if isinstance(meta.mediainfo, dict) else {}
        ).get("media")
        ...
        audio_text = " ".join(
            ...
            if (track := track_value if isinstance(track_value, dict) else {})
            and str(track.get("@type") or track.get("type") or "").casefold()
            == "audio"
        ).casefold()
        if format_name == "M4A":
            return (
    ...
    def validate_music(self, meta: Meta) -> bool:
        ...
        release = (
            MusicRelease.from_dict(release_data)
            if release_data
        ...
        paths = [track.relative_path for track in release.tracks] or [
            str(item)
            for item in meta.filelist or []
            if Path(str(item)).suffix.lower() in audio_suffixes
        ]
        album = str(release.get("album", meta.title or "")).strip()
        root = Path(str(meta.path or ""))
        if root.exists() and not root.is_dir():
            logger.info(
                f"{self.tracker}: [bold red]Music uploads must be contained in a descriptive folder. Skipping upload.[/bold red]"
            )
            return False
        if (
            root.is_dir()
            and album
            and re.sub(r"\W+", " ", album.casefold()).strip()
            not in re.sub(r"\W+", " ", root.name.casefold()).strip()
        ):
            logger.info(
                f"{self.tracker}: [bold red]Music folder name must include the album title. Skipping upload.[/bold red]"
            )
            return False
        ...
        for path in paths:
            ...
            full_relative = "/".join(
                part
                for part in (root.name if root.is_dir() else "", relative)
                if part
            )
            if (
                len(full_relative) > 180
                or filename.startswith(" ")
                or any(part.startswith(" ") for part in relative.split("/"))
            ):
                logger.info(
                    f"{self.tracker}: [bold red]invalid music path: {relative}. Skipping upload."
                )
                return False
            if any(char in filename for char in ':?<>|*"'):
                logger.info(
                    f"{self.tracker}: [bold red]music filename contains prohibited filesystem characters: {filename}. Skipping upload."
                )
                return False
            if re.search(r"\b\w+\.\w+\.\d{1,2}\.\w+", filename) or (
                not unnumbered_single
                and not self._AUDIO_TRACK_PATTERN.match(Path(filename).stem)
            ):
                logger.info(
                    f"{self.tracker}: [bold red]music filename must include a track number and title: {filename}. Skipping upload."
                )
                return False
        return True
    ...
    async def validate_game(self, meta: Meta) -> bool:
        ...
        if (
            ...
            or not rar_files
            or not meta.scene
            or not str(meta.scene_nfo_file or "").strip()
            or str(meta.repack or "").strip()
        ):
            logger.info(
                f"{self.tracker}: [bold red]Games/Apps must be an original RAR'd scene release with its NFO, not a repack or ISO. Skipping upload."
            )
            return False
        instructions = " ".join(
            str(value or "")
    ...
    async def get_audio(self, meta: Meta) -> str:
        ...
        if len(audio) == 1:
            ...
            if (
                original
                and only != original
                and only in self._NORDIC_LANGUAGES
            ):
    ...
    async def get_name(self, meta: Meta) -> dict[str, str]:
        if meta.category == "MUSIC":
            ...
            return {
                "name": self._ensure_group_tag(
                    name, meta.tag, preserve_if_scene=meta.scene
                )
            }
        ...
        has_scene_name = scene_name and not self._is_local_path_name(
            scene_name
        )
        if has_scene_name:
            if meta.scene:
            ...
            if not title_present and not name_present:
                ...
                return {
                    "name": self._ensure_group_tag(
                        normalized_scene_name, meta.tag
                    )
                }
        ...
        if meta.category in {"TV", "MOVIE"} and not meta.scene:
            year = (
                str(meta.manual_year)
                if meta.manual_year not in (None, 0)
                else str(meta.year or "").strip()
            )
            dp_name = self._normalize_aka_year_order(
                dp_name, meta.title, meta.aka, year
            )
    ...
    def _ensure_group_tag(
        ...
        if normalized_name.lower().endswith(normalized_tag.lower()):
            return name
        return (
            f"{normalized_name}{cleaned}"
            if cleaned.startswith("-")
            else f"{normalized_name}-{cleaned}"
        )
    ...
    def _normalize_aka_year_order(
        name: str, title: str, aka: str, year: str
    ) -> str:
        ...
        if re.search(re.escape(expected), name, flags=re.IGNORECASE):
            return re.sub(
                rf"(?i)\b{re.escape(title_value)}\s+{re.escape(year_value)}\s+{re.escape(aka_value)}\b",
    ...
    async def _video_name(self, meta: Meta) -> str:
        ...
        if (
            meta.category == "TV"
            and year
            and not await self._tv_title_needs_year(meta)
        ):
            year = ""
        if meta.no_year:
            year = ""
        ...
        if meta.manual_date:
            ...
            season_episode = (
                ""
                if meta.no_season
                else f"{meta.season or ''}{meta.episode or ''}"
            )
        ...
        resolution = (
            ""
            if str(meta.resolution or "").upper() == "OTHER"
            else str(meta.resolution or "").strip()
        )
        ...
        ds4k = (
            "DS4K"
            if not full_disc_or_remux
            and release_type in {"WEBDL", "WEBRIP", "HDTV"}
            and re.search(r"\bDS4K\b", context, re.IGNORECASE)
            else ""
        )
        hi10p = (
    ...
    def _video_source(meta: Meta, release_type: str) -> str:
        ...
        if release_type in {"WEBDL", "WEBRIP"}:
            return str(meta.service or "").strip() or (
                "" if source.upper() == "WEB" else source
            )
        if release_type == "DVDRIP":
            standard = (
                "NTSC"
                if "NTSC" in source.upper()
                else "PAL"
                if "PAL" in source.upper()
                else ""
            )
        ...
        if release_type == "DISC":
            if source in {"BluRay", "Blu-ray"}:
                ...
                if str(meta.uhd or "").upper() == "UHD":
                    source = "UHD Blu-ray"
            if (
                source in {"PAL DVD", "NTSC DVD"}
                and str(meta.dvd_size or "").strip()
            ):
    ...
    async def _tv_title_needs_year(self, meta: Meta) -> bool:
        title = str(meta.title or "").strip()
        api_key = str(
            self.config.get("DEFAULT", {}).get("tmdb_api", "")
        ).strip()
        if not title:
            return False
        if not api_key:
        ...
        try:
            logger.info(
                f"{self.tracker}: Checking if TMDb has multiple shows with the title '{title}'..."
            )
        ...
        for result_raw in results:
            ...
            if any(
                " ".join(str(candidate or "").casefold().split()) == title_key
                for candidate in names
            ):
                matching_ids.add(result_id)
        matching_ids.discard("")
        return (
            bool(matching_ids - {current_id})
            if current_id
            else len(matching_ids) > 1
        )
    ...
    def _music_name(cls, meta: Meta) -> str:
        ...
        if codec in {"FLAC", "ALAC", "PCM"}:
            ...
            bitrate_mode = (
                str(first_track.get("bitrate_mode") or "").upper().strip()
            )
            if bitrate_mode:
    ...
    def _book_name(meta: Meta) -> str:
        ...
        parts = [
            part
            for part in (author, "-" if author and title else "", title, year)
            if part
        ]
        if (
            not meta.audiobook
            and edition
            and not re.search(r"\b(?:1st|first)\b", edition, re.IGNORECASE)
        ):
        ...
        if meta.audiobook:
            ...
            if (
                format_name in {"MP3", "AAC", "OPUS", "VORBIS"}
                and meta.audiobook_bitrate
            ):
        ...
        if meta.category == "BOOK":
            if type:
                ...
                if t_upper in ("CBR", "CBZ"):
                    ...
                    t_upper = "AUDIOBOOK"
                return {
                    "type_id": type_id.get(t_upper, type_id.get(type, "0"))
                }
            if meta.audiobook:
                meta_type = "AUDIOBOOK"
