...
from src.integrations.trackers.music_validation import MusicValidator, ValidationLevel
...
class DarkPeers(UNIT3D):
    ...
    _AUDIO_TRACK_PATTERN = re.compile(r"^(?:\d{1,3}(?:-\d{1,2})?\.\s+.+|\d{1,3}(?:-\d{1,2})?\s+-\s+.+|\d{1,3}(?:-\d{1,2})?-(?!-).+|.+-\d{1,3}(?:-\d{1,2})?-(?!-).+)$")
    ...
    async def validate_video_quality(self, meta: Meta) -> bool:
        ...
        if min_video_kbps is not None:
            ...
            if video_bitrate < min_video_kbps:
                logger.info(f"{self.tracker}: [bold red]Video bitrate too low for DARKPEERS WEBDL ({video_bitrate} < {min_video_kbps} kbps). Skipping upload.")
                return False
        ...
        if min_audio_kbps is not None and audio_bitrate is not None and audio_bitrate < min_audio_kbps:
            logger.info(f"{self.tracker}: [bold red]Audio bitrate too low for DARKPEERS WEBDL ({audio_bitrate} < {min_audio_kbps} kbps). Skipping upload.")
            return False
    ...
    def validate_video_content(self, meta: Meta) -> bool:
        ...
        if not valid_video_files:
            ...
            return False
        if invalid_files:
            logger.info(f"{self.tracker}: [bold red]Movie/TV uploads should include video files only. Remove non-video files: {', '.join(invalid_files)}. Skipping upload.")
            return False
        return True
    ...
    def _validate_book_file_layout(self, meta: Meta, format_name: str) -> bool:
        ...
        if meta.audiobook:
            ...
            if format_name == "M4B" and len(audio_files) == 1:
                ...
                if actual != expected:
                    logger.info(f"{self.tracker}: [bold red]single-file M4B must be named 'Author - Title - Year.m4b': {audio_files[0].name}. Skipping upload.[/bold red]")
                    return False
