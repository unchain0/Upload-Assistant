...
class Unwalled(UnwalledValidationMixin, UNIT3D):
        ...
        for raw_entry in entries:
            ...
            for plural, name_key, id_key in (
                ...
                if (
                    isinstance(name, str)
                    and name.strip()
                    and str(option_id or "").isdigit()
                ):
                    catalog[plural][cls._normalize_option(name)] = str(
                        option_id
                    )
        return catalog
                ...
                for page in range(1, 101):
                    async with client.stream(
                        ...
                        if 300 <= response.status_code < 400:
                            raise ValueError(
                                "Unwalled option discovery redirect rejected"
                            )
                        response.raise_for_status()
                        bounded_response = await self._bounded_response(
                            response, max_size
                        )
                    ...
                    self.option_catalog["categories"].update(
    ...
    async def get_category_id(
        ...
        catalog = (
            await self.discover_options()
            if mapping_only or reverse
            else self.option_catalog
        )
        if mapping_only:
        ...
        if reverse:
            return {
                option_id: name
                for name, option_id in catalog["categories"].items()
            }
        requested = (
            category
            or meta.unwalled_category
            or str(self.tracker_config.get("category", ""))
        )
        return {
    ...
    async def get_type_id(
        ...
        if reverse:
            return {
                option_id: name for name, option_id in catalog["types"].items()
            }
        requested = (
            type
            or meta.unwalled_type
            or str(self.tracker_config.get("type", ""))
        )
