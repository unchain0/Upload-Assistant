...
class ShareIsland(UNIT3D):
    ...
    INVALID_TAG_PATTERN = re.compile(
        r"-(nogrp|nogroup|unknown|unk)", re.IGNORECASE
    )
    WHITESPACE_PATTERN = re.compile(r"\s{2,}")
    MARKER_PATTERN = re.compile(
        r"\b(UNTOUCHED|VU1080|VU720|VU)\b", re.IGNORECASE
    )
    ...
    async def get_name(self, meta: Meta) -> dict[str, str]:
        ...
        name = self._name_for_type(
            meta, title, audio, languages, release_type, source, extras
        )
    ...
    def _normalized_audio_languages(self, value: Any) -> list[str]:
        ...
        normalized = [
            self._get_language_name(str(language)) for language in values
        ]
        return list(
            dict.fromkeys(language for language in normalized if language)
        )
    ...
    def _two_language_tag(languages: list[str]) -> str:
        ...
        other = next(
            (language for language in languages if language != "ITA"), ""
        )
    ...
    def _hybrid_marker(meta: Meta, title: str) -> str:
        ...
        return (
            "Hybrid"
            if source_is_hybrid and "HYBRID" not in title.upper()
            else ""
        )
    ...
    def _known_type_name(
        ...
        if release_type == "REMUX":
            return self._remux_name(
                meta, title, audio, languages, source, extras
            )
    ...
    def _encoded_type_name(
        ...
        if release_type in {"DVDRIP", "BRRIP"}:
            return self._rip_name(
                meta, title, audio, languages, release_type, extras
            )
    ...
    def _mediainfo_extension(meta: Meta) -> str:
        try:
            return str(
                meta.mediainfo["media"]["track"][0].get("FileExtension", "")
            )
    ...
    def _prompt_region(self) -> str:
        while True:
            value = cli_ui.ask_string(
                "ShareIsland: Region code not found for disc. Please enter it manually (mandatory): "
            )
            region = str(value or "").strip().upper()
            if region:
                return region
            logger.info(
                f"{self.tracker}: Region code is required.",
                extra={"markup": False},
            )
    ...
    async def _validated_region_id(self, region_name: str) -> str:
        ...
        cli_ui.error(
            f"Invalid region code '{region_name}'; skipping ShareIsland."
        )
    ...
    def _distributor_name(meta: Meta) -> str:
        ...
        if existing or meta.unattended:
            return existing
        value = cli_ui.ask_string(
            "ShareIsland: Distributor (optional, Enter to skip): "
        )
    ...
    def _detect_type_from_technical_analysis(self, meta: Meta) -> str:
        ...
        if isinstance(base_type, str) and base_type in (
            "DISC",
            "DVDRIP",
            "BRRIP",
        ):
    ...
    def _analyze_encode_type(self, meta: Meta) -> str:
        ...
        settings_present, settings = self._encoding_field(
            video, "Encoded_Library_Settings"
        )
        library_present, library = self._encoding_field(
        ...
        for detected in (
            ...
            self._service_fingerprint_type(
                service, video, settings, library, library_present
            ),
            self._bluray_encode_type(
                general, video, sources, settings_present, library_present
            ),
            self._web_source_type(general, sources),
            self._source_fallback_type(
                sources, service, settings_present, library_present
            ),
        ):
    ...
    @staticmethod
    def _has_encoding_tools(
        general: dict[str, Any], tools: tuple[str, ...]
    ) -> bool:
        app = str(general.get("Encoded_Application", "")).lower()
        extra = general.get("extra", {})
        frontend = (
            str(extra.get("Writing_frontend", "")).lower()
    ...
    @classmethod
    def _streaming_dv_type(
        cls, general: dict[str, Any], video: dict[str, Any], settings: str
    ) -> str | None:
        profile = str(video.get("HDR_Format_Profile", ""))
        streaming_dv = any(
            marker in profile for marker in ("dvhe.05", "dvhe.07", "dvhe.08")
        )
    ...
    def _service_fingerprint_type(
        ...
        crunchyroll = cls._crunchyroll_type(
            service, settings, library, library_present
        )
        if crunchyroll is not None:
            return crunchyroll
        return (
            "WEBDL"
            if cls._is_netflix_fingerprint(video, settings, library)
            else None
        )

    @classmethod
    def _crunchyroll_type(
        cls, service: str, settings: str, library: str, library_present: bool
    ) -> str | None:
        if service != "CR":
            return None
    ...
    @staticmethod
    def _crunchyroll_library_type(
        library: str, library_present: bool
    ) -> str | None:
    ...
    @staticmethod
    def _is_netflix_fingerprint(
        video: dict[str, Any], settings: str, library: str
    ) -> bool:
    ...
    def _bluray_encode_type(
        ...
        return (
            "ENCODE"
            if cls._bluray_encode_evidence(
                general, video, settings_present, library_present
            )
            else None
        )
    ...
    @classmethod
    def _web_source_type(
        cls, general: dict[str, Any], sources: list[str]
    ) -> str | None:
    ...
    def _source_fallback_type(
        ...
        if cls._has_streaming_service(service):
            return "WEBDL"
        return cls._disc_fallback_type(
            sources, settings_present, library_present
        )
    ...
    @classmethod
    def _disc_fallback_type(
        cls, sources: list[str], settings_present: bool, library_present: bool
    ) -> str | None:
        if cls._has_disc_source(sources):
            return "REMUX"
        dvd_remux = (
            cls._has_dvd_source(sources)
    ...
    def _has_disc_source(sources: list[str]) -> bool:
        return any(
            source in {"BLURAY", "BLU-RAY", "HDDVD"} for source in sources
        )

    @staticmethod
    def _has_dvd_source(sources: list[str]) -> bool:
        return any(
            source in {"NTSC", "PAL", "NTSC DVD", "PAL DVD", "DVD"}
    ...
    def _get_italian_title(self, imdb_info: dict[str, Any]) -> str | None:
        ...
        return (
            ...
            else self._aka_title(akas, "language")
        )
    ...
    async def _get_best_italian_audio_format(self, meta: Meta) -> str:
        ...
        if mediainfo_tracks:
            return await self._formatted_mediainfo_audio(
                meta, mediainfo_tracks
            )
    ...
    def _bdinfo_italian_tracks(cls, meta: Meta) -> list[dict[str, Any]]:
        return [
            track
            for track in cls._bdinfo_audio_tracks(meta)
            if str(track.get("language", "")).lower() in ITALIAN_LANGS
        ]
    ...
    def _mediainfo_italian_tracks(self, meta: Meta) -> list[dict[str, Any]]:
        ...
        return [
            ...
            if self._is_italian_audio_track(track)
        ]
    ...
    @classmethod
    def _bdinfo_audio_quality(
        cls, track: dict[str, Any]
    ) -> tuple[bool, int, bool, int]:
        codec = str(track.get("codec", "")).lower()
        lossless = any(
            marker in codec
            for marker in ("truehd", "dts-hd ma", "flac", "pcm")
        )
        channels = cls._whole_channels(track.get("channels", "2.0"))
        atmos = (
            "atmos" in str(track.get("atmos_why_you_be_like_this", "")).lower()
        )
        bitrate = cls._digits_value(track.get("bitrate", "0"))
        return lossless, channels, atmos, bitrate

    @classmethod
    def _mediainfo_audio_quality(
        cls, track: dict[str, Any]
    ) -> tuple[bool, int, bool, int]:
    ...
    def _safe_bitrate(value: Any) -> int:
        try:
            ...
            cli_ui.warning(
                f"Invalid BitRate value in audio track: {value}\nUsing 0 as default. Error: {error}."
            )
            return 0

    async def _formatted_bdinfo_audio(
        self, meta: Meta, tracks: list[dict[str, Any]]
    ) -> str:
        best = max(tracks, key=self._bdinfo_audio_quality)
        audio, _, _ = await self.audio_manager.get_audio_v2(
            {}, meta, {"audio": [best]}
        )
        return self._clean_audio_string(audio)

    async def _formatted_mediainfo_audio(
        self, meta: Meta, tracks: list[dict[str, Any]]
    ) -> str:
        ...
        audio, _, _ = await self.audio_manager.get_audio_v2(
            {"media": {"track": [general, best]}}, meta, None
        )
        return self._clean_audio_string(audio)

    async def get_description(
        self, meta: Meta, is_test: bool = False
    ) -> dict[str, str]:
    ...
    def _codec_alias(codec: str) -> str:
        ...
        return next(
            (alias for marker, alias in aliases if marker in codec), ""
        )
    ...
    def _description_language(self, meta: Meta) -> str:
        ...
        return (
            preferred
            if preferred
            else self._first_description_language(languages)
        )

    def _description_languages(self, value: Any) -> list[str]:
        values = value if isinstance(value, list) else []
        names = [
            self._get_italian_language_name(self._get_language_code(item))
            for item in values
        ]
    ...
    def _apply_custom_description_header(self, bbcode: str) -> str:
        ...
        header = (
            default.get("custom_description_header", "")
    ...
    async def _fetch_tmdb_italian(self, meta: Meta) -> tuple[str, str]:
        ...
        try:
            ...
            logger.info(
                f"{self.tracker}: [DEBUG] TMDb fetch error: {error}",
                extra={"markup": False},
            )
            return "Riassunto non disponibile.", ""
    ...
    async def _tmdb_media_payload(self, meta: Meta) -> dict[str, Any]:
        ...
        response = await self._tmdb_get(
            url, {"api_key": self._tmdb_api_key(), "language": "it-IT"}
        )
    ...
    @staticmethod
    def _logo_path_for_language(
        logos: list[dict[str, Any]], language: str
    ) -> str:
        return next(
            (
                str(logo.get("file_path") or "")
                for logo in logos
    ...
    def _screen_links(cls, value: Any) -> list[str]:
        ...
        return [
            link for image in images[:6] if (link := cls._screen_link(image))
        ]
    ...
    async def _get_synthetic_mediainfo(
        self, meta: Meta
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.info(
                f"{self.tracker}: [DEBUG] Mediainfo extraction error: {error}",
                extra={"markup": False},
            )
            return None
    ...
    def _preferred_audio_track(
        ...
        italian = next(
            (
                track
                for track in audio
                if self._get_language_code(track) == "it"
            ),
            None,
        )
        return italian if italian is not None else (audio[0] if audio else {})

    def _synthetic_general_info(
        self, meta: Meta, general: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "chap": "Si"
            if self._safe_int(general.get("MenuCount", 0)) > 0
            else "No",
        }
    ...
    def _synthetic_filename(meta: Meta, general: dict[str, Any]) -> str:
        ...
        return (
            Path(str(files[0])).name
            if files
            else str(general.get("FileName", "file.mkv"))
        )
    ...
    def _synthetic_video_info(
        self, meta: Meta, video: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "asp": self._synthetic_aspect_ratio(
                video.get("DisplayAspectRatio")
            ),
        }
    ...
    def _synthetic_video_codec(value: str) -> str:
        ...
        return next(
            (alias for marker, alias in aliases if marker in upper), value
        )
    ...
    def _synthetic_audio_info(self, audio: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "ch": self._synthetic_channels(
                audio.get("Channels", "2") if audio else "2"
            ),
            "aud_br": f"{self._safe_int(audio.get('BitRate', 0)) / 1000:.0f} kb/s"
            if audio
            else "0 kb/s",
            "lang": self._synthetic_audio_language(audio),
        }

    def _synthetic_audio_name(
        self, audio: dict[str, Any], fallback: str
    ) -> str:
        if not audio:
            return fallback
        commercial = self._string_audio_field(
            audio.get("Format_Commercial_IfAny")
        )
        if commercial:
            return commercial
    ...
    def _audio_format_details(audio: dict[str, Any]) -> tuple[str, str]:
        ...
        if (
            raw == "MLP FBA"
            and "atmos"
            in str(audio.get("Format_Commercial_IfAny", "")).lower()
        ):
    ...
    def _synthetic_subtitles(self, tracks: list[dict[str, Any]]) -> str:
        languages = {
            name
            for track in tracks
            if (name := self._subtitle_language_name(track))
        }
    ...
    def _build_bbcode(
        ...
        release_notes = self._release_notes_section(
            release_group, self._tonemapped_text(meta)
        )
    ...
    @staticmethod
    def _release_notes_section(
        release_group: str, tonemapped_text: str
    ) -> str:
        ...
        content = (
            f"{base}\n{tonemapped_text}"
            if island and tonemapped_text
            else (tonemapped_text or base)
        )
    ...
    def _generic_release_group(release_group: str) -> bool:
        return not release_group or release_group.casefold() in {
            "nogroup",
            "nogrp",
            "unknown",
            "unk",
    ...
    def _metadata_link_lines(meta: Meta, category: str) -> list[str]:
        ...
        if meta.imdb:
            lines.append(
                f"[size=11][color=#FFFFFF]IMDb: https://www.imdb.com/title/tt{meta.imdb}/[/color][/size]\n"
            )
        if meta.tmdb:
            media_type = "tv" if category == "TV" else "movie"
            lines.append(
                f"[size=11][color=#FFFFFF]TMDb: https://www.themoviedb.org/{media_type}/{meta.tmdb}[/color][/size]\n"
            )
        return lines
