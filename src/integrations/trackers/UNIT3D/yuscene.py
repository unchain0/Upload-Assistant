...
class YUSCENE(UNIT3D):
    ...
    def _has_english_book_metadata(self, meta: Meta) -> bool:
        ...
        description = " ".join(str(value or "") for value in (meta.book_overview, meta.overview, meta.description, meta.description_file_content) if str(value or "").strip())
    ...
    async def _translate_book_metadata_to_english(self, meta: Meta) -> bool:
        ...
        original_overview = next(
            (str(value).strip() for value in (meta.book_overview, meta.overview, meta.description, meta.description_file_content) if str(value or "").strip()),
            "",
        )
    ...
    async def get_additional_checks(self, meta: Meta) -> bool:
        ...
        if (
            ...
            and self._has_banned_title_chars(release_name)
            and not await self._confirm_or_skip("The release name contains unsupported characters or extra spaces.", meta)
        ):
            return False

        if (
            category in {"MOVIE", "TV", "BOOK"}
            and self._contains_other_tracker_mention(release_name)
            and not await self._confirm_or_skip("The title contains tracker references or tracker domain names.", meta)
        ):
            return False
        ...
        if (
            ...
            and len(filelist) <= 1
            and not await self._confirm_or_skip("Single-file Movie/TV uploads should not be inside a folder for this tracker.", meta)
        ):
            return False
        ...
        if (
            ...
            and not await self._confirm_or_skip("Movie/TV uploads on this tracker require mediainfo in parser field.", meta)
        ):
            return False
        ...
        if category == "TV" and meta.tv_pack:
            ...
            if tv_pack_ended is False and not await self._confirm_or_skip("TV collections are only allowed for ended series on this tracker.", meta):
                return False
            if tv_pack_ended is None and not await self._confirm_or_skip("Unable to confirm TV series status. TV packs are allowed only for ended series at YUSCENE.", meta):
                return False
        ...
        if category in {"MOVIE", "TV"} and screenshot_count < 3 and not await self._confirm_or_skip("YUSCENE requires at least 3 screenshots for Movie/TV uploads.", meta):
            return False
