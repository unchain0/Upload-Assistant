...
class YUSCENE(UNIT3D):
    ...
    async def _confirm_or_skip(self, message: str, meta: Meta) -> bool:
        ...
        if meta.unattended:
            ...
            if not confirmed:
                logger.info(
                    f"{self.tracker}: [bold red]{message} Skipping upload."
                    "[/bold red]"
                )
            return confirmed
    ...
    async def _translate_book_metadata_to_english(self, meta: Meta) -> bool:
        ...
        if not self._has_english_book_metadata(meta):
            return False
        logger.info(
            f"{self.tracker}: [green]Translated BOOK author, title, and"
            " description to English for tracker compliance.[/green]"
        )
        return True
    ...
    async def _translated_book_values(
        ...
        try:
            ...
            logger.info(
                f"{self.tracker}: [bold red]English book metadata translation"
                " HTTP request failed.[/bold red]"
            )
            return None
        except (TypeError, ValueError) as error:
            logger.info(
                f"{self.tracker}: [bold red]English book metadata translation"
                f" failed: {error}[/bold red]"
            )
            return None
    ...
    async def _title_policy(
        ...
        if category not in {"MOVIE", "TV"} or not self._has_banned_title_chars(
            ...
            return True
        return await self._confirm_or_skip(
            "The tracker-formatted release name contains unsupported"
    ...
    async def _folder_policy(
        ...
        if not invalid:
            return True
        return await self._confirm_or_skip(
            "Single-file Movie/TV uploads should not be inside a folder"
            " for this tracker.",
            meta,
        )
    ...
    async def _mediainfo_policy(self, meta: Meta, category: str) -> bool:
        ...
        if not invalid:
            return True
        return await self._confirm_or_skip(
            "Movie/TV uploads on this tracker require mediainfo in parser"
            " field.",
            meta,
        )
    ...
    def _tv_pack_status_message(ended: bool | None) -> str:
        if ended is None:
            return (
                "Unable to confirm TV series status. TV packs are allowed"
                " only for ended series at YUSCENE."
            )
        return (
    ...
    async def _archive_policy(
        ...
        if not archive:
            return True
        return await self._confirm_or_skip(
            f"Archive/RAR files are not allowed for {category} on"
            f" {self.tracker}.",
            meta,
        )
    ...
    async def _game_type_policy(self, meta: Meta, category: str) -> bool:
        ...
        if not valid:
            logger.info(
                f"{self.tracker}: [bold red]No valid tracker type is"
                " available for this GAME package format. Skipping"
                " upload.[/bold red]"
            )
        return valid
    ...
    async def _boxset_policy(
        ...
        if category != "MOVIE" or not any(
            ...
            return True
        return await self._confirm_or_skip(
            "Movie boxset-style naming is not accepted. Upload each movie"
            " separately.",
            meta,
        )
