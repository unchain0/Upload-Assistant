...
class Luminarr(UNIT3D):
    ...
    def _audio_codec(cls, track: dict[str, Any]) -> str:
        ...
        return "VORBIS" if format_name in {"VORBIS", "OGG VORBIS"} else format_name

    @classmethod
    def _mpeg_audio_codec(cls, track: dict[str, Any]) -> str:
        profile = " ".join(str(track.get(key) or "") for key in ("Format_Profile", "CodecID", "CodecID_Hint")).upper()
    ...
    def _is_multi_pass_abr(settings: str) -> bool:
        values = (Luminarr._setting_value(settings, key) for key in ("pass", "stats-read"))
        return any(value.isdigit() and int(value) >= 2 for value in values)

    def _invalid_encode_settings_reason(self, meta: Meta) -> str:
        return next(
            (reason for track in self._video_tracks(meta) if (reason := self._encode_settings_reason(track))),
            "",
        )
    ...
    def _encode_settings_reason(cls, track: dict[str, Any]) -> str:
        settings = cls._settings_text(track.get("Encoded_Library_Settings", ""))
        if not settings:
            return ""
    ...
    def _settings_text(raw_settings: Any) -> str:
        if isinstance(raw_settings, dict):
            return " / ".join(f"{key!s}={value!s}" for key, value in raw_settings.items()).strip()
    ...
    def _is_supplementary_audio(track: dict[str, Any]) -> bool:
        if track.get("is_commentary") is True:
            return True
        description = " ".join(str(track.get(key) or "") for key in ("Title", "title", "ServiceKind", "ServiceKind_String")).casefold()
    ...
    def _invalid_audio_reason(self, meta: Meta) -> str:
        return next(
            (reason for track in self._audio_tracks(meta) if (reason := self._audio_track_reason(meta, track))),
            "",
        )
    ...
    def _invalid_supplementary_codec(self, codec: str, track: dict[str, Any]) -> bool:
        return codec in {"MP3", "VORBIS"} and not self._is_supplementary_audio(track)

    def _invalid_primary_mp2(self, meta: Meta, codec: str, track: dict[str, Any]) -> bool:
    ...
    def _initial_policy_passes(self, meta: Meta, category: str, filelist: list[Any]) -> bool:
    ...
    def _final_policy_passes(self, meta: Meta, category: str, filelist: list[Any]) -> bool:
    ...
    def _format_and_screen_policy_passes(self, meta: Meta, category: str) -> bool:
        return self._format_policy_passes(meta) and self._screens_policy_passes(meta, category)
    ...
    def _category_and_id_policy_passes(self, meta: Meta, category: str) -> bool:
    ...
    def _file_layout_policy_passes(self, meta: Meta, filelist: list[Any]) -> bool:
    ...
    def _multi_file_layout_failure(self, meta: Meta, video_paths: list[Path]) -> str:
        ...
        return "" if meta.tv_pack else "Movie and non-collection TV uploads should contain one video file per title."
    ...
    async def _resolution_policy_passes(self, meta: Meta) -> bool:
        if meta.is_disc in self._DISC_TYPES or meta.resolution in {"8640p", "4320p", "2160p", "1440p", "1080p", "1080i", "720p"}:
            return True
    ...
    def _codec_and_encode_policy_passes(self, meta: Meta) -> bool:
        ...
        if audio_reason:
            logger.info(f"{self.tracker}: [bold red]{audio_reason} Skipping upload.[/bold red]")
            return False
        encode_reason = self._invalid_encode_settings_reason(meta)
        if encode_reason:
            logger.info(f"{self.tracker}: [bold red]{encode_reason} Skipping upload.[/bold red]")
            return False
        return True
    ...
    def _extra_files_policy_passes(self, meta: Meta, filelist: list[Any]) -> bool:
        if meta.is_disc:
            return True
    ...
    def _tv_policy_passes(self, meta: Meta, category: str, filelist: list[Any]) -> bool:
        if category != "TV":
            return True
    ...
    def _tv_pack_and_episode_policy_passes(self, meta: Meta, filelist: list[Any]) -> bool:
