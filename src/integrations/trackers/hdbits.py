...
class HDBits:
    ...
    def __init__(self, config: Config) -> None:
        ...
        tracker_config = trackers.get("HDBITS", {}) if isinstance(trackers, dict) else {}
    ...
    def _concert_category(meta: Meta) -> int | None:
        ...
        return 4 if "concert" in imdb_type or ("video" in imdb_type and "music" in genres) else None
    ...
    async def get_tags(self, meta: Meta) -> list[int]:
        ...
        self._append_mapped_tag(tags, meta.distributor, self._distributor_tags())
    ...
    @staticmethod
    def _append_mapped_tag(tags: list[int], value: Any, mapping: dict[str, int]) -> None:
    ...
    def _append_audio_tags(self, tags: list[int], meta: Meta) -> None:
        ...
        if meta.silent is True:
            logger.info(f"{self.tracker}: [yellow]zxx audio track found, suggesting you tag as silent")
    ...
    def _normalize_hdr_name(name: str, meta: Meta) -> str:
        result = name.replace(" DV ", " DoVi ") if "DV" in str(meta.hdr) else name
    ...
    def _apply_imdb_name(cls, name: str, meta: Meta) -> str:
        ...
        if not imdb:
            return result
        result = result.replace(str(meta.title), str(imdb.get("aka", meta.title)))
    ...
    def _finalize_hdb_name(name: str) -> str:
        result = name.replace("PQ10", "HDR").replace("Dubbed", "").replace("Dual-Audio", "")
        result = result.replace("REMUX", "Remux").replace("BluRay Remux", "Remux").replace("UHD Remux", "Remux")
    ...
    async def upload(self, meta: Meta) -> bool | None:
        ...
        data, files, torrent_path = await self._upload_request_parts(meta, identifiers)
        if meta.debug:
    ...
    def _valid_upload_identifiers(values: dict[str, Any]) -> bool:
        return all(values.get(key) not in {0, "0", None} for key in ("category", "codec", "medium"))

    @staticmethod
    def _dual_audio_forbidden(meta: Meta) -> bool:
        return "Dual-Audio" in str(meta.audio) and not meta.anime and bool(meta.is_disc)
    ...
    async def _rehash_upload_torrent(self, meta: Meta) -> None:
        tracker_url = str(self._tracker_config().get("announce_url", "https://fake.tracker")).strip()
        cooldown = self._rehash_cooldown()
        if cooldown > 0:
            await asyncio.sleep(cooldown)
        torrent_name = f"[{self.tracker}]"
        await TorrentCreator.create_torrent(meta, str(meta.path), torrent_name, tracker_url=tracker_url, piece_size=16)
        await self.common.create_torrent_for_upload(meta, self.tracker, self.source_flag, torrent_filename=torrent_name)
    ...
    def _rehash_cooldown(self) -> int:
        ...
        value = default.get("rehash_cooldown", 0) if isinstance(default, dict) else 0
        try:
            return max(0, int(value or 0))
        except TypeError, ValueError:
            return 0

    async def _upload_request_parts(self, meta: Meta, identifiers: dict[str, Any]) -> tuple[dict[str, Any], dict[str, tuple[str, bytes, str]], Path]:
        ...
        files = {"file": (f"{self._torrent_filename(meta)}.torrent", torrent_bytes, "application/x-bittorrent")}
    ...
    async def _upload_data(self, meta: Meta, identifiers: dict[str, Any], description: str) -> dict[str, Any]:
        data: dict[str, Any] = {
    ...
    async def _post_upload(self, meta: Meta, data: dict[str, Any], files: dict[str, tuple[str, bytes, str]]) -> httpx.Response:
        ...
        async with httpx.AsyncClient(cookies=cookies, timeout=30.0, follow_redirects=True) as client:
            return await client.post(url=f"{self.base_url}/upload/upload", data=data, files=files)

    async def _handle_upload_response(self, meta: Meta, response: httpx.Response, torrent_path: Path) -> bool:
        match = re.match(rf".*?{re.escape(self.base_url.replace('https://', ''))}/details\.php\?id=(\d+)&uploaded=(\d+)", str(response.url))
        if match is None:
            logger.info(response.text)
            raise UploadError(f"Upload to HDBITS Failed: result URL {response.url} ({response.status_code}) was not expected", "red")
    ...
    async def search_existing(self, meta: Meta) -> list[dict[str, Any]]:
        ...
        for term in terms:
            logger.info(f"{self.tracker}: [yellow]Searching {self.tracker} for: {escape(str(term))}")
    ...
    def _fallback_search_candidates(self, meta: Meta) -> list[str]:
        candidates = (str(meta.filename or ""), self._clean_aka(meta.aka), str(meta.basename_no_ext or ""))
    ...
    async def _search_api(self, url: str, data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def _search_results(self, payload: Any) -> list[dict[str, Any]]:
        ...
        return [self._search_result(item) for item in values if isinstance(item, dict)]
    ...
    def _search_result(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "files": filename[:-8] if filename.endswith(".torrent") else filename,
    ...
    async def _torrent_filename_from_api(self, torrent_id: str) -> str:
        ...
        data = {"username": self.username, "passkey": self.passkey, "id": torrent_id}
    ...
    @staticmethod
    def _json_object(response: httpx.Response, api_url: str, data: dict[str, Any]) -> dict[str, Any]:
        try:
            payload = response.json()
        except json.JSONDecodeError as error:
            raise ValueError(f"Failed to parse JSON response from {api_url}. Response content: {response.text}. Data: {data}. Error: {error}") from error
        if not isinstance(payload, dict):
            raise ValueError(f"Invalid JSON response from {api_url}: expected object. Response: {payload}. Data: {data}")
        return cast(dict[str, Any], payload)

    @staticmethod
    def _torrent_filename_from_payload(payload: dict[str, Any], api_url: str, data: dict[str, Any]) -> str:
        rows = payload.get("data")
        if not isinstance(rows, list) or not rows:
            raise ValueError(f"Invalid JSON response from {api_url}: 'data' key missing, not a list, or empty. Response: {payload}. Data: {data}")
        first = rows[0]
        if not isinstance(first, dict) or not first.get("filename"):
            raise ValueError(f"Failed to access filename in response from {api_url}. Response: {payload}. Data: {data}")
        return str(first["filename"])

    async def _download_torrent_response(self, torrent_id: str, filename: str) -> httpx.Response:
    ...
    @staticmethod
    def _validate_torrent_download(response: httpx.Response, filename: str, torrent_id: str) -> None:
        content_type = response.headers.get("content-type", "").lower()
        if "bittorrent" not in content_type and "octet-stream" not in content_type:
            raise ValueError(f"Unexpected content-type for torrent download: {content_type}. File: {filename}. ID: {torrent_id}")
        if not response.content.startswith(b"d"):
            raise ValueError(f"Downloaded content does not appear to be a valid torrent file (does not start with 'd'). File: {filename}. ID: {torrent_id}")
    ...
    def _web_source_note(meta: Meta) -> str:
        if meta.type != "WEBDL" or not meta.service_longname or not meta.description:
            return ""
    ...
    def _disc_description_parts(cls, meta: Meta) -> list[str]:
        ...
        parts.extend(cls._additional_disc_description(disc) for disc in discs[1:])
    ...
    def _comparison_header(meta: Meta) -> str:
        groups = meta.comparison_groups if isinstance(meta.comparison_groups, dict) else {}
        if not groups:
            return ""
        names: list[str] = []
        for index in sorted(groups, key=lambda value: int(str(value))):
            group = groups.get(index, {})
            group_mapping = cast(dict[str, Any], group) if isinstance(group, dict) else {}
    ...
    def _manual_image_links(cls, meta: Meta) -> list[str]:
        ...
        return [link for image in images[:limit] if (link := cls._manual_image_link(image))]
    ...
    def _manual_image_link(image: dict[str, Any]) -> str:
        ...
        return f"[url={web_url}][img]{img_url}[/img][/url]" if img_url and web_url else ""
    ...
    async def hdbimg_upload(self, meta: Meta) -> str | None:
        ...
        return await self._safe_image_upload(meta, upload_files, group_indices, data)

    async def _planned_upload_files(self, meta: Meta, image_files: list[str]) -> dict[str, tuple[str, bytes, str]]:
    ...
    async def _safe_image_upload(
        ...
        try:
            return await self._execute_image_upload(meta, upload_files, group_indices, data)
    ...
    async def _execute_image_upload(
        ...
        bbcode = await self._upload_comparison_images(upload_files, group_indices, data)
        return None if bbcode is None else self._format_comparison_bbcode(bbcode, len(group_indices) or 3)

    async def _image_upload_plan(self, meta: Meta) -> tuple[list[str], str, list[str]] | None:
        if meta.comparison:
    ...
    def _comparison_image_plan(self, meta: Meta) -> tuple[list[str], str, list[str]] | None:
        comparison_path = Path(str(meta.comparison or ""))
        if not comparison_path.is_dir():
            logger.info(f"{self.tracker}: [red]Comparison path not found: {meta.comparison}")
            return None
    ...
    def _comparison_group_images(self, meta: Meta, comparison_path: Path) -> dict[str, list[str]]:
        groups = meta.comparison_groups
        if isinstance(groups, dict):
            return self._declared_comparison_groups(cast(dict[str, Any], groups), comparison_path)
        return self._discovered_comparison_groups(comparison_path)

    @classmethod
    def _declared_comparison_groups(cls, groups: dict[str, Any], comparison_path: Path) -> dict[str, list[str]]:
        return {str(group_index): cls._declared_group_files(value, comparison_path) for group_index, value in groups.items()}

    @classmethod
    def _declared_group_files(cls, value: Any, comparison_path: Path) -> list[str]:
        mapping = cast(dict[str, Any], value) if isinstance(value, dict) else {}
        names = cls._comparison_file_names(mapping.get("files", []))
        paths = [comparison_path / name for name in sorted(names, key=cls._comparison_file_order)]
    ...
    @staticmethod
    def _discovered_comparison_groups(comparison_path: Path) -> dict[str, list[str]]:
        ...
        return {group: [path for _, path in sorted(entries)] for group, entries in groups.items()}

    @staticmethod
    def _interleaved_group_images(groups: dict[str, list[str]], indices: list[str]) -> list[str]:
        count = max((len(groups[index]) for index in indices), default=0)
        return [groups[index][image] for image in range(count) for index in indices if image < len(groups[index])]
    ...
    async def _load_image_upload_files(self, image_files: list[str]) -> dict[str, tuple[str, bytes, str]]:
    ...
    async def _load_image_upload_file(self, file_path: str) -> tuple[str, bytes, str] | None:
        try:
            path = Path(file_path)
            async with aiofiles.open(path, "rb") as handle:
            ...
            logger.error(f"{self.tracker}: [red]Failed to open {escape(file_path)}: {escape(str(error))}")
            return None

    def _image_upload_data(self, meta: Meta, thumb_size: str) -> dict[str, Any]:
        return {"username": self.username, "passkey": self.passkey, "galleryoption": "1", "galleryname": meta.name, "thumbsize": thumb_size}

    async def _upload_image_batch(self, files: dict[str, tuple[str, bytes, str]], data: dict[str, Any]) -> str | None:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post("https://img.hdbits.org/upload_api.php", data=data, files=files)
        if response.status_code == 200:
    ...
    async def _upload_comparison_images(
        ...
        for index, chunk in enumerate(chunks):
            response = await self._upload_comparison_chunk(index, len(chunks), chunk, data)
            if response is None:
                return None
    ...
    def _comparison_rows(
        ...
        return [entries[index : index + group_count] for index in range(0, len(entries), group_count)]
    ...
    def _chunk_comparison_rows(
        ...
        for row in rows:
            ...
            if HDBits._comparison_chunk_full(current, current_size, row_size, max_size):
    ...
    async def _upload_comparison_chunk(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post("https://img.hdbits.org/upload_api.php", data=data, files=files)
        if response.status_code == 200:
            logger.info(f"{self.tracker}: [green]Chunk {index + 1}/{total} upload successful!")
            return response.text
        logger.info(f"{self.tracker}: [red]Chunk {index + 1}/{total} upload failed with status code {response.status_code}")
        return None

    @staticmethod
    def _format_comparison_bbcode(bbcode: str, group_count: int) -> str:
        matches = re.findall(r"\[url=.*?\]\[img\].*?\[/img\]\[/url\]", bbcode)
        rows = [" ".join(matches[index : index + group_count]) for index in range(0, len(matches), group_count)]
        return "\n".join(rows)
    ...
    async def get_info_from_torrent_id(self, hdb_id: int) -> tuple[int | None, int | None, str | None, str | None, str | None]:
        try:
            ...
            logger.info(f"{self.tracker}: [red]Request error: {escape(str(error))}[/red]")
            return None, None, None, None, None
        except (ValueError, KeyError, IndexError, TypeError) as error:
            logger.error(f"{self.tracker}: [red]Failed to parse HDBITS response. Error: {escape(str(error))}[/red]")
            return None, None, None, None, None
    ...
    def _torrent_info_tuple(self, payload: dict[str, Any]) -> tuple[int | None, int | None, str | None, str | None, str | None]:
        if payload.get("status") != 0 or not payload.get("data"):
            logger.info(f"{self.tracker}: [red]API returned error status {payload.get('status', 'unknown')}: {payload.get('message', 'No error message provided')}[/red]")
            return None, None, None, None, None
    ...
    async def search_filename(self, search_term: str, search_file_folder: str, meta: Meta) -> tuple[int | None, int | None, str | None, str | None, str | None, int | None]:
        data = await self._filename_search_payload(search_term, search_file_folder, meta)
        if data is None:
    ...
    async def _filename_search_payload(self, search_term: str, search_file_folder: str, meta: Meta) -> dict[str, Any] | None:
        base = {"username": self.username, "passkey": self.passkey, "limit": 100}
        if search_file_folder == "folder" and meta.is_disc:
            title = await self._disc_search_title(meta)
            if not title:
                return None
            logger.info(f"{self.tracker}: [green]Searching {self.tracker} for title: [bold yellow]{escape(title)}[/bold yellow]")
            return {**base, "search": title}
        filename = Path(search_term).name
        logger.info(f"{self.tracker}: [green]Searching {self.tracker} for file: [bold yellow]{escape(filename)}[/bold yellow]")
    ...
    async def _disc_search_title(self, meta: Meta) -> str:
        ...
        try:
            ...
            logger.error(f"{self.tracker}: [red]Error: File not found at {path}[/red]")
            return ""
    ...
    async def _filename_search_response(self, data: dict[str, Any]) -> dict[str, Any]:
        url = f"{self.base_url}/api/torrents"
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(url, json=data)
    ...
    def _filename_search_result(self, payload: dict[str, Any]) -> tuple[int | None, int | None, str | None, str | None, str | None, int | None]:
        rows = payload.get("data")
        if not isinstance(rows, list):
            logger.error(f"{self.tracker}: [red]Error: 'data' key not found or empty in {self.tracker} API response. Full response: {escape(str(payload))}[/red]")
            return self._empty_filename_result()
        if not rows:
        ...
        logger.info(f"{self.tracker}: [bold green]Matched release with {self.tracker} ID: [yellow]{self.base_url}/details.php?id={result[-1]}[/yellow][/bold green]")
        return result

    @classmethod
    def _filename_result_tuple(cls, entry: dict[str, Any]) -> tuple[int, int, str | None, str | None, str | None, int | None]:
