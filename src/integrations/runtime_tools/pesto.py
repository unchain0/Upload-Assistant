...
class PestoBinaryManager:
    ...
    async def ensure_pesto_binary(
        ...
        if system not in platform_map or machine not in platform_map[system]:
            raise Exception(
                f"Unsupported platform for Pesto: {system} {machine}"
            )
        ...
        logger.info(
            "[yellow]Binary 'pesto' not found. Attempting to download automatically...[/yellow]"
        )
