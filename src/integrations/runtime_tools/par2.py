...
class Par2BinaryManager:
    ...
    async def ensure_par2_binary(
        ...
        platform_map: dict[str, dict[str, dict[str, str]]] = {
            "windows": {
                "x86_64": {
                    "file": f"par2cmdline-turbo-{v_num}-win-x64.zip",
                ...
                "arm64": {
                    "file": f"par2cmdline-turbo-{v_num}-win-arm64.zip",
                    "folder": "windows/arm64",
                },
            },
            "darwin": {
                "arm64": {
                    "file": f"par2cmdline-turbo-{v_num}-macos-arm64.zip",
                    "folder": "macos/arm64",
                },
                "x86_64": {
                    "file": f"par2cmdline-turbo-{v_num}-macos-amd64.zip",
                    "folder": "macos/x86_64",
                },
                "amd64": {
                    "file": f"par2cmdline-turbo-{v_num}-macos-amd64.zip",
            ...
            "linux": {
                "x86_64": {
                    "file": f"par2cmdline-turbo-{v_num}-linux-amd64.zip",
                    "folder": "linux/amd64",
                },
                "amd64": {
                    "file": f"par2cmdline-turbo-{v_num}-linux-amd64.zip",
                ...
                "arm64": {
                    "file": f"par2cmdline-turbo-{v_num}-linux-arm64.zip",
                    "folder": "linux/arm64",
                },
                "aarch64": {
                    "file": f"par2cmdline-turbo-{v_num}-linux-arm64.zip",
        ...
        if system not in platform_map or machine not in platform_map[system]:
            raise Exception(
                f"Unsupported platform for PAR2: {system} {machine}"
            )
        ...
        logger.info(
            "[yellow]Binary 'par2' not found. Attempting to download automatically...[/yellow]"
        )
        ...
        try:
            ...
            logger.debug(
                f"[green]Downloaded PAR2 package: {file_pattern}[/green]"
            )
