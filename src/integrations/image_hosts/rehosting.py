    ...
    *,
    ...
    try:
        for collection_name in ("menu_images", "spectrograms_images", "dynamic_hdr_plot_images"):
            ...
            normalized_collection = [dict(item) for item in collection if isinstance(item, dict)]
...
async def _handle_image_upload(
    ...
    while current_position < len(configured_indices):
        ...
        if current_img_host is None or current_img_host not in approved_image_hosts:
