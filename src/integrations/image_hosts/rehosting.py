...
async def _handle_image_upload(
    ...
    retry_mode = False
    images_reuploaded = False
    filelist = [meta.video] if meta.video else []
    filename = meta.title
    ...
    all_screenshots: list[str] = [
        str(path) for path in manifest_files(base_dir, folder_id, "main")
    ]
    ...
    if meta.image_list and isinstance(meta.image_list, list):
        ...
        all_png_files: list[str] = [
            str(screenshot_path / name)
            for name in await aio_os.listdir(screenshot_path)
        ...
        for image in cast(list[dict[str, str]], meta.image_list):
            for url_key in ["raw_url", "img_url", "web_url"]:
                ...
                if url_value:
                    ...
                    if (
                        filename_from_url
                        and filename_from_url.lower().endswith(".png")
                    ):
        ...
        if image_filenames and meta.debug:
            logger.info(
                f"[cyan]Extracted {len(image_filenames)} filenames from image_list URLs: {image_filenames}"
            )
        ...
        if all_png_files and image_filenames:
            for png_file in all_png_files:
                ...
                if basename in image_filenames:
                    ...
                    logger.debug(
                        f"[green]Found existing screenshot matching URL: {basename}"
                    )
        ...
        if filename and len(all_screenshots) < multi_screens:
            ...
            title_pattern_files = [
                ...
                if Path(f).name.startswith(sanitized_title)
            ]
            logger.debug(
            ...
            if title_pattern_files:
                ...
                logger.debug(
                    f"[green]Found {len(title_pattern_files)} screenshots matching title pattern"
                )
        ...
        for _file in filelist:
            ...
            if meta.is_disc == "DVD":
                existing_screens = await asyncio.to_thread(
                    lambda: [
                        str(p)
                        for p in screenshots_dir(
                            meta.base_dir, meta.uuid
                        ).glob(f"{glob.escape(meta.discs[0]['name'])}-*.png")
                    ]
                )
            else:
                existing_screens = await asyncio.to_thread(
                    lambda fp=filename_pattern: [
                        str(p) for p in screenshot_path.glob(fp)
                    ]
                )
    ...
    if len(all_screenshots) < multi_screens:
        ...
        for pattern in image_patterns:
            glob_results = await asyncio.to_thread(
                lambda p=pattern: [
                    str(path) for path in screenshot_path.glob(p)
                ]
            )
            image_glob.extend(glob_results)
            logger.debug(
                f"[cyan]Found {len(image_glob)} files matching pattern: {pattern}"
            )
        ...
        for pattern in unwanted_patterns:
            ...
            if (
                ...
                or pattern.startswith("PLAYLIST")
                or pattern.startswith("POSTER")
            ):
                hidden_pattern = "." + pattern
                hidden_glob_results = await asyncio.to_thread(
                    lambda hp=hidden_pattern: [
                        str(path) for path in screenshot_path.glob(hp)
                    ]
                )
        ...
        image_glob = [
            file for file in image_glob if file not in unwanted_files
        ]
        image_glob = list(set(image_glob))
        logger.debug(
            f"[cyan]Filtered out {len(unwanted_files)} unwanted files, remaining: {len(image_glob)}"
        )
        ...
        indexed_files: list[str] = [
            ...
            if indexed_pattern.match(Path(file).name)
        ]
        logger.debug(
            f"[cyan]Found {len(indexed_files)} indexed files matching pattern"
        )
        ...
        for screen in indexed_files:
            if screen not in all_screenshots:
                all_screenshots.append(screen)
                logger.debug(
                    f"[green]Found indexed screenshot: {Path(screen).name}"
                )
    ...
    if tracker == "covers":
        ...
        existing_screens = await asyncio.to_thread(
            lambda: [
                str(p)
                for p in artwork_dir(meta.base_dir, meta.uuid).glob(
                    "cover_*.jpg"
                )
            ]
        )
    ...
    if len(all_screenshots) < multi_screens:
        ...
        logger.debug(
            f"[yellow]Found {len(all_screenshots)} screenshots, need {needed_screenshots} more to reach {multi_screens} total."
        )
        ...
        try:
            if meta.is_disc == "BDMV":
                ...
                await takescreens_manager.dvd_screenshots(
                    meta, disc_num=0, retry_cap=True
                )
            elif meta.category == "XXX":
                await takescreens_manager.xxx_contact_sheets(
                    meta.filelist or [], folder_id, base_dir, meta
                )
            else:
                if path:
            ...
            if meta.is_disc == "DVD":
                new_screens = await asyncio.to_thread(
                    lambda: [
                        str(p)
                        for p in screenshots_dir(
                            meta.base_dir, meta.uuid
                ...
                new_screens = [
                    str(path)
                    for path in manifest_files(base_dir, folder_id, "main")
                ]

            # Add any new screenshots to our list (only those not already in all_screenshots)
            if new_screens and meta.debug:
                logger.info(
                    f"[green]Found {len(new_screens)} new screenshots after generation"
                )
            ...
            for screen in new_screens:
                if screen not in all_screenshots:
                    all_screenshots.append(screen)
                    logger.debug(
                        f"[green]Added new screenshot: {Path(screen).name}"
                    )
    ...
    if not all_screenshots:
        logger.info(
            "[red]No screenshots were generated or found. Please check the screenshot generation process."
        )
        return [], True, images_reuploaded
    ...
    for screenshot in all_screenshots:
        ...
        if image_list_entries and any(
            Path(urlparse(_as_str(img.get("raw_url")) or "").path).name
            == basename
    ...
    all_screenshots = (
        all_screenshots
        if tracker == "covers"
        else final_screenshots[:multi_screens]
    )
    ...
    while current_position < len(configured_indices):
        ...
        if (
            current_img_host is None
            or current_img_host not in approved_image_hosts
        ):
            logger.info(
                f"[red]Your preferred image host '{current_img_host}' is not supported at {tracker}, trying next host."
            )
            retry_mode = True
        ...
        selected_host = True
        logger.debug(
            f"[green]Uploading to approved host '{current_img_host}'."
        )
        break

    if not selected_host:
        logger.info(
            "[red]No approved image host was selected; skipping upload."
        )
        return [], True, images_reuploaded
    ...
    if uploaded_images:
        set_tracker_image_collection(
            meta, tracker, "screenshots", uploaded_images
        )

    logger.debug(
        f"[debug] Updated tracker screenshots for {tracker} with {len(uploaded_images)} images."
    )
    for image in uploaded_images:
        logger.debug(
            f"[debug] Response in upload_image_task: {image['img_url']}, {image['raw_url']}, {image['web_url']}"
        )
    ...
    for image in cast(list[dict[str, str]], tracker_images):
        ...
        if mapped_host not in approved_image_hosts:
            logger.info(
                f"[red]Unsupported image host detected in URL '{raw_url}'. Please use one of the approved image hosts."
            )
            if original_imghost:
                meta.imghost = original_imghost
            return (
                ...
                True,
                images_reuploaded,
            )  # Trigger retry_mode if switching hosts
    ...
    if uploaded_images and all(valid_hosts):
        output_file = (
            Path(meta.base_dir) / "tmp" / meta.uuid / "covers.json"
            if tracker == "covers"
            else screenshot_path / "reuploaded_images.json"
        )
        ...
        try:
            async with aiofiles.open(output_file, encoding="utf-8") as f:
                ...
                loaded_value: object = (
                    json.loads(existing_data_raw) if existing_data_raw else []
                )
                if isinstance(loaded_value, list):
                    existing_data = cast(list[dict[str, str]], loaded_value)
                else:
                    logger.info(
                        f"[red]Existing data in {output_file} is not a list. Resetting to an empty list."
                    )
        except Exception:
            existing_data = []
        ...
        updated_data = [
            dict(s) for s in {tuple(d.items()) for d in updated_data}
        ]
        ...
        if tracker == "covers" and "release_url" in meta:
            ...
            logger.info(
                f"[green]Added release URL to {len(updated_data)} cover images: {meta.release_url}"
            )
            ...
            if tracker == "covers":
                ...
                for screenshot in all_screenshots:
                    if _safe_remove(screenshot):
                        deleted_count += 1
                        logger.debug(
                            f"[dim]Deleted cover image file: {screenshot}[/dim]"
                        )

                if deleted_count > 0 and meta.debug:
                    logger.info(
                        f"[green]Cleaned up {deleted_count} cover image files after successful upload[/green]"
                    )
