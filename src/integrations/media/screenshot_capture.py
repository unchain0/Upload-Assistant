...
def _capture_size_is_valid(host: str | None, image_size: int, *, minimum_size: int = 75_000) -> bool:
    ...
    if "imgbb" in normalized:
        return image_host_size_within_limit("imgbb", image_size)
    if normalized in {"imgbox", "pixhost"}:
        return image_host_size_within_limit(normalized, image_size)
    if normalized == "lostimg":
    ...
    try:
        ...
        if meta.frame_overlay:
            ...
            if "pts_time" in frame_info:
                ...
                if pts_time > 1.0 and abs(pts_time - ss_time_value) < 10:
...
async def capture_screenshot(
    ...
    try:
        ...
        def filter_chain(*, hardware_libplacebo: bool) -> str:
            filters: list[str] = []
            if w_sar != 1 or h_sar != 1:
        ...
        def build_command(*, hardware_libplacebo: bool) -> Any:
            chain = filter_chain(hardware_libplacebo=hardware_libplacebo)
            command = (
                cast(Any, ffmpeg)
