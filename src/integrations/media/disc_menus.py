...
class DiscMenus:
    ...
    async def auto_capture_dvd_menus(self, meta: Meta) -> None:
        ...
        has_supported_discs = any(
            disc.get("type") in ("DVD", "HDDVD") for disc in meta.discs
        )
        if not has_supported_discs:
            logger.debug(
                "No supported DVD/HDDVD discs found in metadata; skipping menu auto-capture."
            )
            return
        ...
        try:
            ...
            max_menu_screens = 6
        screenshot_config = (
            cast(Mapping[str, Any], default_section)
        ...
        if configured_ffmpeg is None and platform.system() == "Linux":
            ...
            arch = (
                ...
                else ("arm" if machine in ("aarch64", "arm64") else None)
            )
            if arch:
        ...
        for disc in meta.discs:
            ...
            if disc_type == "HDDVD":
                logger.warning(
                    f"[yellow]HD-DVD menu capture is not supported. Skipping HD-DVD: {disc.get('name', 'Unknown')}[/yellow]"
                )
                continue
            if disc_type != "DVD":
                continue
            ...
            try:
                for file in (p.name for p in Path(disc_path).iterdir()):
                    ...
                    if disc_type == "DVD" and file_lower.endswith(".vob"):
                        ...
                        if file_name == "VIDEO_TS.VOB" or re.match(
                            r"^VTS_\d{2}_0\.VOB$", file_name
                        ):
                            file_path = Path(disc_path) / file
                            if (
                                file_path.is_file()
                                and Path(file_path).stat().st_size > 50000
                            ):
                                menu_files.append((file, file_path))
            except Exception as e:
                logger.error(
                    f"[red]Error scanning directory {disc_path} for menus: {e}[/red]"
                )
                continue
            ...
            for file, file_path in menu_files:
                try:
                    ...
                    if not video_track:
                        logger.debug(
                            f"Skipping {file} because it does not have a video track."
                        )
                        continue

                    # Extract details
                    width = (
                        int(video_track.width) if video_track.width else 720
                    )
                    height = (
                        int(video_track.height) if video_track.height else 480
                    )
                    par = (
                        float(video_track.pixel_aspect_ratio)
                        if video_track.pixel_aspect_ratio
                        else 1.0
                    )
                    dar = (
                        float(video_track.display_aspect_ratio)
                    ...
                    logger.error(
                        f"[red]Error parsing MediaInfo for {file}: {e}[/red]"
                    )
                    width, height, par, dar, duration_ms = (
                        720,
                        480,
                        1.0,
                        1.3333,
                ...
                w_sar, h_sar = screenshot_par_scale_factors(
                    width, height, par, dar, scale_for_par
                )
                ...
                sanitized_disc_name = re.sub(
                    r'[<>:"/\\|?*]', "_", disc.get("name", "dvd")
                )
                vob_base = Path(file).stem
                image_pattern = (
                    Path(output_dir)
                    / f"{sanitized_disc_name}-{vob_base}-%03d.png"
                )
                ...
                if duration_sec < 2.0:
                    ...
                    logger.info(
                        f"Extracting static menu frames from {file} (limit: {limit})..."
                    )
                else:
                    # Motion menu: try scene detection first
                    limit = max(30, max_menu_screens * 3)
                    scene_vf_chain = ",".join(
                        ["select='gt(scene,0.25)'", *vf_filters]
                    )
                    cmd = [
                        ffmpeg_path,
                        "-y",
                    ...
                    logger.info(
                        f"Extracting motion menu frames via scene detection from {file} (limit: {limit})..."
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            f"[red]FFmpeg timed out processing {file}[/red]"
                        )

                    # Gather generated screenshots
                    glob_pattern = (
                        Path(output_dir)
                        / f"{sanitized_disc_name}-{vob_base}-*.png"
                    )
                    found_images = (
                        sorted(
                            str(p)
                            for p in glob_pattern.parent.glob(
                                glob_pattern.name
                            )
                        )
                    ...
                    if process.returncode != 0:
                        logger.error(
                            f"[red]FFmpeg failed processing {file}: {_stderr.decode(errors='replace')}[/red]"
                        )
                        discard_previous_menu_capture_files(image_pattern)
                        ...
                        try:
                            with Image.open(img_path) as img:
                                ...
                                if (
                                    ...
                                    logger.debug(
                                        f"Skipping {Path(img_path).name} because it is a blank/black frame."
                                    )
                            ...
                            logger.debug(
                                f"Failed to check if {img_path} is black: {e}"
                            )
                            valid_images.append(img_path)
                    ...
                    if duration_sec >= 2.0 and not found_images:
                        logger.info(
                            f"Scene detection returned no valid frames for {file}. Falling back to interval sampling..."
                        )
                        ...
                        logger.debug(
                            f"Fallback FFmpeg command: {' '.join(cmd_fallback)}"
                        )
                        discard_previous_menu_capture_files(image_pattern)
                        process_fallback = (
                        ...
                        try:
                            (
                                ...
                                process_fallback.communicate(), timeout=30.0
                            )
                        except TimeoutError:
                            with contextlib.suppress(Exception):
                            ...
                            logger.error(
                                f"[red]FFmpeg fallback timed out processing {file}[/red]"
                            )
                        ...
                        if process_fallback.returncode != 0:
                            logger.error(
                                f"[red]FFmpeg fallback failed processing {file}: {fallback_stderr.decode(errors='replace')}[/red]"
                            )
                            discard_previous_menu_capture_files(image_pattern)
                        valid_images = []
                        for img_path in found_images:
                            try:
                                with Image.open(img_path) as img:
                                    ...
                                    if (
                                        ...
                                        logger.debug(
                                            f"Skipping fallback frame {Path(img_path).name} because it is a blank/black frame."
                                        )
                                        Path(img_path).unlink()
                                        continue
                                valid_images.append(img_path)
                                ...
                                logger.debug(
                                    f"Failed to check if fallback frame {img_path} is black: {e}"
                                )
                                valid_images.append(img_path)
                        found_images = valid_images
                    ...
                    if not found_images and duration_sec >= 2.0:
                        logger.debug(
                            f"FFmpeg fallback/scene detection failed for {file}. Retrying from start (seek_time=0)."
                        )
                        image_path = (
                            Path(output_dir)
                            / f"{sanitized_disc_name}-{vob_base}-001.png"
                        )
                        cmd_retry = [
                            ffmpeg_path,
                            "-y",
                        ...
                        try:
                            await asyncio.wait_for(
                                process_retry.communicate(), timeout=15.0
                            )
                        except TimeoutError:
                            with contextlib.suppress(Exception):
                                process_retry.kill()
                            ...
                            logger.error(
                                f"[red]FFmpeg retry timed out processing {file}[/red]"
                            )
                            ...
                            try:
                                with Image.open(img_path) as img:
                                    ...
                                    if (
                                        ...
                                        logger.debug(
                                            f"Skipping retry frame {Path(img_path).name} because it is a blank/black frame."
                                        )
                                        Path(img_path).unlink()
                                        continue
                                valid_images.append(img_path)
                                ...
                                logger.debug(
                                    f"Failed to check if retry frame {img_path} is black: {e}"
                                )
                                valid_images.append(img_path)
                        found_images = valid_images
                    ...
                    if found_images:
                        logger.info(
                            f"[green]Successfully captured {len(found_images)} menu screenshot(s) for {file}[/green]"
                        )
                        captured_images.extend(found_images)
                    else:
                        logger.info(
                            f"[yellow]No valid menu frames captured for {file} (file may contain only blank/black placeholder screens)[/yellow]"
                        )
                except Exception as e:
                    logger.error(
                        f"[red]Error running ffmpeg for {file}: {e}[/red]"
                    )

        # Apply configurable limit using even spacing
        if len(captured_images) > max_menu_screens:
            logger.info(
                f"[yellow]Captured {len(captured_images)} screenshots, limiting to {max_menu_screens} (configured by max_menu_screens) using even spacing.[/yellow]"
            )
            keep_images = select_evenly_spaced(
        ...
        logger.info(
            f"[cyan]Uploading {len(captured_images)} auto-captured disc menu screenshots...[/cyan]"
        )
