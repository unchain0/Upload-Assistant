...
from src.domain_models.media_identifiers import parse_tmdb_id as parse_tmdb_id_value
from src.domain_models.release import Meta
from src.services.book_input_service import detect_newspaper, sanitize_book_author, sanitize_book_language
from src.services.tracker_catalog_service import get_tracker_comment_hosts, is_known_tracker
...
class Args:
    ...
    def parse(self, argv: Sequence[str], meta: Meta) -> tuple[Meta, CustomArgumentParser, list[str]]:
        ...
        if not parsed_args.get("path") and not parsed_args.get("site_upload"):
            logger.error("[red]Error: Either a path must be provided or --site-upload must be specified.[/red]")
            parser.print_help()
            sys.exit(1)
