...
def test_uhd_all_source_and_type_paths() -> None:
    ...
    assert (
        asyncio.run(
            manager.get_uhd(
                "WEBDL", {"Source": "Ultra HD Blu-ray"}, "1080p", "movie"
            )
        )
        == "UHD"
    )
    assert (
        asyncio.run(manager.get_uhd("WEBDL", {}, "1080p", "Movie.UHD.mkv"))
        == "UHD"
    )
    assert asyncio.run(manager.get_uhd("DISC", {}, "1080p", "movie")) == ""
    assert (
        asyncio.run(manager.get_uhd("ENCODE", {}, "2160p", "movie")) == "UHD"
    )
...
def test_hdr_disc_and_file_variants() -> None:
    ...
    cases = [
        ...
        (
            {**base, "transfer_characteristics_Original": "BT.2020 (10-bit)"},
            "WCG",
        ),
    ...
    for track, expected in cases:
        result = asyncio.run(
            manager.get_hdr(
                {"media": {"track": [{"@type": "General"}, track]}}, None
            )
        )
        assert expected in result

    malformed = {
        "media": {"track": [{"@type": "Video", "colour_primaries": object()}]}
    }
...
def test_video_codec_and_encode_matrix() -> None:
    ...
    assert (
        asyncio.run(manager.get_video_codec({"video": [{"codec": "Unknown"}]}))
        == ""
    )
    ...
    cases = [
        ...
        (mi("HEVC"), "WEBRIP", {}, "x265"),
        (
            mi("MPEG-4 Visual", Encoded_Library_Name="XviD 1.3"),
            "DVDRIP",
            {},
            "XviD",
        ),
        (
            mi("MPEG-4 Visual", Encoded_Library_Name="DivX"),
            "ENCODE",
            {},
            "DivX",
        ),
    ...
    for media_info, type_name, bdinfo, expected in cases:
        encode, _codec, _settings, _depth = asyncio.run(
            manager.get_video_encode(media_info, type_name, bdinfo)
        )
        assert expected in encode

    high10 = mi("AVC", Format_Profile="High 10", BitDepth="10")
    encode, codec, settings, depth = asyncio.run(
        manager.get_video_encode(high10, "ENCODE", {})
    )
    assert (
        encode == "Hi10P x264"
        and codec == "AVC"
    ...
    _encode, codec, _settings, _depth = asyncio.run(
        manager.get_video_encode(mpeg, "WEBDL", {})
    )
    assert codec == "MPEG-2"

    fallback = {"video": [{"codec": "HEVC", "profile": "Main 10"}]}
    encode, codec, _settings, depth = asyncio.run(
        manager.get_video_encode({}, "ENCODE", fallback)
    )
...
def test_get_video_directory_filters_samples_sorting_and_empty(
    ...
    assert (
        str(sample.resolve()) not in files and str(note.resolve()) not in files
    )
    assert first == sorted(files)[0]

    largest, sorted_files = asyncio.run(
        manager.get_video(str(root), "cli", sorted_filelist=True)
    )
    assert largest == str(large.resolve()) and sorted_files[0] == str(
        large.resolve()
    )
    ...
    monkeypatch.setattr(
        Path,
        "iterdir",
        lambda _path: (_ for _ in ()).throw(OSError("list failed")),
    )
...
def test_get_video_arr_prompts_file_and_directory(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = VideoManager()
    ...
    monkeypatch.setattr(
        video.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: True
    )
    assert asyncio.run(manager.get_video(str(arr_file), "cli"))[0] == str(
        arr_file.resolve()
    )

    monkeypatch.setattr(
        video.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: False
    )
    ...
    assert asyncio.run(manager.get_video(str(root), "cli"))[0].endswith(
        "Movie.{imdb-tt1}.mkv"
    )
...
def test_get_resolution_file_and_dvd_fallbacks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = VideoManager()
    folder = "resolution"
    ...
    monkeypatch.setattr(
        video, "mi_resolution", AsyncMock(return_value="1080p")
    )
    resolution, hfr = asyncio.run(
        manager.get_resolution({}, folder, str(tmp_path), Meta(is_disc=""))
    )
    assert resolution == "1080p" and hfr

    dvd_text = "Width : 720\nHeight : 480\nFrame rate : 29.970\nScan type : Interlaced\n"
    dvd = Meta(
        is_disc="DVD", discs=[{"ifo_mi_json": "bad-json", "vob_mi": dvd_text}]
    )
    resolution, hfr = asyncio.run(
        manager.get_resolution({}, "Movie.480i", str(tmp_path), dvd)
    )
    ...
    _resolution, hfr = asyncio.run(
        manager.get_resolution({}, "Movie", str(tmp_path), string_json)
    )
    assert not hfr
    ...
    _resolution, hfr = asyncio.run(
        manager.get_resolution({}, "Movie.1080i", str(tmp_path), malformed)
    )
    assert not hfr
    ...
    _resolution, hfr = asyncio.run(
        manager.get_resolution({}, "Movie", str(tmp_path), no_rate)
    )
    assert not hfr
...
def test_closest_type_3d_sd_duration_and_container(tmp_path: Path) -> None:
    ...
    assert (
        asyncio.run(manager.get_type("anything", False, "", meta)) == "REMUX"
    )
    for filename, expected in (
        ...
        assert (
            asyncio.run(manager.get_type(filename, False, "", Meta()))
            == expected
        )
    assert (
        asyncio.run(manager.get_type("Movie.mkv", False, "BDMV", Meta()))
        == "DISC"
    )
    ...
    assert (
        asyncio.run(manager.is_sd("576p")) == 1
        and asyncio.run(manager.is_sd("1080p")) == 0
    )

    assert (
        asyncio.run(manager.get_video_duration(Meta(category="BOOK"))) is None
    )
    media = Meta(
        category="MOVIE",
        is_disc="",
        mediainfo={
            "media": {"track": [{"@type": "General", "Duration": "3661"}]}
        },
    )
    ...
    for disc_type, expected in (
        ("BDMV", "m2ts"),
        ("HDDVD", "evo"),
        ("DVD", "vob"),
    ):
        assert (
            asyncio.run(manager.get_container(Meta(is_disc=disc_type)))
            == expected
        )
    assert (
        asyncio.run(manager.get_container(Meta(is_disc="", filelist=[]))) == ""
    )
    ...
    assert (
        asyncio.run(
            manager.get_container(
                Meta(is_disc="", filelist=[str(one), str(two)])
            )
        )
        == "mp4"
    )
    assert (
        asyncio.run(
            manager.get_container(
                Meta(is_disc="", filelist=[str(tmp_path / "missing")])
            )
        )
        == ""
    )
