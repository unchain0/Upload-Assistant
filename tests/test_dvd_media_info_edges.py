...
def test_filename_url_and_download_helpers(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    assert (
        dvd.get_filename("windows", "x86_64")
        == "MediaInfo_CLI_23.04_Windows_x64.zip"
    )
    assert (
        dvd.get_filename("linux", "arm64", "cli")
        == "MediaInfo_CLI_23.04_Lambda_arm64.zip"
    )
    assert (
        dvd.get_filename("linux", "x86_64", "lib")
        == "MediaInfo_DLL_23.04_Lambda_x86_64.zip"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda url, path, asset: calls.append((url, path, asset)),
    )
...
def test_extract_windows_success_missing_and_duplicate(tmp_path: Path) -> None:
    ...
    _zip(
        duplicate, ("one/MediaInfo.exe", b"one"), ("two/MediaInfo.exe", b"two")
    )
    with pytest.raises(RuntimeError, match="exactly one"):
        dvd.extract_windows(duplicate, tmp_path / "duplicate-output")


def test_extract_linux_duplicate_cli_and_missing_library(
    tmp_path: Path,
) -> None:
...
def test_download_windows_cache_arm_error_success_and_extract_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_download_unsupported_linux_cache_success_and_missing_outputs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
