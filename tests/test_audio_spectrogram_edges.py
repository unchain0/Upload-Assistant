...
def test_env_cjk_and_selection_helpers(
...
def test_ffprobe_success_failures_and_invalid_json(
    ...
    def run(
        ...
        return subprocess.CompletedProcess(
            [],
            0,
            stdout=json.dumps({"streams": [{"index": 1}, "bad"]}),
            stderr="",
        )
...
def test_config_source_stft_cache_and_font_helpers(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    assert (
        audio_spectrogram._positive_config_int(
            {"DEFAULT": {"x": "bad"}}, "x", 5
        )
        == 5
    )
    assert (
        audio_spectrogram._positive_config_int({"DEFAULT": {"x": 0}}, "x", 5)
        == 5
    )
    assert (
        audio_spectrogram._positive_config_int({"DEFAULT": {"x": 7}}, "x", 5)
        == 7
    )

    disc = _file(tmp_path, "disc.m2ts")
    assert audio_spectrogram.get_spectrogram_sources("MOVIE", [], disc, 2) == [
        disc
    ]
    one = _file(tmp_path, "one.mkv")
    two = _file(tmp_path, "two.mkv")
    assert audio_spectrogram.get_spectrogram_sources(
        "MOVIE", [one, two], None, 5
    ) == [one]
    ...
    assert audio_spectrogram._load_cached_images(cache, "fp") == []
    cache.write_text(
        json.dumps({"fingerprint": "other", "spectrograms_images": []}),
        encoding="utf-8",
    )
    assert audio_spectrogram._load_cached_images(cache, "fp") == []
    cache.write_text(
        json.dumps(
            {"fingerprint": "fp", "spectrograms_images": [{"raw_url": "x"}]}
        ),
        encoding="utf-8",
    )
    assert audio_spectrogram._load_cached_images(cache, "fp") == [
    ...
    monkeypatch.setattr(
        audio_spectrogram.font_manager,
        "FontProperties",
        lambda **_kwargs: (_ for _ in ()).throw(RuntimeError("font")),
    )
    assert (
        audio_spectrogram._font_name_for_file("/fonts/MyFont.ttf") == "MyFont"
    )
    assert audio_spectrogram._build_plot_font_properties(
        "/fonts/MyFont.ttf"
    ) == (None, False)
    assert audio_spectrogram._build_plot_font_properties(None) == (None, False)

    monkeypatch.setattr(
        audio_spectrogram.ft2font,
        "FT2Font",
        lambda _path: (_ for _ in ()).throw(RuntimeError("bad")),
    )
    ...
    assert audio_spectrogram._font_path_supports_cjk(
        "/fonts/plain.ttf", "Noto Sans SC"
    )
    assert not audio_spectrogram._font_path_supports_cjk(
        "/fonts/plain.ttf", "Plain"
    )
    assert not audio_spectrogram._font_path_supports_cjk("/fonts/plain.ttf")


def test_resolve_font_cache_fallback_and_default(
    ...
    monkeypatch.setattr(
        audio_spectrogram, "_font_name_for_file", lambda _path: "Plain"
    )
    monkeypatch.setattr(
        audio_spectrogram, "_font_is_loadable", lambda _path: True
    )
    monkeypatch.setattr(
        audio_spectrogram.font_manager, "findSystemFonts", lambda: []
    )
    ...
    assert audio_spectrogram._resolve_plot_font() == (
        "DejaVu Sans",
        False,
        None,
    )
...
def test_generate_spectrogram_decode_and_audio_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    file = _file(tmp_path)
    output = tmp_path / "out"
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda *_args, **_kwargs: subprocess.CompletedProcess(
            [], 1, stdout=b"", stderr=b"decode failed"
        ),
    )
    with pytest.raises(RuntimeError, match="decode failed"):
        audio_spectrogram.generate_spectrogram(
            1, "Label", "en", file, output, 10, 48000, 1, "file"
        )
    ...
    monkeypatch.setattr(
        audio_spectrogram.librosa,
        "load",
        lambda *_args, **_kwargs: (np.array([]), 48000),
    )
...
def test_generate_spectrogram_success_with_plot_doubles(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    file = _file(tmp_path)
    output = tmp_path / "out"
    ...
    monkeypatch.setattr(
        audio_spectrogram.librosa,
        "load",
        lambda *_args, **_kwargs: (np.ones(128), 48000),
    )
    monkeypatch.setattr(
        audio_spectrogram.librosa,
        "stft",
        lambda *_args, **_kwargs: np.ones((4, 4)),
    )
    monkeypatch.setattr(
        audio_spectrogram.librosa,
        "amplitude_to_db",
    ...
    monkeypatch.setattr(
        audio_spectrogram.librosa.display,
        "specshow",
        lambda *_args, **_kwargs: object(),
    )
    monkeypatch.setattr(
        audio_spectrogram, "_resolve_plot_font", lambda: ("Plain", False, None)
    )
    ...
    monkeypatch.setattr(
        audio_spectrogram.plt, "subplots", lambda **_kwargs: (Figure(), Axis())
    )
    monkeypatch.setattr(audio_spectrogram.plt, "close", lambda _figure: None)
    path = audio_spectrogram.generate_spectrogram(
        1, "Stream_1", "en", file, output, 10, 48000, 1, "source"
    )
    assert path.is_file() and path.name == "spectrogram_source_01_stream_1.png"


def test_process_shortcuts_sources_streams_choice_cache_and_upload(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
            "audio_spectrogram_max_files": 1,
            "process_all_audio_spectrogram": True,
        }
    }
    existing = _meta(tmp_path, spectrograms_images=[{"raw_url": "existing"}])
    assert (
        asyncio.run(
            audio_spectrogram.process_audio_spectrograms(existing, config)
        )
        == []
    )

    missing = _meta(
        tmp_path, uuid="missing", filelist=[str(tmp_path / "missing.flac")]
    )
    assert (
        asyncio.run(
            audio_spectrogram.process_audio_spectrograms(missing, config)
        )
        == []
    )
    ...
    monkeypatch.setattr(
        ...
        "get_audio_streams",
        lambda _path: [
            {"index": 1, "tags": {"language": "en", "title": "Main"}}
        ],
    )
    ...
    result = asyncio.run(
        audio_spectrogram.process_audio_spectrograms(meta, config, uploader)
    )
    ...
    monkeypatch.setattr(
        audio_spectrogram,
        "get_audio_streams",
        lambda _path: [{"index": 1, "tags": {}}],
    )
    fingerprint = audio_spectrogram._cache_fingerprint(
        [Path(cached.filelist[0])], 600, 48000, [(Path(cached.filelist[0]), 1)]
    )
    cache.write_text(
        json.dumps(
            {
                "fingerprint": fingerprint,
                "spectrograms_images": [{"raw_url": "cached"}],
            }
        ),
        encoding="utf-8",
    )
    assert (
        asyncio.run(
            audio_spectrogram.process_audio_spectrograms(
                cached, {"DEFAULT": {}}
            )
        )
        == []
...
def test_process_stream_errors_bdinfo_selection_generation_and_upload_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {"DEFAULT": {}}
    ...
    meta = _meta(
        ...
        bdinfo={
            "path": str(bdroot),
            "files": [{"file": source.name}],
            "audio": [{"language": "French", "codec": "DTS"}],
        },
    ...
    monkeypatch.setattr(
        audio_spectrogram,
        "get_audio_streams",
        lambda _path: [{"index": 2, "tags": {"language": "und"}}],
    )
    assert (
        asyncio.run(audio_spectrogram.process_audio_spectrograms(meta, config))
        == []
    )
...
def test_process_attended_prompt_and_multi_source_limit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        audio_spectrogram, "prompt_audio_stream_positions", lambda: "0"
    )
    plot = tmp_path / "plot.png"
    plot.write_bytes(b"plot")
    monkeypatch.setattr(
    ...
    result = asyncio.run(
        audio_spectrogram.process_audio_spectrograms(
            meta, {"DEFAULT": {"audio_spectrogram_max_files": 1}}
        )
    )
    assert result == [str(plot)]


def test_resolve_plot_font_skips_empty_findfont(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    audio_spectrogram._PLOT_FONT_CACHE = None
    monkeypatch.delenv("UA_AUDIO_SPECTROGRAM_FONT_PATH", raising=False)
