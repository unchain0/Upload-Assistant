...
def test_fallback_single_subtitles_outside_and_nonmedia_path_rules(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = asyncio.run(TorrentCreator.create_torrent(meta, video, "BASE_SUBS"))
    ...
    meta = _meta(tmp_path, category="BOOK", isdir=True, filelist=[str(payload)], keep_folder=False)
    result = asyncio.run(creator.create_torrent(meta, data_dir, "BASE"))
    assert result.kwargs["path"] == str(payload)
    assert result.kwargs["include_globs"] == [] and result.kwargs["exclude_globs"] == []


def test_fallback_keep_folder_directory_disc_pack_and_media_rules(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_fallback(monkeypatch)
    ...
    keep_nfo = _meta(tmp_path, path=str(folder), filelist=[str(video)], isdir=True, keep_folder=True, keep_nfo=True)
    result = asyncio.run(TorrentCreator.create_torrent(keep_nfo, folder, "TRACKER"))
    assert "*.nfo" in result.kwargs["include_globs"] and keep_nfo.mkbrr is False

    keep_folder = _meta(tmp_path, path=str(folder), filelist=[str(video)], isdir=True, keep_folder=True, keep_nfo=False)
    result = asyncio.run(TorrentCreator.create_torrent(keep_folder, folder, "BASE"))
    assert result.kwargs["include_globs"] == [f"Release/{video.name}"]
    assert result.kwargs["exclude_globs"] == ["*", "*/**"]

    directory_nfo = _meta(tmp_path, path=str(folder), filelist=[str(video)], isdir=True, keep_nfo=True)
    result = asyncio.run(TorrentCreator.create_torrent(directory_nfo, folder, "TRACKER"))
    assert "*.nfo" in result.kwargs["include_globs"]

    disc = _meta(tmp_path, path=str(folder), filelist=[str(video)], isdir=True, is_disc="BDMV")
    result = asyncio.run(TorrentCreator.create_torrent(disc, folder, "BASE"))
    assert result.kwargs["include_globs"] == [] and result.kwargs["exclude_globs"] == []

    one_video = _meta(tmp_path, path=str(folder), filelist=[str(video)], isdir=True)
    result = asyncio.run(TorrentCreator.create_torrent(one_video, folder, "BASE"))
    assert result.kwargs["path"] == str(video)
    assert result.kwargs["include_globs"] == ["*.mkv", "*.mp4", "*.ts"]

    pack = _meta(tmp_path, path=str(folder), filelist=[str(video)], isdir=True, tv_pack=True)
    result = asyncio.run(TorrentCreator.create_torrent(pack, folder, "BASE"))
    assert result.kwargs["include_globs"] == [f"Release/{video.name}"]

    standard = _meta(tmp_path, path=str(video), filelist=[str(video)], isdir=False)
    result = asyncio.run(TorrentCreator.create_torrent(standard, video, "BASE"))
    assert result.kwargs["include_globs"] == ["*.mkv", "*.mp4", "*.ts"]
    assert "*sample.mkv" in result.kwargs["exclude_globs"]


def test_fallback_directory_size_and_generation_error_decrements_inflight(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_fallback(monkeypatch)
    folder = tmp_path / "Directory"
    folder.mkdir()
    (folder / "one.bin").write_bytes(b"123")
    (folder / "two.bin").write_bytes(b"12345")
    meta = _meta(tmp_path, category="BOOK", path=str(folder), filelist=[str(folder / "one.bin"), str(folder / "two.bin")], isdir=True, keep_folder=True)
    asyncio.run(TorrentCreator.create_torrent(meta, folder, "BASE"))
    assert _FakeCustomTorrent.instances[-1].piece_size >= creator.PIECE_SIZE_MIN
...
class _MkbrrProcess:
    ...
    def __init__(self, command: list[str], **_kwargs: object) -> None:
        ...
        self.stdout = None if self.stdout_lines is None else iter(self.stdout_lines)
...
def _patch_mkbrr(monkeypatch: pytest.MonkeyPatch, binary: Path) -> None:
    ...
    monkeypatch.setattr(TorrentCreator, "get_mkbrr_path", staticmethod(lambda _meta: str(binary)))
    monkeypatch.setattr(creator.subprocess, "Popen", _MkbrrProcess)
    monkeypatch.setattr(creator, "progress_display", _Progress)


def test_mkbrr_success_command_piece_workers_random_tracker_and_stdout(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = asyncio.run(TorrentCreator.create_torrent(meta, folder, "MKB", tracker_url="https://tracker", piece_size=1))
    ...
    assert "-e" in command and "--workers" in command and "--exclude" in command
    ...
    result = asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "PIECE", piece_size=1))
    command = _MkbrrProcess.commands[-1]
    assert "-l" in command and command[command.index("-l") + 1] == "20"
    assert isinstance(result, str)

    meta = _meta(tmp_path, mkbrr=True, trackers=[])
    asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "DEFAULT"))
    assert "-m" in _MkbrrProcess.commands[-1]

    meta = _meta(tmp_path, mkbrr=True)
    asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "BADPIECE", piece_size=cast(Any, "bad")))
    assert "-l" not in _MkbrrProcess.commands[-1]


def test_mkbrr_stdout_none_nonzero_missing_output_and_fallback_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    binary = tmp_path / "mkbrr"
    binary.write_bytes(b"tool")
    _patch_mkbrr(monkeypatch, binary)
    _patch_fallback(monkeypatch)
    ...
    meta = _meta(tmp_path, mkbrr=True)
    result = asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "NO_STDOUT"))
    assert isinstance(result, str)

    _MkbrrProcess.stdout_lines = []
    _MkbrrProcess.return_code = 2
    meta = _meta(tmp_path, mkbrr=True)
    result = asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "NONZERO"))
    assert isinstance(result, _FakeCustomTorrent) and meta.mkbrr is False

    _MkbrrProcess.return_code = 0
    _MkbrrProcess.create_output = False
    meta = _meta(tmp_path, mkbrr=True)
    result = asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "MISSING_OUTPUT"))
    assert isinstance(result, _FakeCustomTorrent) and meta.mkbrr is False

    missing_path = tmp_path / "missing"
    meta = _meta(tmp_path, mkbrr=True)
    result = asyncio.run(TorrentCreator.create_torrent(meta, missing_path, "MISSING_PATH"))
    assert isinstance(result, _FakeCustomTorrent) and meta.mkbrr is False

    binary.unlink()
    meta = _meta(tmp_path, mkbrr=True)
    result = asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "MISSING_BINARY"))
    assert isinstance(result, _FakeCustomTorrent) and meta.mkbrr is False


def test_mkbrr_called_process_error_falls_back(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    binary = tmp_path / "mkbrr"
    binary.write_bytes(b"tool")
    _patch_fallback(monkeypatch)
    monkeypatch.setattr(TorrentCreator, "get_mkbrr_path", staticmethod(lambda _meta: str(binary)))
    ...
    meta = _meta(tmp_path, mkbrr=True)
    result = asyncio.run(TorrentCreator.create_torrent(meta, Path(meta.path), "CALLED_ERROR"))
    assert isinstance(result, _FakeCustomTorrent) and meta.mkbrr is False


def test_semaphore_wait_debug_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_fallback(monkeypatch)
...
def test_torf_callback_progress_suppressed_and_module_wrapper(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(creator.cli_ui, "info_progress", lambda label, done, total: events.append((label, done, total)))
