...
def test_portugas_nogroup_name_cleanup() -> None:
    ...
    assert __import__("asyncio").run(tracker.get_name(meta)) == {
...
def test_portugas_section_accepts_portuguese_and_rejects_brazilian() -> None:
    assert Portugas._section_is_portuguese(
        "Language : Portuguese\nTitle : European"
    )
    assert not Portugas._section_is_portuguese(
        "Language : Portuguese\nTitle : Brazilian"
    )
    assert not Portugas._section_is_portuguese("Language : Portuguese (BR)")


def test_portugas_read_text_handles_race_and_generic_errors(
    ...
    def missing_read_text(
        current: Path, *args: object, **kwargs: object
    ) -> str:
        if current == path:
