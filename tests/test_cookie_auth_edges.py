...
def _config(tmp_path: Path, **tracker_values: object) -> dict[str, Any]:
    return {
        ...
        "TRACKERS": {
            "TEST": {
                "announce_url": "https://www.tracker.example/announce",
                **tracker_values,
            },
            "ALPHARATIO": {
                "username": "user",
...
class CookieContainer:
    def __init__(
        self, cookies: list[http.cookiejar.Cookie] | None = None
    ) -> None:
...
def _cookie(
    name: str = "session", value: str = "value"
...
def test_attribute_and_upload_error_parsers() -> None:
    ...
    assert (
        cookie_auth._attr_to_string(AttributeValueList(["one", "two"]))
        == "one two"
    )
    ...
    cases = [
        (
            '<div class="notification-border-e"><div class="notification-body"> Error: Modern failure </div></div>',
            "Error: Modern failure",
        ),
        ('<div class="error">Specific old error</div>', "Specific old error"),
        (
            '<div><span class="error">Error</span> Detailed parent failure </div>',
            "Detailed parent failure",
        ),
        ("<h2>Upload failed</h2><p>Sibling failure</p>", "Sibling failure"),
        (
            '<h1 class="dnu_header">Upload not allowed here</h1>',
            "Upload not allowed here",
        ),
...
def test_tracker_domain_registry_config_fallback_and_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert cookie_auth.get_tracker_domain("FILELIST") == "filelist.io"
    config = {
        "TRACKERS": {
            "CUSTOM": {"announce_url": "https://www.Custom.Example/announce"}
        }
    }
    assert cookie_auth.get_tracker_domain("CUSTOM", config) == "custom.example"
    assert (
        cookie_auth.get_tracker_domain("PASSTHEPOPCORN") == "passthepopcorn.me"
    )
    ...
    assert (
        cookie_auth.get_tracker_domain("BROKEN", {"TRACKERS": {"BROKEN": {}}})
        == "broken"
    )


def test_find_cookie_file_custom_filename_domain_multiple_and_fallback(
    tmp_path: Path,
) -> None:
    base = str(tmp_path)
    ...
    assert cookie_auth.find_cookie_file(
        base,
        "TEST",
        {
            "TRACKERS": {
                "TEST": {"cookie_file": str(tmp_path / "absolute.txt")}
            }
        },
    ...
    assert cookie_auth.find_cookie_file(
        base,
        "TEST",
        {"TRACKERS": {"TEST": {"cookie_file": "data/cookies/custom.txt"}}},
    ) == str((tmp_path / "data" / "cookies" / "custom.txt").resolve())
    assert cookie_auth.find_cookie_file(
        base, "TEST", {"TRACKERS": {"TEST": {"cookie_file": "custom.txt"}}}
    ) == str((cookies / "custom.txt").resolve())

    exact = cookies / "TEST.json"
    exact.write_text("{}", encoding="utf-8")
    assert cookie_auth.find_cookie_file(base, "TEST", config) == str(
        exact.resolve()
    )
    exact.unlink()
    ...
    assert cookie_auth.find_cookie_file(base, "FILELIST", config).endswith(
        "FILELIST.json"
    )
    assert cookie_auth.find_cookie_file(
        base, "PASSTHEPOPCORN", config
    ).endswith("PASSTHEPOPCORN.json")
    assert cookie_auth.find_cookie_file(base, "Pterimg", config).endswith(
        "Pterimg.json"
    )
    assert cookie_auth.find_cookie_file(base, "OTHER", config).endswith(
        "OTHER.txt"
    )


def test_find_cookie_file_content_read_error(
    ...
    assert cookie_auth.find_cookie_file(
        str(tmp_path), "TEST", _config(tmp_path)
    ).endswith("TEST.txt")
...
def test_load_save_session_cookies_and_ar_recovery(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    validator = CookieValidator(_config(tmp_path))
    monkeypatch.setattr(
        cookie_auth.http.cookiejar, "MozillaCookieJar", JarDouble
    )
    monkeypatch.setattr(
        cookie_auth,
        "find_cookie_file",
        lambda *_args, **_kwargs: str(tmp_path / "cookies.txt"),
    )

    JarDouble.next_load_error = http.cookiejar.LoadError("bad")
    assert (
        asyncio.run(validator.load_session_cookies(_meta(tmp_path), "TEST"))
        is None
    )
    ...
    assert (
        asyncio.run(
            validator.load_session_cookies(_meta(tmp_path), "ALPHARATIO")
        )
        is None
    )
...
def test_auth_key_read_success_empty_and_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    validator = CookieValidator(_config(tmp_path))
    cookie = tmp_path / "ar.txt"
    monkeypatch.setattr(
        cookie_auth, "find_cookie_file", lambda *_args, **_kwargs: str(cookie)
    )
    auth = tmp_path / "ar_auth.txt"
    auth.write_text(" key \n", encoding="utf-8")
    assert (
        asyncio.run(validator.get_ar_auth_key(_meta(tmp_path), "ALPHARATIO"))
        == "key"
    )
...
def test_ar_login_missing_credentials_status_recovery_validation_and_exceptions(
    tmp_path: Path,
) -> None:
    config = _config(tmp_path)
    ...
    assert not asyncio.run(
        validator.ar_login(
            _meta(tmp_path), "ALPHARATIO", str(tmp_path / "ar.txt")
        )
    )
    ...
    assert not asyncio.run(
        validator.ar_login(
            _meta(tmp_path, debug=True), "ALPHARATIO", str(tmp_path / "ar.txt")
        )
    )
    validator.common.save_html_file.assert_awaited_once()

    Client.reset(
        Response(text="ok"), Response(status=500, text="login.php?act=recover")
    )
    assert not asyncio.run(
        validator.ar_login(
            _meta(tmp_path), "ALPHARATIO", str(tmp_path / "ar.txt")
    ...
    for error in (
        httpx.ConnectTimeout(
            "timeout", request=httpx.Request("GET", "https://alpharatio.cc")
        ),
        httpx.ConnectError(
            "connect", request=httpx.Request("GET", "https://alpharatio.cc")
        ),
        RuntimeError("other"),
    ):
        Client.reset(error)
...
def test_ar_login_success_cookie_and_auth_key(tmp_path: Path) -> None:
    ...
    assert asyncio.run(
        validator.ar_login(_meta(tmp_path), "ALPHARATIO", str(path))
    )
    assert path.is_file()
    assert (path.parent / "ALPHARATIO_auth.txt").read_text(
        encoding="utf-8"
    ) == "SECRET"


def test_cookie_validation_success_failure_indicators_token_and_exceptions(
    tmp_path: Path,
) -> None:
    validator = CookieValidator(_config(tmp_path))
    jar = JarDouble()
    ...
    for kwargs, text, status in (
        ...
        assert not asyncio.run(
            validator.cookie_validation(
                _meta(tmp_path), "TEST", "https://test", **kwargs
            )
        )
    ...
    assert not asyncio.run(
        validator.cookie_validation(_meta(tmp_path), "TEST", "https://test")
    )
    ...
    for error in errors:
        ...
        assert not asyncio.run(
            validator.cookie_validation(
                _meta(tmp_path), "TEST", "https://test"
            )
        )
...
def test_validation_failure_token_and_secure_cookie_io(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    validator = CookieValidator(_config(tmp_path))
    validator.common.save_html_file = AsyncMock(return_value="failure.html")  # type: ignore[method-assign]
    asyncio.run(
        validator.handle_validation_failure(_meta(tmp_path), "TEST", "html")
    )
    validator.common.save_html_file.assert_awaited_once()
    assert (
        asyncio.run(
            validator.find_html_token("TEST", r"TOKEN=(\w+)", "TOKEN=abc")
        )
        == "abc"
    )
    assert (
        asyncio.run(validator.find_html_token("TEST", r"TOKEN=(\w+)", "none"))
        is None
    )
    ...
    assert (
        validator._load_cookies_dict_secure(str(path))["session"]["value"]
        == "value"
    )
    ...
    path.write_text(
        json.dumps({"session": {"value": "x", "domain": None}}),
        encoding="utf-8",
    )
    session = SimpleNamespace(cookies=SessionCookies())
    validator._load_cookies_secure(session, str(path), "TEST")
    ...
    with pytest.raises(json.JSONDecodeError):
        validator._load_cookies_secure(
            SimpleNamespace(cookies=SessionCookies()), str(path), "TEST"
        )
...
def test_secure_cookie_encoding_error(tmp_path: Path) -> None:
    ...
    with pytest.raises(TypeError):
        validator._save_cookies_secure(
            [BadCookie()], str(tmp_path / "bad.json")
        )
...
def _uploader(
    tmp_path: Path,
) -> tuple[CookieAuthUploader, UploaderCommon, Meta]:
...
def test_uploader_rejects_missing_or_ambiguous_success_criteria(
    tmp_path: Path,
) -> None:
...
def test_uploader_debug_load_file_and_additional_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    meta.debug = False
    files = asyncio.run(
        uploader.load_torrent_file(
            meta, "TEST", "torrent", "Explicit.Name", "SRC", "https://announce"
        )
    )
    assert files["torrent"] == (
        "Explicit.Name.torrent",
        b"torrent-bytes",
        "application/x-bittorrent",
    )
    files = asyncio.run(
        uploader.load_torrent_file(
            meta, "TEST", "torrent", "", "SRC", "https://announce"
        )
    )
    assert files["torrent"][0] == "TEST.HASH.placeholder.torrent"


def test_uploader_success_text_list_status_and_error_absence(
    tmp_path: Path,
) -> None:
    variants = [
        (
            ...
            Response(
                ...
                url="https://tracker.example/details?id=1",
            ),
        ),
        (
            {"success_list": ["done", "created"]},
            Response(
                text="created torrent",
                url="https://tracker.example/details?id=2",
            ),
        ),
        (
            ...
            Response(
                status=201,
                text="ok",
                url="https://tracker.example/details?id=3",
            ),
        ),
        (
            {"error_text": "fatal"},
            Response(
                ...
                url="https://tracker.example/details?id=4",
            ),
        ),
    ]
...
def test_uploader_failed_criteria_calls_failure_handler(
    tmp_path: Path,
) -> None:
    variants = [
...
def test_uploader_http_exception_matrix_sets_status_and_builds_seed_torrent(
    tmp_path: Path,
) -> None:
    ...
    errors: list[tuple[BaseException, str]] = [
        ...
        (
            httpx.ProxyError("proxy", request=request),
        ...
        (
            httpx.TooManyRedirects("redirect", request=request),
            "Too many redirects",
        ),
        (
...
def test_handle_successful_upload_url_text_hash_and_no_id(
    tmp_path: Path,
) -> None:
    uploader, common_double, meta = _uploader(tmp_path)
    response = Response(
        text="body id=55", url="https://tracker.example/details?id=44"
    )
    assert asyncio.run(
    ...
    common_double.create_torrent_ready_to_seed = AsyncMock(
        return_value="HASHID"
    )
    uploader.common = common_double  # type: ignore[assignment]
    asyncio.run(
...
def test_handle_failed_upload_all_message_modes_and_tracker_error(
    tmp_path: Path,
) -> None:
    variants = [
        (
            ...
            "Could not find the success text",
        ),
        (
            {
                "success_text": "",
                "success_list": ["one", "two"],
                "error_text": "",
                "success_status_code": "",
            },
            "Could not find any",
    ...
    for index, (kwargs, expected) in enumerate(variants):
        ...
        assert (
            expected in message
            and "Tracker rejected release" in message
            and "failed.html" in message
        )
        common_double.save_html_file.assert_awaited_once()


def test_final_cookie_helper_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert (
        cookie_auth.extract_upload_error(
            "<div>prefix <span>Upload failed!</span></div>"
        )
        == ""
    )
    ...
    monkeypatch.delitem(
        registry.tracker_class_map, "PASSTHEPOPCORN", raising=False
    )
    assert (
        cookie_auth.get_tracker_domain("PASSTHEPOPCORN") == "passthepopcorn.me"
    )
    ...
    assert cookie_auth.find_cookie_file(
        str(tmp_path), "NOCOOKIE", _config(tmp_path)
    ).endswith("NOCOOKIE.txt")


def test_final_cookie_validator_read_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    validator = CookieValidator(_config(tmp_path))
    monkeypatch.setattr(
    ...
    TwoStageJar.loads = 0
    monkeypatch.setattr(
        cookie_auth.http.cookiejar, "MozillaCookieJar", TwoStageJar
    )
    validator.ar_login = AsyncMock(return_value=True)  # type: ignore[method-assign]
    assert (
        asyncio.run(
    ...
    with pytest.raises(OSError):
        validator._load_cookies_secure(
            SimpleNamespace(
                cookies=SimpleNamespace(set=lambda **_kwargs: None)
            ),
