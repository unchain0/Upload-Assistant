...
class _Response:
    status_code = 200
    text = (
        '{"response":{"@attributes":{"API":"OK","NFO":"OK","REGISTER":"OK"}}}'
    )
...
def test_nzbgeek_uses_documented_submission_fields(
    ...
    (nfo_dir / "MEDIAINFO_CLEANPATH.txt").write_text(
    ...
    tracker = NZBGeek(
        {"TRACKERS": {"NZBGEEK": {"api_key": "secret"}}, "USENET": {}}
    )
    monkeypatch.setattr(
        "src.integrations.trackers.USENET.nzbgeek.httpx.AsyncClient",
    ...
    assert (
        meta.tracker_status["NZBGEEK"]["status_message"] == "Upload successful"
    )


def test_nzbgeek_requires_all_success_attributes() -> None:
    assert NZBGeek._successful_response(
        '{"response":{"@attributes":{"API":"OK","REGISTER":"OK"}}}', False
    )
    assert not NZBGeek._successful_response(
        '{"response":{"@attributes":{"API":"OK","REGISTER":"OK"}}}', True
    )
...
def test_nzbgeek_maps_supported_categories() -> None:
    ...
    assert (
        tracker.get_category_id(Meta(category="TV", resolution="1080p"))
        == "5040"
    )
    assert (
        tracker.get_category_id(
            Meta(category="TV", resolution="1080p", anime=True)
        )
        == "5070"
    )
    assert (
        tracker.get_category_id(Meta(category="MUSIC", format="FLAC"))
        == "3040"
    )
