...
share_module = importlib.import_module("src.integrations.trackers.UNIT3D.shareisland")
...
def _meta(tmp_path: Path | None = None, **values: object) -> Meta:
    ...
    state: dict[str, object] = {
        ...
        "mediainfo": {
            "media": {
                "track": [
                    {"@type": "General", "Format": "Matroska", "FileName": "movie"},
                    {
                        "@type": "Video",
                        "Format": "AVC",
                        "BitDepth": "8",
...
def test_shareisland_language_resolution_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(share_module.pycountry.languages, "get", lambda **_kwargs: (_ for _ in ()).throw(LookupError("bad")))
    assert ShareIsland._resolved_alpha2("foo") == "foo"


def test_shareisland_audio_language_tag_branches() -> None:
    tracker = _tracker()
    assert tracker._audio_language_tag(_meta(audio_languages=["Italian", "English"])) == "ITA - ENG"
    assert tracker._audio_language_tag(_meta(audio_languages=["Italian", "English", "French"])) == "ITA - MULTI"
    assert tracker._audio_language_tag(_meta(audio_languages=["English", "French", "German"])) == "MULTI"


def test_shareisland_finalized_name_and_group_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    item = _meta(name="Fallback Name", tag="")
    monkeypatch.setattr(tracker, "_basename_release_group", lambda _meta: "NoGroup")
    monkeypatch.setattr(tracker, "_valid_marker_group", lambda _candidate: False)
    assert tracker._extract_clean_release_group(item) == "NoGroup"
    monkeypatch.setattr(tracker, "_extract_clean_release_group", lambda _meta: "")
...
async def test_shareisland_required_region_unattended_missing() -> None:
    ...
    with pytest.raises(ValueError, match="Region required"):
        await tracker._required_region_name(_meta(region="", unattended=True, unattended_confirm=False))


def test_shareisland_prompt_region_retries(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    answers = iter(("", " eu "))
    monkeypatch.setattr(share_module.cli_ui, "ask_string", lambda *_args, **_kwargs: next(answers))
    assert tracker._prompt_region() == "EU"


@pytest.mark.asyncio
async def test_shareisland_region_distributor_validation_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(share_module.cli_ui, "ask_string", lambda *_args, **_kwargs: " warner ")
    assert tracker._distributor_name(_meta(distributor="", unattended=False)) == "WARNER"
    assert await tracker._optional_distributor_id("") is None


@pytest.mark.asyncio
async def test_shareisland_optional_distributor_and_session_accessors() -> None:
    tracker = _tracker()
...
def test_shareisland_type_detection_remux_and_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_has_remux_marker", lambda _meta: True)
    assert tracker._detect_type_from_technical_analysis(_meta(type="ENCODE")) == "REMUX"

    monkeypatch.setattr(tracker, "_has_remux_marker", lambda _meta: False)
    assert tracker._detect_type_from_technical_analysis(_meta(type="BRRIP")) == "BRRIP"


def test_shareisland_remux_makemkv_branches() -> None:
    tracker = _tracker()
    assert tracker._has_remux_marker(_meta(name="Movie.VU1080.mkv", filename="Movie.VU1080.mkv", filelist=["Movie.VU1080.mkv"]))
    item = _meta(
        mediainfo={
            "media": {
                "track": [
    ...
    assert tracker._video_has_no_encoding_settings(tracker._mediainfo_tracks(item))


def test_shareisland_analyze_encode_fallback_and_specific_helpers() -> None:
    tracker = _tracker()
    assert tracker._analyze_encode_type(_meta(type="OTHER", source="OTHER", service="")) == "OTHER"
    general = {"Encoded_Application": "muxer"}
    video = {"HDR_Format_Profile": "dvhe.05"}
    assert tracker._streaming_dv_type(general, video, "") == "WEBDL"
    assert tracker._service_fingerprint_type("CR", {}, "bitrate=1", "core 142", True) == "WEBDL"
    assert tracker._crunchyroll_type("CR", "", "core 142", True) == "WEBDL"
    assert tracker._bluray_encode_type({}, {}, ["BLURAY"], False, False) is None
...
def test_shareisland_aka_title_country_and_language() -> None:
    tracker = _tracker()
    akas = [
        {"country": "Other", "language": "Italy", "attributes": ["x"], "title": "Skipped"},
...
async def test_shareisland_best_audio_bdinfo_and_mediainfo() -> None:
    tracker = _tracker()
    tracker.audio_manager.get_audio_v2 = AsyncMock(return_value=("DDP 5.1-ITA", "", ""))  # type: ignore[method-assign]
    bd = _meta(bdinfo={"audio": [{"language": "Italian", "codec": "TrueHD", "channels": "7.1", "bitrate": "3000"}]})
    assert await tracker._get_best_italian_audio_format(bd) == "DDP 5.1"
    tracker.audio_manager.get_audio_v2.assert_awaited()

    tracker.audio_manager.get_audio_v2 = AsyncMock(return_value=("DD 5.1-ITA", "", ""))  # type: ignore[method-assign]
    mi = _meta(bdinfo={})
    assert await tracker._get_best_italian_audio_format(mi) == "DD 5.1"


def test_shareisland_non_italian_audio_track() -> None:
    assert not _tracker()._is_italian_audio_track({"@type": "Audio", "Language": "en"})


def test_shareisland_description_title_uses_italian() -> None:
    tracker = _tracker()
    item = _meta(imdb_info={"akas": [{"country": "Italy", "attributes": [], "title": "Titolo Italiano"}]})
    assert tracker._description_title(item) == "Titolo Italiano"


@pytest.mark.asyncio
async def test_shareisland_tmdb_fetch_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_tmdb_media_payload", AsyncMock(side_effect=RuntimeError("offline")))
    assert await tracker._fetch_tmdb_italian(_meta()) == ("Riassunto non disponibile.", "")


@pytest.mark.asyncio
async def test_shareisland_tmdb_media_and_logo_non_200(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(tracker, "_tmdb_get", AsyncMock(return_value=Response()))
...
def test_shareisland_preferred_logo_languages() -> None:
    ...
    assert ShareIsland._preferred_logo_path([{"iso_639_1": "en", "file_path": "/en.png"}]) == "/en.png"


@pytest.mark.asyncio
async def test_shareisland_no_screens() -> None:
    assert await _tracker()._format_screens_italian(_meta(image_list=[])) == "[center]Nessuno screenshot disponibile[/center]"


@pytest.mark.asyncio
async def test_shareisland_synthetic_mediainfo_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_mediainfo_tracks", lambda _meta: (_ for _ in ()).throw(RuntimeError("bad")))
...
def test_shareisland_synthetic_aspect_and_audio_names() -> None:
    ...
    assert tracker._synthetic_audio_name({"Format_Commercial_IfAny": "Dolby Atmos"}, "fallback") == "Dolby Atmos"
    assert tracker._synthetic_audio_name({"Title": "Italian Track"}, "fallback") == "Italian Track"


def test_shareisland_generic_shoutout_and_links(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(share_module.random, "choice", lambda values: values[0])
...
def test_shareisland_italian_language_name_falls_back_when_babel_returns_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    class LocaleValue:
        @staticmethod
        def get_display_name(_language: str) -> None:
            return None

    monkeypatch.setattr(share_module.Locale, "parse", lambda _value: LocaleValue())
    assert _tracker()._get_italian_language_name("en") == "Eng"


def test_shareisland_commentary_audio_is_not_selected() -> None:
    tracker = _tracker()
    assert not tracker._is_italian_audio_track({"@type": "Audio", "Language": "it", "Title": "Director Commentary"})
