...
def _config(*, use_spanish_title: bool = True) -> dict[str, Any]:
    return {
        ...
        "TRACKERS": {
            "EMUWAREZ": {
                "api_key": "test-key",
                "use_spanish_title": use_spanish_title,
            }
        },
    }
...
def _audio_track(
    language: str = "English", *, format_name: str = "AAC", channels: int = 2
) -> dict[str, Any]:
    return {
        "@type": "Audio",
        "Language": language,
...
@pytest.mark.asyncio
async def test_emuwarez_spanish_title_from_imdb_and_tmdb(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    imdb_meta = _meta(
        imdb_info={"akas": [{"country": "Spain", "title": "Título Español"}]}
    )
    assert await tracker._get_title(imdb_meta) == "Título Español"

    assert tracker._imdb_spanish_title(_meta(imdb_info={"akas": "bad"})) == ""
    monkeypatch.setattr(
        tracker.tmdb_manager,
        "get_tmdb_translations",
        AsyncMock(return_value="Título TMDb"),
    )
...
def test_emuwarez_source_and_language_fallbacks() -> None:
    ...
    assert tracker._is_spanish_subtitle(
        {"@type": "Text", "Title": "Castellano"}
    )
...
async def test_emuwarez_dual_audio_string() -> None:
    ...
    assert (
        await tracker._special_audio_string(_meta(), tracks, ["ESP", "ING"])
        == "DUAL AAC 2.0"
    )
...
def test_emuwarez_dual_mixed_codecs_rejected() -> None:
    tracker = _tracker()
    tracks = [
        _audio_track(format_name="AAC"),
        _audio_track(format_name="AC-3"),
    ]
...
async def test_emuwarez_multi_audio_string() -> None:
    tracker = _tracker()
    tracks = [
        _audio_track(language)
        for language in ("Spanish", "English", "French", "German")
    ]
    assert tracker._multi_audio_string(tracks) == "MULTI AAC 2.0"
    assert (
        await tracker._special_audio_string(
            _meta(), tracks, ["ESP", "ING", "FRA", "ALE"]
        )
        == "MULTI AAC 2.0"
    )
...
async def test_emuwarez_single_original_audio_vose_and_vo() -> None:
    ...
    assert (
        await tracker._single_original_audio_string(vo, [track], ["ING"])
        == "V.O. ING AAC 2.0"
    )
    ...
    assert (
        await tracker._single_original_audio_string(
            vose_meta, [track], ["ING"]
        )
        == "VOSE ING AAC 2.0"
    )
...
async def test_emuwarez_single_original_requires_one_track() -> None:
    ...
    assert (
        await tracker._build_audio_string(_meta(audio_languages=["English"]))
        == ""
    )
