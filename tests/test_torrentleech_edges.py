...
def _config(*, api_upload: bool = False, anon: bool = False, img_rehost: bool = True) -> dict[str, Any]:
    return {
        "DEFAULT": {},
        "TRACKERS": {
            "TORRENTLEECH": {
...
def _tracker(*, api_upload: bool = False, anon: bool = False, img_rehost: bool = True) -> TorrentLeech:
    return TorrentLeech(_config(api_upload=api_upload, anon=anon, img_rehost=img_rehost))
...
def _meta(tmp_path: Path | None = None, **values: object) -> Meta:
    ...
    state: dict[str, object] = {
        ...
        "mal_id": 0,
        "scene": False,
        "imdb_info": {"imdbID": "tt123", "imdb_url": "https://imdb.invalid/title/tt123"},
        "tvmaze_id": 0,
        "season": "S01",
        "episode": "E01",
        "year": 2010,
...
def _response(*, status: int = 200, text: str = "", url: str = "https://www.torrentleech.org/torrents/upload/") -> httpx.Response:
    return httpx.Response(status, request=httpx.Request("GET", url), text=text)


@pytest.mark.asyncio
async def test_torrentleech_login_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(tracker, "_validate_cookie_session", AsyncMock(return_value=True))
    assert await tracker.login(_meta())


@pytest.mark.asyncio
async def test_torrentleech_validate_cookie_session_success_failure_and_request_error(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_login_probe", AsyncMock(return_value=_response(status=200, url=tracker.http_upload_url)))
    assert await tracker._validate_cookie_session(False)

    monkeypatch.setattr(tracker, "_login_probe", AsyncMock(return_value=_response(status=200, url="https://www.torrentleech.org/login")))
    assert not await tracker._validate_cookie_session(False)

    error = httpx.RequestError("offline", request=httpx.Request("GET", tracker.base_url))
...
def test_torrentleech_category_disc_and_platform_branches() -> None:
    ...
    assert tracker.get_category(_meta(category="GAME", platform="unknown")) == 17
    assert tracker.get_category(_meta(category="MUSIC")) == 31
    assert tracker.get_category(_meta(category="UNKNOWN")) == 0


@pytest.mark.asyncio
async def test_torrentleech_search_existing_login_fail_and_success(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(tracker, "_search_url", AsyncMock(return_value=[{"name": "Dupe"}]))
    results = await tracker.search_existing(_meta(category="TV", tv_pack=False))
...
async def test_torrentleech_search_url_filters_forbidden() -> None:
    ...
    tracker.session.get = AsyncMock(
        return_value=httpx.Response(
            200,
            request=httpx.Request("GET", "https://www.torrentleech.org/search"),
            json={
                "torrentList": [
    ...
    results = await tracker._search_url("https://www.torrentleech.org/search", ["webrip"])
    assert results == [{"name": "Movie WEB-DL", "size": 10, "link": "https://www.torrentleech.org/torrent/1"}]


@pytest.mark.asyncio
async def test_torrentleech_upload_routes_api_and_cookie(monkeypatch: pytest.MonkeyPatch) -> None:
    api = _tracker(api_upload=True)
...
@pytest.mark.asyncio
async def test_torrentleech_upload_api_debug_and_normal(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker(api_upload=True)
    monkeypatch.setattr(tracker, "_api_torrent_file", AsyncMock(return_value={}))
    monkeypatch.setattr(tracker, "_api_upload_data", AsyncMock(return_value={"name": "Release"}))
    monkeypatch.setattr(tracker, "_debug_api_upload", AsyncMock(return_value=True))
    ...
    monkeypatch.setattr(tracker, "_handle_api_upload_response", AsyncMock(return_value=True))
...
async def test_torrentleech_api_response_error_and_success() -> None:
    tracker = _tracker(api_upload=True)
    item = _meta()
    assert not await tracker._handle_api_upload_response(item, _response(text="error"))
    assert item.tracker_status["TORRENTLEECH"]["status_message"] == "data error: error"

    tracker.common.create_torrent_ready_to_seed = AsyncMock()  # type: ignore[method-assign]
    item = _meta()
    assert await tracker._handle_api_upload_response(item, _response(text="77"))
    assert item.tracker_status["TORRENTLEECH"]["torrent_id"] == "77"
    tracker.common.create_torrent_ready_to_seed.assert_awaited_once()


@pytest.mark.asyncio
async def test_torrentleech_cookie_upload_data_tvmaze_anonymous_and_screens() -> None:
...
@pytest.mark.asyncio
async def test_torrentleech_cookie_upload_login_fail_and_debug(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "generate_description", AsyncMock(return_value="description"))
    monkeypatch.setattr(tracker, "login", AsyncMock(return_value=False))
    item = _meta()
    assert await tracker.cookie_upload(item) is None
    assert "Login with cookies failed" in item.tracker_status["TORRENTLEECH"]["status_message"]

    monkeypatch.setattr(tracker, "login", AsyncMock(return_value=True))
    monkeypatch.setattr(tracker, "get_cookie_upload_data", AsyncMock(return_value={"name": "Release"}))
    monkeypatch.setattr(tracker, "_debug_cookie_upload", AsyncMock(return_value=True))
    assert await tracker.cookie_upload(_meta(debug=True))


@pytest.mark.asyncio
async def test_torrentleech_submit_cookie_upload_request_error(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    error = httpx.RequestError("offline", request=httpx.Request("POST", tracker.http_upload_url))
    monkeypatch.setattr(tracker, "_cookie_upload_response", AsyncMock(side_effect=error))
    item = _meta()
...
@pytest.mark.asyncio
async def test_torrentleech_cookie_upload_response_reads_torrent(tmp_path: Path) -> None:
    tracker = _tracker()
    root = tmp_path / "tmp" / "release"
    root.mkdir(parents=True)
    (root / "[TORRENTLEECH].torrent").write_bytes(b"torrent")
    tracker.session.post = AsyncMock(return_value=_response(status=302))  # type: ignore[method-assign]
    response = await tracker._cookie_upload_response(_meta(tmp_path), {}, "description")
...
@pytest.mark.asyncio
async def test_torrentleech_cookie_upload_response_failure_and_success() -> None:
    tracker = _tracker()
    tracker.common.save_html_file = AsyncMock(return_value="failed.html")  # type: ignore[method-assign]
    item = _meta()
    assert not await tracker._handle_cookie_upload_response(item, _response(status=200, text="bad"))
...
@pytest.mark.asyncio
async def test_torrentleech_submit_cookie_upload_success_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    response = _response(status=200)
    monkeypatch.setattr(tracker, "_cookie_upload_response", AsyncMock(return_value=response))
    monkeypatch.setattr(tracker, "_handle_cookie_upload_response", AsyncMock(return_value=True))
    item = _meta()
    assert await tracker._submit_cookie_upload(item, {}, "description")
    tracker._handle_cookie_upload_response.assert_awaited_once_with(item, response)
