...
def _movie_payload(**values: object) -> list[dict[str, Any]]:
    movie: dict[str, Any] = {
        ...
        "genres": ["Drama"],
        "movieFile": {
            "originalFilePath": "/media/movie.mkv",
            "releaseGroup": "GROUP",
        },
    }
    movie.update(values)
...
def test_radarr_extract_empty_invalid_list_filename_and_release_group() -> (
    None
):
    ...
    assert (
        asyncio.run(manager.extract_movie_data(_movie_payload(), "/other.mkv"))
        is None
    )

    parsed = asyncio.run(
        manager.extract_movie_data(_movie_payload(), "/media/movie.mkv")
    )
    assert parsed == {
        "imdb_id": 1234567,
        "tmdb_id": 123,
    ...
    parsed = asyncio.run(
        manager.extract_movie_data(_movie_payload(imdbId="", movieFile={}))
    )
...
def test_radarr_no_config_invalid_slots_no_query_and_success() -> None:
    assert (
        asyncio.run(RadarrManager({"DEFAULT": {}}).get_radarr_data(tmdb_id=1))
        is None
    )
    config = {
        "DEFAULT": {
            "radarr_api_key": 1,
            "radarr_api_key_1": " ",
            "radarr_api_key_2": "key",
            "radarr_url_2": 1,
        }
    }
    assert (
        asyncio.run(RadarrManager(config).get_radarr_data(tmdb_id=1)) is None
    )
    ...
    _Client.queue = [_Response(payload=_movie_payload())]
    result = asyncio.run(
        RadarrManager(config).get_radarr_data(filename="/media/movie.mkv")
    )
...
def test_radarr_response_errors_then_second_instance_success() -> None:
    ...
    assert (
        asyncio.run(RadarrManager(config).get_radarr_data(tmdb_id=123)) is None
    )
...
def test_radarr_accepts_strong_title_match_when_filename_year_matches_secondary_year() -> (
    None
):
    manager = RadarrManager({"DEFAULT": {}})
    ...
    parsed = asyncio.run(
        manager.extract_movie_data(
            payload, "Tatame.2023.1080p.AMZN.WEB-DL.DDP5.1.H.264.DUAL-FLY.mkv"
        )
    )
...
def test_radarr_rejects_lookup_candidate_when_only_year_matches() -> None:
    ...
    assert (
        asyncio.run(
            manager.extract_movie_data(payload, "Tatame.2023.1080p.WEB-DL.mkv")
        )
        is None
    )
...
def test_sonarr_extract_empty_parse_list_and_invalid() -> None:
    ...
    assert (
        parsed["tvdb_id"] == 456
        and parsed["imdb_id"] == 1234567
        and parsed["release_group"] == "PARSE-GROUP"
    )
    parsed = asyncio.run(
        manager.extract_show_data(
            {"series": {"imdbId": ""}, "parsedEpisodeInfo": {}}
        )
    )
    assert parsed["imdb_id"] is None and parsed["release_group"] is None
    ...
    listed = asyncio.run(
        manager.extract_show_data(_series_payload(imdbId="", releaseGroup=""))
    )
...
def test_sonarr_no_config_invalid_slots_no_query_and_success() -> None:
    assert (
        asyncio.run(SonarrManager({"DEFAULT": {}}).get_sonarr_data(tvdb_id=1))
        is None
    )
    config = {
        "DEFAULT": {
            "sonarr_api_key": 1,
            "sonarr_api_key_1": " ",
            "sonarr_api_key_2": "key",
            "sonarr_url_2": 1,
        }
    }
    assert (
        asyncio.run(SonarrManager(config).get_sonarr_data(tvdb_id=1)) is None
    )
    ...
    result = asyncio.run(
        SonarrManager(config).get_sonarr_data(
            filename="/media/show.mkv", title="Show.S01E01"
        )
    )
    assert result and result["release_group"] == "GROUP"
...
def test_sonarr_response_errors_then_second_instance_success() -> None:
    ...
    assert (
        asyncio.run(SonarrManager(config).get_sonarr_data(tvdb_id=456)) is None
    )
...
def test_radarr_matching_helper_guards_and_invalid_year_values() -> None:
    manager = RadarrManager({"DEFAULT": {}})
    assert not manager._exact_file_match(
        {"movieFile": "bad"}, "/media/movie.mkv"
    )
    assert manager._best_title_similarity("", {"title": "Movie"}) == 0.0
    assert manager._alternate_titles({"alternateTitles": "bad"}) == []
    assert (
        manager._candidate_years({"year": "bad", "secondaryYear": None})
        == set()
    )
...
def test_radarr_alternate_titles_and_release_group_guards() -> None:
    manager = RadarrManager({"DEFAULT": {}})
    assert manager._alternate_titles(
        {"alternateTitles": [{"title": "Alt"}, "bad"]}
    ) == ["Alt"]
