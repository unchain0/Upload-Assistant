...
def test_desktop_installer_detection() -> None:
    assert not game._is_desktop_installer(
        Meta(console_game=True, platform="PC", filelist=["setup.exe"])
    )
    assert not game._is_desktop_installer(
        Meta(platform="SWITCH", filelist=["setup.exe"])
    )
    assert game._is_desktop_installer(
        Meta(platform="PC", filelist=["setup.exe"])
    )
    assert game._is_desktop_installer(Meta(platform="MAC", path="setup.dmg"))
    assert not game._is_desktop_installer(
        Meta(platform="LINUX", filelist=["README.txt"])
    )


def test_read_software_notes_text_nfo_limits_and_errors(
    ...
    assert asyncio.run(
        game._read_software_notes(Meta(filelist=[str(nfo)]))
    ) == ("2. Extract archive\n3. Run setup.exe\n4. Copy crack")
    ...
    assert (
        asyncio.run(game._read_software_notes(Meta(filelist=[str(text)])))
        == ""
    )
...
def test_platform_mapping_and_7z_resolution(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert asyncio.run(
        game.get_7z_path(
            str(tmp_path), {"DEFAULT": {"7z_path": str(configured)}}
        )
    ) == str(configured)
    ...
    monkeypatch.setattr(
        seven_zip.SevenZipBinaryManager,
        "ensure_7z_binary",
        AsyncMock(return_value=str(managed)),
    )
...
def test_archive_listing_success_failure_and_exception(
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=_Process(output)),
    )
    assert asyncio.run(
        game.list_archive_contents_with_7z("game.zip", "7z")
    ) == ["setup.exe", "folder/file.dll"]
    monkeypatch.setattr(
        game.asyncio,
        "create_subprocess_exec",
        AsyncMock(return_value=_Process(output, 1)),
    )
    assert (
        asyncio.run(game.list_archive_contents_with_7z("game.zip", "7z")) == []
    )
    monkeypatch.setattr(
        game.asyncio,
        "create_subprocess_exec",
...
def test_detect_platform_archive_content_and_7zr_skip(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    archive = tmp_path / "game.zip"
    archive.write_bytes(b"archive")
    monkeypatch.setattr(
        game, "get_7z_path", AsyncMock(return_value="/usr/bin/7z")
    )
    monkeypatch.setattr(
        game,
        "list_archive_contents_with_7z",
        AsyncMock(return_value=["steam_api.dll"]),
    )
    assert (
        asyncio.run(
            game.detect_platform_from_files(
                [str(archive)], base_dir=str(tmp_path)
            )
        )
        == "PC"
    )
    ...
    monkeypatch.setattr(
        game, "get_7z_path", AsyncMock(return_value="/usr/bin/7zr")
    )
    ...
    assert (
        asyncio.run(
            game.detect_platform_from_files(
                [str(seven)], base_dir=str(tmp_path)
            )
        )
        == "PC"
    )
...
def test_resolve_game_filelist_priorities_empty_and_single_file(
    tmp_path: Path,
) -> None:
    ...
    for label, files, expected in (
        ...
        selected, filelist, search, kind = game.resolve_game_filelist(
            meta, str(root)
        )
    ...
    assert (
        selected == str(single)
        and filelist == [str(single)]
        and search == "single.iso"
        and kind == "file"
    )
...
def _game_meta(
    tmp_path: Path, name: str = "Example.Game.v1.2-GROUP.iso", **values: object
) -> Meta:
    path = tmp_path / name
...
def _selected_game() -> dict[str, Any]:
    return {
        ...
        "websites": [
            {"type": 13, "url": "https://store.steampowered.com/app/1234/Game"}
        ],
        "external_games": [],
        "language_supports": [
            {
...
def test_gather_game_prep_credentials_title_and_no_results(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    no_credentials = _game_meta(tmp_path)
    asyncio.run(
        game.gather_game_prep(
            no_credentials, no_credentials.path, str(tmp_path), {"DEFAULT": {}}
        )
    )
    assert (
        no_credentials.category == "GAME"
        and no_credentials.game_version == "v1.2"
    )
    ...
    asyncio.run(
        game.gather_game_prep(
            software, software.path, str(tmp_path), {"DEFAULT": {}}
        )
    )
    ...
    empty.title = ""
    asyncio.run(
        game.gather_game_prep(empty, "", str(tmp_path), {"DEFAULT": {}})
    )


def test_gather_game_prep_manual_ids_nfo_steam_and_selection(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    nfo = tmp_path / "scene.nfo"
    nfo.write_text(
        "Build 3.4\nhttps://store.steampowered.com/app/1234/Game",
        encoding="utf-8",
    )
    ...
    asyncio.run(
        game.gather_game_prep(manual, manual.path, str(tmp_path), config)
    )
    ...
    asyncio.run(
        game.gather_game_prep(steam, steam.path, str(tmp_path), config)
    )
    assert steam.steam_manual == "1234" and steam.igdb_id == 42

    _IGDB.reset()
    _IGDB.search_results = [
        _selected_game(),
        {**_selected_game(), "id": 43, "name": "Other"},
    ]
    choices = iter(("Skip - Don't select any match",))
    monkeypatch.setattr(
        game.cli_ui, "ask_choice", lambda *_args, **_kwargs: next(choices)
    )
    interactive = _game_meta(tmp_path, unattended=False)
    asyncio.run(
        game.gather_game_prep(
            interactive, interactive.path, str(tmp_path), config
        )
    )
    ...
    asyncio.run(
        game.gather_game_prep(cancelled, cancelled.path, str(tmp_path), config)
    )


def test_gather_game_prep_full_metadata_steam_cache_and_screenshots(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    monkeypatch.setattr(
        game, "is_cache_miss", lambda value: value is cache.value
    )
    _Response.payload = {
        "1234": {
            "success": True,
    ...
    assert (
        meta.artwork_url.startswith("https:")
        and "t_cover_big" in meta.artwork_url
    )
    ...
    assert (
        ...
        and meta.requirements_recommended == "Recommended"
    )
    assert (
        meta.image_list[0]["raw_url"].startswith("https:")
        and "t_1080p" in meta.image_list[0]["raw_url"]
    )
    assert (
        json.loads(
            (tmp_path / "tmp" / "game" / "image_data.json").read_text()
        )["image_list"]
        == meta.image_list
    )
...
def test_gather_game_prep_external_game_platform_and_steam_failures(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    asyncio.run(
        game.gather_game_prep(
            no_platform, no_platform.path, str(tmp_path), config
        )
    )
    ...
    _IGDB.reset()
    _IGDB.search_results = [
        {**selected, "platforms": [], "websites": [], "external_games": []}
    ]
    missing_platform = _game_meta(tmp_path, "Unknown.Game.bin")
    asyncio.run(
        game.gather_game_prep(
            missing_platform, missing_platform.path, str(tmp_path), config
        )
    )
    ...
    asyncio.run(
        game.gather_game_prep(failed, failed.path, str(tmp_path), config)
    )


def test_gather_game_prep_steam_http_statuses_and_image_write_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    for status, payload in ((404, {}), (500, {}), (200, [])):
        ...
        cache = _Cache(object())
        monkeypatch.setattr(
            game, "cache_for", lambda *_args, _cache=cache, **_kwargs: _cache
        )
        monkeypatch.setattr(
            game,
            "is_cache_miss",
            lambda value, _cache=cache: value is _cache.value,
        )
        meta = _game_meta(
            tmp_path, f"Game-{status}.iso", uuid=f"game-{status}"
        )
        asyncio.run(
            game.gather_game_prep(meta, meta.path, str(tmp_path), config)
        )
...
def test_gather_game_prep_directory_and_discovered_nfo_version(
    tmp_path: Path,
) -> None:
    config = {"DEFAULT": {}}
...
def test_gather_game_prep_latin_nfo_error_and_platform_sorting(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    latin.write_bytes(
        "Steam https://store.steampowered.com/app/777/Gáme".encode("latin-1")
    )
    _IGDB.steam_result = _selected_game()
    meta = Meta(
        path=str(media),
    ...
    sorted_meta = _game_meta(
        tmp_path, "Sorted.Game.bin", platform="PC", manual_platform="PC"
    )
    asyncio.run(
        game.gather_game_prep(
            sorted_meta, sorted_meta.path, str(tmp_path), config
        )
    )
    ...
    monkeypatch.setattr(
        game.cli_ui,
        "ask_choice",
        lambda _message, choices, **_kwargs: choices[1],
    )
    chosen = _game_meta(
        tmp_path,
        "Chosen.Game.bin",
    ...
    asyncio.run(
        game.gather_game_prep(chosen, chosen.path, str(tmp_path), config)
    )
    assert chosen.title == "Other"


def test_gather_game_prep_nfo_outer_error_and_platform_alias_detection(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    _IGDB.search_results = [selected]
    monkeypatch.setattr(
        game, "detect_platform_from_files", AsyncMock(return_value=None)
    )
    alias = _game_meta(tmp_path, "Alias.Game.PS5.bin", platform="")
    asyncio.run(
        game.gather_game_prep(alias, alias.path, str(tmp_path), config)
    )
