...
def test_hdspace_rejects_resolution_below_720p() -> None:
    assert not asyncio.run(
        _tracker().get_additional_checks(_make_meta(resolution="480p"))
    )
...
def test_hdspace_rejects_forbidden_xvid_codecs() -> None:
    assert not asyncio.run(
        _tracker().get_additional_checks(
            _make_meta(video_codec="xvid", video_encode="xvid")
        )
    )
    assert not asyncio.run(
        _tracker().get_additional_checks(
            _make_meta(video_codec="x264", video_encode="divx")
        )
    )
...
def test_hdspace_rejects_rar_payload_files() -> None:
    assert not asyncio.run(
        _tracker().get_additional_checks(
            _make_meta(filelist=["Movie.2024.rar"])
        )
    )
    ...
    assert not asyncio.run(
        _tracker().get_additional_checks(
            _make_meta(
                image_list=[
                    {
                        "raw_url": "https://example.com/frame-1.png",
                        "local_file_path": "",
                    }
                ]
            )
...
def test_hdspace_rejects_non_png_screenshots(tmp_path: Path) -> None:
    ...
    assert not asyncio.run(
        _tracker().get_additional_checks(_make_meta(image_list=image_list))
    )
...
def test_hdspace_rejects_invalid_screenshot_width(tmp_path: Path) -> None:
    image_list = [
        _screenshot_entry(
            _create_png(tmp_path / "frame-1.png", 1280, 720), "frame-1.png"
        ),
        _screenshot_entry(
            _create_png(tmp_path / "frame-2.png", 1366, 768), "frame-2.png"
        ),
        _screenshot_entry(
            _create_png(tmp_path / "frame-3.png", 1920, 1080), "frame-3.png"
        ),
    ]
    assert not asyncio.run(
        _tracker().get_additional_checks(_make_meta(image_list=image_list))
...
def test_hdspace_allows_valid_screenshots_with_allowed_dimensions_and_format(
    tmp_path: Path,
) -> None:
    image_list = [
        _screenshot_entry(
        ...
        _screenshot_entry(
            _create_png(tmp_path / "frame-2.png", 1920, 1080), "frame-2.png"
        ),
        _screenshot_entry(
            _create_png(tmp_path / "frame-3.png", 3840, 2160), "frame-3.png"
        ),
    ]
    assert asyncio.run(
        _tracker().get_additional_checks(_make_meta(image_list=image_list))
