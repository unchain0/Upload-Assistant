...
from src.integrations.security.redaction import (
    Redaction,
    clean_meta_for_export,
    extract_json_blocks,
    redact_private_info,
    redact_value,
)
...
class _Response:
    def __init__(
        ...
        self._payload = payload or {
            "files": [{"url": "https://uguu.invalid/package.tar"}]
        }
...
def test_python_config_repository_reports_all_invalid_sources(
    ...
    annotated.write_text(
        "config: dict[str, object] = {'DEFAULT': {'value': 1}}",
        encoding="utf-8",
    )
    assert (
        repository.load(annotated, ConfigurationSourceKind.RUNTIME).section(
            "DEFAULT"
        )["value"]
        == 1
    )
...
def test_python_config_repository_atomic_copy_and_write_backups(
    tmp_path: Path,
) -> None:
    repository = PythonConfigurationRepository()
    source = tmp_path / "source.py"
    source.write_text(
        "config = {'DEFAULT': {'value': 'new'}}\n", encoding="utf-8"
    )
    source.chmod(0o640)
    destination = tmp_path / "runtime" / "config.py"
    destination.parent.mkdir()
    destination.write_text(
        "config = {'DEFAULT': {'value': 'old'}}\n", encoding="utf-8"
    )

    backup = repository.copy_atomically(source, destination)
    assert backup is not None and backup.read_text(encoding="utf-8").endswith(
        "'old'}}\n"
    )
    ...
    assert (
        repository.load(fresh, ConfigurationSourceKind.RUNTIME).section(
            "DEFAULT"
        )["tmdb_api"]
        == "key"
    )
...
def test_metadata_cache_handles_invalid_settings_serialization_and_io(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = {
        "DEFAULT": {
    ...
    async def exercise() -> None:
        zero = MetadataCache(
            tmp_path, {"DEFAULT": {"metadata_cache_default_ttl_hours": 0}}
        )
...
def test_terminal_link_falls_back_for_invalid_ipv6_url() -> None:
    value = format_terminal_link(
        "Open [now]", "https://[broken/path?q=1", {"embed_links": True}
    )
    assert value.startswith("[link=https://")
    assert "Open \\[now]" in value


def test_console_progress_buffer_and_dynamic_file_edges(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    starts: list[int] = []
    stops: list[int] = []
    monkeypatch.setattr(
        Progress, "start", lambda self: starts.append(id(self))
    )
    ...
    monkeypatch.setattr(
        console_module, "is_cli_progress_suppressed", lambda: True
    )
...
def test_redaction_handles_nested_json_invalid_blocks_and_export(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    redacted = redact_value(
        "https://tracker.invalid/abcdefghijkl/announce?api_key=secret " + text
    )
    ...
    monkeypatch.setattr(
        Redaction, "redact_private_info", lambda *_args, **_kwargs: object()
    )
    ...
    meta = Meta(
        ...
        uuid="redaction",
        tracker_status={
            "BHD": {"status": "ready", "status_message": "secret"}
        },
    ...
    exported = json.loads(
        (tmp_path / "tmp" / "redaction" / "meta.json").read_text(
            encoding="utf-8"
        )
    )
