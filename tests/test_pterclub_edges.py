...
@pytest.mark.asyncio
async def test_pterclub_validate_credentials_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_pterclub_validate_cookies_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    cookie = tmp_path / "PTERCLUB.txt"
    cookie.write_text("cookie", encoding="utf-8")
    monkeypatch.setattr(pter_module, "find_cookie_file", lambda *_args, **_kwargs: str(cookie))
    ...
    class Client:
        ...
        async def get(self, **_kwargs: object) -> httpx.Response:
            return _response(text='<a href="#" data-url="logout.php" id="logout-confirm">')

    monkeypatch.setattr(pter_module.httpx, "AsyncClient", lambda *_args, **_kwargs: Client())
    assert await tracker.validate_cookies(_meta(tmp_path))


@pytest.mark.asyncio
async def test_pterclub_search_existing_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    cookie = tmp_path / "PTERCLUB.txt"
    cookie.write_text("cookie", encoding="utf-8")
    monkeypatch.setattr(pter_module, "find_cookie_file", lambda *_args, **_kwargs: str(cookie))
    ...
    """
    monkeypatch.setattr(tracker, "_search_response", AsyncMock(return_value=_response(text=html)))
...
async def test_pterclub_special_category_and_area() -> None:
    tracker = _tracker()
    assert await tracker.get_type_category_id(_meta(genres=["Documentary"])) == "402"
    assert await tracker.get_type_category_id(_meta(genres=["Animation", "Documentary"])) == "403"
    assert await tracker.get_area_id(_meta(ptgen={"region": ["日本"]})) == 6


@pytest.mark.asyncio
async def test_pterclub_description_signature_and_discs(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    tracker.signature = "SIGNATURE"
    tracker.common.ptgen = AsyncMock(return_value="PTGEN")  # type: ignore[method-assign]
    monkeypatch.setattr(tracker, "_description_screenshots", AsyncMock(return_value=[]))
...
@pytest.mark.asyncio
async def test_pterclub_technical_description_reads_mediainfo(tmp_path: Path) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_pterclub_description_rehost_images(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker(img_rehost=True)
    monkeypatch.setattr(tracker, "pterimg_upload", AsyncMock(return_value=[{"web_url": "w", "img_url": "i"}]))
    assert await tracker._description_image_list(_meta()) == [{"web_url": "w", "img_url": "i"}]


@pytest.mark.asyncio
async def test_pterclub_get_auth_token_saved_and_login_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    cookie = tmp_path / "Pterimg.txt"
    cookie.write_text("cookie", encoding="utf-8")
    monkeypatch.setattr(tracker, "_pterimg_cookie_file", lambda _meta: cookie)
    monkeypatch.setattr(tracker, "_saved_cookie_values", lambda _cookie: {"sid": "x"})
    monkeypatch.setattr(tracker, "_saved_auth_token", AsyncMock(return_value="saved"))
    assert await tracker.get_auth_token(_meta(tmp_path)) == "saved"

    monkeypatch.setattr(tracker, "_saved_auth_token", AsyncMock(return_value=""))
    monkeypatch.setattr(tracker, "_pterimg_login", AsyncMock(return_value="fresh"))
...
def test_pterclub_saved_cookie_values(tmp_path: Path) -> None:
    ...
    cookie.write_text("cookie", encoding="utf-8")
    tracker.cookie_validator._load_cookies_dict_secure = lambda _path: {"sid": {"value": "abc"}}  # type: ignore[method-assign,reportPrivateUsage]
    assert tracker._saved_cookie_values(cookie) == {"sid": "abc"}


@pytest.mark.asyncio
async def test_pterclub_saved_auth_token_success(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    class Client:
        ...
        async def get(self, _url: str) -> httpx.Response:
            return _response(text='auth_token = "token" <a href="https://s3.pterclub.com/logout/?x">logout</a>')
...
@pytest.mark.asyncio
async def test_pterclub_pterimg_login_success_and_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    cookie = tmp_path / "Pterimg.txt"
    saved: list[str] = []
    tracker.cookie_validator._save_cookies_secure = lambda _jar, path: saved.append(path)  # type: ignore[method-assign,reportPrivateUsage]
    ...
    class Client:
        ...
        async def post(self, **_kwargs: object) -> httpx.Response:
            return _response(status=200 if self.success else 403, text='auth_token = "fresh"')

    monkeypatch.setattr(pter_module.httpx, "AsyncClient", lambda *_args, **_kwargs: Client(True))
    assert await tracker._pterimg_login(cookie, {}) == "fresh"
    assert saved == [str(cookie)]

    monkeypatch.setattr(pter_module.httpx, "AsyncClient", lambda *_args, **_kwargs: Client(False))
    with pytest.raises(LoginError):
        await tracker._pterimg_login(cookie, {})


@pytest.mark.asyncio
async def test_pterclub_pterimg_upload_wrapper(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    cookie = tmp_path / "Pterimg.txt"
    cookie.write_text("cookie", encoding="utf-8")
    monkeypatch.setattr(pter_module, "find_cookie_file", lambda *_args, **_kwargs: str(cookie))
    monkeypatch.setattr(tracker, "get_auth_token", AsyncMock(return_value="auth"))
    monkeypatch.setattr(tracker, "_saved_cookie_values", lambda _cookie: {"sid": "x"})
    monkeypatch.setattr(tracker, "_screenshot_paths", lambda _meta: ["one.png"])
    monkeypatch.setattr(tracker, "_upload_pterimg_images", AsyncMock(return_value=[{"web_url": "u", "img_url": "u"}]))
    ...
    monkeypatch.setattr(pter_module.httpx, "AsyncClient", lambda *_args, **_kwargs: Client())
    assert await tracker.pterimg_upload(_meta(tmp_path)) == [{"web_url": "u", "img_url": "u"}]


@pytest.mark.asyncio
async def test_pterclub_upload_pterimg_images_filters_duplicate(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        tracker,
    ...
    assert await tracker._upload_pterimg_images(AsyncMock(), ["a.png", "b.png"], {}) == [{"web_url": "u", "img_url": "u"}]


@pytest.mark.asyncio
async def test_pterclub_upload_pterimg_image_success_duplicate_and_error(tmp_path: Path) -> None:
    tracker = _tracker()
    image = tmp_path / "image.png"
    image.write_bytes(b"image")
    client = AsyncMock()
    client.post = AsyncMock(return_value=_response(json_data={"image": {"url": "https://img/u"}}))
    assert await tracker._upload_pterimg_image(client, str(image), {}) == {"web_url": "https://img/u", "img_url": "https://img/u"}

    client.post = AsyncMock(return_value=_response(status=409, json_data={"error": {"message": "Duplicated upload"}}))
    assert await tracker._upload_pterimg_image(client, str(image), {}) is None

    client.post = AsyncMock(return_value=_response(status=500, json_data={"error": {"message": "boom"}}))
...
@pytest.mark.asyncio
async def test_pterclub_upload_orchestration_debug(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(tracker, "_upload_parts", AsyncMock(return_value=({"name": "Release"}, {}, Path("torrent"))))
...
@pytest.mark.asyncio
async def test_pterclub_ensure_description_calls_edit(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_pterclub_upload_parts_reads_files(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    root = tmp_path / "tmp" / "release"
    root.mkdir(parents=True)
    (root / "[PTERCLUB]DESCRIPTION.txt").write_text("description", encoding="utf-8")
    (root / "[PTERCLUB].torrent").write_bytes(b"torrent")
    monkeypatch.setattr(tracker, "_upload_data", AsyncMock(return_value={"name": "Release"}))
...
async def test_pterclub_small_description_and_debug_upload() -> None:
    tracker = _tracker()
    item = _meta(ptgen={"trans_title": ["译名一", "译名二"], "genre": ["剧情"]})
    assert "译名一 / 译名二" in tracker._small_description(item)
    tracker.common.create_torrent_for_upload = AsyncMock()  # type: ignore[method-assign]
    assert await tracker._debug_upload(item, {"name": "Release"})
    assert item.tracker_status["PTERCLUB"]["status_message"].startswith("Debug mode")


@pytest.mark.asyncio
async def test_pterclub_submit_upload_missing_cookie(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(pter_module, "find_cookie_file", lambda *_args, **_kwargs: str(tmp_path / "missing.txt"))
    assert not await tracker._submit_upload(_meta(tmp_path), {}, {}, tmp_path / "torrent")


@pytest.mark.asyncio
async def test_pterclub_handle_upload_response_success_and_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "download_new_torrent", AsyncMock())
    item = _meta(tmp_path)
    response = _response(url="https://pterclub.net/details.php?id=77&uploaded=1")
    assert await tracker._handle_upload_response(item, response, tmp_path / "torrent")
    assert item.tracker_status["PTERCLUB"]["torrent_id"] == "77"

    with pytest.raises(UploadError, match="not expected"):
        await tracker._handle_upload_response(_meta(tmp_path), _response(url="https://pterclub.net/upload.php"), tmp_path / "torrent")
...
@pytest.mark.asyncio
async def test_pterclub_validate_credentials_success(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_pterclub_saved_auth_token_invalid_session(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_pterclub_pterimg_upload_missing_cookie(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    tracker = _tracker()
    missing = tmp_path / "missing.txt"
    monkeypatch.setattr(pter_module, "find_cookie_file", lambda *_args, **_kwargs: str(missing))
    monkeypatch.setattr(tracker, "get_auth_token", AsyncMock(return_value="auth"))
    assert await tracker.pterimg_upload(_meta(tmp_path)) == []


@pytest.mark.asyncio
async def test_pterclub_upload_non_debug_delegates_submit(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    tracker.common.create_torrent_for_upload = AsyncMock()  # type: ignore[method-assign]
    monkeypatch.setattr(tracker, "_ensure_description", AsyncMock())
    monkeypatch.setattr(tracker, "_upload_parts", AsyncMock(return_value=({"name": "Release"}, {}, Path("torrent"))))
    ...
    tracker._submit_upload.assert_awaited_once_with(item, {"name": "Release"}, {}, Path("torrent"))


@pytest.mark.asyncio
async def test_pterclub_submit_upload_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    cookie = tmp_path / "PTERCLUB.txt"
    cookie.write_text("cookie", encoding="utf-8")
    monkeypatch.setattr(pter_module, "find_cookie_file", lambda *_args, **_kwargs: str(cookie))
