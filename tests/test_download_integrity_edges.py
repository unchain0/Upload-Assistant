...
def test_safe_destination_rejects_absolute_traversal_and_symlink_escape(tmp_path: Path) -> None:
    base = tmp_path / "base"
    base.mkdir()
...
def test_safe_extract_zip_directories_links_special_files_and_regular_member(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(link_path, "w") as archive:
        archive.writestr(_zip_info("link", mode=stat.S_IFLNK | 0o777), b"target")
    with zipfile.ZipFile(link_path) as archive, pytest.raises(RuntimeError, match="links are not allowed"):
    ...
    with zipfile.ZipFile(special_path, "w") as archive:
        archive.writestr(_zip_info("fifo", mode=stat.S_IFIFO | 0o644), b"value")
    with zipfile.ZipFile(special_path) as archive, pytest.raises(RuntimeError, match="Unsupported archive member"):
    ...
    with zipfile.ZipFile(large_path) as archive, pytest.raises(RuntimeError, match="expanded-size limit"):
        integrity.safe_extract_zip(archive, tmp_path / "large-output", max_bytes=4)


def test_extract_zip_regular_member_rejects_invalid_and_extracts(tmp_path: Path) -> None:
    archive_path = tmp_path / "asset.zip"
    with zipfile.ZipFile(archive_path, "w") as archive:
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        with pytest.raises(RuntimeError, match="not a regular file"):
            integrity.extract_zip_regular_member(archive, "directory/", tmp_path / "directory")
        with pytest.raises(RuntimeError, match="expanded-size limit"):
            integrity.extract_zip_regular_member(archive, "tool", tmp_path / "too-large", max_bytes=2)


def test_safe_extract_tar_directory_regular_limit_and_missing_stream(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with tarfile.open(archive_path) as archive, pytest.raises(RuntimeError, match="expanded-size limit"):
...
def test_promote_success_remove_target_and_complete_rollback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    integrity.promote_files_with_rollback([(source, target)], backup, remove_targets=[stale])
...
def test_hash_verification_missing_mismatch_and_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(integrity.SHA256_BY_ASSET, "asset", integrity.sha256_file(asset))
    integrity.verify_downloaded_asset(asset, "asset")


def test_declared_size_none_invalid_valid_and_excessive() -> None:
    integrity._validate_declared_size(SimpleNamespace(headers={}), 10)
    integrity._validate_declared_size(SimpleNamespace(headers={"content-length": "bad"}), 10)
    integrity._validate_declared_size(SimpleNamespace(headers={"content-length": None}), 10)
    integrity._validate_declared_size(SimpleNamespace(headers={"content-length": "10"}), 10)
    with pytest.raises(RuntimeError, match="10-byte limit"):
        integrity._validate_declared_size(SimpleNamespace(headers={"content-length": "11"}), 10)


def test_verified_download_success_failure_cleanup_async_and_sync(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    asyncio.run(integrity.download_verified_asset(object(), "https://example.invalid", destination, "asset"))
    ...
    monkeypatch.setitem(integrity.SHA256_BY_ASSET, "asset", digest)
    integrity.download_verified_asset_sync("https://example.invalid", destination, "asset")
...
def test_promote_rolls_back_already_promoted_target_and_reports_unlink_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for path, data in ((first_source, b"new-one"), (second_source, b"new-two"), (first_target, b"old-one"), (second_target, b"old-two")):
    ...
    with pytest.raises(OSError, match="second promotion failed"):
        integrity.promote_files_with_rollback([(first_source, first_target), (second_source, second_target)], backup)
    ...
    def fail_promoted_unlink(path: Path, *args: object, **kwargs: object) -> None:
