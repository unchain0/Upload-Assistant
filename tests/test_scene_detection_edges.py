...
class Response:
    def __init__(self, payload: object = None, *, status: int = 200, text: str = "", content: bytes = b"nfo") -> None:
...
def test_scene_search_cached_match_lowercase_and_imdb(tmp_path: Path) -> None:
    ...
    (cache / "movie.release-group.json").write_text(json.dumps(_scene_result()), encoding="utf-8")

    video, scene, imdb = asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta))
...
def test_scene_search_cache_invalid_then_remote_no_match(tmp_path: Path) -> None:
    meta = _meta(tmp_path, debug=True)
    cache = tmp_path / "tmp" / "scene" / "srrdb" / "search"
    cache.mkdir(parents=True)
    (cache / "Movie.Release-GROUP.json").write_text("bad-json", encoding="utf-8")
...
def test_scene_search_request_error_and_predb_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    request = httpx.RequestError("offline", request=httpx.Request("GET", "https://srrdb.com"))
    Client.reset(request)
    manager = SceneManager({"DEFAULT": {"check_predb": True}})
    monkeypatch.setattr(manager, "predb_check", lambda *_args: asyncio.sleep(0, result=True))
    meta = _meta(tmp_path)
...
def test_scene_nfo_details_remote_and_download(tmp_path: Path) -> None:
    ...
    Client.reset(Response(_scene_result(hasNFO="yes")), Response(details), Response(content=b"NFO DATA"))

    _, scene, _ = asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta))
...
def test_scene_nfo_cached_details_and_existing_local_file(tmp_path: Path) -> None:
    meta = _meta(tmp_path)
...
def test_scene_nfo_detail_errors_and_failed_download_are_nonfatal(tmp_path: Path) -> None:
    meta = _meta(tmp_path)
    Client.reset(
    ...
    Client.reset(Response(_scene_result(hasNFO="yes")), RuntimeError("details failed"))
    _, scene, _ = asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta))
    assert scene and not meta.nfo
...
def test_scene_imdb_manual_prevents_remote_override(tmp_path: Path) -> None:
    ...
    _, scene, imdb = asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta, imdb=7654321))
...
def test_game_folder_name_is_used_for_search(tmp_path: Path) -> None:
    ...
    video, scene, _ = asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta))
...
def test_lower_search_match_download_and_existing_nfo(tmp_path: Path) -> None:
    meta = _meta(tmp_path, filename="Movie Release.mkv", tag="-GROUP", imdb_id=1234567)
    result = _scene_result(hasNFO="yes", imdbId="1234567")
    Client.reset(Response(result), Response(content=b"NFO"))
    video, scene, _ = asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta, lower=True))
    assert scene and meta.scene and video == "Movie.Release-GROUP"
    assert meta.nfo and meta.auto_nfo

    meta = _meta(tmp_path, uuid="existing", filename="Movie Release.mkv", tag="-GROUP", imdb_id=1234567)
    result = _scene_result(hasNFO="yes", imdbId="1234567")
    ...
    asyncio.run(SceneManager({"DEFAULT": {}}).is_scene(meta.filename, meta, lower=True))
    assert meta.nfo and meta.auto_nfo


def test_lower_search_missing_mismatch_no_results_and_errors(tmp_path: Path) -> None:
    manager = SceneManager({"DEFAULT": {}})
    missing = _meta(tmp_path, filename=None, tag=None)  # type: ignore[arg-type]
    _, scene, _ = asyncio.run(manager.is_scene("video.mkv", missing, lower=True))
    ...
    _, scene, _ = asyncio.run(manager.is_scene(mismatch.filename, mismatch, lower=True))
    assert not scene

    no_results = _meta(tmp_path, uuid="none", tag="-GROUP")
    Client.reset(Response({"resultsCount": 0, "results": []}))
    _, scene, _ = asyncio.run(manager.is_scene(no_results.filename, no_results, lower=True))
    assert not scene

    error = _meta(tmp_path, uuid="err", tag="-GROUP")
    Client.reset(RuntimeError("search failed"))
    _, scene, _ = asyncio.run(manager.is_scene(error.filename, error, lower=True))
    assert not scene
...
def test_predb_success_group_no_group_and_all_failures(tmp_path: Path) -> None:
    ...
    assert asyncio.run(SceneManager({"DEFAULT": {}}).predb_check(meta, "Movie.Release-GROUP"))
    ...
    assert not asyncio.run(SceneManager({"DEFAULT": {}}).predb_check(_meta(tmp_path), "Movie.Release-GROUP"))

    Client.reset(Response(status=500, text=""))
    assert not asyncio.run(SceneManager({"DEFAULT": {}}).predb_check(_meta(tmp_path), "Movie.Release-GROUP"))

    Client.reset(httpx.RequestError("offline", request=httpx.Request("GET", "https://predb.pw")))
    assert not asyncio.run(SceneManager({"DEFAULT": {}}).predb_check(_meta(tmp_path), "Movie.Release-GROUP"))
...
def test_invalid_details_cache_and_lower_nfo_exception(tmp_path: Path) -> None:
    ...
    details_dir.mkdir(parents=True)
    (details_dir / "Movie.Release-GROUP.json").write_text("bad-json", encoding="utf-8")
    Client.reset(
