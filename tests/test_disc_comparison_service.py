...
def _meta(tmp_path: Path, summary: str = "", extended: str = "") -> Meta:
    ...
    if extended:
        (state / "BD_SUMMARY_EXT_00.txt").write_text(
...
def test_load_format_and_content_helpers(tmp_path: Path) -> None:
    ...
    assert comparison.load_bdinfo_file(
        Meta(base_dir=str(tmp_path), uuid="missing")
    ) == ("", "")
    ...
    assert (
        comparison.has_bdinfo_content({"description": "Disc Label: Movie"})
        == "Disc Label: Movie"
    )
    assert comparison.has_bdinfo_content({"description": "plain"}) == ""


def test_get_relevant_lines_summary_extended_and_full_modes(
    tmp_path: Path,
) -> None:
    ...
    source, target = comparison.get_relevant_lines(
        meta, "Disc Title: Movie\nAudio: DTS / 1500 kbps"
    )
    normalized_summary, normalized_extended, _ = (
        comparison.remove_playlist_variations(summary, extended, "")
    )
    assert source == comparison.normalize_and_filter(normalized_summary)
    assert target == ["Audio: DTS / 1500 kbps"]

    source, target = comparison.get_relevant_lines(
        meta, "PLAYLIST REPORT:\nAudio: DTS / 1500 kbps"
    )
...
def test_compare_bdinfo_differences_matches_and_missing_content(
    tmp_path: Path,
) -> None:
    ...
    warning, result = comparison.compare_bdinfo(
        meta, {"name": "Same", "bd_info": source}, "OTHER"
    )
    assert "No differences found" in warning
    assert "⚠" in result

    warning, result = comparison.compare_bdinfo(
        meta, {"name": "Missing"}, "AITHER"
    )
...
def test_warning_and_sort_priorities() -> None:
    ...
    assert "No differences" in comparison.generate_warning(
    ...
    assert (
        comparison.sorting_priority({"content": "Presentation Graphics"})[0]
        == 2
    )
