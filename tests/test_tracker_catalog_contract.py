...
def test_tracker_catalog_preserves_auth_and_category_capabilities() -> None:
    for tracker_name, tracker_class in tracker_class_map.items():
        ...
        assert definition.auth_type == str(
            getattr(tracker_class, "auth_type", "") or ""
        )
        assert definition.supported_categories == tuple(
            str(value).upper()
            for value in (
                getattr(tracker_class, "supported_categories", ()) or ()
            )
        )
        assert definition.is_usenet is bool(
            getattr(tracker_class, "is_usenet", False)
        )
