...
def test_add_language_section_missing_existing_and_insert() -> None:
    manager = LanguagesManager()
    assert (
        manager._add_language_to_audio_section(
            "General\nFormat : Matroska\n", "English"
        )
        == "General\nFormat : Matroska\n"
    )
    ...
    assert (
        manager._add_language_to_audio_section(existing, "English") == existing
    )
    missing = "General\n\nAudio #1\nFormat : AAC\n\nText\nLanguage : English\n"
...
def test_confirmed_single_audio_language_all_guards(tmp_path: Path) -> None:
    ...
    assert not asyncio.run(
        manager.apply_confirmed_single_audio_language(
            _meta(tmp_path, category="BOOK", manual_language="en")
        )
    )
    assert not asyncio.run(
        manager.apply_confirmed_single_audio_language(
            _meta(tmp_path, is_disc="BDMV", manual_language="en")
        )
    )
    assert not asyncio.run(
        manager.apply_confirmed_single_audio_language(
            _meta(tmp_path, manual_language="und")
        )
    )
    assert not asyncio.run(
        manager.apply_confirmed_single_audio_language(
            _meta(tmp_path, manual_language="en", dual_audio=True)
        )
    )
    assert not asyncio.run(
        manager.apply_confirmed_single_audio_language(
    ...
    assert not asyncio.run(
        manager.apply_confirmed_single_audio_language(
            _meta(
                ...
                mediainfo={
                    "media": {
                        "track": [{"@type": "Audio", "Language": "French"}]
                    }
                },
            ),
        )
...
def test_confirmed_language_writes_only_existing_files(tmp_path: Path) -> None:
    ...
    meta = _meta(
        ...
        manual_language="en",
        mediainfo={
            "media": {"track": [{"@type": "Audio", "Language": "unknown"}]}
        },
    )
    ...
    assert "English" in (directory / "MEDIAINFO.txt").read_text(
        encoding="utf-8"
    )
    assert not (directory / "MediaInfo.json").exists()


def test_parse_bluray_missing_error_and_rich_summary(
...
def test_parsed_mediainfo_sections_missing_and_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    meta = _meta(tmp_path)
...
def test_process_non_video_and_already_checked(tmp_path: Path) -> None:
    ...
    assert (
        meta.language_checked
        and meta.audio_languages == []
        and meta.subtitle_languages == []
    )
...
def test_process_file_languages_from_tracks_and_titles(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    meta = _meta(tmp_path)
...
def test_process_file_missing_audio_attended_prompt_and_cancel(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    monkeypatch.setattr(
        manager,
        "parsed_mediainfo",
        AsyncMock(return_value={"audio": [{"title": "Unknown"}], "text": []}),
    )
    monkeypatch.setattr(
        language_adapter.cli_ui,
        "ask_string",
        lambda *_args, **_kwargs: "English, French",
    )
    meta = _meta(tmp_path, unattended=False)
    asyncio.run(manager.process_desc_language(meta, "TEST"))
    assert (
        meta.audio_languages == ["English", "French"]
        and meta.write_audio_languages
    )

    cleanup = AsyncMock()
    monkeypatch.setattr(language_adapter.cleanup_manager, "cleanup", cleanup)
    monkeypatch.setattr(
        language_adapter.cleanup_manager, "reset_terminal", lambda: None
    )
    monkeypatch.setattr(
        language_adapter.cli_ui,
        "ask_string",
...
def test_process_file_unattended_missing_audio_subtitles_and_hardcoded(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    monkeypatch.setattr(
        manager,
        "parsed_mediainfo",
        AsyncMock(return_value={"audio": [{}], "text": [{}]}),
    )
    ...
    assert (
        hardcoded.subtitle_languages == ["English"]
        and hardcoded.write_hc_languages
    )
...
def test_process_file_subtitle_and_hardcoded_prompts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    monkeypatch.setattr(
    ...
    asyncio.run(manager.process_desc_language(meta, "TEST"))
    assert (
        meta.subtitle_languages == ["Portuguese", "French"]
        and meta.write_subtitle_languages
    )
    ...
    monkeypatch.setattr(
        ...
        "parsed_mediainfo",
        AsyncMock(
            return_value={"audio": [{"language": "English"}], "text": []}
        ),
    )
    monkeypatch.setattr(
        language_adapter.cli_ui,
        "ask_string",
        lambda *_args, **_kwargs: "Spanish",
    )
    hardcoded = _meta(
        tmp_path, uuid="hc-prompt", hardcoded_subs=True, unattended=False
    )
    asyncio.run(manager.process_desc_language(hardcoded, "TEST"))
    assert (
        hardcoded.subtitle_languages == ["Spanish"]
        and hardcoded.write_hc_languages
    )


def test_process_bluray_commentary_mapping_string_and_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        manager,
        "parse_blu_ray",
        AsyncMock(side_effect=RuntimeError("bad bdinfo")),
    )
...
def test_remaining_language_adapter_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = LanguagesManager()
    ...
    directory = _release_dir(meta)
    (directory / "BD_SUMMARY_00.txt").write_text(
        "Disc Title: Test\n\nAudio: English / AAC\n", encoding="utf-8"
    )
    assert (
        asyncio.run(manager.parse_blu_ray(meta))["disc_info"]["disc_title"]
        == "Test"
    )
    ...
    assert (
        asyncio.run(manager.parsed_mediainfo(regular))["general"]["format"]
        == "Matroska"
    )
    ...
    monkeypatch.setattr(
        language_adapter.cli_ui, "ask_string", lambda *_args, **_kwargs: ""
    )
    blank_audio = _meta(tmp_path, uuid="blank-audio", unattended=False)
    asyncio.run(manager.process_desc_language(blank_audio, "TEST"))
    assert (
        blank_audio.unattended_audio_skip
        and blank_audio.tracker_status["TEST"]["skip_upload"]
    )
    ...
    assert (
        blank_sub.unattended_subtitle_skip
        and blank_sub.tracker_status["TEST"]["skip_upload"]
    )
    ...
    hardcoded_cancel = _meta(
        tmp_path, uuid="hc-cancel", hardcoded_subs=True, unattended=False
    )
    with pytest.raises(OperationAbortedError, match="cancelled"):
    ...
    hardcoded_blank = _meta(
        tmp_path, uuid="hc-blank", hardcoded_subs=True, unattended=False
    )
    asyncio.run(manager.process_desc_language(hardcoded_blank, "TEST"))
    assert (
        hardcoded_blank.unattended_subtitle_skip
        and hardcoded_blank.tracker_status["TEST"]["skip_upload"]
    )
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value={
                "audio": [],
                "subtitles": [{"language": "English"}, "bad"],
            }
        ),
    )
    ...
    monkeypatch.setattr(
        manager,
        "parse_blu_ray",
        AsyncMock(side_effect=OperationAbortedError("cancelled")),
    )
    with pytest.raises(OperationAbortedError, match="cancelled"):
        asyncio.run(
            manager.process_desc_language(
                _meta(tmp_path, uuid="bd-cancel", is_disc="BDMV")
            )
        )
    ...
    monkeypatch.setattr(
        language_adapter.langcodes, "find", lambda _word: InvalidLanguage()
    )
