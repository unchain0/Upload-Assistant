...
def _config(*, announce_url: object = "https://nexus.invalid/announce", anon: bool = False) -> dict[str, Any]:
    return {
...
def _response(text: str = "", *, status: int = 200, url: str = "https://nexus.invalid/torrents.php") -> httpx.Response:
...
async def test_nexus_search_requires_announce_and_cookie_login() -> None:
    ...
    tracker.session.get = AsyncMock(return_value=_response("login.php", url="https://nexus.invalid/login.php"))  # type: ignore[method-assign]
    meta = _meta()
...
def test_nexus_search_params_and_names() -> None:
    ...
    assert tracker._search_name(_meta(category="TV", title="Show", tv_pack=True, season="S02")) == "Show S02"
    assert tracker._search_name(_meta(category="TV", title="Show", tv_pack=False, season="S02", episode="E03")) == "Show S02E03"
    assert tracker._search_name(_meta(category="TV", title="Show", tv_pack=False, season="", episode="")) == "Show"
...
def test_nexus_search_html_helpers() -> None:
    ...
    list_link = BeautifulSoup('<a href="details.php?id=7" title="Release"></a>', "html.parser").find("a")
    ...
    bad_link = BeautifulSoup('<a href="bad.php">bad</a>', "html.parser").find("a")
    assert bad_link is not None
    assert NEXUSPHP._torrent_id_from_link(bad_link) == ""


@pytest.mark.asyncio
async def test_nexus_parse_search_results_with_bdinfo(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    html = """
    <table class='torrents'>
      <tr><th>header</th></tr>
    ...
    assert results == [{"name": "Release", "link": "https://nexus.invalid/details.php?id=77", "bd_info": "BDINFO"}]

    monkeypatch.setattr(tracker, "get_dupe_bdinfo", AsyncMock(return_value=""))
    row = NEXUSPHP._search_rows(html)[0]
    assert await tracker._search_row(_meta(is_disc="BDMV"), row) == {"name": "Release", "link": "https://nexus.invalid/details.php?id=77"}


@pytest.mark.asyncio
async def test_nexus_search_existing_success(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    html = """
    <table class='torrents'>
      <tr><th>header</th></tr>
    ...
    assert await tracker.search_existing(_meta()) == [{"name": "Release", "link": "https://nexus.invalid/details.php?id=77"}]


@pytest.mark.asyncio
async def test_nexus_get_dupe_bdinfo_success_empty_and_error() -> None:
    tracker = _tracker()
    success_html = '<div class="nexus-media-info-raw"><pre>DISC INFO</pre></div>'
    tracker.session.get = AsyncMock(return_value=_response(success_html, url="https://nexus.invalid/details.php?id=1"))  # type: ignore[method-assign]
    ...
    tracker.session.get = AsyncMock(side_effect=httpx.RequestError("offline", request=httpx.Request("GET", "https://nexus.invalid")))  # type: ignore[method-assign]
...
def _tmdb_movie_data() -> dict[str, Any]:
    return {
        ...
        "credits": {
            "crew": [
                {"job": "Director", "name": "导演", "original_name": "Director"},
                {"job": "Writer", "name": "编剧", "original_name": "Writer"},
            ],
            "cast": [
                {"name": "演员一", "original_name": "Actor One", "character": "Hero"},
...
async def test_nexus_standard_desc_movie_and_empty() -> None:
    ...
    text = await tracker.standard_desc(
        _meta(
            imdb_info={"rating": "8.5", "votes": "1000", "imdb_url": "https://imdb.invalid/title/tt1"},
            douban_rating="9.0",
            douban_votes="2000",
            douban_id=123,
        )
...
async def test_nexus_standard_desc_tv_runtime_branches() -> None:
    ...
    tracker.tmdb_data = {
        ...
        "seasons": [{"season_number": "S01", "name": "第一季", "episode_count": 10}],
        "episode_run_time": [45],
        "credits": {},
    }
    text = await tracker.standard_desc(_meta(category="TV", season="S01", year=2020))
...
def test_nexus_standard_desc_helpers_remaining_branches() -> None:
    ...
    assert lines == []
    assert NEXUSPHP._localized_title(_meta(category="MOVIE"), {"name": "Movie"}) == "Movie"
    assert NEXUSPHP._season_title("Show 第 S01 季", "S01", []) == "Show 第 S01 季"
    ...
    assert lines == []
    NEXUSPHP._append_tv_runtime(lines, _meta(category="TV", season=""), {"episode_run_time": []})
    assert lines == ["◎季　　数　"]

    assert NEXUSPHP._crew_names(["bad", {"job": "Director", "name": "D", "original_name": "DO"}], {"Director"}) == ["D DO"]
...
@pytest.mark.asyncio
async def test_nexus_get_description_uses_builder(monkeypatch: pytest.MonkeyPatch) -> None:
    class Builder:
        def __init__(self, *_args: object, **_kwargs: object) -> None:
            pass

        async def general_description_generator(self, meta: Meta, **kwargs: object) -> str:
    ...
    monkeypatch.setattr(tracker, "standard_desc", AsyncMock(return_value="localized"))
...
def test_nexus_base_mapping_methods_and_urls() -> None:
    ...
    assert base.get_douban_url(_meta(douban_id=123)) == "https://movie.douban.com/subject/123/"
    assert base.get_douban_url(_meta(douban_id=0)) == ""
    assert base.get_imdb_url(_meta(imdb_id=1, imdb_info={"imdb_url": "https://imdb.invalid"})) == "https://imdb.invalid"
...
async def test_nexus_technical_info_and_data_wrappers(tmp_path: Path) -> None:
    ...
    assert await tracker.get_technical_info(_meta(tmp_path)) == {"technical_info": "MI"}
    assert await tracker.get_technical_info(_meta(tmp_path, is_disc="BDMV")) == {"technical_info": "BD"}
    assert await tracker.get_name(_meta()) == {"name": "Example Movie 2024 1080p"}
    ...
    assert await tracker.get_resolution_data(_meta()) == {"standard_sel[4]": 404}
    assert await tracker.get_group_tag_data(_meta()) == {"team_sel[4]": 505}
    assert await tracker.get_checkboxes_data(_meta()) == {"tags[4][]": ["tag-a", "tag-b"]}
    assert await tracker.get_anonymous_data(_meta()) == {"uplver": "yes"}
    assert await tracker.get_imdb_data(_meta(imdb_id=1, imdb_info={"imdb_url": "https://imdb.invalid"})) == {"url": "https://imdb.invalid"}
    assert await tracker.get_douban_data(_meta(douban_id=2)) == {"pt_gen": "https://movie.douban.com/subject/2/"}
    assert await tracker.get_audio_codec_data(_meta()) == {"audiocodec_sel[4]": 606}
    assert await tracker.get_region_data(_meta()) == {"source_sel[4]": 707}
    assert await tracker.get_container_data(_meta()) == {"processing_sel[4]": 808}
...
@pytest.mark.asyncio
async def test_nexus_get_data_merges_all_fields(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    root = tmp_path / "tmp" / "release"
    root.mkdir(parents=True)
    (root / "MEDIAINFO_CLEANPATH.txt").write_text("MI", encoding="utf-8")
    monkeypatch.setattr(tracker, "get_description", AsyncMock(return_value={"descr": "desc"}))
...
@pytest.mark.asyncio
async def test_nexus_upload_delegates_cookie_uploader(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    tracker.cookie_validator.load_session_cookies = AsyncMock(return_value=httpx.Cookies({"sid": "1"}))  # type: ignore[method-assign]
    monkeypatch.setattr(tracker, "get_data", AsyncMock(return_value={"type": 101}))
