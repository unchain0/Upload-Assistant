...
def test_matrix_keeps_domain_and_engines_independent_of_effectful_layers() -> (
    None
):
    assert ALLOWED_IMPORTS["domain_models"] == frozenset({"domain_models"})
    assert ALLOWED_IMPORTS["engines"] == frozenset(
        {"domain_models", "engines"}
    )
    assert "integrations" in ALLOWED_IMPORTS["services"]
    assert ALLOWED_IMPORTS["integrations"] == frozenset(
        {"domain_models", "integrations"}
    )
...
def test_checker_source_is_parseable_without_importing_application() -> None:
    path = (
        ...
        / "scripts"
        / "check_masa_architecture.py"
    )
    ast.parse(path.read_text(encoding="utf-8"), filename=str(path))


def test_checker_rejects_process_termination_outside_delivery(
    tmp_path: Path,
) -> None:
...
def test_checker_allows_standalone_tool_exit_only_under_main_guard(
    tmp_path: Path,
) -> None:
    source_root = tmp_path / "src"
