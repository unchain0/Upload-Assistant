...
async def test_existing_local_mkbrr_is_used_on_unsupported_platform(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.system", lambda: "Android")
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.machine", lambda: "aarch64")
...
def test_find_existing_binary_uses_docker_platform_layout(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    binary_path.write_text("#!/bin/sh\n", encoding="utf-8")
    binary_path.chmod(binary_path.stat().st_mode | stat.S_IXUSR)
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.system", lambda: "Linux")
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.machine", lambda: "x86_64")

    assert MkbrrBinaryManager.find_existing_binary(tmp_path) == str(binary_path)
...
def test_find_existing_binary_version_checks_managed_platform_cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.system", lambda: "Linux")
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.machine", lambda: "x86_64")
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.shutil.which", lambda _: None)

    assert MkbrrBinaryManager.find_existing_binary(tmp_path, "v1.24.0") is None

    (binary_path.parent / "v1.24.0").touch()
    assert MkbrrBinaryManager.find_existing_binary(tmp_path, "v1.24.0") == str(binary_path)
...
def test_find_existing_binary_uses_freebsd_platform_layout(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    binary_path.write_text("#!/bin/sh\n", encoding="utf-8")
    binary_path.chmod(binary_path.stat().st_mode | stat.S_IXUSR)
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.system", lambda: "FreeBSD")
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.machine", lambda: "amd64")
...
async def test_ensure_mkbrr_binary_rejects_unsupported_platforms(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, system: str, machine: str) -> None:
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.system", lambda: system)
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.platform.machine", lambda: machine)
    monkeypatch.setattr("src.integrations.runtime_tools.mkbrr.shutil.which", lambda _: None)
