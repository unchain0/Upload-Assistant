...
def test_temp_root_rejects_writable_directory_owned_by_another_user(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("pathlib.Path.chmod", lambda _path, _mode: (_ for _ in ()).throw(PermissionError("not owner")))
    monkeypatch.setattr("pathlib.Path.stat", lambda _path: SimpleNamespace(st_uid=12345, st_mode=stat.S_IFDIR | 0o1777))
...
def test_temp_root_accepts_root_owned_sticky_shared_directory(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("pathlib.Path.chmod", lambda _path, _mode: (_ for _ in ()).throw(PermissionError("not owner")))
    monkeypatch.setattr("pathlib.Path.stat", lambda _path: SimpleNamespace(st_uid=0, st_mode=stat.S_IFDIR | 0o1777))
...
def test_release_temp_dir_uses_safe_pending_id_when_release_id_is_empty(tmp_path: Path) -> None:
    assert release_temp_dir(tmp_path, "") == tmp_path / "tmp" / "release-pending"


@pytest.mark.parametrize("release_id", ["..", "../escape", r"..\\escape", "/absolute", r"C:\\escape"])
def test_release_temp_dir_rejects_unsafe_release_id(tmp_path: Path, release_id: str) -> None:
...
def test_music_release_snapshot_uses_safe_pending_id_when_release_id_is_empty(tmp_path: Path) -> None:
    assert music_release_snapshot_path(tmp_path, "") == tmp_path / "tmp" / "music-release-pending" / "music_release.json"


def test_music_release_snapshot_uses_state_dir_when_base_dir_is_empty(tmp_path: Path, monkeypatch) -> None:
    state_dir = tmp_path / "state"
    monkeypatch.setattr("src.integrations.filesystem.temp_paths.STATE_DIR", state_dir)
