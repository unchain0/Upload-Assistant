...
def test_digitalcore_search_payload_empty_and_matching_entry() -> None:
    ...
    entry = DigitalCore._dupe_entry(
        {"category": 6, "id": 123, "name": "Release", "size": 42, "numfiles": 2},
        6,
    )
...
def test_digitalcore_firstpic_uses_hosted_artwork() -> None:
    meta = Meta(category="BOOK", hosted_artwork=[{"raw_url": "https://example.com/cover.jpg"}])
    assert asyncio.run(_tracker().get_firstpic(meta)) == "https://example.com/cover.jpg"


@pytest.mark.asyncio
async def test_digitalcore_upload_release_success(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_digitalcore_upload_release_http_status_error(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_digitalcore_upload_release_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_submit_upload", AsyncMock(side_effect=httpx.TimeoutException("timeout")))
    status: dict[str, Any] = {}
...
@pytest.mark.asyncio
async def test_digitalcore_upload_release_request_error(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_digitalcore_submit_upload_posts_torrent(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    assert files["file"] == ("Release.torrent", b"torrent", "application/x-bittorrent")
