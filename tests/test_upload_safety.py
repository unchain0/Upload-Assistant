...
def test_single_file_with_spaces_is_blocked() -> None:
    ...
    assert content_paths_with_spaces(meta) == ["To Philly with Love 2026 1080p WEB-DL BONE.mkv"]
...
def test_allow_spaces_cli_option_sets_explicit_override() -> None:
    meta, _parser, _unused = Args({"DEFAULT": {"screens": 3}}).parse(["--allow-spaces", "Movie With Spaces.mkv"], Meta())
...
def test_global_guard_skips_every_tracker_before_processing() -> None:
    ...
    meta = Meta(path=path, filelist=[path], trackers=["LUMINARR", "ZENITH"], name="To Philly with Love", unattended=True)
    ...
    assert all(status["skipped"] is True and status["upload"] is False for status in meta.tracker_status.values())
    assert all("--allow-spaces" in status["skip_reason"] for status in meta.tracker_status.values())
