...
def _response(text: str, *, url: str = "https://immortalseed.me/browse.php") -> httpx.Response:
...
async def test_immortalseed_search_unsupported_and_login_failure() -> None:
    ...
    tracker.session.get = AsyncMock(return_value=_response("login.php", url="https://immortalseed.me/login.php"))  # type: ignore[method-assign]
...
def test_immortalseed_search_entries_guard_and_success() -> None:
    ...
    """
    assert ImmortalSeed._search_entries(html) == [{"name": "Release", "size": "1.5 GB", "link": "details.php?id=7"}]


def test_immortalseed_special_categories() -> None:
    tracker = _tracker()
    assert tracker.get_category_id(_meta(category="MOVIE", genres=["Documentary"], sd=0)) == 54
    assert tracker.get_category_id(_meta(category="TV", genres=["Documentary"], sd=1)) == 53
    assert tracker.get_category_id(_meta(category="TV", anime=True)) == 32
    assert tracker.get_category_id(_meta(category="TV", keywords=["cartoon"])) == 31


@pytest.mark.asyncio
async def test_immortalseed_existing_nfo_and_hosted_cover(tmp_path: Path) -> None:
    tracker = _tracker()
    ...
    assert await tracker.get_nfo(meta) == {"nfofile": ("release.nfo", b"nfo", "application/octet-stream")}
