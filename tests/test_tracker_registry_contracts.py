...
class _Tracker:
    ...
    tracker_urls: ClassVar[list[str]] = [
        "https://legacy.tracker.invalid/torrents"
    ]
    ...
    async def get_type_id(
        self, _meta: Meta, mapping_only: bool = False
    ) -> object:
...
def _request_error() -> httpx.RequestError:
    return httpx.RequestError(
        "offline", request=httpx.Request("GET", "https://tracker.invalid")
    )
...
async def test_banned_group_catalog_supports_lists_pagination_cache_and_errors(
    ...
    _Client.reset(
        _Response(
            200, {"data": [{"name": "ONE"}], "meta": {"next_cursor": "next"}}
        ),
        _Response(
            200, {"data": [{"name": "TWO"}], "meta": {"next_cursor": None}}
        ),
    )
...
async def test_banned_group_checks_cover_dynamic_files_notes_and_prompt_paths(
    ...
    assert (
        await setup.check_banned_group(
            "TEST", ["GROUP"], _meta(tmp_path, tag="")
        )
        is False
    )
    assert (
        await setup.check_banned_group(
            "TEST", ["TAOE"], _meta(tmp_path, tag="-TAoE", unattended=True)
        )
        is True
    )
    assert (
    ...
    monkeypatch.setattr(
        setup, "get_banned_groups", AsyncMock(return_value="empty")
    )
    assert await setup.check_banned_group("LUMINARR", [], dynamic) is False
    monkeypatch.setattr(
        setup, "get_banned_groups", AsyncMock(return_value=None)
    )
    assert await setup.check_banned_group("LUMINARR", [], dynamic) is False

    banned_file = tmp_path / "banned.json"
    banned_file.write_text(
        json.dumps({"banned_groups": "GROUP, OTHER"}), encoding="utf-8"
    )
    monkeypatch.setattr(
        setup, "get_banned_groups", AsyncMock(return_value=banned_file)
    )
    ...
    monkeypatch.setattr(
        registry.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: True
    )
    assert (
        await setup.check_banned_group("TEST", ["GROUP"], interactive) is False
    )
    monkeypatch.setattr(
        registry.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: False
    )
    assert (
        await setup.check_banned_group("TEST", ["GROUP"], interactive) is True
    )
    monkeypatch.setattr(
        registry.cli_ui,
        "ask_yes_no",
    ...
    monkeypatch.setattr(
        registry.cleanup_manager, "reset_terminal", lambda: None
    )
    assert (
        await setup.check_banned_group("TEST", ["GROUP"], interactive) is True
    )
...
async def test_claim_catalog_writes_fetches_and_matches(
    ...
    await setup.write_internal_claims_to_file(
        claims_file, [{"invalid": True}, valid]
    )
    assert claims_file.is_file()
    assert await setup.check_tracker_claims(meta, "TEST") is True
    assert (
        await setup.check_tracker_claims(
            _meta(tmp_path, tmdb=999), ["TEST", "MISSING"]
        )
        is False
    )
...
async def test_request_endpoints_parse_supported_payloads_and_fail_safely(
    ...
    assert (
        await setup.get_tracker_requests(meta, "TEST", _Tracker.requests_url)
    )[0]["name"] == "Request"
    hawke = {"id": 2, "attributes": item}
    _Client.reset(_Response(200, {"results": [hawke]}))
    assert (
        await setup.get_tracker_requests(
            meta, "HAWKEUNO", _Tracker.requests_url
        )
    )[0]["id"] == 2
    assert (
        await setup.get_tracker_requests(
            _meta(tmp_path, tmdb=None), "TEST", _Tracker.requests_url
        )
        == []
    )
    ...
    for response in (
        ...
        assert (
            await setup.get_tracker_requests(
                meta, "TEST", _Tracker.requests_url
            )
            == []
        )
    for error in (
    ...
    assert (
        await setup.bhd_request_check(
            meta, "BEYONDHD", "https://bhd.invalid/api"
        )
    )[0]["name"] == "BHD"
...
async def test_tracker_request_logs_movie_tv_beyondhd_and_custom_matches(
    ...
    monkeypatch.setattr(
        setup, "get_tracker_requests", AsyncMock(return_value=[base_request])
    )
    assert await setup.tracker_request(_meta(tmp_path), "TEST") is True
    log = tmp_path / "tmp" / "TEST_request_results.json"
    assert (
        json.loads(log.read_text(encoding="utf-8"))[0]["match_type"]
        if False
        else log.is_file()
    )
    ...
    monkeypatch.setattr(
        setup, "get_tracker_requests", AsyncMock(return_value=[tv_request])
    )
    assert (
        await setup.tracker_request(
            _meta(tmp_path, category="TV"), ["TEST", "MISSING"]
        )
        is True
    )
    ...
    monkeypatch.setattr(
        setup, "bhd_request_check", AsyncMock(return_value=[bhd])
    )
    assert (
        await setup.tracker_request(
            _meta(tmp_path, resolution="2160p", HDR=""), "BEYONDHD"
        )
        is True
    )

    monkeypatch.setattr(
        _Tracker, "get_requests", AsyncMock(return_value=[{"id": 1}])
    )
...
async def test_trump_search_processing_and_report_creation_cover_all_modes(
    ...
    requests, status = await setup.get_tracker_trumps(
        "TEST", _Tracker.trumping_url, "7"
    )
    assert status == 200 and requests[0]["trumping_torrent"] == [
        {"id": 8, "name": "Replacement"}
    ]
    _Client.reset(_Response(500, {}, "fail"))
    assert await setup.get_tracker_trumps(
        "TEST", _Tracker.trumping_url, "7"
    ) == ([], 500)
    _Client.reset(_request_error())
    assert (
        await setup.get_tracker_trumps("TEST", _Tracker.trumping_url, "7")
    )[0] == []

    meta = _meta(tmp_path)
    meta["TEST_trumpable_id"] = "7"
    normalized_report = dict(
        report, trumping_torrent=[{"id": 8, "name": "Replacement"}]
    )
    monkeypatch.setattr(
        setup,
        "get_tracker_trumps",
    ...
    monkeypatch.setattr(
        registry.cli_ui, "ask_string", lambda *_args, **_kwargs: "d"
    )
    ...
    answers: Iterator[str] = iter(
        ("L", "https://reported/1, https://reported/2", "https://trumping/1")
    )
    monkeypatch.setattr(
        registry.cli_ui, "ask_string", lambda *_args, **_kwargs: next(answers)
    )
    assert await setup.process_trumpables(meta, "TEST") is True
    ...
    meta = _meta(
        tmp_path, debug=True, tracker_status={"TEST": {"torrent_id": 9}}
    )
    ...
    lst = _meta(
        tmp_path, debug=True, tracker_status={"LST": {"torrent_id": 9}}
    )
    lst["LST_reported_torrent_id"] = "7"
    monkeypatch.setattr(
        registry.cli_ui, "ask_string", lambda *_args, **_kwargs: "reason"
    )
    ...
    assert (
        await setup.make_trumpable_report(_meta(tmp_path), "MISSING") is False
    )


def test_comment_host_catalog_normalizes_class_and_runtime_urls(
    config: dict[str, Any], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        registry,
...
def test_registry_configuration_helper_edges(
    ...
    monkeypatch.setattr(
        registry, "example_config", {"TRACKERS": {"TEST": "bad"}}
    )
    ...
    assert registry._as_comment_values("https://tracker.invalid") == [
...
async def test_registry_request_helper_edges(
    ...
    assert (
        await setup._request_context(meta, "NOEND", _NoEndpoints(setup.config))
        is None
    )

    values = await setup._mapped_id(
        _Tracker(setup.config).get_type_id, meta, "UNKNOWN", "Type"
    )
    assert values == [None]
    ...
    setup._log_unit3d_request(
        meta, "TEST", partial, "https://tracker.invalid/requests/1", "partial"
    )
    entry = setup._request_log_entry(
        meta, partial, "https://tracker.invalid/requests/1", "partial"
    )
...
def test_registry_beyondhd_matching_edges(
    tmp_path: Path, setup: registry.TrackerSetup
) -> None:
    meta = _meta(tmp_path)
    assert setup._beyondhd_resolution_matches(
        _meta(tmp_path, is_disc="BDMV", resolution="1080p"), "BD 50"
    )
    assert setup._beyondhd_resolution_matches(meta, "1080p")
    assert not setup._beyondhd_remux_resolution(
        _meta(tmp_path, type="ENCODE"), "uhd remux"
    )
    assert setup._beyondhd_disc_resolution(
        _meta(tmp_path, resolution="2160p"), "uhd disc"
    )
    assert setup._beyondhd_type_matches(
        _meta(tmp_path, type="ENCODE"), "Blu-ray", "1080p"
    )
    assert setup._beyondhd_type_matches(
        _meta(tmp_path, type="WEBDL"), "WEB", "1080p"
    )
    ...
    assert (
        setup._beyondhd_hdr_match_kind({"dv": False, "hdr": False})
        == "hdr_mismatch"
    )
    assert (
        setup._beyondhd_hdr_match_kind({"dv": True, "hdr": False}) == "partial"
    )
    ...
    data: list[registry.JsonDict] = []
    setup._append_beyondhd_request_log(
        meta, data, existing, request, {"claimed_status": ""}
    )
    assert data == []
...
async def test_registry_trumpable_helper_edges(
    ...
    monkeypatch.setattr(
        setup, "get_tracker_trumps", AsyncMock(return_value=([], 500))
    )
    assert not await setup._existing_trump_reports_allow_upload(
        meta, "TEST", _Tracker.trumping_url, "7"
    )
    assert "TEST" in meta.skip_upload_trackers

    meta.skip_upload_trackers = []
    monkeypatch.setattr(
        setup, "get_tracker_trumps", AsyncMock(return_value=([], 200))
    )
    assert await setup._existing_trump_reports_allow_upload(
        meta, "TEST", _Tracker.trumping_url, "7"
    )

    setup._log_trump_report(
        {"id": 1, "title": "Pending", "trumping_torrent": []}
    )
    monkeypatch.setattr(
        registry.cli_ui,
        "ask_yes_no",
    ...
    assert not await setup._collect_trump_comparisons(
        _meta(tmp_path, tv_pack=False), "TEST"
    )
...
async def test_registry_trump_fetch_edge_cases(
    ...
    assert await setup._fetch_trump_pages(
        "TEST", _Tracker.trumping_url, {}, {}
    ) == ([], None)
    monkeypatch.setattr(
        setup, "_trump_page_loop", AsyncMock(side_effect=RuntimeError("bad"))
    )
    assert await setup._fetch_trump_pages(
        "TEST", _Tracker.trumping_url, {}, {}
    ) == ([], None)
    ...
    assert setup._normalized_trumping_torrents([{"id": 1}, "bad"]) == [
        {"id": 1}
    ]
...
async def test_registry_trump_report_context_payload_and_post_edges(
    ...
    assert not await setup._post_trump_report(
        "TEST", "https://tracker.invalid", {}
    )
    monkeypatch.setattr(
        registry.httpx,
        "AsyncClient",
...
def test_registry_last_uncovered_helper_edges(
    ...
    monkeypatch.setattr(
        ...
        "_trash_spec_value",
        classmethod(
            lambda _cls, _spec: (_ for _ in ()).throw(ValueError("bad"))
        ),
    )
    assert setup._trash_spec_groups({}) == []
    ...
    assert (
        __import__("asyncio").run(
            setup._process_tracker_request(_meta(tmp_path), "NOEND")
        )
        is False
    )
    assert (
        __import__("asyncio").run(
            setup._request_context(_meta(tmp_path), "BEYONDHD", noend)
        )
        is None
    )
