...
def test_yuscene_translates_non_english_book_metadata_before_upload():
    ...
    assert (
        meta.name
        == "Kenji Miyazawa - Complete Collection of Children's Stories 2016 JAPANESE AUDIOBOOK"
    )
...
def test_yuscene_accepts_verified_english_book_meta_166519():
    meta = _book_meta(
        ...
        book_overview=(
            "This complete audiobook collection presents the celebrated children's stories written by Kenji Miyazawa. Original Japanese title: 宮沢賢治童話全集."
        ),
    )
    ...
    assert (
        asyncio.run(
            _tracker().get_additional_checks(
                _movie_meta(
                    description=(
                        "Screens: https://i.imgur.com/example1.jpg\nhttps://www.imdb.com/title/tt1234567/"
                    ),
                )
            )
        )
