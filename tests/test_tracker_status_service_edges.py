...
def test_tracker_status_defensive_mapping_and_missing_game_edges(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(tracker_status_service, "missing_game_fields", lambda _meta: [])
    assert manager._missing_game_field_reason(_meta(tmp_path, category="GAME")) == ""
...
async def test_tracker_status_additional_checks_accept_and_ptp_missing_method(
    ...
    assert await manager._additional_checks_allow("TEST", meta, Accepts(), status)
    assert await manager._ptp_additional_checks(object(), "PASSTHEPOPCORN", meta, status)
...
async def test_tracker_status_store_prepared_meta_repairs_invalid_container(
    ...
    await TrackerStatusManager._store_prepared_meta(meta, "TEST", local, asyncio.Lock())
...
def test_tracker_status_completed_episode_preserves_dupe_result() -> None:
    ...
    result = tracker_status_service._TrackerResult("TEST", status, None, _Tracker)
    assert TrackerStatusManager._block_completed_episode_result(result) is False
