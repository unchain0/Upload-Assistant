...
def test_lst_music_payload_includes_discogs_release_and_master_ids():
    meta = Meta(
        ...
        music_release={
            "external_ids": {
                "discogs_release": "https://www.discogs.com/release/12345-example",
                "discogs_master": "master/67890",
            }
        },
    )

    data = asyncio.run(
        LST({"DEFAULT": {}, "TRACKERS": {"LST": {}}}).get_additional_data(meta)
    )
...
def test_lst_music_payload_omits_discogs_existence_flag_for_invalid_ids():
    meta = Meta(
        category="MUSIC",
        music_release={"external_ids": {"discogs_release": "not-an-id"}},
    )
...
def test_lst_music_name_uses_technical_fields_for_lossless_releases():
    ...
    name = asyncio.run(
        LST({"DEFAULT": {}, "TRACKERS": {"LST": {}}}).get_name(meta)
    )["name"]
...
def test_lst_ebook_name_includes_edition_type_and_isbn():
    ...
    assert (
        name
        == "Liu Cixin - The Three-Body Problem Revised Edition 2008 PDF OCR 9780765377067-GROUP"
    )
...
def test_lst_lossless_audiobook_name_includes_depth_and_sample_rate():
    ...
    assert (
        ...
        == "Narrator - Lossless Book 2025 WEB FLAC 24-bit 48 kHz"
    )
