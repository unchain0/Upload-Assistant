...
def test_unsupported_cache_and_linux_install(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert asyncio.run(nyuu.NyuuBinaryManager.ensure_nyuu_binary(tmp_path)) == str(binary)
    ...
    async def download(_client, _url: str, destination: Path, asset: str) -> None:
...
def test_linux_oversize_and_download_failure_cleanup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_windows_auto_7z_success_and_nonzero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(SevenZipBinaryManager, "ensure_7z_binary", AsyncMock(return_value="7zr.exe"))
    ...
    async def create(*args: object, **_kwargs: object):
        output = next(str(value)[2:] for value in args if str(value).startswith("-o"))
    ...
    monkeypatch.setattr(nyuu.asyncio, "create_subprocess_exec", lambda *_args, **_kwargs: _awaitable(Failed()))
    with pytest.raises(Exception, match="7z extraction failed"):
        asyncio.run(nyuu.NyuuBinaryManager.ensure_nyuu_binary(tmp_path, path_7z="7zr.exe"))
...
def test_windows_extraction_timeout_calls_termination(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    process = Process()
    monkeypatch.setattr(nyuu.asyncio, "create_subprocess_exec", lambda *_args, **_kwargs: _awaitable(process))
    terminated = AsyncMock()
    monkeypatch.setattr(nyuu.NyuuBinaryManager, "_terminate_process_tree", terminated)
...
def test_terminate_process_tree_windows_warning_timeout_oserror_and_posix(monkeypatch: pytest.MonkeyPatch) -> None:
    class Process:
        pid = 42
        returncode = None
    ...
    monkeypatch.setattr(nyuu.asyncio, "create_subprocess_exec", lambda *_args, **_kwargs: _awaitable(TreeKiller()))
    asyncio.run(nyuu.NyuuBinaryManager._terminate_process_tree(process))
    assert process.killed and process.communications == 1

    process = Process()
    monkeypatch.setattr(nyuu.asyncio, "create_subprocess_exec", lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("missing taskkill")))
    asyncio.run(nyuu.NyuuBinaryManager._terminate_process_tree(process))
...
def test_terminate_process_tree_windows_taskkill_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    class Process:
        pid = 77
        returncode = None
    ...
    monkeypatch.setattr(nyuu.platform, "system", lambda: "Windows")
    monkeypatch.setattr(nyuu.asyncio, "create_subprocess_exec", lambda *_args, **_kwargs: _awaitable(tree_killer))
    calls = 0
