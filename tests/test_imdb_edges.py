        ...
        "data": {
            "title": {
                        ...
                        "credits": [
                            {
                                "name": {
                                    "id": "nm1",
                                    "nameText": {"text": "Director"},
                                }
                            },
                            {
                                "name": {
                                    "id": "",
                                    "nameText": {"text": "Ignored"},
                                }
                            },
                        ],
                    },
                    {
                        "category": {"text": "Creators"},
                        "credits": [
                            {
                                "name": {
                                    "id": "nm2",
                                    "nameText": {"text": "Creator"},
                                }
                            }
                        ],
                    },
                    ...
                    "edges": [
                        {
                            "node": {
                                "seconds": 3720,
                                "displayableProperty": {
                                    "value": {"plainText": "Director's Cut"}
                                },
                                "attributes": [
                                    {"text": "extended"},
                                    {"text": ""},
                ...
                "technicalSpecifications": {
                    "soundMixes": {
                        "items": [{"text": "Dolby Digital"}, {"bad": True}]
                    }
                },
            }
        }
...
def test_imdb_info_cache_http_errors_and_empty_title(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    manager = ImdbManager()
    cache = _install(monkeypatch, cache=_Cache({"cached": True}))
    assert asyncio.run(
        manager.get_imdb_info_api(123, base_dir=str(tmp_path))
    ) == {"cached": True}
    ...
    assert (
        asyncio.run(manager.get_imdb_info_api(123, base_dir=str(tmp_path)))
        == {}
    )
    ...
    cache = _install(
        monkeypatch,
        _Response(
            {"errors": [{"message": "missing"}], "data": {"title": None}}
        ),
    )
    assert (
        asyncio.run(manager.get_imdb_info_api(123, base_dir=str(tmp_path)))
...
def test_imdb_info_rich_payload(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    cache = _install(monkeypatch, _Response(_rich_title_payload()))
    result = asyncio.run(
        ImdbManager().get_imdb_info_api(
            1234567, manual_language="pt", base_dir=str(tmp_path)
        )
    )
    ...
    assert (
        result["year"] == 2024
        and result["end_year"] == 2026
        and result["tv_year"] == 2026
    )
    assert result["runtime"] == "60" and result["genres"] == "Drama, Comedy"
    assert result["directors"] == ["Director"] and result["creators"] == [
        "Creator"
    ]
    assert result["writers"] == ["Writer"] and result["stars"] == ["Star"]
    assert (
        result["edition_count"] == 1 and "Director's Cut" in result["editions"]
    )
    assert result["edition_details"]["62"]["attributes"] == ["extended"]
    assert result["akas"][0]["country"] == "BR"
    assert len(result["episodes"]) == 3
    assert result["seasons_summary"] == [
        {"season": 1, "year": 2024, "year_range": "2024-2025"}
    ]
...
def test_imdb_info_empty_country_runtime_episode_and_closest_year(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    result = asyncio.run(
        ImdbManager().get_imdb_info_api("tt1234567", base_dir=str(tmp_path))
    )
    assert result["aka"] == "Example"
    assert result["country"] == "" and result["country_list"] == ""
    assert result["runtime"] == "60" and result["tv_year"] == 2025
    assert result["seasons_summary"] == [
        {"season": 2, "year": 2025, "year_range": "2025"}
    ]

    payload["data"]["title"]["episodes"] = None
    _install(monkeypatch, _Response(payload))
    result = asyncio.run(
        ImdbManager().get_imdb_info_api(1234567, base_dir=str(tmp_path))
    )
    assert (
        result["episodes"] == []
        and result["seasons_summary"] == []
        and result["tv_year"] is None
    )


def test_search_quickie_constraints_type_and_year_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    _install(
        monkeypatch,
        _Response(_search_payload(_edge("tt1111111", "A & B", year=2026))),
    )
    sleep = AsyncMock()
    ...
    assert (
        'start: "2025-01-01"' in query
        and "min: 110" in query
        and "max: 130" in query
    )
    ...
    _install(
        ...
        _Response(
            _search_payload(
                _edge("tt2222222", "Show", title_type="TV Series", year=2026)
            )
        ),
    )
    assert (
        asyncio.run(
            manager.search_imdb("Show", 2026, quickie=True, category="TV")
        )
        == 2222222
    )
    ...
    _install(
        monkeypatch,
        _Response(_search_payload(_edge("tt3333333", "Movie", year=2025))),
    )
    assert (
        asyncio.run(
            manager.search_imdb("Movie", 2026, quickie=True, category="MOVIE")
        )
        == 0
    )
    ...
    _install(
        monkeypatch,
        _Response(_search_payload(_edge("tt4444444", "Movie", year=None))),
    )
    assert (
        asyncio.run(
            manager.search_imdb("Movie", None, quickie=True, category="MOVIE")
        )
        == 4444444
    )
    ...
    _install(
        ...
        _Response(
            _search_payload(
                _edge("tt5555555", "Movie", title_type="TV Series", year=2026)
            )
        ),
    )
    assert (
...
def test_search_single_multiple_similarity_and_unattended(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    _install(
        monkeypatch, _Response(_search_payload(_edge("tt1000001", "Single")))
    )
    assert (
        asyncio.run(manager.search_imdb("Single", 2026, category="MOVIE"))
        == 1000001
    )
    ...
    _install(
        monkeypatch, _Response(_search_payload(close, previous_year, distant))
    )
    assert (
        asyncio.run(manager.search_imdb("Exact Match", 2026, category="MOVIE"))
        == 1000002
    )
    ...
    assert (
        asyncio.run(
            manager.search_imdb(
                "Unrelated", 2026, category="MOVIE", unattended=True
            )
        )
        == 1000005
    )
...
def test_search_fallback_prefix_wide_parsed_reduced_and_further(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    ...
    monkeypatch.setattr(
        imdb, "guessit_fn", lambda *_args, **_kwargs: {"title": "Parsed Input"}
    )
    monkeypatch.setattr(
        imdb,
        "anitopy_parse_fn",
    ...
    assert (
        asyncio.run(
            manager.search_imdb(
                ...
                None,
                category="MOVIE",
                untouched_filename="Parsed Input",
            )
        )
        == 2000003
    )
    ...
    assert (
        asyncio.run(
            manager.search_imdb("Alpha Beta Gamma", None, category="MOVIE")
        )
        == 2000004
    )
    ...
    assert (
        asyncio.run(
            manager.search_imdb("One Two Three Four", None, category="MOVIE")
        )
        == 2000005
    )


def test_search_secondary_and_graphql_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    _install(
    ...
    assert (
        asyncio.run(
            manager.search_imdb(
                "Primary", 2026, category="MOVIE", secondary_title="Secondary"
            )
        )
        == 3000001
    )
    ...
    _install(
        monkeypatch,
        *[httpx.RequestError("offline", request=request) for _ in range(8)],
    )
    monkeypatch.setattr(imdb, "guessit_fn", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(imdb, "anitopy_parse_fn", lambda *_args, **_kwargs: {})
    assert (
        asyncio.run(
            manager.search_imdb(
                "No Result", None, category="MOVIE", unattended=True
            )
        )
        == 0
    )
...
def test_search_interactive_manual_numeric_skip_and_cancellation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    edges = [
    ...
    monkeypatch.setattr(
        imdb,
        "prompt_in_thread",
        lambda *_args, **_kwargs: asyncio.sleep(0, result=next(prompts)),
    )
    assert (
...
def test_search_no_results_manual_invalid_unattended_and_eof(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    ...
    monkeypatch.setattr(
        imdb,
        "prompt_in_thread",
        lambda *_args, **_kwargs: asyncio.sleep(0, result="tt5000001"),
    )
    assert (
    ...
    monkeypatch.setattr(
        imdb,
        "prompt_in_thread",
        lambda *_args, **_kwargs: asyncio.sleep(0, result="ttbad"),
    )
    assert (
        asyncio.run(manager.search_imdb("No Result", None, category="MOVIE"))
        == 0
...
def test_get_imdb_from_episode_all_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    ...
    assert result["previous_episode"] == {
        "id": "tt6000000",
        "title": "Previous",
    }
...
def test_search_attempted_none_extension_removal_and_split_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    monkeypatch.setattr(imdb.asyncio, "sleep", AsyncMock())
...
def test_search_wide_and_parsed_exception_guards(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
...
def test_search_previous_year_similarity_and_manual_parse_exceptions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = ImdbManager()
    ...
    assert (
        asyncio.run(manager.search_imdb("Target", 2026, category="MOVIE"))
        == 8000001
    )
    ...
    edges = [
        _edge("tt8000003", "Candidate One"),
        _edge("tt8000004", "Candidate Two"),
    ]
...
def test_guessit_wrapper(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        imdb.guessit_module,
        "guessit",
        lambda value, options: {"title": value, "options": options},
    )
