...
def test_warning_rendering_and_dict_dressing() -> None:
    ...
    assert (
        str(ConfigValidationWarning("message", key="key")) == "[key] message"
    )
    assert (
        str(ConfigValidationWarning("message", section="SECTION"))
        == "[SECTION] message"
    )
    assert (
        str(ConfigValidationWarning("message", key="key", section="SECTION"))
        == "[SECTION][key] message"
    )
...
def test_tracker_sources_client_cross_references_and_unknown_sections() -> (
    None
):
    config = _base_config()
    config["TRACKERS"] = {
    ...
    valid, errors, warnings = validate_config(
        config, active_trackers=["TRACKER"]
    )
...
def test_client_list_shape_and_default_client_variants() -> None:
    ...
    config["DEFAULT"].update(
        {
            "injecting_client_list": 7,
            "searching_client_list": "bad",
            "default_torrent_client": "client",
        }
    )
    ...
    assert any(
        "No default_torrent_client" in warning.message for warning in warnings
    )
...
def test_usenet_required_types_numeric_and_missing_section_paths() -> None:
    ...
    assert not valid and any(
        "Missing required config section: 'USENET'" in error
        for error in errors
    )
    ...
    errors, warnings = _validate_usenet_section(
        {"port": "119", "connections": 4}, False
    )
    assert errors == [] and warnings == []


def test_default_section_warns_for_types_values_hosts_and_numeric_limits() -> (
    None
):
    ...
    errors, warnings = _validate_default_section(
        {
            "tmdb_api": 7,
            "image_upload_concurrency": object(),
            "image_upload_delay": object(),
        }
    )
    assert errors
    rendered = "\n".join(str(warning) for warning in warnings)
...
def test_tracker_and_client_section_helpers_cover_missing_and_invalid_configs() -> (
    None
):
...
def test_group_and_format_validation_results() -> None:
    ...
    assert "Warnings" not in format_validation_results(
        False, [], warnings, show_warnings=False
    )
    assert (
        format_validation_results(True, [], []) == "Config validation passed."
    )
    assert "passed with 1 warning" in format_validation_results(
        True, [], [ConfigValidationWarning("warning")]
    )


def test_default_tracker_string_and_list_sources() -> None:
    string_config = _base_config()
    string_config["TRACKERS"] = {
        "default_trackers": " tracker , ",
        "TRACKER": {"announce_url": "https://tracker/<PASSKEY>"},
    }
    ...
    list_config["TRACKERS"] = {
        "default_trackers": [" tracker ", 7, ""],
        "TRACKER": {"announce_url": "https://tracker/<PASSKEY>"},
    }
...
def test_required_default_key_contract(monkeypatch) -> None:
    ...
    errors, _ = validation._validate_default_section(
        {"tmdb_api": "key", "required": 1}
    )
    assert errors == ["DEFAULT['required'] must be str, got int"]
    errors, _ = validation._validate_default_section(
        {"tmdb_api": "key", "required": "value"}
    )
    assert errors == []
