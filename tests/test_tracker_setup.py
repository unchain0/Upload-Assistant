    ...
    setup = TrackerSetup(
        {
            "TRACKERS": {
                ...
                "ORPHEUS": {
                    "api_key": "token",
                    "announce_url": "https://orpheus.example/announce",
                },
    ...
    setup = TrackerSetup(
        {
            "TRACKERS": {
                "CATHODERAYTUBE": {
                    "announce_url": "https://signal.cathode-ray.tube/passkey/announce"
                }
            }
        }
    )
...
class _BannedGroupsResponse:
    ...
    def json(self) -> dict[str, Any]:
        return {
            "count": 2,
            "groups": [
                {"name": "BannedGroup", "reason": "Rule"},
                {"name": "AnotherGroup", "reason": "Rule"},
            ],
...
@pytest.mark.asyncio
async def test_capybarabr_fetches_banned_groups_with_api_token(
    tmp_path, monkeypatch: pytest.MonkeyPatch
):
    requests: list[dict[str, Any]] = []
    monkeypatch.setattr(
        "src.integrations.trackers.registry.httpx.AsyncClient",
        lambda: _BannedGroupsClient(requests),
    )
    setup = TrackerSetup(
        {"TRACKERS": {"CAPYBARABR": {"api_key": "test-token"}}}
    )

    file_path = await setup.get_banned_groups(
        Meta(base_dir=str(tmp_path)), "CAPYBARABR"
    )
    ...
    assert (
        file_path
        == tmp_path / "data" / "banned" / "CAPYBARABR_banned_groups.json"
    )
    assert (
        json.loads(file_path.read_text(encoding="utf-8"))["banned_groups"]
        == "BannedGroup, AnotherGroup"
    )
