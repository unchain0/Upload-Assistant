...
def test_seedpool_routes_sports_titles_to_sports_category():
    assert asyncio.run(_tracker().get_category_id(Meta(category="TV", name="NFL GameNight 2026"))) == {"category_id": "8"}
...
def test_seedpool_maps_remaining_game_platforms():
    ...
    for platform, expected in cases:
        assert asyncio.run(tracker.get_type_id(Meta(category="GAME", platform=platform))) == {"type_id": expected}


def test_seedpool_removes_known_video_extension_from_generated_name():
    meta = Meta(scene=False, is_disc=False, name="Ignored Name", basename_no_ext="Scene File.mkv", mal_id=0)
    assert asyncio.run(_tracker().get_name(meta)) == {"name": "Scene.File"}


def test_seedpool_low_resolution_prompt_paths(monkeypatch):
    tracker = _tracker()
    meta = Meta(category="MOVIE", resolution="720p", unattended=False, unattended_confirm=False, keywords=[], combined_genres=[])

    monkeypatch.setattr("src.integrations.trackers.UNIT3D.seedpool.cli_ui.ask_yes_no", lambda *_args, **_kwargs: True)
    assert asyncio.run(tracker.get_additional_checks(meta)) is True

    monkeypatch.setattr("src.integrations.trackers.UNIT3D.seedpool.cli_ui.ask_yes_no", lambda *_args, **_kwargs: False)
    assert asyncio.run(tracker.get_additional_checks(meta)) is False

    unattended = Meta(category="MOVIE", resolution="720p", unattended=True, unattended_confirm=False, keywords=[], combined_genres=[])
...
def test_seedpool_adult_keyword_prompt_paths(monkeypatch):
    tracker = _tracker()
    meta = Meta(category="MOVIE", resolution="1080p", unattended=False, unattended_confirm=False, keywords=["porn"], combined_genres=[])
    ...
    unattended = Meta(category="MOVIE", resolution="1080p", unattended=True, unattended_confirm=False, keywords=["porn"], combined_genres=[])
    assert asyncio.run(tracker.get_additional_checks(unattended)) is False
