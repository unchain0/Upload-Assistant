...
def test_unit3d_search_name_and_book_title_shortening() -> None:
    ...
    assert tracker._book_search_name(_meta(category="BOOK"), "A Long Title: Subtitle") == "A Long Title"
    assert tracker._book_search_name(_meta(category="BOOK"), "One: Subtitle") == "One: Subtitle"
...
async def test_unit3d_search_params_video_and_nonvideo() -> None:
    ...
    tv = await tracker._search_params_dict(_meta(category="TV", title="Show", season="S01", tmdb=99))
    assert tv["name"] == "Show S01"
    assert tv["tmdbId"] == "99"
    book = await tracker._search_params_dict(_meta(category="BOOK", title="Long Book Title: A Subtitle", tmdb=None))
...
def test_unit3d_search_result_helpers() -> None:
    tracker = _tracker()
    pending = {"id": 1, "tmdb_id": 123, "name": "Pending", "files": [{"name": "a.mkv"}], "size": 10}
    assert tracker._search_result(_meta(is_disc=""), pending, True)["link"] == "/torrents/pending"  # type: ignore[index]
...
def test_unit3d_response_entries_guard() -> None:
    ...
    assert UNIT3D._response_entries(httpx.Response(200, request=request, json=[])) == []
    assert UNIT3D._response_entries(httpx.Response(200, request=request, json={"data": "bad"})) == []


@pytest.mark.asyncio
async def test_unit3d_description_and_media_files(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    class Builder:
    ...
    tracker = _tracker()
    assert await tracker.get_description(_meta()) == {"description": "description"}
    ...
    assert await tracker.get_mediainfo(_meta(tmp_path, bdinfo={"x": 1})) == {"mediainfo": ""}
    assert await tracker.get_mediainfo(_meta(tmp_path, category="BOOK")) == {"mediainfo": ""}
    assert await tracker.get_bdinfo(_meta(tmp_path, bdinfo={"x": 1})) == {"bdinfo": "BD"}
    assert await tracker.get_bdinfo(_meta(tmp_path, bdinfo={})) == {"bdinfo": ""}
...
async def test_unit3d_mapping_modes_flags_and_identity_getters() -> None:
    ...
    assert (await tracker.get_category_id(meta, mapping_only=True))["MOVIE"] == "1"
    assert (await tracker.get_category_id(meta, reverse=True))["2"] == "TV"
    assert await tracker.get_category_id(meta, category="UNKNOWN") == {"category_id": "0"}
    assert await tracker.get_type_id(meta, mapping_only=True) == tracker._type_mapping()
    assert (await tracker.get_type_id(meta, reverse=True))["4"] == "WEBDL"
    assert await tracker.get_type_id(meta, type="UNKNOWN") == {"type_id": "0"}
    assert (await tracker.get_resolution_id(meta, mapping_only=True))["1080p"] == "3"
    assert (await tracker.get_resolution_id(meta, reverse=True))["2"] == "2160p"
    assert await tracker.get_resolution_id(meta, resolution="unknown") == {"resolution_id": "10"}
    ...
    assert await tracker.get_distributor_id(_meta(distributor="Criterion")) == {"distributor_id": "5"}
    ...
    assert await tracker.get_region_id(_meta(region="US")) == {"region_id": "7"}
    assert await tracker.get_region_id(_meta(region="")) == {}

    assert await tracker.get_tmdb(_meta(tmdb=None)) == {"tmdb": "0"}
    assert await tracker.get_imdb(_meta(category="BOOK", imdb_id=123)) == {"imdb": "0"}
    assert await tracker.get_tvdb(_meta(category="MOVIE", tvdb_id=123)) == {"tvdb": "0"}
    assert await tracker.get_mal(_meta(mal_id=55)) == {"mal": "55"}
    assert await tracker.get_igdb(_meta(category="GAME", igdb_id=88)) == {"igdb": "88"}
    assert await tracker.get_igdb(_meta(category="MOVIE", igdb_id=88)) == {"igdb": "0"}
    ...
    assert await tracker.get_personal_release(_meta(personalrelease=True)) == {"personal_release": "1"}
...
async def test_unit3d_keywords_limit_and_truncation() -> None:
    tracker = _tracker()
    assert await tracker.get_keywords(_meta(keywords=[" one ", "", "two"])) == {"keywords": "one, two"}
    long = "x" * 300
...
@pytest.mark.asyncio
async def test_unit3d_get_data_merges_and_sets_exclusive(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "get_description", AsyncMock(return_value={"description": "desc"}))
    monkeypatch.setattr(tracker, "get_mediainfo", AsyncMock(return_value={"mediainfo": "mi"}))
    monkeypatch.setattr(tracker, "get_bdinfo", AsyncMock(return_value={"bdinfo": ""}))
    monkeypatch.setattr(tracker, "get_region_id", AsyncMock(return_value={}))
    monkeypatch.setattr(tracker, "get_distributor_id", AsyncMock(return_value={}))
...
@pytest.mark.asyncio
async def test_unit3d_get_image_file_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(unit3d_module, "is_valid_image_bytes", lambda _data: False)
    assert await tracker.get_image_file(image) is None

    monkeypatch.setattr(unit3d_module, "is_valid_image_bytes", lambda _data: True)
...
@pytest.mark.asyncio
async def test_unit3d_additional_files_nfo_and_artwork(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    root = tmp_path / "game"
    root.mkdir()
    nfo = root / "scene.nfo"
    nfo.write_bytes(b"nfo")
    game = _meta(tmp_path, category="GAME", path=str(root / "game.iso"), scene_nfo_file="scene.nfo", filelist=[])
    ...
    files = await tracker.get_additional_files(_meta(tmp_path, category="MOVIE"))
    assert files["nfo"][0] == "temp.nfo"

    monkeypatch.setattr(tracker, "get_image_file", AsyncMock(side_effect=[("cover.png", b"c", "image/png"), ("banner.png", b"b", "image/png")]))
    book = _meta(tmp_path, category="BOOK", artwork_path="cover.png", artwork_banner_path="banner.png")
...
@pytest.mark.asyncio
async def test_unit3d_upload_request_parts_and_debug(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker(api_key="key")
    root = tmp_path / "tmp" / "release"
    root.mkdir(parents=True)
    (root / "BASE.torrent").write_bytes(b"torrent")
    monkeypatch.setattr(tracker, "get_upload_torrent_filename", AsyncMock(return_value="BASE"))
    monkeypatch.setattr(tracker, "get_additional_files", AsyncMock(return_value={}))
...
@pytest.mark.asyncio
async def test_unit3d_post_attempt_success_redirect_and_api_rejection(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    success = httpx.Response(200, request=httpx.Request("POST", "https://tracker.invalid/upload"), json={"success": True, "message": "ok", "data": "/123.file"})
    monkeypatch.setattr(tracker, "_upload_response", AsyncMock(return_value=success))
    ...
    redirect = httpx.Response(302, request=httpx.Request("POST", "https://tracker.invalid/upload"), headers={"location": "/login"})
    tracker.follow_upload_redirects = False
    monkeypatch.setattr(tracker, "_upload_response", AsyncMock(return_value=redirect))
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._post_with_retries(item, {}, {}, {}) is None
    assert item.tracker_status["TEST"]["status_message"] == "data error: Upload redirect rejected"

    tracker.follow_upload_redirects = True
    rejected = httpx.Response(200, request=httpx.Request("POST", "https://tracker.invalid/upload"), json={"success": False, "message": "bad"})
    monkeypatch.setattr(tracker, "_upload_response", AsyncMock(return_value=rejected))
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._post_with_retries(item, {}, {}, {}) is None
...
@pytest.mark.asyncio
async def test_unit3d_post_with_retries_success_and_value_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_post_upload_attempt", AsyncMock(return_value={"data": "https://download.invalid/torrent"}))
    assert await tracker._post_with_retries(_meta(tracker_status={"TEST": {}}), {}, {}, {}) == "https://download.invalid/torrent"

    monkeypatch.setattr(tracker, "_post_upload_attempt", AsyncMock(side_effect=ValueError("bad json")))
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._post_with_retries(item, {}, {}, {}) is None
    assert "Invalid JSON response" in item.tracker_status["TEST"]["status_message"]


@pytest.mark.asyncio
async def test_unit3d_http_error_decisions(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._http_error_decision(item, error(403), 0, 40.0) == (False, 40.0)
    assert "Forbidden" in item.tracker_status["TEST"]["status_message"]

    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._http_error_decision(item, error(302), 0, 40.0) == (False, 40.0)
    assert "Redirect" in item.tracker_status["TEST"]["status_message"]

    duplicate_text = '{"data":{"name":["The name has already been taken."]}}'
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._http_error_decision(item, error(422, duplicate_text), 0, 40.0) == (False, 40.0)
    assert item.tracker_status["TEST"]["dupe"] is True

    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._http_error_decision(item, error(401), 0, 40.0) == (True, 40.0)
    assert await tracker._http_error_decision(item, error(401), 1, 40.0) == (False, 40.0)

    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._http_error_decision(item, error(500), 0, 40.0) == (True, 40.0)
    assert await tracker._http_error_decision(item, error(520), 1, 40.0) == (False, 40.0)
    assert "cloudflare" in item.tracker_status["TEST"]["status_message"]


@pytest.mark.asyncio
async def test_unit3d_timeout_request_and_dispatch_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_retry_delay", AsyncMock())
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._timeout_error_decision(item, 0, 40.0) == (True, 60.0)
    assert await tracker._timeout_error_decision(item, 1, 60.0) == (False, 60.0)
    assert "timed out" in item.tracker_status["TEST"]["status_message"]

    request_error = httpx.RequestError("offline", request=httpx.Request("POST", "https://tracker.invalid"))
    item = _meta(tracker_status={"TEST": {}})
    assert await tracker._request_error_decision(item, request_error, 0, 40.0, {}) == (True, 40.0)
    assert await tracker._request_error_decision(item, request_error, 1, 40.0, {"message": "x"}) == (False, 40.0)
    assert "offline" in item.tracker_status["TEST"]["status_message"]

    item = _meta(tracker_status={"TEST": {}})
    retry, _ = await tracker._upload_error_decision(item, httpx.TimeoutException("timeout"), 1, 40.0, {})
    assert not retry
...
async def test_unit3d_download_uploaded_torrent_paths() -> None:
    ...
    item = _meta(tracker_status={"TEST": {}})
    assert not await tracker._download_uploaded_torrent(item, {}, "https://tracker.invalid/file")

    tracker.download_url_hosts = ()
    tracker.common.download_tracker_torrent = AsyncMock(return_value=None)  # type: ignore[method-assign]
    assert await tracker._download_uploaded_torrent(_meta(tracker_status={"TEST": {}}), {}, "https://tracker.invalid/file")


@pytest.mark.asyncio
async def test_unit3d_upload_wrapper_success_and_debug(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_upload_request_parts", AsyncMock(return_value=({}, {}, {})))
    monkeypatch.setattr(tracker, "_post_with_retries", AsyncMock(return_value="https://download.invalid/file"))
    monkeypatch.setattr(tracker, "_download_uploaded_torrent", AsyncMock(return_value=True))
...
async def test_unit3d_bounded_response_limits() -> None:
    request = httpx.Request("GET", "https://tracker.invalid")
    too_large_header = httpx.Response(200, request=request, headers={"content-length": "100"}, content=b"x")
    with pytest.raises(ValueError, match="exceeds"):
        await UNIT3D._bounded_response(too_large_header, 10)

    too_large_body = httpx.Response(200, request=request, content=b"12345678901")
    with pytest.raises(ValueError, match="exceeds"):
...
async def test_unit3d_response_parsers() -> None:
    ...
    assert await tracker.process_response_data({"success": True, "message": "ok"}) == "ok"
    assert await tracker.process_response_data({"success": False, "message": "bad"}) == "API response: bad"
    assert await tracker.process_response_data({"success": False, "data": 1}) == "API response: {'success': False, 'data': 1}"


def test_unit3d_book_search_name_nonbook_passthrough() -> None:
    assert UNIT3D._book_search_name(_meta(category="MOVIE"), "Title: Subtitle") == "Title: Subtitle"


@pytest.mark.asyncio
async def test_unit3d_search_redirect_and_failed_status_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    tracker.follow_search_redirects = False
    redirect = httpx.Response(302, request=httpx.Request("GET", "https://tracker.invalid/search"))
    monkeypatch.setattr(tracker, "_request_search_response", AsyncMock(return_value=redirect))
    with pytest.raises(ValueError, match="redirect rejected"):
        await tracker._search_response(AsyncMock(), "https://tracker.invalid/search", [], {}, False)

    failed = httpx.Response(500, request=httpx.Request("GET", "https://tracker.invalid/search"), json={"data": []})
    ...
    ok = httpx.Response(
        ...
        json={"data": [{"id": 1, "attributes": {"name": "Release", "size": 10, "files": []}}]},
    )
...
@pytest.mark.asyncio
async def test_unit3d_bounded_search_response(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    tracker.max_json_response_size = 1024
    response = httpx.Response(200, request=httpx.Request("GET", "https://tracker.invalid/search"), json={"data": []})
    bounded = httpx.Response(200, request=response.request, json={"data": []})
    monkeypatch.setattr(tracker, "_bounded_response", AsyncMock(return_value=bounded))
    result = await tracker._request_search_response(_SearchStreamClient(response), "https://tracker.invalid/search", [], {})  # type: ignore[arg-type]
    assert result is bounded
    tracker._bounded_response.assert_awaited_once()


@pytest.mark.asyncio
async def test_unit3d_read_image_oserror(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    image = tmp_path / "cover.png"
    image.write_bytes(b"png")
    monkeypatch.setattr(tracker, "_image_path_allowed", lambda *_args: (_ for _ in ()).throw(OSError("stat failed")))
...
@pytest.mark.asyncio
async def test_unit3d_upload_returns_false_when_post_never_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_upload_request_parts", AsyncMock(return_value=({}, {}, {})))
    monkeypatch.setattr(tracker, "_post_with_retries", AsyncMock(return_value=None))
    assert not await tracker.upload(_meta(tracker_status={"TEST": {}}))


@pytest.mark.asyncio
async def test_unit3d_post_with_retries_exhausts_retry_loop(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_post_upload_attempt", AsyncMock(side_effect=RuntimeError("retry")))
    monkeypatch.setattr(tracker, "_upload_error_decision", AsyncMock(return_value=(True, 40.0)))
    assert await tracker._post_with_retries(_meta(tracker_status={"TEST": {}}), {}, {}, {}) is None
...
@pytest.mark.asyncio
async def test_unit3d_upload_response_direct_and_bounded(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    response = httpx.Response(200, request=httpx.Request("POST", "https://tracker.invalid/upload"), json={"success": True})
    monkeypatch.setattr(unit3d_module.httpx, "AsyncClient", lambda **kwargs: _UploadClient(response, **kwargs))
    assert await tracker._upload_response({}, {}, {}, 40.0) is response

    tracker.max_json_response_size = 1024
    bounded = httpx.Response(200, request=response.request, json={"success": True})
    monkeypatch.setattr(tracker, "_bounded_response", AsyncMock(return_value=bounded))
...
def test_unit3d_json_object_rejects_non_mapping() -> None:
    response = httpx.Response(200, request=httpx.Request("POST", "https://tracker.invalid/upload"), json=[])
    with pytest.raises(ValueError, match="JSON object"):
        UNIT3D._json_object(response)


@pytest.mark.asyncio
async def test_unit3d_upload_error_dispatch_http_and_request(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_http_error_decision", AsyncMock(return_value=(False, 40.0)))
    request = httpx.Request("POST", "https://tracker.invalid/upload")
    response = httpx.Response(500, request=request)
    http_error = httpx.HTTPStatusError("bad", request=request, response=response)
    assert await tracker._upload_error_decision(_meta(), http_error, 0, 40.0, {}) == (False, 40.0)

    monkeypatch.setattr(tracker, "_request_error_decision", AsyncMock(return_value=(False, 40.0)))
    request_error = httpx.RequestError("offline", request=request)
    assert await tracker._upload_error_decision(_meta(), request_error, 0, 40.0, {}) == (False, 40.0)
...
@pytest.mark.asyncio
async def test_unit3d_retry_delay_calls_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    sleep = AsyncMock()
...
@pytest.mark.asyncio
async def test_unit3d_bounded_response_body_limit_without_content_length() -> None:
    with pytest.raises(ValueError, match="exceeds"):
