...
class Client:
    ...
    @classmethod
    def _next(
        cls, method: str, url: str, kwargs: dict[str, object]
    ) -> Response:
...
def _files(meta: Meta, *, bdinfo: bool = False) -> Path:
    ...
    (root / "[SWARMAZON]DESCRIPTION.txt").write_text(
...
def test_category_mapping_movie_tv_anime_and_unknown(tmp_path: Path) -> None:
    ...
    assert asyncio.run(
        tracker._upload_category(_meta(tmp_path, category="TV", tv_pack=False))
    ) == ("2", "5")
    assert asyncio.run(
        tracker._upload_category(_meta(tmp_path, category="TV", tv_pack=True))
    ) == ("2", "6")
    assert asyncio.run(
        tracker._upload_category(
            _meta(tmp_path, category="MUSIC", mal_id=10, demographic="Seinen")
        )
    ) == ("7", "28")
    assert asyncio.run(
        tracker._upload_category(
            _meta(tmp_path, category="MUSIC", mal_id=10, demographic="Unknown")
        )
    ) == ("7", "47")
    assert asyncio.run(
        tracker._upload_category(_meta(tmp_path, category="MUSIC", mal_id=0))
    ) == ("", "")
...
def test_media_dump_and_upload_file_readers(tmp_path: Path) -> None:
    ...
    _files(regular)
    assert asyncio.run(tracker._read_media_dump(regular)) == (
        "mediainfo",
        None,
    )
    assert asyncio.run(tracker._read_upload_files(regular)) == (
        "description",
        b"torrent",
    )
...
def test_upload_success_creates_ready_torrent(
    ...
    Client.reset(
        Response({"success": True, "link": "https://tracker.invalid/1"})
    )

    assert asyncio.run(tracker.upload(meta))
    assert (
        meta.tracker_status["SWARMAZON"]["status_message"]
        == "https://tracker.invalid/1"
    )
    assert FakeCommon.ready


def test_upload_request_error_and_response_failures(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = Swarmazon(_config())
    monkeypatch.setattr(tracker, "edit_desc", AsyncMock())
    ...
    request = httpx.RequestError(
        "offline", request=httpx.Request("POST", "https://swarmazon.club")
    )
...
def test_debug_upload_does_not_post(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = Swarmazon(_config())
    ...
    assert asyncio.run(tracker.upload(meta))
    assert Client.calls == []
    assert meta.tracker_status["SWARMAZON"]["status_message"].startswith(
        "Debug mode"
    )
    assert len(FakeCommon.created) == 2


def test_disc_upload_appends_bd_summary(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = Swarmazon(_config())
...
def test_edit_desc_skips_incomplete_images_and_keeps_complete_image(
    tmp_path: Path,
) -> None:
    tracker = Swarmazon(_config())
    meta = _meta(
        ...
        image_list=[
            ...
            {
                "web_url": "https://web.invalid/1",
                "img_url": "https://img.invalid/1",
            },
        ],
    )
    root = release_temp_dir(meta.base_dir, meta.uuid)
...
def test_search_existing_all_parameter_paths(tmp_path: Path) -> None:
    ...
    assert asyncio.run(
        tracker.search_existing(
            _meta(tmp_path, imdb_id=0, category="MOVIE", title="Movie")
        )
    ) == ["One"]
    ...
    assert asyncio.run(
        tracker.search_existing(
            _meta(
                tmp_path, imdb_id=0, category="TV", title="Show", season="S02"
            )
        )
    ) == ["Two"]
    ...
    assert asyncio.run(
        tracker.search_existing(
            _meta(tmp_path, imdb_id=1, category="TV", season="S03")
        )
    ) == ["Three"]
    ...
    assert asyncio.run(
        tracker.search_existing(
            _meta(tmp_path, imdb_id=1, category="MOVIE", resolution="2160p")
        )
    ) == ["Four"]
