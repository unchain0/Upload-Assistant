...
def _tracker(**settings: object) -> Unwalled:
    tracker_settings: dict[str, object] = {
        "api_key": "token",
        "announce_url": "https://unwalled.cc/announce/test-token",
        **settings,
    }
...
def test_podcast_prep_rejects_mixed_audio_and_video(tmp_path: Path) -> None:
    ...
    meta = Meta(
        path=str(tmp_path), category="PODCAST", manual_category="podcast"
    )
    ...
    try:
        with patch(
            "src.services.podcast_preparation._detected_media_kind",
            side_effect=detected_kind,
        ):
...
def test_podcast_prep_builds_an_audio_pack_without_tmdb(
    tmp_path: Path,
) -> None:
    ...
    with (
        ...
        patch(
            "src.services.podcast_preparation.export_info",
            new=AsyncMock(return_value={"media": {"track": []}}),
        ),
    ):
        asyncio.run(gather_podcast_prep(meta))
...
def test_unwalled_podcast_title_requires_year_format_and_audio_bitrate() -> (
    None
):
    assert (
        Unwalled._valid_podcast_title(
            "Example Show [2026/MP3 - 128kbps]", audio=True
        )
        is True
    )
    assert (
        Unwalled._valid_podcast_title(
            "Example Show [2026-08-11/MP4]", audio=False
        )
        is True
    )
    assert (
        Unwalled._valid_podcast_title(
            "Example Show [MP3 - 128kbps]", audio=True
        )
        is False
    )
    assert (
        Unwalled._valid_podcast_title("Example Show [2026/MP3]", audio=True)
        is False
    )


@pytest.mark.asyncio
async def test_unwalled_additional_checks_reject_invalid_generated_title() -> (
    None
):
    meta = Meta(
        category="PODCAST", type="AUDIO", name="Example Show [MP3 - 128kbps]"
    )
...
@pytest.mark.asyncio
async def test_podcast_prep_offloads_per_file_media_scanning(
    tmp_path: Path,
) -> None:
    episode = tmp_path / "001 - Pilot.mp3"
    episode.write_bytes(b"audio")
    ...
    with (
        patch(
            "src.services.podcast_preparation.asyncio.to_thread",
    ...
    offloaded_functions = [
        call.args[0].__name__ for call in to_thread.await_args_list
    ]
...
@pytest.mark.asyncio
async def test_export_info_offloads_standard_mediainfo_parsing(
    tmp_path: Path,
) -> None:
    ...
    with (
        patch(
            "src.integrations.media.media_info_export.MediaInfo.parse",
            side_effect=parse_media,
        ),
        patch(
            "src.integrations.media.media_info_export.asyncio.to_thread",
            new=AsyncMock(side_effect=run_in_thread),
        ) as to_thread,
    ):
        result = await export_info(
            str(media), True, "mediainfo-offload", str(tmp_path)
        )
...
@pytest.mark.asyncio
async def test_unwalled_single_episode_directory_keeps_folder_torrent(
    tmp_path: Path,
) -> None:
    ...
    torrent = Torrent.read(
        tmp_path / "tmp" / meta.uuid / f"{filename}.torrent"
    )
...
def test_podcast_prep_rejects_symlinks_and_disguised_archives(
    tmp_path: Path,
) -> None:
...
def test_podcast_prep_rejects_symlinked_ancestors_and_artwork(
    tmp_path: Path,
) -> None:
    ...
    ancestor_meta = Meta(
        path=str(linked_dir / episode.name),
        base_dir=str(tmp_path),
        uuid="ancestor-link",
        category="PODCAST",
    )
...
def test_podcast_prep_rejects_media_with_a_mismatched_extension(
    tmp_path: Path,
) -> None:
    disguised_video = tmp_path / "video.mp3"
    disguised_video.write_bytes(
        b"\x00\x00\x00\x18ftypisom\x00\x00\x02\x00isomiso2"
    )
    meta = Meta(
        path=str(disguised_video),
        base_dir=str(tmp_path),
        uuid="mismatched-media",
        category="PODCAST",
    )
    ...
    with (
        ...
        pytest.raises(ValueError, match="extension does not match"),
    ):
        asyncio.run(gather_podcast_prep(meta))


def test_podcast_prep_rejects_unidentified_declared_media(
    tmp_path: Path,
) -> None:
...
def test_unwalled_discovers_category_and_type_ids_from_unit3d_results() -> (
    None
):
    ...
    assert _tracker().catalog_from_response(payload) == {
        "categories": {"technology": "14"},
        "types": {"free audio": "3"},
    }
...
async def test_unwalled_discovers_options_across_all_result_pages() -> None:
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        payload: object = (
            first_page if request.url.params["page"] == "1" else second_page
        )
        return httpx.Response(200, json={"data": payload})

    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    with patch(
        "src.integrations.trackers.UNIT3D.unwalled.httpx.AsyncClient",
        return_value=client,
    ):
        catalog = await _tracker().discover_options()

    assert catalog == {
        "categories": {"science": "15", "technology": "14"},
        "types": {"free audio": "3", "premium audio": "4"},
    }
...
async def test_unwalled_retries_incomplete_option_discovery_cache() -> None:
    ...
    async def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
            json={
                "data": [{"attributes": {"type": "Free Audio", "type_id": 3}}]
            },
        )
...
async def test_unwalled_bounds_option_and_upload_json_responses() -> None:
    ...
    with patch(
        "src.integrations.trackers.UNIT3D.unwalled.httpx.AsyncClient",
        return_value=option_client,
    ):
        assert await _tracker().discover_options() == {
            "categories": {},
            "types": {},
        }

    response_client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    async with response_client.stream(
        "GET", "https://unwalled.cc/api/test"
    ) as response:
...
def test_unwalled_resolves_names_or_explicit_numeric_ids() -> None:
    tracker = _tracker()
    tracker.option_catalog = {
        "categories": {"technology": "14"},
        "types": {"free audio": "3"},
    }
    named = Meta(
        category="PODCAST",
        unwalled_category="Technology",
        unwalled_type="Free Audio",
    )
    numeric = Meta(
        category="PODCAST", unwalled_category="99", unwalled_type="42"
    )

    assert asyncio.run(tracker.get_category_id(named)) == {"category_id": "14"}
    assert asyncio.run(tracker.get_type_id(named)) == {"type_id": "3"}
    assert asyncio.run(tracker.get_category_id(numeric)) == {
        "category_id": "99"
    }
...
async def test_unwalled_video_payload_omits_unknown_resolution() -> None:
    ...
    with (
        patch.object(
            tracker, "get_description", new=AsyncMock(return_value={})
        ),
...
@pytest.mark.asyncio
async def test_unwalled_duplicate_search_uses_upload_title_and_bounds_json() -> (
    None
):
    seen_names: list[str] = []
    ...
    meta = Meta(
        category="PODCAST",
        title="source-folder",
        name="Final Podcast Title",
        podcast_title="Final Podcast Title",
        unwalled_category="14",
        unwalled_type="3",
    )
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
...
def test_unwalled_requires_valid_distinct_jpeg_cover_and_banner(
    tmp_path: Path,
) -> None:
    cover = tmp_path / "cover.jpg"
    banner = tmp_path / "banner.jpg"
...
@pytest.mark.parametrize(
    "filename", ["   ", "bad:name.mp3", "episode.mp3.", "episode.mp3 "]
)
...
def test_unwalled_rejects_invalid_nested_paths_and_missing_announce(
    tmp_path: Path,
) -> None:
    cover = tmp_path / "cover.jpg"
    banner = tmp_path / "banner.jpg"
    ...
    if os.name != "nt":
        ...
        assert (
            asyncio.run(_tracker().get_additional_checks(invalid_meta))
            is False
        )
    ...
    assert (
        asyncio.run(_tracker(announce_url="").get_additional_checks(meta))
        is False
    )


@pytest.mark.asyncio
async def test_unwalled_builds_private_v1_torrent_with_source_and_announce(
    tmp_path: Path,
) -> None:
    episode = tmp_path / "001 - Pilot.mp3"
    ...
    filename = await _tracker(
        announce_url=announce
    ).get_upload_torrent_filename(meta)
    ...
    assert (
        _tracker(announce_url=announce)._valid_upload_bundle(
            meta, release_dir / f"{filename}.torrent"
        )
        is False
    )
...
def test_unwalled_rejects_huge_v1_lengths_without_overflow(
    tmp_path: Path,
) -> None:
...
@pytest.mark.parametrize(
    "piece_length",
    [16 * 1024, 32 * 1024 * 1024, 64 * 1024 * 1024, 128 * 1024 * 1024],
)
...
@pytest.mark.parametrize(
    "piece_length", [8 * 1024, 24 * 1024, 256 * 1024 * 1024]
)
def test_unwalled_rejects_unsupported_v1_piece_sizes(
    piece_length: int,
) -> None:
    info: dict[bytes, object] = {
        b"name": b"episode.mp3",
...
@pytest.mark.asyncio
async def test_unwalled_rejects_cross_host_torrent_download_redirect(
    tmp_path: Path,
) -> None:
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            302,
            headers={"location": "https://attacker.invalid/stolen.torrent"},
        )
    ...
    (tmp_path / "tmp" / meta.uuid).mkdir(parents=True)
    with (
        patch("src.integrations.trackers.registry.http_trackers", []),
        patch(
...
async def test_unwalled_option_discovery_does_not_follow_redirects() -> None:
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        requests.append(request)
        return httpx.Response(
            302, headers={"location": "https://attacker.invalid/options"}
        )
...
@pytest.mark.asyncio
async def test_unwalled_rejects_oversized_torrent_download(
    tmp_path: Path,
) -> None:
    async def handler(_request: httpx.Request) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_unwalled_upload_uses_bounded_same_host_torrent_download(
    tmp_path: Path,
) -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                "success": True,
                "message": "uploaded",
                "data": "https://unwalled.cc/torrents/download/1",
            },
        )
    ...
    with (
        ...
        patch.object(
            tracker,
            "get_upload_torrent_filename",
            new=AsyncMock(return_value="[UNWALLED]"),
        ),
        patch.object(
            tracker, "get_additional_files", new=AsyncMock(return_value={})
        ),
        patch.object(
            tracker.common,
...
@pytest.mark.asyncio
async def test_unwalled_upload_does_not_follow_redirects(
    tmp_path: Path,
) -> None:
    requests: list[httpx.Request] = []
    async_client_class = httpx.AsyncClient
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        requests.append(request)
        return httpx.Response(
            307, headers={"location": "https://attacker.invalid/collect"}
        )
    ...
    with (
        ...
        patch.object(
            tracker,
            "get_data",
            new=AsyncMock(return_value={"name": "Private Show"}),
        ),
        patch.object(
            tracker,
            "get_upload_torrent_filename",
    ...
    assert [request.url.host for request in requests] == ["unwalled.cc"]
    assert (
        meta.tracker_status["UNWALLED"]["status_message"]
        == "data error: Upload redirect rejected"
    )


@pytest.mark.asyncio
async def test_unwalled_omits_tracker_controlled_error_messages(
    tmp_path: Path,
) -> None:
    ...
    async def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200, json={"success": False, "message": sentinel}
        )
...
@pytest.mark.asyncio
async def test_unwalled_rejects_v2_base_and_oversized_final_bundle(
    tmp_path: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_unwalled_debug_torrent_never_contains_personal_announce(
    tmp_path: Path,
) -> None:
    ...
    filename = await _tracker(
        announce_url="https://unwalled.cc/announce/personal-token"
    ).get_upload_torrent_filename(meta)
    torrent = Torrent.read(release_dir / f"{filename}.torrent")
...
def test_unwalled_rejects_malformed_announce_ports() -> None:
    assert (
        _tracker()._valid_announce_url(
            "https://unwalled.cc:invalid/announce/token"
        )
        is False
    )


def test_unwalled_rejects_malformed_or_path_traversing_v1_torrents(
    tmp_path: Path,
) -> None:
...
def test_unwalled_rejects_decompression_bomb_errors(tmp_path: Path) -> None:
    ...
    with patch(
        "src.integrations.trackers.UNIT3D.unwalled_validation.Image.open",
...
def test_unwalled_catalog_guard_branches() -> None:
    tracker = _tracker()
    assert tracker.catalog_from_response({"data": "bad"}) == {
        "categories": {},
        "types": {},
    }
    payload = {"data": ["bad-entry", {"attributes": "bad-attributes"}]}
    assert tracker.catalog_from_response(payload) == {
        "categories": {},
        "types": {},
    }
...
async def test_unwalled_mapping_modes_and_unknown_option() -> None:
    ...
    assert await tracker.get_category_id(meta, reverse=True) == {
        "14": "technology"
    }
    assert await tracker.get_type_id(meta, mapping_only=True) == {
        "free audio": "3"
    }
...
async def test_unwalled_additional_checks_require_title() -> None:
    assert not await _tracker().get_additional_checks(
        Meta(category="PODCAST", podcast_title="", name="")
    )


@pytest.mark.asyncio
async def test_unwalled_option_resolution_error_is_a_clean_rejection(
...
@pytest.mark.asyncio
async def test_unwalled_rejects_artwork_that_consumes_bundle_budget(
...
@pytest.mark.asyncio
async def test_unwalled_upload_filename_requires_valid_personal_announce() -> (
    None
):
    tracker = _tracker(announce_url="not-a-valid-url")
    with pytest.raises(
        ValueError, match="valid personal Unwalled announce URL"
    ):
