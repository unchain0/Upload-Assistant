...
def test_guessit_wrapper(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert tags.guessit_fn("Release", {"type": "movie"}) == {
        "title": "Release",
        "options": {"type": "movie"},
    }
...
def test_get_tag_anime_and_non_anime_paths(tmp_path: Path) -> None:
    assert (
        asyncio.run(
            tags.get_tag("[SubsPlease] Show - 01.mkv", _meta(anime=True))
        )
        == "-SubsPlease"
    )
    assert (
        asyncio.run(
            tags.get_tag("Show.S01E01.1080p-GROUP.mkv", _meta(anime=True))
        )
        == "-GROUP"
    )
    ...
    folder = _meta(
        category="MOVIE", keep_folder=True, uuid="Movie.2026.1080p-FOLDER"
    )
    assert asyncio.run(tags.get_tag("ignored.mkv", folder)) == "-FOLDER"

    assert (
        asyncio.run(tags.get_tag("Movie.2026.1080p.x265-GROUP", _meta()))
        == "-GROUP"
    )
    assert (
        asyncio.run(tags.get_tag("Movie.2026.1080p-GROUP.mkv", _meta()))
        == "-GROUP"
    )
...
def test_get_tag_preserves_valid_prefix_group_for_tv_without_anime_flag(
    filename: str,
) -> None:
    expected = filename.split("]", 1)[0].lstrip("[")
    assert (
        asyncio.run(tags.get_tag(filename, _meta(category="TV", anime=False)))
        == f"-{expected}"
    )
...
@pytest.mark.parametrize(
    "candidate",
    ["S01", "S01E05", "E05", "01x05", "EP05", "Episode 05", "-_S01E05_"],
)
...
def test_release_group_semantics_validate_prefix_candidates() -> None:
    ...
    assert not is_valid_prefixed_release_group(
        "BILI.WEB-DL_1080P_HEVC_AAC_D-SUB"
    )
...
def test_get_tag_never_returns_episode_syntax_as_release_group(
    filename: str,
) -> None:
    assert (
        asyncio.run(tags.get_tag(filename, _meta(category="TV", anime=False)))
...
def test_get_tag_book_game_title_author_and_hyphen_guards() -> None:
    ...
    title_match = _meta(
        ...
        title="Example Group",
        author="Author",
        uuid="Book-Example.Group.epub",
    )
    assert (
        asyncio.run(tags.get_tag("Book-Example.Group.epub", title_match)) == ""
    )
    ...
    author_match = _meta(
        ...
        title="Book",
        author="Alice Writer",
        uuid="Book-Alice.Writer.epub",
    )
    assert (
        asyncio.run(tags.get_tag("Book-Alice.Writer.epub", author_match)) == ""
    )
...
def test_get_tag_rewrites_z0n3_long_scene_and_space_group() -> None:
    assert (
        asyncio.run(tags.get_tag("Movie.1080p-Z0N3.mkv", _meta())) == "-D-Z0N3"
    )
    assert (
        asyncio.run(
            tags.get_tag(
                "Movie.1080p-VERYLONGGROUPNAME.mkv", _meta(scene=False)
            )
        )
        == ""
    )
    assert (
        asyncio.run(
            tags.get_tag(
                "Movie.1080p-VERYLONGGROUPNAME.mkv", _meta(scene=True)
            )
        )
        == "-VERYLONGGROUPNAME"
    )
    assert (
        asyncio.run(tags.get_tag("Movie 1080p BONE.mkv", _meta())) == "-BONE"
    )


def test_get_tag_disc_guessit_success_error_bdmv_validation_and_generic_tags(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        tags,
        "guessit_fn",
        lambda _value, _options=None: {"release_group": "DISCGRP"},
    )
    assert (
        asyncio.run(tags.get_tag("DVD Folder", _meta(is_disc="DVD")))
        == "-DISCGRP"
    )
    assert (
        asyncio.run(tags.get_tag("BDMV DISCGRP", _meta(is_disc="BDMV")))
        == "-DISCGRP"
    )
    ...
    monkeypatch.setattr(
        tags,
        "guessit_fn",
        lambda *_args, **_kwargs: {"release_group": "HD.MA.5.1"},
    )
    assert asyncio.run(tags.get_tag("DVD Folder", _meta(is_disc="DVD"))) == ""
    monkeypatch.setattr(
        tags,
        "guessit_fn",
        lambda *_args, **_kwargs: {"release_group": "UNTOUCHED"},
    )
    assert asyncio.run(tags.get_tag("DVD Folder", _meta(is_disc="DVD"))) == ""


def test_tag_override_in_name_type_personal_template_and_arbitrary_values(
    tmp_path: Path,
) -> None:
    data = tmp_path / "data"
    data.mkdir()
    ...
    meta = _meta(
        base_dir=str(tmp_path), path="Movie.mkv", tag="-GROUP", type="WEBDL"
    )
    result = asyncio.run(tags.tag_override(meta))
    assert result.type == "WEBDL"


def test_tag_override_missing_malformed_and_boolean_values(
    tmp_path: Path,
) -> None:
    ...
    (data / "tags.json").write_text(
        json.dumps({"BAD": None}), encoding="utf-8"
    )
    assert asyncio.run(tags.tag_override(meta)) is meta
