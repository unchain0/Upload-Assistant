...
def check_subtitles(tmp_path: Path, *names: str) -> bool:
    meta = SimpleNamespace(
        subtitle_files=[str(tmp_path / name) for name in names]
    )
...
def test_detects_portuguese_from_text_when_filename_is_ambiguous(
    tmp_path: Path,
):
    subtitle = tmp_path / "movie.external.srt"
    subtitle.write_text(
        "Olá, você está aqui? Vamos para casa.", encoding="utf-8"
    )
...
def test_does_not_accept_unidentified_text_subtitle(tmp_path: Path):
    subtitle = tmp_path / "movie.external.srt"
    subtitle.write_text(
        "This is a subtitle. Where are you going?", encoding="utf-8"
    )
