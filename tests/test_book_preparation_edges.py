...
def _book_file(
    tmp_path: Path, name: str = "Alice Writer - Example Book.m4b"
) -> Path:
    path = tmp_path / name
...
def _patch_external_lookups(
    monkeypatch: pytest.MonkeyPatch, *, mam=None, google=None, openlibrary=None
) -> None:
    monkeypatch.setattr(
...
def test_media_info_helpers_cover_case_lists_and_unescaping() -> None:
    ...
    assert (
        book_preparation._mi_extra({"extra": {"series": "  Saga  "}}, "SERIES")
        == "Saga"
    )
    assert (
        book_preparation._mi_extra(
            {"extra": {"series": {"bad": True}}}, "SERIES"
        )
        == ""
    )
    assert (
        book_preparation._mi_extra({"extra": {"series": "  "}}, "SERIES") == ""
    )
...
def test_epub_content_identifier_limits_errors_and_values(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    bad = tmp_path / "bad.epub"
    bad.write_bytes(b"not zip")
    assert book_preparation._epub_content_identifiers(str(bad)) == (
        set(),
        set(),
    )
    ...
    assert book_preparation._epub_content_identifiers(str(rich)) == (
        {"9780306406157"},
        {"B012345678"},
    )
    ...
    assert book_preparation._epub_content_identifiers(str(many)) == (
        set(),
        set(),
    )
    ...
    assert book_preparation._epub_content_identifiers(str(huge)) == (
        set(),
        set(),
    )
    ...
    monkeypatch.setattr(
        book_preparation, "_safe_zip_member_bytes", lambda *_args: None
    )
    assert book_preparation._epub_content_identifiers(str(safe)) == (
        set(),
        set(),
    )


def test_reconcile_epub_identifiers_priorities_and_conflicts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        book_preparation, "_validated_isbns", lambda _value: {"9780306406157"}
    )
    monkeypatch.setattr(
        book_preparation,
        "_epub_content_identifiers",
        lambda _path: ({"9781861972712"}, {"B012345678"}),
    )
    ...
    monkeypatch.setattr(
        book_preparation,
        "_validated_isbns",
        lambda _value: {"9780306406157", "9781861972712"},
    )
    ...
    monkeypatch.setattr(
        book_preparation, "_validated_isbns", lambda _value: set()
    )
    monkeypatch.setattr(
        book_preparation,
        "_epub_content_identifiers",
        lambda _path: ({"9780306406157", "9781861972712"}, set()),
    )
    with pytest.raises(ItemProcessingError, match="Conflicting EPUB ISBNs"):
        book_preparation._reconcile_epub_identifiers(meta, {}, str(path))
    ...
    monkeypatch.setattr(
        book_preparation,
        "_epub_content_identifiers",
        lambda _path: ({"9780306406157"}, set()),
    )
    metadata = {}
    book_preparation._reconcile_epub_identifiers(meta, metadata, str(path))
    assert metadata["isbn"] == "9780306406157"
...
def test_rich_embedded_mediainfo_populates_audiobook_fields(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(path), str(tmp_path), {"DEFAULT": {}}
        )
    )
...
def test_embedded_metadata_uses_fallback_author_description_asin_and_audio_language(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_embedded_metadata_text_language_and_error_are_safe(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_source_extractors_cli_overrides_and_invalid_values(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    for suffix, attribute, payload in (
        ...
        (
            ".cbz",
            "_extract_cbr_cbz_metadata",
            {"title": "Comic title", "author": "Comic Author"},
        ),
        (
            ".mobi",
            "_extract_mobi_metadata",
            {"title": "MOBI title", "author": "MOBI Author"},
        ),
    ):
        path = _book_file(tmp_path, f"book{suffix}")
        monkeypatch.setattr(
            book_preparation, attribute, lambda _path, value=payload: value
        )
        if suffix == ".epub":
            monkeypatch.setattr(
                book_preparation,
                "_get_epubmeta_output",
                lambda _path: "epubmeta",
            )
            monkeypatch.setattr(
                book_preparation,
                "_reconcile_epub_identifiers",
                lambda *_args: None,
            )
        meta = _meta(
            tmp_path,
    ...
    monkeypatch.setattr(
        book_preparation, "_extract_pdf_page_count", lambda _path: 123
    )
    monkeypatch.setattr(
        book_preparation,
        "_extract_isbn_from_pdf",
        lambda _path: "9780306406157",
    )
    meta = _meta(tmp_path, pdf, mediainfo={})
    _patch_external_lookups(monkeypatch)
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(pdf), str(tmp_path), {"DEFAULT": {}}
        )
    )
    assert meta.page_count == 123 and meta.isbn == "9780306406157"


def test_media_info_export_errors_are_normalized(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = _book_file(tmp_path)
    _patch_external_lookups(monkeypatch)
...
def test_mam_host_comment_google_openlibrary_and_lookup_failures(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    google_data = {
        ...
        "artwork_url": "https://cover.invalid/google.jpg",
        "isbn": "9780306406157",
    }
    open_data = {
        "title": "Open Title",
        "overview": "Publisher: Open Press",
        "isbn": "9780306406157",
    }
    _patch_external_lookups(
        monkeypatch, mam=mam_data, google=google_data, openlibrary=open_data
    )
        ...
        torrent_comments=[
            {
                ...
                "comment": "MID=1",
                "tracker_urls": [
                    {"url": "https://foo.myanonamouse.net/announce"}
                ],
            },
        ],
    )
    asyncio.run(
        book_preparation.gather_book_prep(
            ...
            str(path),
            str(tmp_path),
            {"DEFAULT": {"mam_api_key": "key", "google_books_api_key": "key"}},
        )
    )
    ...
    meta = _meta(
        ...
        isbn="9780306406157",
        torrent_comments=[
            {"trackers": "myanonamouse.net", "comment": "MID=2"}
        ],
    )
    asyncio.run(
        book_preparation.gather_book_prep(
...
def test_openlibrary_work_lookup_publisher_inference_and_invalid_isbn(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    _patch_external_lookups(
        ...
        openlibrary={
            ...
            "year": 2022,
            "search_year": 2022,
            "overview": "Publisher: Inferred Press",
        },
    )
    monkeypatch.setattr(
        book_preparation, "_get_epubmeta_output", lambda _path: ""
    )
    monkeypatch.setattr(
        book_preparation, "_extract_epub_metadata", lambda _path: {}
    )
    ...
    meta = _meta(
        tmp_path, path, mediainfo={}, isbn="invalid", book_isbn="invalid"
    )
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(path), str(tmp_path), {"DEFAULT": {}}
...
def test_audiobook_duration_and_bitrate_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    class Track:
        def __init__(
            self, track_type: str, *, duration=None, data=None
        ) -> None:
    ...
    total, formatted = asyncio.run(
        book_preparation.get_audiobook_duration(
            [*(str(path) for path in files[:4]), str(missing), "not-audio.txt"]
        )
    )
    assert total == 3825.0
    assert formatted == "01h 03m 45s"
    assert asyncio.run(book_preparation.get_audiobook_duration([])) == (
        0.0,
        "",
    )
    ...
    assert (
        asyncio.run(
            book_preparation.get_audiobook_bitrate(
    ...
    monkeypatch.setattr(
        book_preparation.MediaInfo,
        "parse",
        lambda _path: SimpleNamespace(tracks=[]),
    )
    assert (
        asyncio.run(book_preparation.get_audiobook_bitrate([str(files[0])]))
        is None
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _path: SimpleNamespace(
            tracks=[Track("Audio", data={"bit_rate": "999"})]
        ),
    )
    assert (
        asyncio.run(book_preparation.get_audiobook_bitrate([str(files[0])]))
...
def test_missing_fields_and_embedded_extra_isbn_existing_keywords_and_parser_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    meta = _meta(
        tmp_path, path, mediainfo={"media": {"track": [BrokenTrack()]}}
    )
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(path), str(tmp_path), {"DEFAULT": {}}
...
def test_filename_series_and_client_lookup_success_and_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = _book_file(
        tmp_path, "Alice Writer - Example Series #2 - Example Book.m4b"
    )
    _patch_external_lookups(monkeypatch)
    monkeypatch.setattr(
        book_preparation, "export_info", AsyncMock(return_value={})
    )
...
def test_mam_host_edge_values_and_year_without_search_year(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = _book_file(tmp_path)
    _patch_external_lookups(
        monkeypatch, mam={"year": 2023, "title": "MAM Title"}
    )
    comments = [
        ...
        {"trackers": "http://", "comment": "MID=0"},
        {
            "trackers": "not-a-host",
            "comment": "MID=0",
            "tracker_urls": ["https://myanonamouse.net/announce"],
        },
    ]
    meta = _meta(tmp_path, path, mediainfo={}, torrent_comments=comments)
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(path), str(tmp_path), {"DEFAULT": {"mam_id": "key"}}
        )
    )
    assert meta.year == 2023 and meta.search_year == 2023


def test_google_year_fallback_exception_openlibrary_override_and_year(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = _book_file(tmp_path)
    _patch_external_lookups(
        monkeypatch,
        google={"year": 2024, "title": "Google Title"},
        openlibrary=None,
    )
    ...
    asyncio.run(
        book_preparation.gather_book_prep(
            ...
            str(path),
            str(tmp_path),
            {"DEFAULT": {"google_books_api_key": "key"}},
        )
    )
    assert meta.year == 2024 and meta.search_year == 2024
    ...
    _patch_external_lookups(
        monkeypatch, google={"title": "Google Title"}, openlibrary=None
    )
    meta = _meta(
        tmp_path,
        path,
    ...
    _patch_external_lookups(
        monkeypatch,
        google=None,
        openlibrary={"year": 2025, "title": "Open Title"},
    )
    meta = _meta(
        tmp_path,
        path,
...
def test_exact_edition_filename_title_swap_and_unattended_conflict(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    meta = _meta(
        tmp_path, path, mediainfo={}, isbn="9780306406157", title="", author=""
    )
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(path), str(tmp_path), {"DEFAULT": {}}
    ...
    asyncio.run(
        book_preparation.gather_book_prep(
            meta, str(swapped), str(tmp_path), {"DEFAULT": {}}
        )
    )
    ...
    with pytest.raises(ItemProcessingError, match="conflicts"):
        asyncio.run(
            book_preparation.gather_book_prep(
                meta, str(conflict), str(tmp_path), {"DEFAULT": {}}
            )
        )


def test_bitrate_missing_file_is_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    missing = tmp_path / "missing.mp3"
    monkeypatch.setattr(
        ...
        lambda _path: (_ for _ in ()).throw(
            AssertionError("should not parse missing file")
        ),
    )
    assert (
        asyncio.run(book_preparation.get_audiobook_bitrate([str(missing)]))
        is None
    )
