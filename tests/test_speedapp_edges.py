...
def _config(*, channel: object = "", use_metadata_name: bool = False) -> dict[str, Any]:
    return {
        "DEFAULT": {},
        "TRACKERS": {
            "SPEEDAPP": {
...
def _response(payload: Any, *, status: int = 200, method: str = "GET", url: str = "https://speedapp.io/api/channel") -> httpx.Response:
...
def test_speedapp_special_categories_and_direct_channel() -> None:
    ...
    assert asyncio.run(tracker.get_cat_id(_meta(category="TV", origin_country=["RO"]))) == 60
    assert asyncio.run(tracker.get_cat_id(_meta(genres=["Documentary"], audio_languages=["Romanian"]))) == 63
...
async def test_speedapp_channel_lookup_match_and_no_match() -> None:
    tracker = _tracker()
    tracker.session.get = AsyncMock(return_value=_response([{"id": 7, "tag": "release"}]))  # type: ignore[method-assign]
    assert await tracker._lookup_channel("release") == 7

    tracker.session.get = AsyncMock(return_value=_response([{"id": 8, "tag": "other"}]))  # type: ignore[method-assign]
...
async def test_speedapp_metadata_name_and_existing_nfo(tmp_path: Path) -> None:
    ...
    encoded = await tracker.get_nfo(_meta(base_dir=str(tmp_path), uuid="release"))
    assert encoded is not None


def test_speedapp_requirements_join_and_strip_bbcode() -> None:
    meta = _meta(requirements_minimum="<b>Minimum</b>", requirements_recommended="<i>Recommended</i>")
...
@pytest.mark.asyncio
async def test_speedapp_game_requirements_and_debug_nfo(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "get_requirements", lambda _meta: "Requirements")
    data: dict[str, Any] = {}
    await tracker._apply_category_upload_data(data, _meta(category="GAME", console_game=False))
...
@pytest.mark.asyncio
async def test_speedapp_upload_skips_missing_channel(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "fetch_data", AsyncMock(return_value={}))
    monkeypatch.setattr(tracker, "search_channel", AsyncMock(return_value=None))
    meta = _meta()
...
@pytest.mark.asyncio
async def test_speedapp_upload_release_exception_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
async def test_speedapp_upload_response_success_and_failures() -> None:
    ...
    status: dict[str, Any] = {}
    assert not await tracker._handle_upload_response(_meta(), status, _response({"status": False, "error": True}, method="POST"))

    status = {}
    assert not await tracker._handle_upload_response(_meta(), status, _response({"status": True, "error": False}, method="POST"))

    status = {}
    payload = {"status": True, "error": False, "downloadUrl": "present", "torrent": {"id": 99}}
    assert await tracker._handle_upload_response(_meta(), status, _response(payload, method="POST"))
