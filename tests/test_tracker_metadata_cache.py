...
def test_explicit_tracker_id_reuses_cached_metadata(tmp_path):
    async def run():
        ...
        first_meta = Meta(
            {
                ...
                "tracker_ids": {"PASSTHEPOPCORN": "12345"},
            }
        )
        (
            _,
            first_match,
        ) = await first_manager.update_metadata_from_explicit_tracker(
            "PASSTHEPOPCORN", object(), first_meta, "Amour", "Amour", False
        )
        ...
        (
            _,
            second_match,
        ) = await second_manager.update_metadata_from_explicit_tracker(
            "PASSTHEPOPCORN", object(), second_meta, "Amour", "Amour", False
        )
