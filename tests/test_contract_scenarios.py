...
def _aliased_release_rule(
    meta: Meta, config: dict[str, Any], choice: str = ""
) -> bool:
    ...
    return (
        category == "MOVIE"
        and minimum >= 3
        and normalized_choice in {"yes", "y"}
    )


def test_literal_branch_scenarios_resolve_local_aliases_and_nested_mappings() -> (
    None
):
    scenarios = literal_branch_scenarios(
        _aliased_release_rule, Meta.__dataclass_fields__
    )

    assert any(meta.get("category") == "MOVIE" for meta, _params in scenarios)
    assert any(
        params.get("choice") in {"yes", "y"} for _meta, params in scenarios
    )
    assert any(
        isinstance(params.get("config"), dict)
        and isinstance(params["config"].get("DEFAULT"), dict)
        and params["config"]["DEFAULT"].get("minimum") in {2, 4}
