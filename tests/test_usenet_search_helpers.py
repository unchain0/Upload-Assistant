...
def test_newznab_category_quality_and_book_branches() -> None:
    assert (
        helpers.get_newznab_search_category_id(
            Meta(category="MOVIE", resolution="2160p")
        )
        == "2045"
    )
    assert (
        helpers.get_newznab_search_category_id(
            Meta(category="MOVIE", resolution="480p")
        )
        == "2030"
    )
    assert (
        helpers.get_newznab_search_category_id(
            Meta(category="TV", resolution="2160p")
        )
        == "5045"
    )
    assert (
        helpers.get_newznab_search_category_id(
            Meta(category="BOOK", audiobook=True)
        )
        == "3030"
    )
    assert (
        helpers.get_newznab_search_category_id(Meta(category="GAME")) == "4050"
    )
    assert (
        helpers.get_newznab_search_category_id(Meta(category="MUSIC"))
        == "3000"
    )
    assert (
        helpers.get_newznab_search_category_id(Meta(category="OTHER"))
        == "2000"
    )
...
def test_newznab_search_query_branches() -> None:
    assert (
        helpers.build_newznab_search_query(
            Meta(category="TV", title="Show", season_int=1, episode_int=2)
        )
        == "Show S01E02"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="TV", title="Show", season_int=1, episode_int=0)
        )
        == "Show S01"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="TV", title="Show", season_int=0, episode_int=0)
        )
        == "Show"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="TV", title="", basename_no_ext="Fallback")
        )
        == "Fallback"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="MOVIE", title="Movie", year=2026)
        )
        == "Movie 2026"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="MOVIE", title="Movie", year="bad")
        )
        == "Movie"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="MOVIE", title="", basename_no_ext="Movie.Fallback")
        )
        == "Movie.Fallback"
    )
    assert (
        helpers.build_newznab_search_query(
            Meta(category="BOOK", title="Book", basename_no_ext="Book.File")
        )
        == "Book.File"
    )
...
def test_parse_newznab_dupes_attributes_and_links() -> None:
    ...
    """
    dupes = helpers.parse_newznab_dupes(
        xml, "https://indexer.invalid/details/", use_guid_attr_as_id=True
    )
    assert dupes[0] == {
        "name": "Release One",
        "files": "Release One",
        "size": 200,
        "link": "https://indexer.invalid/details/abc123",
    }
...
def test_newznab_attribute_and_link_helpers() -> None:
    item = ElementTree.fromstring(
        '<item xmlns:newznab="http://www.newznab.com/DTD/2010/feeds/attributes/"><newznab:attr name="other" value="x"/></item>'
    )
    attr = item[0]
    assert helpers._apply_newznab_attribute(
        attr, "guid", "1", use_guid_attr_as_id=True
    ) == ("guid", "1")
    assert not helpers._should_use_guid_attribute("guid", "x", "", False)
    assert helpers._needs_torrent_url_prefix("", "guid", "https://x/") is False
    assert (
        helpers._needs_torrent_url_prefix("https://x/1", "guid", "https://x/")
        is False
    )
...
def test_daily_limit_and_counter_filename_guards(tmp_path: Path) -> None:
    ...
    assert (
        helpers._get_api_hit_counter_path(str(tmp_path), "NZB Geek").name
        == "nzb_geek.json"
    )
    assert (
        helpers._get_api_hit_counter_lock_path(
            str(tmp_path), "NZB Geek"
        ).suffix
        == ".lock"
    )
...
def test_lock_and_unlock_msvcrt_branch(
...
def test_lock_and_unlock_fcntl_branch(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    fake = _FakeFcntl()
...
def test_lock_raises_without_platform_backend(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(helpers, "msvcrt", None)
    monkeypatch.setattr(helpers, "fcntl", None)
    with (
        (tmp_path / "lock").open("a+b") as lock_file,
        pytest.raises(RuntimeError, match="No supported file locking"),
    ):
        helpers._lock_api_hit_counter_file(lock_file)
...
def test_acquire_lock_timeout(
    ...
    monkeypatch.setattr(
        helpers,
        "_lock_api_hit_counter_file",
        lambda _file: (_ for _ in ()).throw(BlockingIOError()),
    )
    times = iter((0.0, 11.0))
...
def test_release_lock_closes_file(
    ...
    monkeypatch.setattr(
        helpers, "_unlock_api_hit_counter_file", lambda _file: None
    )
...
def test_api_hit_cache_write_load_and_recent_filters(tmp_path: Path) -> None:
    ...
    now = time.time()
    recent = helpers._recent_api_hits(
        [now, now - helpers.API_HIT_WINDOW_SECONDS - 1, "bad", True], now
    )
...
def test_reserve_daily_api_hit_sync_success_and_limit(tmp_path: Path) -> None:
    allowed, used = helpers._reserve_daily_api_hit_sync(
        str(tmp_path), "NZBGEEK", 1
    )
...
async def test_reserve_daily_api_hit_async_wrapper(tmp_path: Path) -> None:
    allowed, used = await helpers.reserve_daily_api_hit(
        str(tmp_path), "DRUNKENSLUG", 2
    )
    assert allowed and used == 1
...
def test_import_fcntl_fallback_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        "_search_helpers_without_fcntl", source_path
    )
...
def test_acquire_lock_retries_before_success(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    calls = 0
