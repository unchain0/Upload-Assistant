...
class _FakeResponse:
    def __init__(
        self, status_code: int, payload: object, text: str = ""
    ) -> None:
    ...
    *,
    ...
    async def exercise() -> dict[str, Any]:
        return await upload_image_task(
            (str(image), host, {"DEFAULT": defaults}, Meta(category=category))
        )

    with patch(
        "src.integrations.image_hosts.uploader.httpx.AsyncClient",
        return_value=_FakeClient(response, requests),
    ):
...
def test_image_host_registry_covers_every_configurable_host() -> None:
    ...
    assert all(
        spec.documentation_url or not spec.public_api
        for spec in IMAGE_HOST_SPECS.values()
    )
...
def test_imgbb_uses_official_multipart_v1_contract(tmp_path: Path) -> None:
    ...
    result, requests = _upload(
        tmp_path,
        host="imgbb",
        defaults={"imgbb_api": "secret-key"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "passtheimage",
            "passtheima_ge_api",
            "https://passtheima.ge/api/1/upload",
        ),
    ],
)
def test_chevereto_hosts_use_source_and_header_auth(
    tmp_path: Path, host: str, api_key_name: str, endpoint: str
) -> None:
    payload = {
        "status_code": 200,
    ...
    result, requests = _upload(
        tmp_path,
        host=host,
        defaults={api_key_name: "host-key"},
        response=_FakeResponse(200, payload),
    )
    ...
    assert requests[0][1]["files"] == {
        "source": ("screen.png", b"image-bytes")
    }
...
def test_chevereto_xxx_upload_marks_nsfw(tmp_path: Path) -> None:
    payload = {
        "status_code": 200,
        "image": {
            "url": "https://host/raw.png",
            "url_viewer": "https://host/view",
            "thumb": {"url": "https://host/thumb.png"},
        },
    }
...
def test_chevereto_error_message_is_preserved_without_html_noise(
    tmp_path: Path,
) -> None:
    payload = {
        "status_code": 400,
...
def test_pixhost_v2_uses_accept_header_and_nsfw_content_type(
    tmp_path: Path,
) -> None:
    payload = {
        "name": "screen.png",
...
def test_pixhost_raw_url_falls_back_when_thumbnail_shape_is_unknown() -> None:
    assert (
        _pixhost_raw_url("https://cdn.example/thumb.png")
        == "https://cdn.example/thumb.png"
    )


def test_imgbox_failure_is_marked_unavailable_for_fast_fallback(
    tmp_path: Path,
) -> None:
    image = tmp_path / "screen.png"
    ...
    async def exercise() -> dict[str, Any]:
        return await upload_image_task(
            (str(image), "imgbox", {"DEFAULT": {}}, Meta(category="MOVIE"))
        )

    async def fake_imgbox(
        *_args: object, return_dict: dict[str, Any], **_kwargs: object
    ) -> list[dict[str, str]]:
    ...
    with patch(
        "src.integrations.image_hosts.uploader.imgbox_upload",
        new=AsyncMock(side_effect=fake_imgbox),
    ):
        result = asyncio.run(exercise())
