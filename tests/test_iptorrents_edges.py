...
def _config(*, anon: bool = False, force_data: bool = False) -> dict[str, Any]:
    return {"DEFAULT": {}, "TRACKERS": {"IPTORRENTS": {"anon": anon, "force_data": force_data}}}
...
def _meta(**values: object) -> Meta:
    state: dict[str, object] = {
        ...
        "debug": False,
        "tmdb_id": 123,
        "tracker_status": {"IPTORRENTS": {"torrent_id": "42", "status_message": "ok"}},
        "ua_signature": "UA",
    }
    state.update(values)
    return Meta(state)
...
def _response(text: str, *, url: str = "https://iptorrents.com/t?72=&q=Example") -> httpx.Response:
...
async def test_iptorrents_search_none_and_login_paths() -> None:
    ...
    tracker.session.get = AsyncMock(return_value=_response("login", url="https://iptorrents.com/login.php"))  # type: ignore[method-assign]
    meta = _meta()
...
async def test_iptorrents_search_parses_and_filters_rows() -> None:
    ...
    assert result == [{"name": "Example Movie 1080p WEB-DL", "size": "8.5 GB", "link": "https://iptorrents.com/t/1"}]


def test_iptorrents_search_parser_guards() -> None:
    assert IPTorrents._parse_search_results("<html></html>", []) == []
    assert IPTorrents._parse_search_results("<table id='torrents'></table>", []) == []


def test_iptorrents_category_edge_paths() -> None:
    tracker = _tracker()
    assert tracker.get_category_id(_meta(category="UNKNOWN")) == 0
    assert tracker._movie_secondary_special(_meta(type="BDRIP", source="WEB")) == 90
    assert tracker._movie_secondary_special(_meta(type="OTHER", resolution="480p", source="WEB")) == 77
    assert tracker._movie_secondary_special(_meta(type="OTHER", resolution="1080p", source="CAM")) == 96
    assert tracker._movie_language_category(_meta(genres=["Family"])) == 54
    assert tracker._movie_language_category(_meta(genres=[], original_language="fr")) == 38
    assert tracker._book_special_category(_meta(category="BOOK", comic=True)) == 94
    assert tracker._book_special_category(_meta(category="BOOK", magazine=True)) == 92


def test_iptorrents_torrent_multifile_size() -> None:
    payload = bencodepy.encode({b"info": {b"files": [{b"length": 5}, {b"length": 7}]}})
    assert IPTorrents._torrent_size_bytes(payload) == 12


@pytest.mark.asyncio
async def test_iptorrents_freeleech_missing_invalid_and_large_torrent(tmp_path: Path) -> None:
    tracker = _tracker()
    ...
    (root / "BASE.torrent").write_bytes(bencodepy.encode({b"info": {b"length": 9 * 1024**3}}))
    assert await tracker.get_is_freeleech(meta)


@pytest.mark.asyncio
async def test_iptorrents_get_data_freeleech_and_anonymous(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker(anon=True)
    monkeypatch.setattr(tracker, "generate_description", AsyncMock(return_value="description"))
    monkeypatch.setattr(tracker, "get_is_freeleech", AsyncMock(return_value=True))
...
@pytest.mark.asyncio
async def test_iptorrents_upload_force_data(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker(force_data=True)
    cookies = httpx.Cookies()
    cookies.set("sid", "value")
    tracker.cookie_validator.load_session_cookies = AsyncMock(return_value=cookies)  # type: ignore[method-assign]
    monkeypatch.setattr(tracker, "get_data", AsyncMock(return_value={"type": 72}))
...
@pytest.mark.asyncio
async def test_iptorrents_edit_post_upload_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "generate_description", AsyncMock(return_value="description"))
    tracker.session.post = AsyncMock(return_value=httpx.Response(200, request=httpx.Request("POST", "https://iptorrents.com")))  # type: ignore[method-assign]
    meta = _meta()
    await tracker.edit_post_upload(meta)
    assert meta.tracker_status["IPTORRENTS"]["status_message"].endswith("Failed to edit torrent.")


def test_iptorrents_search_row_without_link_returns_none() -> None:
    soup = __import__("bs4").BeautifulSoup("<tr><td>0</td><td>name only</td><td>2</td><td>3</td><td>4</td><td>1 GB</td></tr>", "html.parser")
    row = soup.find("tr")
