...
def test_infinityhd_nested_language_and_lookup_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    assert (
        tracker._language_value({"Language": {"String": "English"}})
        == "English"
    )
    monkeypatch.setattr(
        infinity_module.pycountry.languages,
        "get",
        lambda **_kwargs: (_ for _ in ()).throw(AttributeError("bad")),
    )
...
def test_infinityhd_original_language_match_and_commentary_rejection() -> None:
    ...
    assert not tracker._track_matches_original(
        {"@type": "Audio", "Language": "English", "Title": "Commentary"},
        {"en"},
    )


def test_infinityhd_foreign_name_prefix(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        infinity_module.languages_manager,
    ...
    assert asyncio.run(tracker.get_name(meta)) == {
        "name": "Movie FRENCH 1080p WEB-DL"
    }


def test_infinityhd_non_disc_language_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
...
def test_infinityhd_non_disc_english_audio_passes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(
        infinity_module.languages_manager, "has_english_language", has_english
    )
    tracker.common.check_and_confirm_adult_media_upload = AsyncMock(
        return_value=True
    )  # type: ignore[method-assign]
...
def test_infinityhd_subtitle_fallback_passes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
...
def test_infinityhd_original_language_short_circuits_external_checks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
    ...
    meta = Meta(
        original_language=["en"],
        mediainfo={
            "media": {"track": [{"@type": "Audio", "Language": "English"}]}
        },
        audio_languages=[],
        subtitle_languages=[],
    )
