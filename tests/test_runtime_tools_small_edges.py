...
def test_ffmpeg_docker_platform_missing_success_and_main(
    ...
    assert (
        ffmpeg_docker.FfmpegBinaryManager.download_ffmpeg_for_docker("ignored")
        == "/usr/bin/ffmpeg"
    )
...
def test_pesto_unsupported_cached_linux_windows_success_and_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        pesto, "tool_install_dir", lambda *_args: _pesto_paths(tmp_path)[0]
    )
    ...
    async def download(
        _client: object, _url: str, target: Path, _label: str
    ) -> None:
...
def test_zentag_unsupported_and_cached(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    marker.write_text(
        zentag_binary.ZentagBinaryManager.VERSION, encoding="utf-8"
    )
    ...
    monkeypatch.setitem(
        zentag_binary.ZentagBinaryManager.BINARY_CHECKSUMS,
        asset,
        hashlib.sha256(payload).hexdigest(),
    )
    assert asyncio.run(
        zentag_binary.ZentagBinaryManager.ensure_binary(tmp_path)
    ) == str(binary)


def test_zentag_zip_and_tar_install_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async def install(
        system: str, machine: str, root: Path, binary_name: str, archive_writer
    ) -> str:
        root.mkdir()
        monkeypatch.setattr(
            zentag_binary, "tool_install_dir", lambda *_args: root
        )
        ...
        monkeypatch.setitem(
            zentag_binary.ZentagBinaryManager.CHECKSUMS, asset, archive_hash
        )
        monkeypatch.setitem(
            zentag_binary.ZentagBinaryManager.BINARY_CHECKSUMS,
            asset,
        ...
        result = await zentag_binary.ZentagBinaryManager.ensure_binary(
            tmp_path
        )
    ...
    assert asyncio.run(
        install("Windows", "AMD64", windows, "zentag.exe", _zip_payload)
    ) == str(windows / "zentag.exe")
    ...
    linux.rmdir()
    result = asyncio.run(
        install("Linux", "x86_64", linux, "zentag", _tar_payload)
    )
    assert result == str(linux / "zentag")
    assert bool((linux / "zentag").stat().st_mode & stat.S_IEXEC)


def test_zentag_archive_checksum_binary_checksum_missing_and_size_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async def write_bytes(payload: bytes, _name: str = "zentag.exe"):
        ...
        monkeypatch.setattr(zentag_binary, "download_bounded_asset", download)
        monkeypatch.setitem(
            zentag_binary.ZentagBinaryManager.CHECKSUMS,
            asset,
    ...
    monkeypatch.setitem(
        zentag_binary.ZentagBinaryManager.CHECKSUMS, asset, "bad"
    )
    ...
    monkeypatch.setitem(
        zentag_binary.ZentagBinaryManager.BINARY_CHECKSUMS, asset, "bad"
    )
    with pytest.raises(
...
def test_zentag_tar_missing_member_size_and_stream_limit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_dvd_filename_url_download_extract_errors_and_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    assert (
        dvd_media_info.get_filename("windows", "x86_64")
        == "MediaInfo_CLI_23.04_Windows_x64.zip"
    )
    assert dvd_media_info.get_filename("windows", "arm64") == ""
    assert "Lambda_x86_64" in dvd_media_info.get_filename(
        "linux", "x86_64", "cli"
    )
    assert "MediaInfo_DLL" in dvd_media_info.get_filename(
        "linux", "arm64", "lib"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda url, output, label: called.append((url, output, label)),
    )
...
def test_download_dvd_mediainfo_platform_cache_success_and_missing_outputs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    (win_dir / f"version_{dvd_media_info.MEDIAINFO_VERSION}").write_text(
        "version", encoding="utf-8"
    )
    monkeypatch.setattr(dvd_media_info.platform, "machine", lambda: "AMD64")
    assert dvd_media_info.download_dvd_mediainfo(str(windows_root)) == str(
        win_cli
    )
    ...
    assert dvd_media_info.download_dvd_mediainfo(
        str(windows_install)
    ).endswith("MediaInfo.exe")

    monkeypatch.setattr(dvd_media_info, "extract_windows", lambda *_args: None)
    with pytest.raises(RuntimeError, match="Failed to extract"):
        dvd_media_info.download_dvd_mediainfo(
            str(tmp_path / "windows-missing")
        )
    ...
    (linux_dir / f"version_{dvd_media_info.MEDIAINFO_VERSION}").write_text(
        "version", encoding="utf-8"
    )
    monkeypatch.setattr(dvd_media_info.platform, "system", lambda: "Linux")
    monkeypatch.setattr(dvd_media_info.platform, "machine", lambda: "x86_64")
    assert dvd_media_info.download_dvd_mediainfo(str(linux_cache)) == str(
        linux_cli
    )
    ...
    assert dvd_media_info.download_dvd_mediainfo(str(linux_install)).endswith(
        "mediainfo"
    )
...
def test_zentag_tar_stream_can_exceed_declared_member_size(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        zentag_binary.tarfile, "open", lambda *_args, **_kwargs: Archive()
    )
    with pytest.raises(RuntimeError, match="exceeds"):
        asyncio.run(zentag_binary.ZentagBinaryManager.ensure_binary(tmp_path))
