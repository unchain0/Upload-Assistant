...
def test_sanitize_book_author_empty_fallback_manual_and_detected_translator() -> (
    None
):
...
def test_clean_translator_patterns_underscores_fallbacks_and_deduplication() -> (
    None
):
    ...
    author, translator = book_input_service.clean_translator_from_author(
        "Jane_Doe_John_Smith_tradutor"
    )
    assert author == "Jane_Doe"
    assert translator == "John Smith"

    author, translator = book_input_service.clean_translator_from_author(
        "Jane Doe translated by John Smith"
    )
    ...
    author, translator = book_input_service.clean_translator_from_author(
        "Jane Doe - John Smith translator; John Smith translated by"
    )
