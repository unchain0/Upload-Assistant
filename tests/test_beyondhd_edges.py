...
def _response(
    ...
    *,
    status: int = 200,
    method: str = "POST",
    url: str = "https://beyond-hd.me/api/upload",
) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_beyondhd_upload_success_delegates_seed(
    ...
    monkeypatch.setattr(
        tracker,
        "_post_upload",
        AsyncMock(return_value="https://beyond-hd.me/details/77"),
    )
    monkeypatch.setattr(
        tracker, "_seed_uploaded_torrent", AsyncMock(return_value=True)
    )
    item = _meta()
    assert await tracker.upload(item)
    tracker._seed_uploaded_torrent.assert_awaited_once_with(
        item, "https://beyond-hd.me/details/77"
    )
...
async def test_beyondhd_debug_upload() -> None:
    ...
    assert item.tracker_status["BEYONDHD"]["status_message"].startswith(
        "Debug mode"
    )
...
@pytest.mark.asyncio
async def test_beyondhd_post_upload_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        tracker, "_submit_request", AsyncMock(side_effect=RuntimeError("boom"))
    )
    item = _meta()
...
@pytest.mark.asyncio
async def test_beyondhd_invalid_imdb_retry_and_invalid_name(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    retry = _response(
        {
            "status_code": 1,
            "status_message": "https://beyond-hd.me/torrent/download/release.55.torrent",
        }
    )
    monkeypatch.setattr(
        tracker, "_submit_request", AsyncMock(return_value=retry)
    )
    data = {"imdb_id": 123, "name": "Release"}
    payload = {"status_code": 0, "status_message": "Invalid imdb_id 123"}
    result = await tracker._maybe_retry_invalid_imdb(
        _meta(), data, {}, payload
    )
    assert result["status_code"] == 1
    assert data["imdb_id"] == 1
    tracker._log_invalid_name(
        {"status_code": 0, "status_message": "Invalid name value"}, "Release"
    )
...
async def test_beyondhd_details_link_paths() -> None:
    ...
    item = _meta()
    assert (
        tracker._details_link_from_payload(
            item, {"status_message": "uploaded but no download"}
        )
        == ""
    )
    assert (
        "No valid details link"
        in item.tracker_status["BEYONDHD"]["status_message"]
    )

    item = _meta()
    payload = {
        "status_message": "https://beyond-hd.me/torrent/download/release.88.torrent"
    }
    assert (
        tracker._details_link_from_payload(item, payload)
        == "https://beyond-hd.me/details/88"
    )
...
async def test_beyondhd_seed_uploaded_torrent_failure() -> None:
    tracker = _tracker()
    tracker.common.create_torrent_ready_to_seed = AsyncMock(
        side_effect=RuntimeError("seed failure")
    )  # type: ignore[method-assign]
    assert not await tracker._seed_uploaded_torrent(
        _meta(), "https://beyond-hd.me/details/1"
    )


@pytest.mark.asyncio
async def test_beyondhd_description_content_exercises_disc_comparison_header_and_screens(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(
        ...
        "base_description",
        lambda _meta: (
            "before https://old.invalid/a.png after [img]cover[/img]"
        ),
    )
    ...
    monkeypatch.setattr(
        bhd_module, "get_tracker_image_collection", image_collection
    )
    item = _meta(
...
def test_beyondhd_description_helper_guards(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    item = _meta(menu_images="bad")
    monkeypatch.setattr(
        bhd_module, "get_tracker_image_collection", lambda *_args: "bad"
    )
    assert (
        tracker._replace_rehosted_collection(item, "text", "menu_images")
        == "text"
    )
...
@pytest.mark.asyncio
async def test_beyondhd_additional_check_failure_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    internal = _meta(
        name="Movie.1080p-FraMeSToR", unattended=True, unattended_confirm=False
    )
    assert not await tracker.get_additional_checks(internal)

    assert not await tracker.get_additional_checks(
        _meta(valid_mi_settings=False)
    )
    assert not await tracker.get_additional_checks(
        _meta(type="REMUX", container="avi")
    )
    assert not await tracker.get_additional_checks(
        _meta(
            type="ENCODE",
    ...
    monkeypatch.setattr(
        bhd_module.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: True
    )
    assert tracker._optional_policy_override(_meta(unattended=False))


@pytest.mark.asyncio
async def test_beyondhd_search_existing_rss_and_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    class Client:
        ...
        async def post(
            self, _url: str, *, params: dict[str, Any]
        ) -> httpx.Response:
            assert params["rsskey"] == "rss"
            return _response(
                payload, url="https://beyond-hd.me/api/torrents/key"
            )

    monkeypatch.setattr(
        bhd_module.httpx, "AsyncClient", lambda *_args, **_kwargs: Client()
    )
    ...
    params, has_rss = await tracker._search_params(
        _meta(category="TV", season="S02", sd=1, is_disc="DVD")
    )
    assert has_rss
    ...
    with pytest.raises(RuntimeError, match="BEYONDHD API Error"):
        tracker._search_payload(
            _response({"status_code": 0, "message": "bad"})
        )
...
async def test_beyondhd_get_edition_custom_branch() -> None:
    custom, edition = await _tracker().get_edition(
        _meta(edition="Fan Cut"), []
    )
...
@pytest.mark.asyncio
async def test_beyondhd_upload_returns_false_without_details_link(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    tracker.common.create_torrent_for_upload = AsyncMock()  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_beyondhd_invalid_imdb_non_matching_error_returns_payload() -> (
    None
):
    tracker = _tracker()
    payload = {"status_code": 0, "status_message": "Different API error"}
    assert (
        await tracker._maybe_retry_invalid_imdb(_meta(), {}, {}, payload)
        is payload
    )
