...
class _Response:
    def __init__(self, status_code: int = 200, payload: object = None, text: str | None = None) -> None:
...
def test_payload_error_and_chevereto_helpers_cover_all_shapes() -> None:
    ...
    assert uploader._chevereto_urls({"image": {"url": "raw", "url_viewer": "web", "medium": {"url": "medium"}}}) == (
        "medium",
        "raw",
        "web",
    )
...
def test_chevereto_missing_configuration_invalid_json_status_and_incomplete(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key=None, request_timeout=1, nsfw=False))["reason"] == "Missing ptscreens API key"

    original = IMAGE_HOST_SPECS["ptscreens"]
    monkeypatch.setitem(IMAGE_HOST_SPECS, "ptscreens", replace(original, upload_url=None))
    result = asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key="key", request_timeout=1, nsfw=False))
    assert result["reason"] == "No upload URL configured for ptscreens"
    monkeypatch.setitem(IMAGE_HOST_SPECS, "ptscreens", original)

    _Client.reset(_Response(500, ValueError("bad json"), "<!doctype html><html>HTTP 500 something went wrong</html>"))
    result = asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key="key", request_timeout=1, nsfw=False))
    ...
    result = asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key="key", request_timeout=1, nsfw=True))
...
def test_chevereto_timeout_request_and_file_errors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "outcome unknown" in asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key="key", request_timeout=1, nsfw=False))["reason"]

    _Client.reset(_request_error())
    result = asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key="key", request_timeout=1, nsfw=False))
    assert result["host_unavailable"] and "request failed" in result["reason"]

    monkeypatch.setattr(uploader, "_read_image_bytes", lambda _path: (_ for _ in ()).throw(OSError("read failed")))
    result = asyncio.run(uploader._upload_chevereto(str(image), host_key="ptscreens", api_key="key", request_timeout=1, nsfw=False))
...
def test_imgbb_success_url_fallbacks_and_all_failures(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _Client.reset(_Response(200, {"success": True, "data": {"url": "raw", "display_url": "display"}}))
    result = asyncio.run(uploader._upload_imgbb(str(image), "key", request_timeout=1))
...
def test_pixhost_url_patterns_and_upload_task_missing_hosts(tmp_path: Path) -> None:
    image = _image(tmp_path)
...
def test_dalexni_success_and_response_failures(tmp_path: Path) -> None:
    ...
    _Client.reset(_Response(200, {"success": True, "data": {"image": {"url": "raw"}, "medium": {"url": "medium"}, "url_viewer": "web"}}))
    ...
    _Client.reset(_Response(502, ValueError("bad"), "<!doctype html>HTTP 502 something went wrong"))
    result = asyncio.run(uploader.upload_image_task([str(image), "dalexni", config, meta]))
...
def test_pixhost_success_size_invalid_json_status_and_network(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _Client.reset(_Response(200, {"th_url": "https://t1.pixhost.to/thumbs/a.png", "show_url": "https://pixhost.to/show/a"}))
...
def test_zipline_midnightscene_success_shapes_and_failures(tmp_path: Path) -> None:
    image = _image(tmp_path)
    meta = Meta(category="MOVIE")
    config = _config()

    for host in ("zipline", "midnightscene"):
        for file_value in ({"url": "https://files.invalid/a.png"}, "https://files.invalid/a.png"):
...
def test_seedpool_sharex_lostimg_success_and_failures(tmp_path: Path) -> None:
    ...
    _Client.reset(_Response(201, {"files": [{"url": "raw", "thumbnail_url": "thumb", "variants": {"medium": "medium"}}]}))
    ...
    _Client.reset(_Response(200, {"files": [{"url": "raw", "variants": {"thumb": "variant-thumb"}}]}))
    assert asyncio.run(uploader.upload_image_task([str(image), "seedpool_cdn", config, meta]))["img_url"] == "variant-thumb"
    for payload in ({"files": []}, {"files": ["bad"]}, {"files": [{"url": ""}]}):
        _Client.reset(_Response(200, payload))
    ...
    for payload, expected in (({"data": {"link": "share-link"}}, "share-link"), ({"link": "root-link"}, "root-link")):
        _Client.reset(_Response(200, payload))
...
def test_upload_image_task_imgbox_chevereto_routing_and_outer_exception(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        uploader,
        "imgbox_upload",
        lambda *_args, **_kwargs: asyncio.sleep(0, result=[{"img_url": "thumb", "raw_url": "raw", "web_url": "web"}]),
    )
    ...
    async def chevereto(_image: str, *, host_key: str, api_key: str | None, request_timeout: float, nsfw: bool) -> dict[str, Any]:
        assert api_key and request_timeout == 60
        routed.append((host_key, nsfw))
        return {"status": "success", "img_url": "i", "raw_url": "r", "web_url": "w"}

    monkeypatch.setattr(uploader, "_upload_chevereto", chevereto)
    for host in ("ptscreens", "utppm", "onlyimage", "lensdump", "passtheimage"):
        assert asyncio.run(uploader.upload_image_task([str(image), host, config, meta]))["status"] == "success"
    assert routed == [(host, True) for host in ("ptscreens", "utppm", "onlyimage", "lensdump", "passtheimage")]

    monkeypatch.setattr(uploader, "_upload_imgbb", lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("boom")))
...
def test_imgbox_upload_success_incomplete_failure_exception_and_gallery_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _Gallery.submissions = {
        str(one): [{"success": True, "web_url": "web", "thumbnail_url": "thumb", "image_url": "raw"}],
        str(two): [{"success": True, "web_url": "web", "thumbnail_url": None, "image_url": "raw"}],
    ...
    state: dict[str, Any] = {}
    result = asyncio.run(uploader.imgbox_upload(tmp_path, [str(one), str(two), str(three), str(four)], state, adult=True))
...
def test_upload_screens_no_plan_zero_existing_missing_sources_and_custom_success(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path
    ...
    cwd = Path.cwd()
    try:
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="", debug=False)
        config = {"DEFAULT": {}}
        assert asyncio.run(uploader._upload_screens(config, meta, 1, 1, 0, 1, [], {})) == ([], 0)

        config = _config()
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", debug=False)
        assert asyncio.run(uploader._upload_screens(config, meta, 0, 1, 0, 0, [], {})) == ([], 0)

        existing = {"img_url": "i", "raw_url": "r", "web_url": "w"}
        meta = Meta(base_dir=str(root), uuid="release", image_list=[existing], imghost="imgbb", debug=False)
        ...
        async def success(args: list[object]) -> dict[str, Any]:
            return {"status": "success", "img_url": "thumb", "raw_url": "raw", "web_url": "web", "local_file_path": args[0]}

        monkeypatch.setattr(uploader, "upload_image_task", success)
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], image_sizes={}, imghost="imgbb", debug=True, cutoff=1)
...
def test_upload_screens_manifest_menu_duplicate_stat_and_images_needed_zero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        duplicate = {"img_url": "old", "raw_url": "raw:Release-1.png", "web_url": "old"}
        meta = Meta(base_dir=str(root), uuid="release", image_list=[duplicate], image_sizes={}, imghost="imgbb", cutoff=2)
        ...
        meta = Meta(base_dir=str(root), uuid="release", image_list=[duplicate], imghost="imgbb", cutoff=1)
...
def test_upload_screens_api_key_retry_exception_gather_and_no_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        ...
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", cutoff=1)
        assert asyncio.run(uploader._upload_screens(_config(img_host_2=""), meta, 1, 1, 0, 1, [image], {}, max_retries=2)) == ([], 0)
        assert calls == 1
        ...
        async def retry_then_success(args: list[object]) -> dict[str, Any]:
            ...
            calls += 1
            if calls == 1:
                return {"status": "failed", "reason": "temporary payload failure"}
            return {"status": "success", "img_url": "i", "raw_url": "r", "web_url": "w", "local_file_path": args[0]}

        monkeypatch.setattr(uploader, "upload_image_task", retry_then_success)
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", cutoff=1)
        images, count = asyncio.run(uploader._upload_screens(_config(img_host_2=""), meta, 1, 1, 0, 1, [image], {}, max_retries=2))
        ...
        async def raise_then_success(args: list[object]) -> dict[str, Any]:
            ...
            return {"status": "success", "img_url": "i", "raw_url": "r2", "web_url": "w", "local_file_path": args[0]}

        monkeypatch.setattr(uploader, "upload_image_task", raise_then_success)
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", cutoff=1)
        images, count = asyncio.run(uploader._upload_screens(_config(img_host_2=""), meta, 1, 1, 0, 1, [image], {}, max_retries=1))
...
def test_upload_screens_wait_timeout_and_cancellation_cleanup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def exercise() -> None:
        task = asyncio.create_task(
            uploader._upload_screens(
                _config(img_host_2=""),
                Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", cutoff=1),
                1,
                1,
                0,
                1,
...
def test_upload_screens_duplicate_raw_and_negative_retry_guard(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        existing_raw_only = {"raw_url": "same-raw", "img_url": "", "web_url": ""}
        monkeypatch.setattr(uploader, "manifest_files", lambda *_args: [image])
        meta = Meta(base_dir=str(root), uuid="release", image_list=[existing_raw_only], image_sizes={}, imghost="imgbb", cutoff=1)
        ...
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", cutoff=1)
        assert asyncio.run(uploader._upload_screens(_config(img_host_2=""), meta, 1, 1, 0, 1, [image], {}, max_retries=-1)) == ([], 0)
        assert calls == 0
    finally:
        os.chdir(cwd)
...
def test_upload_screens_exception_exhaustion(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        meta = Meta(base_dir=str(root), uuid="release", image_list=[], imghost="imgbb", cutoff=1)
        assert asyncio.run(uploader._upload_screens(_config(img_host_2=""), meta, 1, 1, 0, 1, [image], {}, max_retries=0)) == ([], 0)
    finally:
        os.chdir(cwd)
