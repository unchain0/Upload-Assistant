...
from src.integrations.torrent_clients import transmission as transmission_module
...
def test_deluge_connected_and_disconnected_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    DelugeClientMixin().deluge("/local/release/file.mkv", "release.torrent", torrent, "/local", "/remote", config)  # type: ignore[arg-type]
    ...
    assert args[2] == {"download_location": "/remote/release", "seed_mode": True}
...
def test_transmission_connection_failure_and_label_priority(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(transmission_module.transmission_rpc, "Client", _TransmissionClient)
    torrent = _Torrent()
    mixin = TransmissionClientMixin()

    _TransmissionClient.fail = True
    mixin.transmission("/local/release/file.mkv", torrent, "/local", "/remote", _transmission_config(), Meta())  # type: ignore[arg-type]
    ...
    mixin.transmission("/local/release/file.mkv", torrent, "/local", "/remote", config, Meta(transmission_label="meta-label"))  # type: ignore[arg-type]
    ...
    mixin.transmission("/local/release/file.mkv", torrent, "/local", "/remote", config, Meta(transmission_label=None))  # type: ignore[arg-type]
    assert _TransmissionClient.instances[-1].calls[-1]["labels"] == ["client-label"]
