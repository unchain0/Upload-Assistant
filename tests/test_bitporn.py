...
def test_bitporn_infers_category_from_basename_only() -> None:
    ...
    assert asyncio.run(bitporn.get_category_id(Meta(category="XXX", basename_no_ext="OnlyFans.2026.Creator.Big.Tits.1080p"))) == {"category_id": "10"}
    assert asyncio.run(bitporn.get_category_id(Meta(category="XXX", basename_no_ext="ManyVids.Creator.1080p"))) == {"category_id": "20"}
    assert asyncio.run(bitporn.get_category_id(Meta(category="XXX", basename_no_ext="Plain.Release.1080p"))) == {"category_id": "52"}
...
def test_bitporn_category_mappings_and_no_type_field() -> None:
    ...
    assert asyncio.run(bitporn.get_category_id(meta, mapping_only=True))["Ai Generated"] == "54"
    assert asyncio.run(bitporn.get_category_id(meta, reverse=True))["52"] == "Uncategorized"
...
def test_bitporn_resolution_mapping() -> None:
    ...
    assert asyncio.run(bitporn.get_resolution_id(meta)) == {"resolution_id": "18"}
    assert asyncio.run(bitporn.get_resolution_id(meta, "2048p")) == {"resolution_id": "14"}
    assert asyncio.run(bitporn.get_resolution_id(meta, "1080i")) == {"resolution_id": "11"}
