...
def _config() -> dict[str, Any]:
    return {
        "TRACKERS": {"MTEAM": {"api_key": "key", "base_url": "kp.m-team.cc"}},
...
def _response(
    ...
    *,
    status: int = 200,
    url: str = "https://api.m-team.cc/api/test",
) -> httpx.Response:
    return httpx.Response(
        status, request=httpx.Request("POST", url), json=payload
    )
    ...
    assert result == [
        {
            "Name": "Example",
            "Reward": "100",
            "Link": "https://kp.m-team.cc/seekDetail?id=5",
        }
    ]
...
@pytest.mark.asyncio
async def test_mteam_douban_error_and_fallback_description(
    ...
    text = await tracker.mteam_standard_desc(
        _meta(year=2025, overview="Plot", tmdb_poster_path="/poster.jpg")
    )
...
@pytest.mark.asyncio
async def test_mteam_additional_check_policy_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_mteam_search_no_imdb_api_error_and_valid_entry(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    payload = {
        ...
        "data": {
            "data": [
                {"id": 7, "name": "Release", "size": "42", "file_count": 2}
            ]
        },
    }
    tracker.session.post = AsyncMock(return_value=_response(payload))  # type: ignore[method-assign]
...
def test_mteam_unknown_audio_codec_and_hdr_name() -> None:
    ...
    assert (
        asyncio.run(
            tracker.get_name(_meta(name="Movie hdr10 60fps 60fps.mkv"))
        )
        == "Movie HDR10 HFR"
    )
...
@pytest.mark.asyncio
async def test_mteam_upload_release_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    create_response = _response({"message": "SUCCESS", "data": {"id": 9}})
    monkeypatch.setattr(
        tracker, "_submit_upload", AsyncMock(return_value=create_response)
    )
    tracker.session.post = AsyncMock(
        return_value=_response({"data": "https://download.invalid/9"})
...
@pytest.mark.asyncio
async def test_mteam_upload_error_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_mteam_submit_upload_posts_torrent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
...
async def test_mteam_handle_upload_missing_id_and_download_url() -> None:
    ...
    assert not await tracker._handle_upload_response(
        _meta(), status, _response({"message": "SUCCESS", "data": {}})
    )
...
@pytest.mark.asyncio
async def test_mteam_douban_success_description(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    payload = {
    ...
    monkeypatch.setattr(
        tracker, "get_douban_info", AsyncMock(return_value=payload)
    )
    assert "◎片　　名** Example" in await tracker.mteam_standard_desc(_meta())


@pytest.mark.asyncio
async def test_mteam_additional_checks_propagate_upscale_and_screenshot_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        tracker, "_upscale_policy_passes", AsyncMock(return_value=False)
    )
    ...
    monkeypatch.setattr(
        tracker, "_screenshot_policy_passes", lambda _meta: False
    )
    assert not await tracker.get_additional_checks(_meta())
...
async def test_mteam_dupe_bdinfo_falls_back_to_description() -> None:
    ...
    tracker.session.post = AsyncMock(
        return_value=_response(
            {"data": {"mediainfo": "", "descr": "BDINFO fallback"}}
        )
    )  # type: ignore[method-assign]
