...
def _media(
    tmp_path: Path, name: str = "Episode.S01E01.1080p.WEB-DL.x264-GROUP.mkv"
) -> Path:
    path = tmp_path / name
...
def test_duplicate_helper_contracts(tmp_path: Path) -> None:
    async def exercise() -> None:
        ...
        assert not await DupeChecker.is_exact_match(
            _entry("Other", files=["other.mkv"]), meta
        )
        assert (
            await DupeChecker.normalize_filename("Movie.Name-GROUP")
            == "movie name -group"
        )
        assert (
            await duplicate_service.normalize_filename(
                {"name": "Movie.Name-GROUP"}
            )
            == "movie name -group"
        )
        ...
        for arguments, result in expected.items():
            assert (
                await DupeChecker.is_season_episode_match(*arguments) == result
            )

        assert await DupeChecker.refine_hdr_terms(None) == set()
        assert await DupeChecker.refine_hdr_terms("DoVi HDR10+") == {
            "DV",
            "HDR",
        }
        ...
        assert await duplicate_service.has_matching_hdr(
            {"HDR10+"}, {"HDR"}, web
        )
...
def test_exact_match_edge_shapes(tmp_path: Path) -> None:
    async def exercise() -> None:
        ...
        cases = [
            (
                _entry(
                    "Same", files="Episode.S01E01.1080p.WEB-DL.x264-GROUP.mkv"
                ),
                _meta(tmp_path),
            ),
            (
                _entry("Same", files=[], file_count=0),
                _meta(tmp_path, is_disc="BDMV", filelist=[]),
            ),
            (
                _entry("Same", files=[], file_count=3),
                _meta(tmp_path, is_disc="BDMV", filelist=[]),
            ),
            (
                _entry("Same", size=None),
                _meta(tmp_path, source_size=None, filelist=[]),
            ),
            (
                {"name": " exact name ", "size": None},
                _meta(
                    tmp_path, name="Exact Name", source_size=None, filelist=[]
                ),
            ),
            (
                {"name": ""},
        ...
        outcomes = [
            await checker.is_exact_match(candidate, meta)
            for candidate, meta in cases
        ]
...
def test_tv_season_pack_luminarr_and_internal_trump_paths(
    tmp_path: Path,
) -> None:
    async def exercise() -> None:
        checker = DupeChecker(_config())
        ...
        assert isinstance(
            await checker.filter_dupes([pack], episode, "OTHER"), list
        )
        assert episode.season_pack_exists is True

        missing = _meta(tmp_path, category="TV", season="S01", episode="E03")
        assert isinstance(
            await checker.filter_dupes([pack], missing, "OTHER"), list
        )
        ...
        assert isinstance(
            await checker.filter_dupes(
                [_entry("Show.S01E02.1080p.WEB-DL")], luminarr, "LUMINARR"
            ),
            list,
        )
        ...
        assert isinstance(
            await checker.filter_dupes([candidate], internal, "AITHER"), list
        )
    ...
    async def exercise() -> None:
        ...
        for platform, candidate_type in game_cases:
            ...
            result = await checker.filter_dupes(
                [
                    _entry(
                        "Super.Game.Update.v1.0.4.2026-PC-GOG",
                        type=candidate_type,
                    )
                ],
                meta,
        ...
        for tracker in ("OTHER", "CAPYBARABR"):
            ...
            result = await checker.filter_dupes(
                list(book_cases), meta, tracker
            )
            assert isinstance(result, list)

        special_cases = (
            (
                _meta(tmp_path, source_size=100),
                _entry("Other", trumpable=True, res="1080p"),
                "AITHER",
            ),
            (
                _meta(tmp_path, type="WEBDL", source="WEB"),
                _entry("Movie.1080p.HDTV"),
                "OTHER",
            ),
            (
                _meta(tmp_path, type="ENCODE", source="BluRay"),
                _entry("Movie.1080p.WEB-DL"),
                "OTHER",
            ),
            (
                _meta(tmp_path, type="WEBDL", hdr="HDR"),
                _entry("Movie.1080p.SDR", flags=[]),
                "OTHER",
            ),
            (
                _meta(
                    tmp_path,
                    name="Movie REMUX 1080p",
                    uuid="Movie.REMUX.1080p",
                ),
                _entry("Movie.1080p.ENCODE"),
                "OTHER",
            ),
            (
                _meta(
                    tmp_path,
                    name="Movie REPACK 1080p",
                    uuid="Movie.REPACK.1080p",
                ),
                _entry("Movie.1080p"),
                "PREFERS",
            ),
        ...
        for meta, candidate, tracker in special_cases:
            assert isinstance(
                await checker.filter_dupes([candidate], meta, tracker), list
            )
