...
def test_no_disc_and_dvd_size_variants(tmp_path: Path) -> None:
    ...
    assert asyncio.run(manager.get_disc(_meta(tmp_path, source))) == (
        "",
        str(source),
        None,
        [],
...
def test_bdmv_site_check_ensure_error_edit_and_normal(
    ...
    monkeypatch.setattr(
        disc_info.BDInfoBinaryManager, "ensure_bdinfo_binary", ensure
    )
    ...
    asyncio.run(
        manager.get_disc(_meta(tmp_path, root, edit=True, discs=existing))
    )
    assert parser.get_bdinfo.await_args.args[1] == existing


def test_dvd_and_hddvd_dispatch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    manager = DiscInfoManager({"DEFAULT": {}})
    parser = SimpleNamespace(
        get_dvdinfo=AsyncMock(
            return_value=[{"name": "Movie", "size": "DVD9"}]
        ),
    ...
    monkeypatch.setattr(
        ...
        lambda base: downloads.append(base),
    )
    result = asyncio.run(manager.get_disc(_meta(tmp_path, dvd_root)))
    assert result[0] == "DVD" and result[3] == [
        {"name": "Movie", "size": "DVD9"}
    ]
