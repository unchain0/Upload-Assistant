...
def test_config_rounding_host_and_manager_delegation(
    ...
    assert (
        capture.task_limit == 1
        and capture.cutoff == 1
        and capture.desat == 10.0
    )
    ...
    assert (
        capture.screenshot_par_scale_factors(720, 576, 0.8, 4 / 3, True)[1]
        != 1
    )
    assert capture.screenshot_par_scale_factors(
        720, 576, 1.2, 4 / 3, True
    ) == (1.2, 1.0)

    assert (
        asyncio.run(
            capture.get_image_host(Meta(imghost=[None, 7, " ImgBox "]))
        )
        == "imgbox"
    )
    ...
    manager = capture.TakeScreensManager(
        {"DEFAULT": {"xxx_contact_sheet_rows": 2}}
    )
    monkeypatch.setattr(
        capture, "sanitize_filename", AsyncMock(return_value="safe")
    )
    monkeypatch.setattr(
...
def test_contact_sheet_empty_existing_retake_cached_failure_and_exception(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    meta = Meta(base_dir=str(tmp_path), uuid="xxx", category="XXX", screens=1)
    assert (
        asyncio.run(
            capture.xxx_contact_sheets([], meta.uuid, meta.base_dir, meta)
        )
        == []
    )
    ...
    capture.register_screenshots(
        meta.base_dir, meta.uuid, [str(cached)], "main"
    )
    meta.retake = False
    result = asyncio.run(
        capture.xxx_contact_sheets(
    ...
    monkeypatch.setattr(
        capture.ffmpeg, "probe", lambda _path: {"format": {"duration": "0"}}
    )
    assert (
        asyncio.run(
            capture.xxx_contact_sheets(
    ...
    monkeypatch.setattr(
        capture.ffmpeg, "probe", lambda _path: {"format": {"duration": "60"}}
    )
    monkeypatch.setattr(
...
class _OwnedProcess:
    ...
    async def wait(self) -> int:
        ...
        self.returncode = (
            self.returncode if self.returncode is not None else -15
        )
        return self.returncode


@pytest.mark.asyncio
async def test_run_ffmpeg_linux_arch_bundled_pipe_and_cancel_kill(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Command:
    ...
    assert (
        capture.get_ffmpeg_output_path(
            FallbackCommand(), ["ffmpeg", "output.png"]
        )
        == "output.png"
    )


def test_audiobook_cover_all_container_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import mutagen
    ...
    assert not asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
            Meta(path=str(tmp_path / "missing")), str(output)
        )
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _path: SimpleNamespace(
            pictures=[Picture(b"back", 4), Picture(b"front", 3)], tags={}
        ),
    )
    assert asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
            Meta(filelist=[str(audio)]), str(output), confirmed_only=True
        )
    )
    ...
    monkeypatch.setattr(
        ...
        "File",
        lambda _path: SimpleNamespace(
            pictures=[Picture(b"first", 4)], tags={}
        ),
    )
    assert asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
            Meta(filelist=[str(audio)]), str(output)
        )
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _path: SimpleNamespace(
            pictures=[],
            tags={
                "APIC:one": APIC(b"fallback", 4),
                "APIC:front": APIC(b"front", 3),
            },
        ),
    )
    assert asyncio.run(
    ...
    monkeypatch.setattr(
        ...
        "File",
        lambda _path: SimpleNamespace(
            pictures=[], tags={"APIC:one": APIC(b"fallback", 4)}
        ),
    )
    assert asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
    ...
    assert not asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
            Meta(filelist=[str(audio), "other.txt"]), str(output)
        )
    )
...
@pytest.mark.asyncio
async def test_epub_cover_manifest_html_svg_case_fallbacks(
    tmp_path: Path,
) -> None:
    output = tmp_path / "cover.bin"
    ...
    assert await capture.extract_epub_cover(
        str(direct), str(output), confirmed_only=True
    )
    ...
    assert await capture.extract_epub_cover(
        str(html), str(output), confirmed_only=True
    )
    ...
    _write_epub(
        zip_cover, {"book.opf": "<package/>", "assets/COVER.JPG": b"zip-cover"}
    )
    ...
    _write_epub(
        first,
        {
            "book.opf": '<package><manifest><item id="one" href="a.png" media-type="image/png"/></manifest></package>',
            "a.png": b"first",
        },
    )
    ...
    assert not await capture.extract_epub_cover(
        str(no_cover), str(output), confirmed_only=True
    )
    assert not await capture.extract_epub_cover(
        str(tmp_path / "invalid.epub"), str(output)
    )


def test_document_cover_pdf_cbz_cbr_and_failures(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import fitz
    ...
    assert not asyncio.run(
        capture.extract_document_cover(str(tmp_path / "book.txt"), str(output))
    )
    ...
    monkeypatch.setattr(
        fitz, "open", lambda *_args, **_kwargs: EmptyDocument()
    )
    assert not asyncio.run(
        capture.extract_document_cover(str(pdf), str(output))
    )
    ...
    assert not asyncio.run(
        capture.extract_document_cover(str(no_images), str(output))
    )

    broken = tmp_path / "broken.cbr"
    broken.write_bytes(b"bad")
    assert not asyncio.run(
        capture.extract_document_cover(str(broken), str(output))
    )
...
def test_prepare_book_cover_all_priorities(
    ...
    monkeypatch.setattr(
        ...
        lambda path: str(path) == str(existing),
    )
    meta = Meta(artwork_path=str(existing), retake=False)
    assert asyncio.run(
        capture.prepare_book_cover(str(source), "book", str(tmp_path), meta)
    ) == str(existing)

    output = capture.artwork_dir(str(tmp_path), "book") / "POSTER.png"
    output.write_bytes(b"image")
    monkeypatch.setattr(
        capture, "is_valid_cover_image", lambda path: str(path) == str(output)
    )
    ...
    for confirmed, downloaded, unconfirmed, expected in (
        ...
        monkeypatch.setattr(
            ...
            AsyncMock(
                side_effect=[confirmed, unconfirmed]
                if not confirmed
                else [True]
            ),
        )
        ...
        result = asyncio.run(
            capture.prepare_book_cover(
                str(source), "audio", str(tmp_path), meta
            )
        )
        assert bool(result) is expected

    monkeypatch.setattr(
        capture, "extract_epub_cover", AsyncMock(side_effect=[True])
    )
    meta = Meta(audiobook=False, artwork_path="", retake=False)
    assert asyncio.run(
        capture.prepare_book_cover(str(source), "epub", str(tmp_path), meta)
    )

    monkeypatch.setattr(
        capture, "extract_epub_cover", AsyncMock(side_effect=[False, True])
    )
    monkeypatch.setattr(
        capture, "download_artwork_from_meta", AsyncMock(return_value=False)
    )
    meta = Meta(audiobook=False, artwork_path="", retake=False)
    assert asyncio.run(
        capture.prepare_book_cover(str(source), "epub2", str(tmp_path), meta)
    )
    ...
    monkeypatch.setattr(
        capture, "extract_document_cover", AsyncMock(return_value=True)
    )
    assert asyncio.run(
        capture.prepare_book_cover(str(pdf), "pdf", str(tmp_path), Meta())
    )
    monkeypatch.setattr(
        capture, "extract_document_cover", AsyncMock(return_value=False)
    )
    assert (
        asyncio.run(
            capture.prepare_book_cover(str(pdf), "pdf2", str(tmp_path), Meta())
        )
        is None
    )


def test_download_artwork_redirects_mam_candidates_validation_and_limits(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    artwork = tmp_path / "cover.png"
    meta = Meta(
        artwork_url="https://www.myanonamouse.net/covers/book.webp",
        artwork_path="",
    )
    capture.default_config = {"mam_api_key": "mam-key"}
    monkeypatch.setattr(
        capture, "is_valid_cover_image", lambda path: Path(path).exists()
    )
    monkeypatch.setattr(
        capture, "is_valid_image_bytes", lambda data: data.startswith(b"image")
    )
    monkeypatch.setattr(
        capture, "is_public_http_url", lambda url: "private" not in url
    )
    ...
    Client.queue = [
        Response(302, location="/covers/book.jpg"),
        Response(200, content=b"image-png"),
    ]
...
def test_epub_additional_standard_id_case_and_no_rootfile(
    tmp_path: Path,
) -> None:
    output = tmp_path / "cover.bin"
    ...
    _write_epub(
        no_root,
        {"META-INF/container.xml": "<container/>", "readme.txt": "none"},
    )
    assert not asyncio.run(
        capture.extract_epub_cover(str(no_root), str(output))
    )
    ...
    assert not asyncio.run(
        capture.extract_epub_cover(
            str(confirmed), str(output), confirmed_only=True
        )
    )


def test_generate_ebook_screenshots_cbz_cbr_pdf_epub_and_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import fitz
    from PIL import Image
    ...
    meta = Meta(
        base_dir=str(tmp_path), uuid="comic", retake=False, artwork_path=""
    )
    png = tmp_path / "page1.png"
    jpg = tmp_path / "page2.jpg"
    ...
    monkeypatch.setattr(
        "random.sample", lambda population, count: list(population)[:count]
    )
    screenshots = asyncio.run(
        capture.generate_ebook_screenshots(
            str(cbz), "Comic", meta.uuid, meta.base_dir, meta, num_screens=2
        )
    )
    assert len(screenshots) == 2
    assert (
        Path(meta.artwork_path).is_file()
        and Path(meta.artwork_banner_path).is_file()
    )
    ...
    screenshots = asyncio.run(
        capture.generate_ebook_screenshots(
            str(cbz), "Comic", meta.uuid, meta.base_dir, meta, num_screens=1
        )
    )
    ...
    cbr_meta = Meta(
        base_dir=str(tmp_path), uuid="cbr", retake=False, artwork_path=""
    )
    assert asyncio.run(
        capture.generate_ebook_screenshots(
            str(cbr),
    ...
    assert (
        asyncio.run(
            capture.generate_ebook_screenshots(
                str(invalid), "Invalid", "invalid", str(tmp_path), Meta()
            )
        )
        == []
    )
    ...
    assert (
        asyncio.run(
            capture.generate_ebook_screenshots(
                str(empty), "Empty", "empty", str(tmp_path), Meta()
            )
        )
        == []
    )
    ...
    document.close()
    pdf_meta = Meta(
        base_dir=str(tmp_path), uuid="pdf", retake=False, artwork_path=""
    )
    pages = asyncio.run(
        capture.generate_ebook_screenshots(
            str(pdf),
    ...
    assert (
        asyncio.run(
            capture.generate_ebook_screenshots(
                str(pdf), "Empty", "pdf-empty", str(tmp_path), Meta()
            )
        )
        == []
    )
    ...
    monkeypatch.setattr(
        capture,
        "extract_epub_cover",
        AsyncMock(side_effect=RuntimeError("cover failed")),
    )
    epub_meta = Meta(
    ...
    assert (
        asyncio.run(
            capture.generate_ebook_screenshots(
                str(pdf), "Broken", "broken-pdf", str(tmp_path), Meta()
            )
        )
        == []
    )
...
def test_unrar_configuration_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        capture, "configured_binary", lambda _name: "/usr/bin/unrar"
    )
    invalid = tmp_path / "invalid.cbr"
    invalid.write_bytes(b"invalid")
    assert not asyncio.run(
        capture.extract_document_cover(
            str(invalid), str(tmp_path / "cover.png")
        )
    )
    assert (
        asyncio.run(
            capture.generate_ebook_screenshots(
                str(invalid), "Invalid", "unrar", str(tmp_path), Meta()
            )
        )
        == []
    )
...
def test_epub_meta_cover_html_without_image_parent_parts_and_empty_manifest(
    tmp_path: Path,
) -> None:
    output = tmp_path / "cover.bin"
    ...
    assert asyncio.run(
        capture.extract_epub_cover(str(meta_image), str(output))
    )
    ...
    assert not asyncio.run(
        capture.extract_epub_cover(str(html_none), str(output))
    )

    empty_manifest = tmp_path / "empty-manifest.epub"
    _write_epub(
        empty_manifest,
        {"book.opf": "<package><manifest/></package>", "readme.txt": "none"},
    )
    assert not asyncio.run(
        capture.extract_epub_cover(str(empty_manifest), str(output))
    )


def test_cbz_falls_back_to_rar_reader(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        ...
        "ZipFile",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            zipfile_module.BadZipFile()
        ),
    )
    monkeypatch.setattr(rarfile, "RarFile", FakeRar)
    assert asyncio.run(
        capture.extract_document_cover(str(archive), str(output))
    )
    assert output.is_file()
...
def test_epub_absolute_style_href_is_safely_resolved_inside_archive(
    tmp_path: Path,
) -> None:
    output = tmp_path / "cover.bin"
