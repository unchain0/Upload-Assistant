...
@pytest.mark.asyncio
async def test_avistaz_get_media_code_unsupported_and_lookup_paths(
    ...
    monkeypatch.setattr(
        tracker, "_media_lookup_attempt", AsyncMock(return_value=True)
    )
    ...
    monkeypatch.setattr(
        tracker, "_handle_missing_media", AsyncMock(return_value=True)
    )
    assert await tracker.get_media_code(_meta())


@pytest.mark.asyncio
async def test_avistaz_media_lookup_attempt_delayed_match(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(
        tracker,
        "_media_lookup_data",
        AsyncMock(return_value={"data": [{"id": 42, "imdb": "tt123"}]}),
    )
    identifiers = {"imdb": "tt123", "tmdb": "456", "title": "Example"}
    assert await tracker._media_lookup_attempt(
        _meta(), "1", identifiers, {}, delayed=True
    )
    assert tracker.media_code == "42"
    assert tracker._matching_media_item(
        {"data": [{"id": 1, "tmdb": "456"}]}, identifiers
    ) == {"id": 1, "tmdb": "456"}


@pytest.mark.asyncio
async def test_avistaz_missing_media_unattended_and_user_decline(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    identifiers = {"imdb": "tt123", "tmdb": "456", "title": "Example"}
    ...
    monkeypatch.setattr(
        az_module, "prompt_in_thread", AsyncMock(return_value=False)
    )
...
async def test_avistaz_add_media_success_and_exception() -> None:
    tracker = _tracker()
    assert await tracker._record_add_media_response(
        _meta(), _response(status=302)
    )
    tracker.session.post = AsyncMock(side_effect=RuntimeError("boom"))  # type: ignore[method-assign]
    assert not await tracker.add_media_to_db(
        _meta(), "Example", "1", "tt123", "456"
    )


@pytest.mark.asyncio
async def test_avistaz_validate_credentials_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_avistaz_additional_checks_branches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        tracker, "_rules_policy", AsyncMock(return_value=False)
    )
    ...
    monkeypatch.setattr(
        tracker, "_privatehd_group_policy", AsyncMock(return_value=False)
    )
    assert not await tracker.get_additional_checks(_meta())

    monkeypatch.setattr(
        tracker, "_privatehd_group_policy", AsyncMock(return_value=True)
    )
    monkeypatch.setattr(tracker, "_apply_session_cookies", AsyncMock())
    monkeypatch.setattr(
        tracker, "get_media_code", AsyncMock(return_value=True)
    )
    assert await tracker.get_additional_checks(_meta())

    monkeypatch.setattr(
        tracker, "get_media_code", AsyncMock(return_value=False)
    )
    assert not await tracker.get_additional_checks(_meta())
...
async def test_avistaz_policy_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert not await tracker._confirm_policy_override(
        _meta(unattended=True, unattended_confirm=False)
    )

    private = _tracker("PRIVATEHD")
    monkeypatch.setattr(
        private, "_confirm_policy_override", AsyncMock(return_value=True)
    )
    assert await private._privatehd_group_policy(
        _meta(type="ENCODE", tag="EVO")
    )
...
@pytest.mark.asyncio
async def test_avistaz_page_duplicate_helpers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    soup = BeautifulSoup(
    ...
    monkeypatch.setattr(
        tracker,
        "_row_duplicate",
        AsyncMock(return_value={"name": "Dupe", "size": "1 GB", "link": "x"}),
    )
    assert await tracker._page_duplicates(_meta(), soup, "WEB-DL") == [
        {"name": "Dupe", "size": "1 GB", "link": "x"}
    ]

    no_body = BeautifulSoup(
        "<table class='table-bordered'></table>", "html.parser"
    )
    assert tracker._torrent_rows(no_body) == []


@pytest.mark.asyncio
async def test_avistaz_row_duplicate_match_and_bdinfo(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    row = BeautifulSoup(
    ...
    row_small = BeautifulSoup(
        "<tr><td><a class='torrent-filename'>Name</a></td></tr>", "html.parser"
    ).find("tr")
...
@pytest.mark.asyncio
async def test_avistaz_dupe_bdinfo_success_and_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            side_effect=httpx.HTTPStatusError(
                "bad", request=request, response=status_response
            )
        ),
    )
    ...
    monkeypatch.setattr(
        tracker.session,
        "get",
        AsyncMock(side_effect=httpx.RequestError("offline", request=request)),
    )
    assert await tracker.get_dupe_bdinfo("https://x") == ""

    monkeypatch.setattr(
        tracker.session, "get", AsyncMock(side_effect=ValueError("bad parse"))
    )
    assert await tracker.get_dupe_bdinfo("https://x") == ""
...
def test_avistaz_log_dupe_error_helpers() -> None:
    ...
    tracker._log_dupe_bdinfo_error(
        "https://x",
        httpx.HTTPStatusError("bad", request=request, response=response),
    )
    tracker._log_dupe_bdinfo_error(
        "https://x", httpx.RequestError("bad", request=request)
    )
    tracker._log_dupe_bdinfo_error("https://x", ValueError("bad"))
    assert (
        tracker._bdinfo_from_html(
            "<div id='collapseMediaInfo'><pre>INFO</pre></div>"
        )
        == "INFO"
    )
...
@pytest.mark.asyncio
async def test_avistaz_file_languages_and_prompt(
    ...
    monkeypatch.setattr(
        tracker,
        "_ask_missing_audio_languages",
        AsyncMock(return_value="English, French"),
    )
...
def test_avistaz_classify_track_language_missing_and_text() -> None:
    ...
    assert missing
    tracker._classify_track_language(
        {"@type": "Text", "Language": "en"}, audio, subs, missing
    )
...
async def test_avistaz_img_host_success_failure_and_exception() -> None:
    ...
    tracker.session.post = AsyncMock(
        return_value=_response(json_data={"success": True, "imageId": 9})
    )  # type: ignore[method-assign]
    assert await tracker.img_host(_meta(), "ref", b"x", "x.png") == "9"

    tracker.session.post = AsyncMock(
        return_value=_response(json_data={"success": False, "error": "bad"})
    )  # type: ignore[method-assign]
    assert await tracker.img_host(_meta(), "ref", b"x", "x.png") is None

    tracker.session.post = AsyncMock(
        return_value=_response(status=500, text="error")
    )  # type: ignore[method-assign]
    assert await tracker.img_host(_meta(), "ref", b"x", "x.png") is None
...
@pytest.mark.asyncio
async def test_avistaz_screenshot_helpers(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    monkeypatch.setattr(
        tracker, "img_host", AsyncMock(return_value="image-id")
    )
    assert (
        await tracker._upload_local_screenshot(_meta(tmp_path), local)
        == "image-id"
    )

    tracker.session.get = AsyncMock(side_effect=RuntimeError("offline"))  # type: ignore[method-assign]
    assert (
        await tracker._upload_remote_screenshot(_meta(), "https://x/image.png")
        is None
    )
    ...
    monkeypatch.setattr(
        tracker, "_upload_local_screenshot", AsyncMock(return_value="local-id")
    )
    results = await tracker._append_local_images(meta, [], [local], 1)
    assert results == ["local-id"]
    monkeypatch.setattr(
        tracker,
        "_upload_remote_screenshot",
        AsyncMock(return_value="remote-id"),
    )
    assert await tracker._append_remote_images(meta, [], ["https://x"], 1) == [
        "remote-id"
    ]


@pytest.mark.asyncio
async def test_avistaz_requests_disabled_error_and_parsing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    assert (
        tracker._request_row(
            BeautifulSoup("<tr><td>none</td></tr>", "html.parser").find("tr")
        )
        is None
    )
    row = BeautifulSoup(
        "<tr><td><a class='torrent-filename' href='/r/1'>Req</a></td><td></td></tr>",
        "html.parser",
    ).find("tr")
    assert row is not None
    assert tracker._request_row(row) == {
        "Name": "Req",
        "Link": "/r/1",
        "Reward": "N/A",
    }
    tracker._log_request_results(
        [{"Name": "Req", "Link": "/r/1", "Reward": "10"}]
    )


@pytest.mark.asyncio
async def test_avistaz_fetch_tag_and_get_tags(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    tracker.session.get = AsyncMock(
        return_value=_response(
            json_data={"data": [{"tag": "drama", "id": "12"}]}
        )
    )  # type: ignore[method-assign]
    ...
    tracker.session.get = AsyncMock(
        return_value=_response(
            json_data={"data": [{"tag": "drama", "id": "bad"}]}
        )
    )  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_avistaz_edit_desc_and_render_fallback(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(
        tracker,
        "_raw_description",
        AsyncMock(return_value="[b]Hello[/b] https://x"),
    )
...
@pytest.mark.asyncio
async def test_avistaz_create_task_debug_invalid_redirect_and_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        tracker, "_task_payload", AsyncMock(return_value={"x": 1})
    )
    assert await tracker.create_task_id(_meta(debug=True)) == {}

    response = _response(
        status=302, headers={"Location": "https://x/not-a-number"}
    )
    item = _meta()
...
def test_avistaz_name_helper_edge_paths() -> None:
    ...
    assert (
        tracker._reposition_cinemaz_hybrid(item, "Example 2024 WEB-DL")
        == "Example 2024 WEB-DL"
    )
    assert (
        tracker._reposition_cinemaz_hybrid(item, "Example 2024 Hybrid WEB-DL")
        == "Example 2024 Hybrid WEB-DL"
    )
    assert (
        tracker._tv_year(_meta(category="TV", no_year=True, year=2020)) == 2020
    )


@pytest.mark.asyncio
async def test_avistaz_fetch_data_skip_and_task_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_apply_session_cookies", AsyncMock())
    ...
    monkeypatch.setattr(
        tracker, "get_screenshots", AsyncMock(side_effect=RuntimeError("bad"))
    )
    data: dict[str, Any] = {}
    await tracker._apply_task_upload_fields(
        data, _meta(debug=False), {"redirect_url": "url"}
    )
...
@pytest.mark.asyncio
async def test_avistaz_upload_skip_debug_and_live(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_avistaz_live_upload_failure_and_success(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    tracker.session.post = AsyncMock(
        return_value=_response(
            status=302,
            headers={"Location": "https://avistaz.invalid/torrent/7"},
        )
    )  # type: ignore[method-assign]
    monkeypatch.setattr(
        tracker, "_record_successful_upload", AsyncMock(return_value=True)
    )
    assert await tracker._live_upload(_meta(), {})


@pytest.mark.asyncio
async def test_avistaz_record_successful_upload_register_failure_and_success() -> (
    None
):
    tracker = _tracker()
    response = _response(
        status=302, headers={"Location": "https://avistaz.invalid/torrent/77"}
    )
...
@pytest.mark.asyncio
async def test_avistaz_language_ids_prompt_for_missing_audio(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        tracker,
        "_prompt_missing_audio_languages",
        AsyncMock(return_value={"9"}),
    )
    audio, subtitles = await tracker._language_ids_from_tracks(
        _meta(), [{"@type": "Audio"}]
    )
    assert audio == {"9"}
...
@pytest.mark.asyncio
async def test_avistaz_raw_description_includes_episode_overview(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        az_module.DescriptionBuilder,
        "get_tv_info",
        AsyncMock(return_value=("Pilot", "Episode overview")),
    )
    monkeypatch.setattr(
        az_module.DescriptionBuilder,
        "get_user_description",
        AsyncMock(return_value="User notes"),
    )
    monkeypatch.setattr(
        az_module.DescriptionBuilder,
        "get_tonemapped_header",
        AsyncMock(return_value="Tone"),
    )
...
@pytest.mark.asyncio
async def test_avistaz_create_task_returns_successful_task(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    response = _response(
        status=302, headers={"Location": "https://avistaz.invalid/upload/123"}
    )
    expected = {
        "task_id": "123",
        "info_hash": "hash",
    ...
    monkeypatch.setattr(
        tracker, "_submit_task_step_one", AsyncMock(return_value=response)
    )
    monkeypatch.setattr(
        tracker, "_task_from_response", AsyncMock(return_value=expected)
    )
...
def test_avistaz_raw_urls_ignores_non_mapping_items() -> None:
    assert AZTrackerBase._raw_urls(
        ["bad", {"raw_url": "https://x/image.png"}]
    ) == ["https://x/image.png"]


def test_avistaz_season_summary_entries_rejects_non_list() -> None:
    assert (
        AZTrackerBase._season_summary_entries({"seasons_summary": "bad"}) == []
    )
