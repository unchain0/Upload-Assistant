...
from src.integrations.trackers.UNIT3D.znth import Zenith, _normalized_tracker_names, _uses_zenith_music_layout, prepare_zenith_music_layout
from tests.test_znth import _audiobook_meta, _book_meta, _movie_meta, _tracker, _tv_meta
...
def test_zenith_music_release_error_for_root_and_missing_tracks() -> None:
    tracker = _tracker()
    file_root = Meta(category="MUSIC", path="track.flac", artist="Artist", title="Album", music_release={})
    assert tracker._music_release_error(file_root, ["track.flac"]) == "music uploads must be inside a directory"
    ...
    missing_tracks = Meta(
        ...
        artist="Artist",
        title="Album",
        music_release={"fields": {"artist": {"value": "Artist"}, "album": {"value": "Album"}}, "tracks": []},
    )
    assert tracker._music_release_error(missing_tracks, []) == "music metadata does not contain any audio tracks"
...
def test_zenith_audiobook_layout_remaining_branches() -> None:
    ...
    assert tracker._audiobook_layout_error(meta, [], meta.path) == "audiobook does not contain a supported audio file"

    multi = [Path("01. First - Book (2020).mp3"), Path("02. Second - Book (2020).mp3")]
    multi_meta = _book_meta(title="Book", year=2020, path="Book")
    assert tracker._audiobook_layout_error(multi_meta, [str(path) for path in multi], "Book") == ""

    m4b = [Path("01. First - Book (2020).m4b"), Path("02. Second - Book (2020).mp3")]
    assert "M4B audiobooks" in tracker._multi_audiobook_layout_error(multi_meta, m4b)

    invalid = [Path("bad name.mp3"), Path("02. Second - Book (2020).mp3")]
    assert "bad name.mp3" in tracker._multi_audiobook_layout_error(multi_meta, invalid)


def test_zenith_audiobook_language_without_audio_track() -> None:
    assert _tracker()._audiobook_language_error(_audiobook_meta(mediainfo={"media": {"track": []}})) == "MediaInfo does not contain an audio track"


@pytest.mark.asyncio
async def test_zenith_video_storage_and_tv_scope_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    movie = _movie_meta(is_disc="")
    assert tracker._video_storage_policy(movie, movie.filelist, tracker._collect_video_paths(movie.filelist))

    monkeypatch.setattr(tracker.common, "extract_tv_seasons", lambda _files: {1, 2})
    assert not await tracker._tv_scope_policy(_tv_meta(), ["S01E01.mkv", "S02E01.mkv"])
    ...
    assert not tracker._tv_episode_policy(_tv_meta(tv_pack=False), ["S01E01.mkv", "S01E02.mkv"])


@pytest.mark.asyncio
async def test_zenith_deferred_audiobook_and_book_identity_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
def test_zenith_movie_episode_token_and_misc_book_name() -> None:
    tracker = _tracker()
    assert not tracker._video_episode_title_policy(_movie_meta(), "MOVIE", "Movie S01E01 1080p WEB-DL")
...
def test_zenith_ebook_series_edition_and_source_branches() -> None:
    ...
    assert tracker._ebook_source(_book_meta(manual_source="RETAIL"), "EPUB") == "RETAIL"
...
def test_zenith_music_bitrate_invalid_value_is_ignored() -> None:
    assert Zenith._music_bitrate_label({"bitrate": "not-a-number"}, "MP3") == ""


@pytest.mark.asyncio
async def test_zenith_additional_files_adds_audiobook_cover(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(UNIT3D, "get_additional_files", AsyncMock(return_value={}))
    monkeypatch.setattr(tracker, "get_image_file", AsyncMock(return_value=("cover.jpg", b"cover", "image/jpeg")))
...
def test_zenith_video_storage_rejects_unsupported_disc_structure() -> None:
    ...
    assert not _tracker()._video_storage_policy(meta, meta.filelist, _tracker()._collect_video_paths(meta.filelist))
