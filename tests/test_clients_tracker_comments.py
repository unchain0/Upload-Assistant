...
def test_matches_single_file_inside_folder_torrent_content_path():
    ...
    assert client._matches_qbit_content_path(torrent, Meta({"path": media, "uuid": "Heat.mkv"}))
...
def test_matches_torrent_name_when_content_path_is_unavailable():
    ...
    assert client._matches_qbit_content_path(torrent, Meta({"path": media, "filelist": [media], "uuid": "different"}))
