...
class _Progress:
    ...
    def add_task(self, description: str, *, total: object = None) -> int:
        ...
        self.tasks.append(
            SimpleNamespace(description=description, total=total, completed=0)
        )
        return task_id
...
def test_random_poster_sizes_paths_dynamic_and_connections(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    offsets = iter((0, 1, 1, 3))
    monkeypatch.setattr(
        creator.secrets, "randbelow", lambda _limit: next(offsets)
    )
    monkeypatch.setattr(creator.secrets, "choice", lambda values: values[0])
    assert (
        creator.generate_random_poster()
        == "Aaaaa Aaaaaa <aaaaaaa@aaaaaaaa.com>"
    )
...
def test_check_binary_path_managers_download_failures_and_missing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        creator.shutil,
        "which",
        lambda path: f"/resolved/{path}" if path == "configured" else None,
    )
    assert (
        asyncio.run(creator.check_binary("7z", "configured"))
        == "/resolved/configured"
    )
    ...
    monkeypatch.setattr(
        ...
        "ensure_7z_binary",
        AsyncMock(return_value="/auto/7z"),
    )
    monkeypatch.setattr(
        NyuuBinaryManager,
        "ensure_nyuu_binary",
        AsyncMock(return_value="/auto/nyuu"),
    )
    monkeypatch.setattr(
        Par2BinaryManager,
        "ensure_par2_binary",
        AsyncMock(return_value="/auto/par2"),
    )
    monkeypatch.setattr(
        PestoBinaryManager,
        "ensure_pesto_binary",
        AsyncMock(return_value="/auto/pesto"),
    )
    meta = _meta(tmp_path)
    assert asyncio.run(creator.check_binary("7z", meta=meta)) == "/auto/7z"
    assert (
        asyncio.run(
            creator.check_binary("nyuu", meta=meta, path_7z="/auto/7z")
        )
        == "/auto/nyuu"
    )
    assert asyncio.run(creator.check_binary("par2", meta=meta)) == "/auto/par2"
    assert (
        asyncio.run(creator.check_binary("pesto", meta=meta)) == "/auto/pesto"
    )
...
def test_run_command_logging_success_failure_and_spawn_error(
    ...
    asyncio.run(
        creator.run_command_with_logging(
            ["tool", "-p", "secret", "--username", "user"], "tool"
        )
    )
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=_Process(returncode=2, communicate=(b"out", b"err"))
        ),
    )
    with pytest.raises(RuntimeError, match="failed with exit code 2"):
        asyncio.run(
            creator.run_command_with_logging(
                ["tool", "--password", "secret"], "tool"
            )
        )
...
def test_nzb_validation_injection_and_verification(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    valid.write_text(
        "<nzb><head></head>" + "x" * 1200 + "</nzb>", encoding="utf-8"
    )
    ...
    with_head.write_text(
        "<nzb><head><meta type='title'>x</meta></head></nzb>", encoding="utf-8"
    )
...
def test_run_7z_progress_success_failure_psutil_and_spawn_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        creator.psutil,
        "Process",
        lambda _pid: (_ for _ in ()).throw(creator.psutil.AccessDenied()),
    )
    monkeypatch.setattr(
        creator.asyncio,
        "create_subprocess_exec",
    ...
    with pytest.raises(RuntimeError, match="exit code 2"):
        asyncio.run(
            creator.run_7z_with_progress(
                ["7z", "a", "archive", "source"], directory, "release", None, 0
            )
        )
...
def test_run_par2_progress_actions_failures_and_missing_stdout(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    asyncio.run(
        creator.run_par2_with_progress(
            ["par2", "c", "-p", "secret", "file"], cwd=str(tmp_path)
        )
    )
    descriptions = [
        str(update.get("description", ""))
        for _task, update in _Progress.instances[-1].updates
    ]
    assert any("matrix" in value for value in descriptions)
    assert _Progress.instances[-1].tasks[0].completed == 100

    failed = _Process(
        stdout=_Stream(chunks=[b"Writing 25%\nout"]), returncode=3
    )
    monkeypatch.setattr(
        creator.asyncio,
        "create_subprocess_exec",
...
def test_run_nyuu_progress_post_check_failure_and_missing_stdout(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    lines = [
    ...
    asyncio.run(
        creator.run_nyuu_with_progress(
            ["nyuu", "-u", "user", "-p", "pass"], cwd=str(tmp_path)
        )
    )
    assert _Progress.instances[-1].tasks[0].completed == 100
    ...
    no_total = _Process(
        stdout=_Stream(
            lines=[b"Article posting progress: 1 read, 1 posted\n"]
        ),
        returncode=0,
    )
    monkeypatch.setattr(
        creator.asyncio,
        "create_subprocess_exec",
        AsyncMock(return_value=no_total),
    )
...
def _pesto_lines(*events: dict[str, object] | str) -> list[bytes]:
    return [
        (json.dumps(event) if isinstance(event, dict) else event).encode()
        + b"\n"
        for event in events
    ]
...
def test_run_pesto_all_events_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    asyncio.run(
        creator.run_pesto_with_progress(
            ["pesto", "-u", "user", "--auth-password", "secret"]
        )
    )
    assert len(_Progress.instances[-1].tasks) >= 4


def test_run_pesto_failures_missing_articles_generic_and_streams(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    events = _pesto_lines(
        {
    ...
    process = _Process(
        stdout=_Stream(lines=events),
        stderr=_Stream(lines=[b"provider rejected\n"]),
        returncode=5,
    )
    monkeypatch.setattr(
        creator.asyncio,
...
def test_remaining_progress_and_nzb_helper_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        creator,
        "threading",
        SimpleNamespace(Event=OneShotEvent, Thread=ImmediateThread),
    )
    process = _Process(
        stdout=None, stderr=_Stream(chunks=[b"25%", b""]), returncode=0
    )
    monkeypatch.setattr(
        creator.asyncio,
        "create_subprocess_exec",
    ...
    asyncio.run(
        creator.run_7z_with_progress(
            ["7z", "a"], directory, "release", None, 10
        )
    )
    assert any(
        "processed" in str(update.get("description", ""))
        for _task, update in _Progress.instances[-1].updates
    )

    # Password redaction and blank progress records in PAR2.
    process = _Process(
        stdout=_Stream(chunks=[b"\rWriting 10%\r"]), returncode=0
    )
    monkeypatch.setattr(
        creator.asyncio,
        "create_subprocess_exec",
    ...
    asyncio.run(
        creator.run_par2_with_progress(
            ["par2", "c", "-p", "secret"], cwd=str(tmp_path)
        )
    )
    ...
    assert asyncio.run(creator.is_valid_nzb(medium))
    asyncio.run(
        creator.inject_nzb_password(tmp_path / "missing.nzb", "secret")
    )


def _patch_usenet_tools(
    monkeypatch: pytest.MonkeyPatch,
    captured: dict[str, list[list[str]]] | None = None,
    ...
    async def seven(cmd: list[str], *_args: object, **_kwargs: object) -> None:
        ...
        target = (
            Path(f"{archive}.001")
            if any(str(arg).startswith("-v") for arg in cmd)
            else archive
        )
    ...
    async def nyuu(cmd: list[str], cwd: str | None = None) -> None:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(
            "<nzb><head></head>" + "x" * 120 + "</nzb>", encoding="utf-8"
        )
...
def test_prepare_missing_config_input_existing_nzb_and_path_fallbacks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    assert (
        asyncio.run(creator.prepare_and_upload_usenet(_meta(tmp_path), {}))
        is None
    )
    assert (
        asyncio.run(
            creator.prepare_and_upload_usenet(_meta(tmp_path, path=""), _cfg())
        )
        is None
    )
    ...
    assert asyncio.run(
        creator.prepare_and_upload_usenet(
            meta, _cfg(nzb_output_dir=str(output))
        )
    ) == str(existing)
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            meta,
            _cfg(
                nzb_output_dir=str(invalid_output),
                usenet_tmp_dir=str(invalid_tmp),
                skip_archive=True,
            ),
            prepare_only=True,
        )
...
def test_prepare_random_static_password_obfuscation_and_reuse(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _patch_usenet_tools(monkeypatch)
    tokens = iter(("-invalid", "random-secret"))
    monkeypatch.setattr(
        creator.secrets, "token_urlsafe", lambda _length: next(tokens)
    )
    monkeypatch.setattr(
        creator.secrets, "token_hex", lambda _length: "obfuscated"
    )
    meta = _meta(tmp_path, debug=True)
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            meta,
            _cfg(archive_password=RANDOM_PASSWORD_MODE),
            prepare_only=True,
        )
    )
    assert result
    ...
    asyncio.run(
        creator.prepare_and_upload_usenet(
            static,
            _cfg(archive_password=STATIC_ARCHIVE_PASSWORD),
            prepare_only=True,
        )
    )
    assert (
        static.archive_password == "configured"
        and static.usenet_archive_password_is_random is False
    )


def test_prepare_binary_missing_debug_and_non_debug_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(debug, _cfg(), prepare_only=True)
    )
    ...
    assert (
        asyncio.run(
            creator.prepare_and_upload_usenet(
                pesto, _cfg(usenet_uploader="pesto")
            )
        )
        is None
    )
...
def test_prepare_skip_archive_directory_file_missing_and_password(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            meta, _cfg(skip_archive=True), prepare_only=True
        )
    )
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            single, _cfg(skip_archive=True), prepare_only=True
        )
    )
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            missing, _cfg(skip_archive=True), prepare_only=True
        )
    )
    assert result and Path(result, "missing.bin").is_file()


def test_prepare_archive_auto_single_copy_and_prepared_reuse(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured = _patch_usenet_tools(monkeypatch)
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            meta, _cfg(rar_volume_size="auto"), prepare_only=True
        )
    )
    assert result
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(copied, _cfg(), prepare_only=True)
    )
    assert result
    assert any(
        Path(path).name == "copy.bin" for path in copied.usenet_prepared_files
    )
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(reused, _cfg(), prepare_only=True)
    )
    assert result and captured["7z"] == [] and captured["par2"] == []


def test_prepare_debug_nyuu_simulation_password_and_final_move(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    async def missing(*_args: object, **_kwargs: object) -> str:
    ...
    monkeypatch.setattr(creator, "check_binary", missing)
    monkeypatch.setattr(
        creator.secrets, "token_hex", lambda _length: "hex-subject"
    )
    meta = _meta(
        tmp_path,
        uuid="debug-post",
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            meta, _cfg(archive_password=ARCHIVE_PASSWORD)
        )
    )
...
def test_prepare_nyuu_command_ids_checks_subject_and_cleanup(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured = _patch_usenet_tools(monkeypatch)
    meta = _meta(
        tmp_path, uuid="nyuu-command", usenet_subject="Custom Subject"
    )
    config = _cfg(
        random_poster=False,
    ...
    assert cmd[
        cmd.index("--check-connections") : cmd.index("--check-connections") + 2
    ] == ["--check-connections", "2"]
    ...
    assert (
        "-S" not in cmd
        and "--check-connections" not in cmd
        and "--filename" not in cmd
    )


def test_prepare_pesto_command_options_no_check_and_password(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured = _patch_usenet_tools(monkeypatch)
    meta = _meta(
        tmp_path, uuid="pesto-command", archive_password=ARCHIVE_PASSWORD
    )
    config = _cfg(
    ...
    no_check = _meta(
        tmp_path,
        uuid="pesto-no-check",
        basename_no_ext="Pesto No Check",
        tmdb_id="bad",
        imdb_tt="",
        tvdb_id="bad",
        mal_id="bad",
...
def test_prepare_upload_failures_remove_nzb_and_cleanup(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured = _patch_usenet_tools(monkeypatch)
    ...
    with pytest.raises(RuntimeError, match="pesto failed"):
        asyncio.run(
            creator.prepare_and_upload_usenet(
                meta, _cfg(usenet_uploader="pesto", rar_volume_size="100m")
            )
        )
    assert captured["7z"]


def test_prepare_move_and_cleanup_failures_are_nonfatal(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _patch_usenet_tools(monkeypatch)
...
def test_remaining_pesto_progress_and_archive_monitor_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    process = _Process(
        stdout=_Stream(
    ...
    process = _Process(
        stdout=_Stream(chunks=[]), stderr=_Stream(chunks=[]), returncode=0
    )
    monkeypatch.setattr(
        creator.asyncio,
        "create_subprocess_exec",
...
def test_prepare_empty_basename_file_and_binary_failure_stages(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured = _patch_usenet_tools(monkeypatch)
    ...
    normal = _meta(
        tmp_path, uuid="pesto-missing", basename_no_ext="Pesto Missing"
    )
    assert (
        asyncio.run(
            creator.prepare_and_upload_usenet(
                normal, _cfg(usenet_uploader="pesto")
            )
        )
        is None
    )
    debug = _meta(
        tmp_path,
        uuid="pesto-missing-debug",
        basename_no_ext="Pesto Missing Debug",
        debug=True,
    )
    result = asyncio.run(
        creator.prepare_and_upload_usenet(debug, _cfg(usenet_uploader="pesto"))
    )
    assert result and Path(result).is_file()
    ...
    normal = _meta(
        tmp_path, uuid="par2-missing", basename_no_ext="Par2 Missing"
    )
    assert (
        asyncio.run(
            creator.prepare_and_upload_usenet(
...
def test_prepare_debug_missing_single_file_pesto_simulation_and_absolute_prepared(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _patch_usenet_tools(monkeypatch)
    missing = _meta(
    ...
    result = asyncio.run(
        creator.prepare_and_upload_usenet(missing, _cfg(), prepare_only=True)
    )
    assert result and Path(result, "missing-source.bin").is_file()

    async def pesto_missing(
    ...
    pesto = _meta(
        tmp_path,
        uuid="pesto-debug-sim",
        basename_no_ext="Pesto Debug",
        debug=True,
    )
    result = asyncio.run(
        creator.prepare_and_upload_usenet(pesto, _cfg(usenet_uploader="pesto"))
    )
    assert result and Path(result).is_file()
...
def test_prepare_skips_deleted_upload_file_and_real_password_injection(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured = _patch_usenet_tools(monkeypatch)
    source = tmp_path / "source-dir"
    ...
    meta = _meta(
        ...
        source,
        uuid="deleted-upload",
        basename_no_ext="Deleted Upload",
    )
    result = asyncio.run(
        creator.prepare_and_upload_usenet(meta, _cfg(rar_volume_size="100m"))
    )
    assert result and Path(result).is_file()
    ...
    secured = _meta(
        tmp_path,
        uuid="secured-real",
        basename_no_ext="Secured Real",
        archive_password=password,
    )
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            secured, _cfg(archive_password=password)
        )
    )
    assert result and asyncio.run(creator.verify_nzb_has_password(result))


def test_remaining_monitor_and_initial_pesto_tasks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=_Process(
                stdout=_Stream(chunks=[b""]),
                stderr=_Stream(chunks=[b""]),
                returncode=0,
            )
        ),
    )
    asyncio.run(
        creator.run_7z_with_progress(
            ["7z", "a"], directory, "archive", None, 10
        )
    )
    ...
    assert (
        "Calculating PAR2 parity" in descriptions
        and "Writing PAR2 recovery files" in descriptions
    )


def test_prepare_remaining_binary_debug_name_and_missing_source_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert asyncio.run(
        creator.prepare_and_upload_usenet(
            empty_name, _cfg(), prepare_only=True
        )
    )
    ...
    async def debug_missing(
        _name: str, *_args: object, **_kwargs: object
    ) -> str:
    ...
    debug_pesto = _meta(
        ...
        debug_source,
        uuid="debug-pesto",
        basename_no_ext="Debug Pesto",
        debug=True,
    )
    result = asyncio.run(
        creator.prepare_and_upload_usenet(
            debug_pesto, _cfg(usenet_uploader="pesto")
        )
    )
    assert result and Path(result).is_file()
    ...
    absent_meta = _meta(
        tmp_path,
        tmp_path / "placeholder.bin",
        uuid="absent",
        basename_no_ext="Absent",
        debug=True,
    )
    absent_meta.path = str(absent)
    staged = asyncio.run(
        creator.prepare_and_upload_usenet(
            absent_meta, _cfg(), prepare_only=True
        )
    )
    assert staged and any(
        Path(path).name == "absent-source.bin"
        for path in absent_meta.usenet_prepared_files
    )
...
def test_prepare_relative_fallback_disappearing_files_pesto_debug_and_real_password_injection(
    ...
    password_result = asyncio.run(
        creator.prepare_and_upload_usenet(
            password_meta, _cfg(archive_password=ARCHIVE_PASSWORD)
        )
    )
    assert password_result and '<meta type="password">secret</meta>' in Path(
        password_result
