...
def test_hdb_description_removes_internal_material_and_extracts_external_images() -> None:
...
def test_hdb_description_only_bbcode_returns_empty() -> None:
    cleaned, images = BBCODE().clean_hdb_description("[b][/b]\n[center][/center]")
    assert cleaned == "" and images == []


def test_bhd_description_framestor_flux_images_and_empty(tmp_path: Path) -> None:
...
"""
    ...
    empty, _ = BBCODE().clean_bhd_description("[img]https://x.example/a.jpg[/img]", Meta(base_dir=str(tmp_path), uuid="empty", flux=True))
    assert empty == ""
    no_flux, _ = BBCODE().clean_bhd_description("text", Meta(base_dir=str(tmp_path), uuid="noflux", flux=False))
    assert no_flux == ""
...
"""
    ...
    assert any(image["raw_url"] == "https://loose.example/screen.jpg" for image in images)
    assert "[comparison=" not in cleaned

    dvd, _ = bb.clean_ptp_description("[mediainfo]DVD INFO[/mediainfo]\nDVD notes", "DVD")
...
def test_unit3d_description_site_images_spoilers_centers_and_signatures() -> None:
    bb = BBCODE()
...
Notes
"""
    cleaned, images = bb.clean_unit3d_description(desc, "https://tracker.example")
    ...
    assert all("thumbs" not in value and "favicon" not in value for value in urls)
    assert "inside.example/spoiler.jpg" in cleaned

    assert bb.clean_unit3d_description("[center][/center]", "https://tracker.example")[0] == ""
    assert bb.clean_unit3d_description("[b][/b]", "https://tracker.example")[0] == ""
...
def test_simple_bbcode_transformers() -> None:
    ...
    assert bb.convert_hide_to_spoiler("[hide=x]a[/hide]") == "[spoiler=x]a[/spoiler]"
    assert bb.convert_spoiler_to_hide("[spoiler=x]a[/spoiler]") == "[hide=x]a[/hide]"
    assert bb.remove_hide("[hide=x]a[/hide]") == "a"
    assert bb.convert_named_spoiler_to_named_hide("[spoiler=Name]x[/spoiler]") == "[hide=Name]x[/hide]"
    assert bb.remove_spoiler("[spoiler=Name]x[/spoiler]") == "x"
    assert bb.remove_color("[color=red]x[/color]") == "x"
    assert bb.convert_named_spoiler_to_normal_spoiler("[spoiler=Name]x[/spoiler]") == "[spoiler]x[/spoiler]"
    assert bb.convert_spoiler_to_code("[spoiler=Name]x[/spoiler]") == "[code=Name]x[/code]"
    ...
    assert bb.convert_to_align("[center]x[/center][right]y[/right]") == "[align=center]x[/align][align=right]y[/align]"
...
[/spoiler]"""
    converted = bb.convert_collapse_to_comparison(six_images, "spoiler", [six_images])
    assert "[comparison=Source, Encode]" in converted

    hide = six_images.replace("spoiler", "hide")
    assert "[comparison=Source, Encode]" in bb.convert_collapse_to_comparison(hide, "hide", [hide])
...
def test_collapse_conversion_skips_invalid_or_small_groups() -> None:
    bb = BBCODE()
    small = "[spoiler=Comparison]" + "".join(f"[img]https://x/{i}.jpg[/img]" for i in range(3)) + "[/spoiler]"
    assert bb.convert_collapse_to_comparison(small, "spoiler", [small]) == small
    invalid = "".join(f"[img]https://x/{i}.jpg[/img]" for i in range(6))
    assert bb.convert_collapse_to_comparison(invalid, "spoiler", [invalid]) == invalid
    assert bb.convert_collapse_to_comparison(invalid, "hide", [invalid]) == invalid
...
def test_centered_comparison_caps_width_and_empty_collapse_sources() -> None:
    ...
    empty_sources = "[spoiler=]" + "".join(f"[img]https://x.example/{index}.jpg[/img]" for index in range(6)) + "[/spoiler]"
    assert bb.convert_collapse_to_comparison(empty_sources, "spoiler", [empty_sources]) == empty_sources
