...
def test_capture_size_and_cleanup_policies() -> None:
    ...
    assert capture._should_cleanup_after_capture(
        Meta(tv_pack=False, discs=[]), True
    )
    assert not capture._should_cleanup_after_capture(
        Meta(tv_pack=True, discs=[]), True
    )
    assert not capture._should_cleanup_after_capture(
        Meta(tv_pack=False, discs=[{}, {}]), True
    )
...
def test_disc_screenshots_early_exits_and_invalid_fps(
    ...
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="imgbb")
    )

    existing = _disc_meta(
        tmp_path, image_list=[{"img_url": "https://img.invalid/a.png"}]
    )
    capture.cutoff = 1
    assert (
        asyncio.run(
            capture.disc_screenshots(
                existing,
                "Disc",
                bdinfo,
                existing.uuid,
    ...
    assert (
        asyncio.run(
            capture.disc_screenshots(
                zero, "Disc", bdinfo, zero.uuid, zero.base_dir, False
            )
        )
        == []
    )
    ...
    bad_fps = {
        **bdinfo,
        "video": [{"fps": "bad fps", "codec": "AVC", "hdr_dv": ""}],
    }
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["1", "2"])
    )
    monkeypatch.setattr(
        capture, "capture_disc_task", AsyncMock(return_value=None)
    )
    result = asyncio.run(
        capture.disc_screenshots(
            _disc_meta(tmp_path),
...
def test_disc_screenshots_capture_overlay_smallest_registration_and_cleanup(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source, bdinfo = _disc_source(tmp_path)
    meta = _disc_meta(
        tmp_path, frame_overlay=True, hdr="HDR10", discs=[{"name": "disc"}]
    )
    ...
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["10", "20"])
    )
    monkeypatch.setattr(
        capture,
        "get_frame_info",
        AsyncMock(side_effect=[{"frame_type": "I"}, {"frame_type": "P"}]),
    )
    cleanup = AsyncMock()
    ...
    async def capture_frame(
        index: int, file: str, _ss_time: str, image_path: str, *_args: object
    ) -> tuple[int, str]:
        nonlocal calls
        calls += 1
    ...
    result = asyncio.run(
        capture.disc_screenshots(
            meta, "Disc:Title", bdinfo, meta.uuid, meta.base_dir, False
        )
    )
    assert len(result) == 1
    assert calls == 2
...
def test_disc_screenshots_vapoursynth_lostimg_and_reuse(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _source, bdinfo = _disc_source(tmp_path)
    meta = _disc_meta(tmp_path, screens=2, imghost="lostimg", debug=False)
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="lostimg")
    )
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["1", "2", "3"])
    )

    module = ModuleType("src.integrations.media.vapoursynth")

    def vs_screengn(
        *, source: str, encode: object, num: int, dir: str
    ) -> None:
        del source, encode
    ...
    monkeypatch.setitem(
        sys.modules, "src.integrations.media.vapoursynth", module
    )
    result = asyncio.run(
        capture.disc_screenshots(
            meta,
...
def test_disc_screenshots_retake_success_failure_and_remove_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _source, bdinfo = _disc_source(tmp_path)
    meta = _disc_meta(tmp_path, imghost="pixhost", screens=1, debug=False)
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="pixhost")
    )
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["1", "2"])
    )
    monkeypatch.setattr(
        capture, "random", SimpleNamespace(uniform=lambda *_args: 5.0)
    )
    calls = 0

    async def retaking(
        index: int, _file: str, _time: str, image_path: str, *_args: object
    ) -> tuple[int, str] | None:
        nonlocal calls
        calls += 1
    ...
    failure = _disc_meta(
        tmp_path, imghost="unknown", screens=1, uuid="failure", debug=False
    )
    monkeypatch.setattr(
        capture, "capture_disc_task", AsyncMock(return_value=None)
    )
...
def test_capture_disc_task_filters_verbose_failure_and_exception(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    result = asyncio.run(
        capture.capture_disc_task(
            0, str(source), "10", str(output), "none", "verbose", True, meta
        )
    )
    ...
    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(return_value=(1, b"", b"failed"))
    )
    assert (
        asyncio.run(
            capture.capture_disc_task(
    ...
    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(side_effect=RuntimeError("boom"))
    )
    assert (
        asyncio.run(
            capture.capture_disc_task(
...
def test_dvd_screenshots_early_reuse_valid_capture_and_cleanup(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    capture.cutoff = 1
    existing = _dvd_meta(
    ...
    existing_path = (
        capture.screenshots_dir(meta.base_dir, meta.uuid) / "DISC_ONE-0.png"
    )
    _write_large(existing_path)
    capture.register_screenshots(
        meta.base_dir, meta.uuid, [str(existing_path)], "DISC_ONE"
    )
    ...
    _write_large(two)
    asyncio.run(
        capture.dvd_screenshots(
            meta, 0, num_screens=2, cleanup_after_capture=False
        )
    )
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["1", "2"])
    ...
    cleanup = AsyncMock()
    monkeypatch.setattr(capture.cleanup_manager, "cleanup", cleanup)
    capture_meta = _dvd_meta(
        tmp_path, uuid="dvd-capture", screens=1, frame_overlay=True
    )

    async def dvd_capture(
    ...
    assert (
        len(
            capture.manifest_files(
                capture_meta.base_dir, capture_meta.uuid, "DISC_ONE"
            )
        )
        == 1
    )
...
def test_dvd_screenshots_vob_fallback_retake_and_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    asyncio.run(
        capture.dvd_screenshots(
            meta, 0, num_screens=1, cleanup_after_capture=False
        )
    )
    ...
    asyncio.run(
        capture.dvd_screenshots(
            failure, 0, num_screens=1, cleanup_after_capture=False
        )
    )


def test_capture_dvd_screenshot_scaling_overlay_duration_and_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(capture, "run_ffmpeg", success)
    result = asyncio.run(
        capture.capture_dvd_screenshot(
            (1, str(source), str(output), "10", meta, 720, 576, 1.2, 0.8)
        )
    )
    ...
    output.unlink()
    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(return_value=(0, b"", b""))
    )
    assert asyncio.run(
        capture.capture_dvd_screenshot(
            (1, str(source), str(output), "2", meta, 720, 576, 1, 1)
        )
    ) == (1, None)
    ...
    monkeypatch.setattr(
        capture.MediaInfo,
        "parse",
        lambda _path: (_ for _ in ()).throw(RuntimeError("bad media")),
    )
    assert asyncio.run(
        capture.capture_dvd_screenshot(
            (1, str(source), str(output), "2", meta, 720, 576, 1, 1)
...
def _screen_meta(
    tmp_path: Path, folder_id: str = "release", **values: object
) -> Meta:
...
def test_screenshots_category_dispatch_existing_and_mediainfo_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert (
        asyncio.run(
            capture.screenshots(
                game.path, "Game", game.uuid, game.base_dir, game
            )
        )
        == []
    )
    ...
    book = _screen_meta(
        tmp_path, folder_id="book", category="BOOK", audiobook=True
    )
    ...
    assert (
        asyncio.run(
            capture.screenshots(
                book.path, "Book", book.uuid, book.base_dir, book
            )
        )
        == []
    )
    ...
    ebook = _screen_meta(
        tmp_path, folder_id="ebook", category="BOOK", audiobook=False
    )
    monkeypatch.setattr(
        capture,
        "generate_ebook_screenshots",
        AsyncMock(return_value=["page.png"]),
    )
    assert asyncio.run(
        capture.screenshots(
            ebook.path, "Book", ebook.uuid, ebook.base_dir, ebook
        )
    ) == ["page.png"]

    capture.cutoff = 1
    existing = _screen_meta(
        tmp_path,
        folder_id="existing",
    ...
    assert (
        asyncio.run(
            capture.screenshots(
                invalid.path, "Movie", invalid.uuid, invalid.base_dir, invalid
            )
        )
        is None
    )
...
def test_screenshots_safe_float_manual_frames_and_local_reuse(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        capture, "determine_tonemapping", AsyncMock(return_value=False)
    )
    monkeypatch.setattr(
        capture, "capture_screenshot", AsyncMock(return_value=None)
    )
    assert (
        asyncio.run(
            capture.screenshots(
...
def test_screenshots_capture_overlay_success_gather_errors_and_cleanup(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    meta = _screen_meta(tmp_path, frame_overlay=True, screens=2)
    _write_media_info(tmp_path, meta.uuid)
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="ptscreens")
    )
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["1", "2"])
    )
    ...
    result = asyncio.run(
        capture.screenshots(meta.path, "Movie", meta.uuid, meta.base_dir, meta)
    )
    ...
    async def fail_gather(
        *awaitables: object, **_kwargs: object
    ) -> list[object]:
    ...
    with pytest.raises(
        ScreenshotCaptureError, match="Screenshot capture failed"
    ):
        asyncio.run(
            capture.screenshots(
                broken.path, "Movie", broken.uuid, broken.base_dir, broken
            )
        )


def test_screenshots_retake_success_exhaustion_manual_lostimg_and_random_fallback(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="imgbox")
    )
    monkeypatch.setattr(
        capture, "valid_ss_time", AsyncMock(return_value=["1"])
    )
    monkeypatch.setattr(
        capture, "determine_tonemapping", AsyncMock(return_value=False)
    )
    ...
    exhausted = _screen_meta(
        tmp_path, folder_id="exhausted", imghost="unknown"
    )
    _write_media_info(tmp_path, exhausted.uuid)
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="unknown")
    )
    monkeypatch.setattr(capture, "capture_screenshot", small_then_large)
    assert (
        asyncio.run(
...
def test_capture_screenshot_invalid_paths_directory_and_basic_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    output = tmp_path / "frame.png"
...
def test_capture_screenshot_libplacebo_warmup_retry_fallback_and_timeout(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
...
def test_capture_screenshot_overlay_pts_compile_failure_complex_error_and_cancellation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
    ...
    monkeypatch.setattr(
        ...
        lambda _command: (_ for _ in ()).throw(RuntimeError("compile failed")),
    )
    monkeypatch.setattr(
        capture,
        "run_ffmpeg",
        AsyncMock(
            return_value=(1, b"", b"Error initializing complex filters")
        ),
    )
    assert asyncio.run(
        capture.capture_screenshot(
    ...
    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(side_effect=asyncio.CancelledError)
    )
    ...
    bad = Meta(
        frame_overlay=True, frame_info_map={}, resolution="invalid", debug=True
    )
    assert (
        asyncio.run(
            capture.capture_screenshot(
...
def test_get_frame_info_patterns_none_and_exception(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
    ...
    monkeypatch.setattr(
        ...
        "run_ffmpeg",
        AsyncMock(
            return_value=(0, b"", b"showinfo pict_type:I pts_time:2.500")
        ),
    )
    ...
    monkeypatch.setattr(
        capture,
        "run_ffmpeg",
        AsyncMock(return_value=(0, b"", b"showinfo type:P ")),
    )
    assert (
        asyncio.run(capture.get_frame_info(str(source), "2", meta))[
            "frame_type"
        ]
        == "P"
    )
    ...
    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(return_value=(None, b"", b""))
    )
    ...
    assert asyncio.run(
        capture.get_frame_info(str(source), "2", Meta(frame_rate=None))
    ) == {"frame_type": "Unknown", "frame_number": 48}


def test_libplacebo_compatibility_all_outcomes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
    ...
    assert asyncio.run(
        capture.check_libplacebo_compatibility(
            1.2,
            0.8,
            1920,
            1080,
    ...
    assert (
        calls == 1
        and not Path(str(output).replace(".png", "_test.png")).exists()
    )
    ...
    monkeypatch.setattr(
        capture,
        "run_ffmpeg",
        AsyncMock(side_effect=lambda _command: next(outcomes)),
    )
    assert asyncio.run(
        capture.check_libplacebo_compatibility(
            1, 1, 1920, 1080, str(source), "1", str(output), "quiet", meta
        )
    ) == (False, True)

    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(return_value=(1, b"", b""))
    )
    assert asyncio.run(
        capture.check_libplacebo_compatibility(
            1, 1, 1920, 1080, str(source), "1", str(output), "quiet", meta
    ...
    monkeypatch.setattr(
        capture, "run_ffmpeg", AsyncMock(side_effect=RuntimeError("failed"))
    )
    assert asyncio.run(
        capture.check_libplacebo_compatibility(
            1, 1, 1920, 1080, str(source), "1", str(output), "quiet", meta
...
def test_determine_tonemapping_and_warmup_edges(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
    ...
    assert asyncio.run(
        capture.determine_tonemapping(
            1, 1, 1920, 1080, str(source), "1", str(output), "quiet", good
        )
    )
    ...
    monkeypatch.setattr(
        capture,
        "check_libplacebo_compatibility",
    ...
    assert asyncio.run(
        capture.determine_tonemapping(
            1, 1, 1920, 1080, str(source), "1", str(output), "quiet", overlay
        )
    )
    ...
    assert (
        asyncio.run(
            capture.libplacebo_warmup(
                str(source), Meta(libplacebo=False), "quiet"
            )
        )
        is None
    )
    assert (
        asyncio.run(
            capture.libplacebo_warmup(
                str(source),
                Meta(libplacebo=True, libplacebo_warmed=True),
                "quiet",
            )
        )
        is None
    ...
    assert (
        asyncio.run(
            capture.libplacebo_warmup(
                str(tmp_path / "missing"), Meta(libplacebo=True), "quiet"
            )
        )
        is None
    )
    ...
    monkeypatch.setattr(
        capture,
        "run_ffmpeg",
        AsyncMock(side_effect=RuntimeError("warmup failed")),
    )
    warmed = Meta(libplacebo=True, libplacebo_warmed=False, debug=True)
    asyncio.run(capture.libplacebo_warmup(str(source), warmed, "verbose"))
    assert warmed.libplacebo_warmed
...
def test_run_ffmpeg_arm_and_bundled_candidate(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    output = tmp_path / "frame.png"
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
    candidate = (
        Path(capture.__file__).resolve().parent.parent
        / "bin"
        / "ffmpeg"
        / "arm"
        / "ffmpeg"
    ...
    monkeypatch.setattr(
        capture, "configured_binary", lambda *_args, **_kwargs: None
    )
...
def test_disc_screenshot_error_result_remove_failure_and_exhaustion(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _source, bdinfo = _disc_source(tmp_path)
    meta = _disc_meta(
        tmp_path, uuid="disc-errors", screens=1, imghost="unknown", debug=False
    )
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="unknown")
    )
    ...
    meta = _disc_meta(
        tmp_path, uuid="disc-exhaust", screens=1, imghost="imgbox", debug=False
    )
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="imgbox")
    )
...
def test_dvd_existing_files_error_strings_delete_failure_and_exhaustion(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    def parse(_path: str, **kwargs: object) -> object:
        if kwargs.get("output") == "JSON":
    ...
    error_meta = _dvd_meta(
        tmp_path, uuid="dvd-error-string", screens=1, debug=False
    )
    monkeypatch.setattr(
        capture,
        "capture_dvd_screenshot",
        AsyncMock(return_value=(0, "Error from DVD capture")),
    )
    asyncio.run(
        capture.dvd_screenshots(
            error_meta, 0, num_screens=1, cleanup_after_capture=False
        )
    )

    failure = _dvd_meta(
        tmp_path, uuid="dvd-retake-failure", screens=1, debug=False
    )
    attempts = 0

    async def always_small(
        task: tuple[int, str, str, str, Meta, float, float, float, float],
    ) -> tuple[int, str]:
        nonlocal attempts
        attempts += 1
    ...
    exception = _dvd_meta(
        tmp_path, uuid="dvd-retake-exception", screens=1, debug=False
    )
    calls = 0

    async def initial_then_error(
        task: tuple[int, str, str, str, Meta, float, float, float, float],
    ) -> tuple[int, str]:
        nonlocal calls
        calls += 1
    ...
    asyncio.run(
        capture.dvd_screenshots(
            exception, 0, num_screens=1, cleanup_after_capture=False
        )
    )
    assert calls >= 2


def test_capture_dvd_no_filters_format_branch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.vob"
    source.write_bytes(b"video")
    output = tmp_path / "frame.png"
    monkeypatch.setattr(
        capture.MediaInfo, "parse", lambda _path: SimpleNamespace(tracks=[])
    )
    commands: list[str] = []
...
def test_audiobook_fallback_no_cover_direct_path_skips_and_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import mutagen
    import mutagen.mp4
    ...
    monkeypatch.setattr(
        mutagen,
        "File",
        lambda _path: (_ for _ in ()).throw(ValueError("broken chapters")),
    )
    ...
    assert not asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
            Meta(path=str(audio), filelist=[]), str(output)
        )
    )
    ...
    monkeypatch.setattr(
        mutagen.mp4,
        "MP4Tags",
        lambda *_args: (_ for _ in ()).throw(RuntimeError("fallback failed")),
    )
    assert not asyncio.run(
        capture.extract_embedded_cover_from_audiobook(
            Meta(
...
def test_artwork_invalid_written_file_epub_parent_resolution_and_comic_jpg(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert not asyncio.run(
        capture.download_artwork_from_meta(meta, str(artwork), force=True)
    )
    ...
    assert not asyncio.run(
        capture.extract_epub_cover(str(malformed), str(artwork))
    )
...
def test_prepare_epub_downloaded_artwork_branch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(capture, "is_valid_cover_image", lambda _path: False)
    monkeypatch.setattr(
        capture, "extract_epub_cover", AsyncMock(return_value=False)
    )
    monkeypatch.setattr(
        capture, "download_artwork_from_meta", AsyncMock(return_value=True)
    )
    meta = Meta(audiobook=False, artwork_path="")
    result = asyncio.run(
        capture.prepare_book_cover(
            str(epub), "downloaded", str(tmp_path), meta
        )
    )
    assert result and meta.artwork_path == result


def test_generate_epub_cover_success_and_cached_banner(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import fitz
    ...
    banner = (
        capture.artwork_dir(meta.base_dir, meta.uuid) / "POSTER_BANNER.png"
    )
    ...
    class Page:
        def get_pixmap(self, *, matrix: object) -> object:
            del matrix
            return SimpleNamespace(
                save=lambda path: Path(path).write_bytes(b"page")
            )
    ...
    monkeypatch.setattr(
        capture, "prepare_book_cover", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(capture, "extract_epub_cover", extract)
    pages = asyncio.run(
        capture.generate_ebook_screenshots(
            str(epub), "Book", meta.uuid, meta.base_dir, meta, num_screens=1
        )
    )
    assert (
        pages and cover.is_file() and meta.artwork_banner_path == str(banner)
    )


def test_screenshots_remaining_result_exception_keyboard_random_and_force(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    meta = _screen_meta(tmp_path, folder_id="registered", screens=1)
    _write_media_info(tmp_path, meta.uuid)
    registered = (
        capture.screenshots_dir(meta.base_dir, meta.uuid) / "registered.png"
    )
    _write_large(registered)
    capture.register_screenshots(
        meta.base_dir, meta.uuid, [str(registered)], "main"
    )
    assert asyncio.run(
        capture.screenshots(meta.path, "Movie", meta.uuid, meta.base_dir, meta)
    )
    ...
    _write_media_info(
        ...
        values.uuid,
        general="bad",
        video={"Duration": "bad", "Width": 1920, "Height": 1080},
    )
    monkeypatch.setattr(
        capture, "get_image_host", AsyncMock(return_value="imgbb")
    )
    ...
    exception_meta = _screen_meta(
        tmp_path, folder_id="result-exception", screens=1
    )
    ...
    with pytest.raises(KeyboardInterrupt):
        asyncio.run(
            capture.screenshots(
                ...
                "Movie",
                cancel_meta.uuid,
                cancel_meta.base_dir,
                cancel_meta,
            )
        )
    monkeypatch.setattr(capture.asyncio, "gather", real_gather)

    random_meta = _screen_meta(
        tmp_path,
        folder_id="random-retake",
        screens=1,
        imghost="imgbox",
        retake=True,
    )
...
def test_libplacebo_zscale_cleanup_and_determine_non_hardware(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = tmp_path / "source.mkv"
    source.write_bytes(b"video")
...
def test_dvd_retake_remains_small_until_exhausted(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    meta = _dvd_meta(
        tmp_path, uuid="dvd-small-exhaust", screens=1, debug=False
    )
...
def test_screenshots_default_request_and_existing_remote_zero_capture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert (
        asyncio.run(
            capture.screenshots(
                remote.path, "Movie", remote.uuid, remote.base_dir, remote
            )
        )
        is None
    )
...
def test_screenshots_empty_manual_frame_list_uses_default_screen_count(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
