...
def test_unwalled_torrent_path_root_guards(tmp_path: Path) -> None:
    ...
    assert not Unwalled._valid_torrent_paths(
        Meta(path=str(missing), filelist=[])
    )

    invalid_root = tmp_path / ".pad-folder"
    invalid_root.mkdir()
    assert not Unwalled._valid_torrent_paths(
        Meta(path=str(invalid_root), filelist=[])
    )


def test_unwalled_prepared_root_handles_resolve_error(
...
def test_unwalled_v1_header_and_content_mode_guards() -> None:
    assert not Unwalled._valid_v1_info(
        {**_valid_header(), b"name": b"CON", b"length": 1}
    )
...
def test_unwalled_single_and_multi_v1_guards() -> None:
    assert not Unwalled._valid_single_file_info(
        {b"length": True}, b"x" * 20, 16384
    )
    assert not Unwalled._valid_multi_file_info(
        {b"files": []}, b"x" * 20, 16384
    )

    assert (
        Unwalled._complete_v1_file_entry({b"length": True, b"path": [b"a"]})
        is None
    )
    assert (
        Unwalled._complete_v1_file_entry({b"length": 1, b"path": []}) is None
    )
    assert (
        Unwalled._complete_v1_file_entry(
            {b"length": 1, b"path": [b"a"], b"attr": b"l"}
        )
        is None
    )
...
def test_unwalled_torrent_file_matching_guards(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    missing = tmp_path / "missing"
    assert not Unwalled._torrent_matches_files(
        {b"name": b"missing"}, Meta(path=str(missing), filelist=[])
    )
    ...
    assert not Unwalled._single_torrent_matches(
        {b"length": 1}, Meta(filelist=[]), root
    )
    assert (
        Unwalled._expected_files(
            Meta(filelist=[str(root / "missing.mp3")]), root
        )
        is None
    )
    ...
    monkeypatch.setattr(
        Unwalled,
        "_torrent_root_matches",
        classmethod(
            lambda _cls, _info, _root: (_ for _ in ()).throw(ValueError("bad"))
        ),
    )
    assert not Unwalled._torrent_matches_files(
        {b"name": b"root"}, Meta(path=str(root), filelist=[])
    )


def test_unwalled_directory_match_expected_files_failure(
    tmp_path: Path,
) -> None:
    root = tmp_path / "root"
    root.mkdir()
...
def test_unwalled_artwork_invalid_cover_and_identity_branch(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    meta = Meta(
        artwork_path=str(tmp_path / "missing.jpg"),
        artwork_banner_path=str(tmp_path / "missing-banner.jpg"),
    )
    ...
    monkeypatch.setattr(
        tracker, "_image_details", lambda _value: next(details)
    )
    assert not tracker._valid_artwork(meta)
...
def test_unwalled_upload_bundle_helper_failures(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    ...
    meta = Meta(
        ...
        artwork_banner_path=str(tmp_path / "banner.jpg"),
        debug=False,
    )

    monkeypatch.setattr(
        tracker,
        "_validated_upload_torrent",
        lambda *_args: (
            {b"announce": b"https://unwalled.cc/announce/token"},
            {b"private": 1, b"source": b"Unwalled"},
        ),
    )
    monkeypatch.setattr(
        tracker, "_upload_metadata_is_valid", lambda *_args: False
    )
    ...
    bad_metadata = (
        {b"announce": b"https://unwalled.cc/announce/token"},
        {b"private": 0, b"source": b"Unwalled"},
    )
    assert not tracker._upload_metadata_is_valid(meta, bad_metadata)


def test_unwalled_upload_asset_and_torrent_failures(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    torrent = tmp_path / "upload.torrent"
    ...
    assert (
        tracker._validated_upload_torrent(meta, tmp_path / "missing.torrent")
        is None
    )
...
def test_unwalled_announce_and_private_metadata_guards() -> None:
    ...
    assert not tracker._private_metadata_is_valid(
        Meta(debug=False), {}, "https://unwalled.cc/announce/token"
    )
...
def test_unwalled_bundle_files_reject_missing_paths(tmp_path: Path) -> None:
    ...
    assert not _tracker()._bundle_files_are_safe(
        meta, tmp_path / "missing.torrent"
    )
...
def test_unwalled_upload_metadata_invalid_announce_and_private_fields() -> (
    None
):
    tracker = _tracker()
...
def test_unwalled_validated_upload_torrent_rejects_invalid_v1(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
