...
def test_nordicquality_filters_unsupported_categories():
    ...
    assert meta.tracker_status["NORDICQUALITY"] == {"upload": False, "skipped": True}
...
def test_nordicquality_category_ids():
    ...
    assert asyncio.run(tracker.get_category_id(Meta(category="MUSIC"))) == {"category_id": "3"}
    assert asyncio.run(tracker.get_category_id(Meta(category="GAME"))) == {"category_id": "4"}
...
def test_nordicquality_type_ids_for_music_books_and_games():
    ...
    assert asyncio.run(tracker.get_type_id(Meta(category="MUSIC", format="FLAC"))) == {"type_id": "8"}
    assert asyncio.run(tracker.get_type_id(Meta(category="BOOK", format="EPUB"))) == {"type_id": "9"}
    assert asyncio.run(tracker.get_type_id(Meta(category="BOOK", format="PDF"))) == {"type_id": "10"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME", platform="Windows"))) == {"type_id": "11"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME", platform="Linux"))) == {"type_id": "17"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME", platform="macOS"))) == {"type_id": "12"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME", platform="Android"))) == {"type_id": "13"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME", platform="iOS"))) == {"type_id": "14"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME", console_game=True))) == {"type_id": "18"}
    assert asyncio.run(tracker.get_type_id(Meta(category="GAME"))) == {"type_id": "15"}
...
def test_nordicquality_sanitizes_upload_name():
    ...
    assert asyncio.run(_tracker().get_name(meta)) == {"name": "AEroskobing.Ar.2025.HDR10P.DDP.DTS-X.and"}
...
def test_nordicquality_preserves_release_suffix_in_extensionless_uuid():
    ...
    assert asyncio.run(_tracker().get_name(meta)) == {"name": "Snatched.2017.UHD.BluRay.2160p.DTS-HD.MA.7.1.HEVC.REMUX-FraMeSToR"}
...
def test_nordicquality_strips_only_known_media_extensions():
    ...
    assert asyncio.run(_tracker().get_name(meta)) == {"name": "Movie.2025.1080p.BluRay.REMUX-GROUP"}
...
def test_nordicquality_strips_m2ts_media_extension():
    ...
    assert asyncio.run(_tracker().get_name(meta)) == {"name": "Movie.2025.1080p.WEB-DL-GROUP"}
...
def test_nordicquality_falls_back_to_generated_name():
    ...
    assert asyncio.run(_tracker().get_name(meta)) == {"name": "Movie.2025.1080p.BluRay.REMUX.DTS-HD.MA.7.1-GROUP"}
