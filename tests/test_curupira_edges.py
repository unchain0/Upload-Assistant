...
def _config(*, anon: bool = False) -> dict[str, Any]:
    return {"DEFAULT": {}, "TRACKERS": {"CURUPIRA": {"api_key": "key", "anon": anon}}}
...
def _response(payload: Any | None = None, *, text: str | None = None, status: int = 200) -> httpx.Response:
...
async def test_curupira_cache_and_exact_search_params(tmp_path: Path) -> None:
    ...
    assert await tracker.search_existing(_meta(tmp_path)) == ["Example.2025.1080p"]
...
def test_curupira_identifier_and_movie_search_paths() -> None:
    tracker = _tracker()
    assert tracker._tv_identifier(_meta(category="TV", imdb_id=123, imdb=123)) == {"imdbid": "tt123"}
    assert tracker._movie_search_params(_meta(imdb_id=123, imdb=123)) == {"t": "movie", "imdbid": "tt123"}
    assert tracker._movie_search_params(_meta(tmdb_id=456)) == {"t": "movie", "tmdbid": "456"}
...
def test_curupira_parse_error_and_duplicate_suppression(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_parse_dupes_from_response", lambda _text: (_ for _ in ()).throw(ElementTree.ParseError("bad")))
    dupes: list[dict[str, Any]] = []
    tracker._extend_unique_dupes("bad", dupes, set())
    assert dupes == []

    seen = {"same"}
    tracker._append_unique_dupe({"name": "duplicate", "link": "same"}, dupes, seen)
    assert dupes == []


@pytest.mark.asyncio
async def test_curupira_prepare_files_with_optional_nfo(tmp_path: Path) -> None:
    tracker = _tracker()
    ...
    assert await tracker._prepare_files(_meta(tmp_path, nzb_path=str(nzb))) is None
...
def test_curupira_cover_fallback_and_nonvideo_cover_data() -> None:
    tracker = _tracker()
    meta = _meta(hosted_artwork=["bad"], artwork_url="https://example.com/cover.jpg")
    assert tracker.get_cover(meta) == "https://example.com/cover.jpg"
    data: dict[str, Any] = {}
    tracker._apply_cover_data(data, _meta(category="BOOK", artwork_url="https://example.com/book.jpg"))
    assert data["custom_cover_url"] == "https://example.com/book.jpg"


@pytest.mark.asyncio
async def test_curupira_upload_missing_files_debug_and_transport(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    tracker = _tracker()
    tracker.common.check_nzb_file = AsyncMock(return_value=False)  # type: ignore[method-assign]
    meta = _meta(tmp_path)
    ...
    monkeypatch.setattr(tracker, "_prepare_files", AsyncMock(return_value=files))
    monkeypatch.setattr(tracker, "_prepare_data", AsyncMock(return_value={"name": "Release"}))
    debug = _meta(tmp_path, debug=True, tracker_status={})
    assert await tracker.upload(debug)

    monkeypatch.setattr(tracker, "_upload_release", AsyncMock(return_value=True))
...
@pytest.mark.asyncio
async def test_curupira_upload_release_errors_and_response(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    meta = _meta(tmp_path)
    files = {"nzb_file": ("release.nzb", b"nzb", "application/x-nzb")}
    for error in (
    ...
    status = {}
    assert not await tracker._handle_upload_response(meta, status, _response({}, status=500))
    assert "HTTP 500" in status["status_message"]

    status = {}
    assert await tracker._handle_upload_response(meta, status, _response({"public_id": "abc"}, status=201))
...
def test_curupira_extend_unique_dupes_adds_result(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_parse_dupes_from_response", lambda _text: [{"name": "One", "link": "1"}])
    dupes: list[dict[str, Any]] = []
...
@pytest.mark.asyncio
async def test_curupira_media_info_read_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from src.integrations.trackers.USENET import curupira as curupira_module

    path = tmp_path / "tmp" / "release" / "MEDIAINFO_CLEANPATH.txt"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("mediainfo", encoding="utf-8")
    monkeypatch.setattr(curupira_module.aiofiles, "open", lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("broken")))
    assert await _tracker().get_media_info(_meta(tmp_path)) == ""


def test_curupira_cover_prefers_hosted_artwork() -> None:
    meta = _meta(hosted_artwork=[{"raw_url": "https://example.com/hosted.jpg"}], artwork_url="https://example.com/fallback.jpg")
