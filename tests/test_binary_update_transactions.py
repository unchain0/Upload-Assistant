...
@pytest.mark.parametrize(
    ...
    [
        (get_7z, get_7z.SevenZipBinaryManager.ensure_7z_binary, "bin/7z/linux/amd64", "7zz", "25.00"),
        (get_bdinfo, get_bdinfo.BDInfoBinaryManager.ensure_bdinfo_binary, "bin/bdinfo/linux/amd64", "bdinfo", "v0.2.0"),
        (
            get_mkbrr,
            lambda base_dir: get_mkbrr.MkbrrBinaryManager.ensure_mkbrr_binary(base_dir, "v1.24.0"),
...
def test_failed_dynamic_hdr_update_preserves_existing_installation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    marker.write_text("working marker", encoding="utf-8")
    monkeypatch.setattr(get_dynamic_hdr_tools.shutil, "which", lambda _command: None)
    monkeypatch.setattr(get_dynamic_hdr_tools.platform, "system", lambda: "Linux")
    monkeypatch.setattr(get_dynamic_hdr_tools.platform, "machine", lambda: "x86_64")
    monkeypatch.setattr(get_dynamic_hdr_tools.httpx, "AsyncClient", lambda **_kwargs: _Client())
    monkeypatch.setattr(get_dynamic_hdr_tools, "download_bounded_asset", fail_download)
...
def test_failed_zentag_update_preserves_existing_installation(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(get_zentag, "HTTPX", type("HTTPXStub", (), {"AsyncClient": lambda **_kwargs: _Client()}))
