...
def _response(
    *, status: int = 200, text: str = "", url: str = "https://totheglory.im/"
) -> httpx.Response:
...
async def test_ttg_type_matrix_documentary_animation_and_uhd() -> None:
    tracker = _tracker()
    assert (
        await tracker.get_type_id(
            _meta(genres=["Documentary"], resolution="720p")
        )
        == 62
    )
    assert (
        await tracker.get_type_id(
            _meta(genres=["Documentary"], resolution="1080p")
        )
        == 63
    )
    assert (
        await tracker.get_type_id(
            _meta(genres=["Documentary"], resolution="480p")
        )
        == 0
    )
    assert (
        await tracker.get_type_id(
            _meta(genres=["Documentary"], is_disc="BDMV")
        )
        == 64
    )
    assert (
        await tracker.get_type_id(
            _meta(genres=["Animation"], resolution="1080p")
        )
        == 58
    )
    assert (
        await tracker.get_type_id(
            _meta(genres=["Animation"], resolution="2160p")
        )
        == 108
    )
    assert (
        await tracker.get_type_id(
            _meta(genres=["Animation"], resolution="2160p", is_disc="BDMV")
        )
        == 109
    )
...
@pytest.mark.asyncio
async def test_ttg_upload_non_debug_delegates_response(
    ...
    tracker._handle_upload_response.assert_awaited_once_with(
        ANY, response, torrent_path
    )


@pytest.mark.asyncio
async def test_ttg_post_upload_uses_cookie_payload(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    tracker = _tracker()
    ...
    monkeypatch.setattr(
        ttg_module.httpx, "AsyncClient", lambda *_args, **_kwargs: Client()
    )
    data: dict[str, Any] = {}
...
@pytest.mark.asyncio
async def test_ttg_handle_upload_response_success_and_errors(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    tracker = _tracker()
    ...
    assert item.tracker_status["TOTHEGLORY"]["status_message"].endswith(
        "id=77"
    )
    ...
    with pytest.raises(UploadError):
        await tracker._handle_upload_response(
            _meta(),
            _response(url="https://totheglory.im/upload.php"),
            str(tmp_path / "x"),
        )
    ...
    with pytest.raises(UploadError, match="torrent id missing"):
        await tracker._handle_upload_response(
            _meta(), bad_url, str(tmp_path / "x")
        )


def test_ttg_search_release_name_parser() -> None:
    link = BeautifulSoup(
        '<a href="/t/1"><b><font>Release.Name<br></font></b></a>',
        "html.parser",
    ).find("a")
    assert link is not None
    assert ToTheGlory._release_name_from_link(link) == "Release.Name"
    unrelated = BeautifulSoup('<a href="/foo">Foo</a>', "html.parser").find(
        "a"
    )
...
@pytest.mark.asyncio
async def test_ttg_search_existing_success(
...
@pytest.mark.asyncio
async def test_ttg_validate_cookies_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_ttg_validate_credentials_and_recreate_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    ...
    assert not await tracker.validate_credentials(
        _meta(tmp_path, unattended=True, unattended_confirm=False)
    )

    monkeypatch.setattr(
        ttg_module.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: False
    )
    assert not await tracker._maybe_recreate_session(_meta(tmp_path), cookie)

    monkeypatch.setattr(
        ttg_module.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: True
    )
...
@pytest.mark.asyncio
async def test_ttg_two_factor_branches(
    ...
    client = AsyncMock()
    assert (
        await tracker._maybe_complete_two_factor(client, direct, _meta())
        is direct
    )

    missing = _response(
        text="<html></html>", url="https://totheglory.im/2fa.php"
    )
    with pytest.raises(UploadError, match="authenticity token"):
        await tracker._maybe_complete_two_factor(client, missing, _meta())

    page = _response(
        text='<input name="authenticity_token" value="token">',
        url="https://totheglory.im/2fa.php",
    )
    unattended = _meta(unattended=True, unattended_confirm=False)
    assert (
        await tracker._maybe_complete_two_factor(client, page, unattended)
        is page
    )
    ...
    monkeypatch.setattr(
        ttg_module, "prompt_in_thread", AsyncMock(return_value="123456")
    )
    monkeypatch.setattr(ttg_module.asyncio, "sleep", AsyncMock())
    assert await tracker._submit_two_factor(client, "token") is completed
    assert (
        tracker._authenticity_token(
...
@pytest.mark.asyncio
async def test_ttg_save_login_result_failure(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    tracker = _tracker()
    client = AsyncMock()
    monkeypatch.setattr(ttg_module.asyncio, "sleep", AsyncMock())
    await tracker._save_login_result(
        client,
        _response(text="bad", url="https://totheglory.im/login.php"),
        str(tmp_path / "cookie"),
    )


@pytest.mark.asyncio
async def test_ttg_description_preamble_technical_and_screens(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    common = AsyncMock()
    ...
    item = _meta(
        tmp_path, type="WEBDL", service_longname="Netflix", description=""
    )
    ...
    assert tracker._disc_description(
        {"type": "BDMV", "name": "DISC", "summary": "BDINFO"}
    ).startswith("[quote=DISC]")
    assert "VOB" in tracker._disc_description(
        {
            "type": "DVD",
            "vob": "VOB",
            "vob_mi": "VOBMI",
...
@pytest.mark.asyncio
async def test_ttg_technical_description_reads_mediainfo(
    tmp_path: Path,
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_ttg_download_new_torrent_failure(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    tracker = _tracker()
...
@pytest.mark.asyncio
async def test_ttg_remaining_type_and_validate_success(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    assert (
...
@pytest.mark.asyncio
async def test_ttg_maybe_complete_two_factor_submits(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    page = _response(
    ...
    completed = _response(url="https://totheglory.im/my.php")
    monkeypatch.setattr(
        tracker, "_submit_two_factor", AsyncMock(return_value=completed)
    )
    assert (
        await tracker._maybe_complete_two_factor(AsyncMock(), page, _meta())
        is completed
    )
...
async def test_ttg_save_login_result_success() -> None:
    ...
    await tracker._save_login_result(
        client, _response(url="https://totheglory.im/my.php"), "cookie.pkl"
    )
