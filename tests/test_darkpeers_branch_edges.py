...
@pytest.mark.asyncio
async def test_darkpeers_keep_folder_confirmation_rejection(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "validate_video_languages", AsyncMock(return_value=True))
    monkeypatch.setattr(tracker, "validate_video_resolution", AsyncMock(return_value=True))
    monkeypatch.setattr(tracker, "validate_video_quality", AsyncMock(return_value=True))
    ...
    monkeypatch.setattr(tracker, "validate_video_screenshots", lambda _meta: True)
    monkeypatch.setattr(tracker, "_confirm_or_skip", AsyncMock(return_value=False))
    meta = Meta(
        category="MOVIE",
...
def test_darkpeers_config_min_bitrate_guards() -> None:
    ...
    tracker.tracker_config["mixed"] = {"Good": "100", "Bad": "x", 123: 100, "Negative": -1}
    assert tracker._config_min_bitrate("mixed") == {"good": 100}


@pytest.mark.asyncio
async def test_darkpeers_video_quality_none_and_low_audio(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "_min_webl_bitrate", lambda *_args: None)
    assert await tracker.validate_video_quality(Meta(type="WEBDL", resolution="1080p"))
    ...
    meta = Meta(type="WEBDL", resolution="1080p", video_bitrate=5000, audio_bitrate=500)
...
@pytest.mark.asyncio
async def test_darkpeers_book_missing_identifier_after_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
...
async def test_darkpeers_audiobook_missing_narrator_and_runtime() -> None:
    tracker = _tracker()
    missing_narrator = _book_meta(audiobook=True, type="M4B", narrator="", audiobook_duration=3600)
    assert not await tracker.validate_book(missing_narrator)

    missing_runtime = _book_meta(audiobook=True, type="M4B", narrator="Narrator", audiobook_duration=0)
    assert not await tracker.validate_book(missing_runtime)


@pytest.mark.asyncio
async def test_darkpeers_audiobook_declined_edition_confirmation(monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker.common, "prompt_user_for_confirmation", AsyncMock(return_value=False))
    meta = _book_meta(audiobook=True, type="M4B", narrator="Narrator", audiobook_duration=3600)
...
def test_darkpeers_ebook_filename_must_contain_author_and_title(tmp_path: Path) -> None:
...
def test_darkpeers_book_format_aliases_from_codec() -> None:
    tracker = _tracker()
    m4a_alac = _book_meta(audiobook=True, type="M4A", mediainfo={"media": {"track": [{"@type": "Audio", "Format": "ALAC"}]}})
    m4a_aac = _book_meta(audiobook=True, type="M4A", mediainfo={"media": {"track": [{"@type": "Audio", "Format": "AAC"}]}})
    ogg_opus = _book_meta(audiobook=True, type="OGG", mediainfo={"media": {"track": [{"@type": "Audio", "Format": "Opus"}]}})
    ogg_vorbis = _book_meta(audiobook=True, type="OGG", mediainfo={"media": {"track": [{"@type": "Audio", "Format": "Vorbis"}]}})
...
def _music_meta(root: Path, relative_path: str, *, album: str = "Album", release_type: str = "Album") -> Meta:
    return Meta(
        category="MUSIC",
        title=album,
        path=str(root),
...
def test_darkpeers_music_path_policy_branches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(darkpeers_module.MusicValidator, "validate", lambda _self, _release: [])
    tracker = _tracker()

    root_file = tmp_path / "music.flac"
    root_file.write_bytes(b"x")
    assert not tracker.validate_music(_music_meta(root_file, "01 - Track.flac"))

    wrong_folder = tmp_path / "Wrong Folder"
    wrong_folder.mkdir()
    assert not tracker.validate_music(_music_meta(wrong_folder, "01 - Track.flac", album="Album"))

    nonexistent = tmp_path / "missing"
    assert not tracker.validate_music(_music_meta(nonexistent, " 01 - Track.flac"))
    assert not tracker.validate_music(_music_meta(nonexistent, "01 - Bad?.flac"))
...
async def test_darkpeers_audio_label_matrix_remaining_branches() -> None:
    tracker = _tracker()
    assert await tracker.get_audio(Meta(language_checked=True, is_disc="", original_language="Japanese", audio_languages=["English"])) == "Dubbed"
    assert await tracker.get_audio(Meta(language_checked=True, is_disc="", original_language="Japanese", audio_languages=["Swedish"])) == "Swedish Dubbed"
    assert await tracker.get_audio(Meta(language_checked=True, is_disc="", original_language="Japanese", audio_languages=["Japanese", "French", "German"])) == "MULTi"
    assert await tracker.get_audio(Meta(language_checked=True, is_disc="", original_language="Japanese", audio_languages=["English", "Swedish", "Danish"])) == "MULTi"
    assert await tracker.get_audio(Meta(language_checked=True, is_disc="", original_language="Japanese", audio_languages=["Swedish", "French"])) == "French MULTi"
...
def test_darkpeers_small_name_helpers_cover_guard_branches() -> None:
    ...
    assert DarkPeers._normalize_aka_year_order("Movie 2020 AKA", " ", "AKA", "2020") == "Movie 2020 AKA"
    assert DarkPeers._normalize_aka_year_order("Movie 2020 AKA", "Movie", "AKA", " ") == "Movie 2020 AKA"
...
def test_darkpeers_dub_element_insertion_and_blank_audio() -> None:
    assert DarkPeers._apply_dub_element("Movie AAC 2.0 x264", "") == "Movie AAC 2.0 x264"
    assert DarkPeers._apply_dub_element("Movie AAC 2.0 x264", "Dubbed") == "Movie Dubbed AAC 2.0 x264"
...
class _Response:
    ...
    def json(self) -> dict[str, Any]:
        return {"results": ["bad", {"id": 1, "name": "Example", "original_name": "Example"}, {"id": 2, "name": "Example", "original_name": "Other"}]}
...
@pytest.mark.asyncio
async def test_darkpeers_tv_year_skips_non_mapping_results(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_darkpeers_explicit_book_type_mapping_branches() -> None:
    ...
    assert await tracker.get_type_id(Meta(category="BOOK", type="CBR", audiobook=False, comic=True)) == {"type_id": "17"}
