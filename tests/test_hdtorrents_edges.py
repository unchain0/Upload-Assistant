...
def _response(
    text: str, *, url: str = "https://hd-torrents.org/torrents.php"
) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_hdtorrents_search_login_redirect_marks_skip(
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=_response(
                "login.php", url="https://hd-torrents.org/login.php"
            )
        ),
    )
    meta = Meta()
...
@pytest.mark.asyncio
async def test_hdtorrents_search_parses_rows_and_updates_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    ...
    assert results == [
        {
            "name": "Release One",
            "size": "1.5 GiB",
            "link": "https://hd-torrents.org/details.php?id=10",
        },
        {
            "name": "Release Two",
            "size": None,
            "link": "https://hd-torrents.org/details.php?id=11",
        },
    ]
...
def test_hdtorrents_row_result_header_and_missing_link() -> None:
    ...
    assert (
        tracker._search_results(
            '<tr><td class="mainblockcontent">Other</td></tr>'
        )
        == []
    )
...
async def test_hdtorrents_get_nfo_reads_first_file(tmp_path: Path) -> None:
    ...
    assert result == {
        "nfos": ("release.nfo", b"nfo-data", "application/octet-stream")
    }


@pytest.mark.asyncio
async def test_hdtorrents_upload_applies_loaded_cookies(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    cookies = [SimpleNamespace(domain="hd-torrents.org")]
    ...
    monkeypatch.setattr(
        tracker, "get_data", AsyncMock(return_value={"category": 1})
    )
