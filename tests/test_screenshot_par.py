...
def test_screenshot_par_scaling_preserves_coded_dimensions_when_disabled() -> (
    None
):
    width_scale, height_scale = screenshot_par_scale_factors(
        1920.0, 1040.0, 481 / 480, 1.85, apply_par_scaling=False
    )

    assert (width_scale, height_scale) == (1.0, 1.0)


def test_screenshot_par_scaling_restores_square_pixel_conversion_when_enabled() -> (
    None
):
    width_scale, height_scale = screenshot_par_scale_factors(
        1920.0, 1040.0, 481 / 480, 1.85, apply_par_scaling=True
    )
