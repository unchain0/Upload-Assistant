...
def test_crt_cover_source_language_and_tag_helper_branches() -> None:
    ...
    assert (
        CathodeRayTube._cover_source_url(item)
    ...
    tags = []
    CathodeRayTube._append_resolution_tags(
        tags, meta(resolution="OTHER", sd=True)
    )
    assert tags == ["sd"]

    tags = []
    CathodeRayTube._append_feature_tags(
        tags,
        meta(is_disc="BDMV", three_d="3D", extras=True, has_commentary=True),
    )
...
def test_crt_platform_and_audio_tag_remaining_branches() -> None:
    ...
    assert "nintendo" in site.get_tags(
        meta(category="GAME", platform="Nintendo Switch")
    )
    assert "atari" in site.get_tags(
        meta(category="GAME", platform="Atari 2600")
    )
    assert "ddp" in site.get_tags(meta(audio="DD+ 5.1"))
    assert "dd" in site.get_tags(
        meta(audio="Dolby Digital 2.0", channels="2.0")
    )
    assert "flac" in site.get_tags(meta(audio="FLAC 2.0", channels="2.0"))


@pytest.mark.asyncio
async def test_crt_host_cover_short_circuits_and_missing_cases(
    tmp_path: Path,
) -> None:
    site = tracker()
...
@pytest.mark.asyncio
async def test_crt_cover_upload_invalid_result_and_no_success(
    tmp_path: Path,
) -> None:
    site = tracker()
    cover = tmp_path / "cover.png"
    ...
    site.rehost_images_manager.uploadscreens_manager.upload_screens = (
        AsyncMock(
            return_value=([{"raw_url": "https://evil.invalid/x.png"}], 1)
        )
    )
...
@pytest.mark.asyncio
async def test_crt_generate_description_debug_writes_file(
    tmp_path: Path,
) -> None:
    site = tracker()
...
def test_crt_content_archive_screenshot_and_age_edge_branches() -> None:
    ...
    assert not asyncio.run(
        site.get_additional_checks(meta(filelist=["release.rar"]))
    )
    ...
    assert asyncio.run(
        site.get_additional_checks(
            meta(release_date="not-a-date", year="2000")
        )
    )
...
def test_crt_content_name_and_bdinfo_guard_branches() -> None:
    ...
    empty_table = (
        "<div id='files_1'><table><tr><td>only one</td></tr></table></div>"
    )
    assert CathodeRayTube._content_name(empty_table) == ""
    assert (
        CathodeRayTube._bd_info(
            "<div class='section-details'>No disc details</div>"
        )
        == ""
    )
...
def test_crt_link_extraction_guard_branches() -> None:
    ...
    response = httpx.Response(
        200, request=request, text='<a href="/torrents.php?id=44">Torrent</a>'
    )
    ...
    bad_link = BeautifulSoup(
        '<a href="/details.php?id=abc">bad</a>', "html.parser"
    ).find("a")
...
def test_crt_row_link_and_detail_guard_branches() -> None:
    ...
    response = httpx.Response(
        ...
        request=httpx.Request(
            "GET", "https://www.cathode-ray.tube/torrents.php"
        ),
        text="",
    )
    row = BeautifulSoup("<tr><td>missing links</td></tr>", "html.parser").find(
        "tr"
    )
    assert row is not None
    ...
    bad_title = BeautifulSoup(
        '<tr><td><a href="">Title</a></td><td><a href="?action=download">DL</a></td></tr>',
        "html.parser",
    ).find("tr")
    ...
    assert site._seen_link(
        "https://www.cathode-ray.tube/torrents.php?id=1", seen
    )

    detail = httpx.Response(
        200,
        request=httpx.Request(
            "GET", "https://www.cathode-ray.tube/torrents.php?id=1"
        ),
        text="<html></html>",
    )
...
async def test_crt_search_response_and_auth_helpers() -> None:
    ...
    login = httpx.Response(
        200,
        request=httpx.Request("GET", "https://www.cathode-ray.tube/login.php"),
        text="login",
    )
...
async def test_crt_find_log_upload_success_and_failure() -> None:
    ...
    response = httpx.Response(
        200,
        request=httpx.Request("GET", "https://www.cathode-ray.tube/log.php"),
        text=html,
    )
    ...
    assert (
        await site._find_log_upload(meta())
        == "https://www.cathode-ray.tube/torrents.php?id=77"
    )

    site.session.get = AsyncMock(
        side_effect=httpx.RequestError(
            "offline", request=httpx.Request("GET", site.base_url)
        )
    )  # type: ignore[method-assign]
...
@pytest.mark.asyncio
async def test_crt_upload_missing_auth_and_submit_failure(
    ...
    try:
        ...
        assert (
            "Failed to load authenticated upload form"
            in item.tracker_status["CATHODERAYTUBE"]["status_message"]
        )

        CathodeRayTube.auth_token = "csrf"
        monkeypatch.setattr(
            site, "_submit_upload", AsyncMock(return_value=False)
        )
        assert not await site.upload(
            meta(tracker_status={"CATHODERAYTUBE": {}})
        )
    finally:
...
@pytest.mark.asyncio
async def test_crt_upload_cookie_submit_and_finalize_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    site = tracker()
    cookies = httpx.Cookies({"session": "ok"})
    ...
    monkeypatch.setattr(
        ...
        "_find_log_upload",
        AsyncMock(
            return_value="https://www.cathode-ray.tube/torrents.php?foo=bar"
        ),
    )
    assert await site._finalize_upload(
        meta(tracker_status={"CATHODERAYTUBE": {}})
    ...
    monkeypatch.setattr(
        ...
        "_find_log_upload",
        AsyncMock(
            return_value="https://www.cathode-ray.tube/torrents.php?id=88"
        ),
    )
    monkeypatch.setattr(site, "_record_uploaded_torrent", AsyncMock())
    item = meta(tracker_status={"CATHODERAYTUBE": {}})
    assert await site._finalize_upload(item)
    site._record_uploaded_torrent.assert_awaited_once_with(
        item, "88", "https://www.cathode-ray.tube/torrents.php?id=88"
    )
...
async def test_crt_record_uploaded_torrent() -> None:
    site = tracker()
    site.config["TRACKERS"]["CATHODERAYTUBE"]["announce_url"] = (
        "https://signal.cathode-ray.tube/passkey/announce"
    )
    site.common.create_torrent_ready_to_seed = AsyncMock()  # type: ignore[method-assign]
    item = meta(tracker_status={"CATHODERAYTUBE": {}})
    await site._record_uploaded_torrent(
        item, "99", "https://www.cathode-ray.tube/torrents.php?id=99"
    )
...
@pytest.mark.asyncio
async def test_crt_host_cover_returns_empty_when_local_cover_cannot_be_prepared(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    site = tracker()
    monkeypatch.setattr(
        site, "_ensure_local_cover", AsyncMock(return_value=None)
    )
    assert await site._host_cover(meta()) == ""


@pytest.mark.asyncio
async def test_crt_search_row_entry_returns_none_when_detail_has_no_name(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    site = tracker()
    response = httpx.Response(
...
@pytest.mark.asyncio
async def test_crt_upload_success_delegates_to_finalize(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    site = tracker()
    original = CathodeRayTube.auth_token
    try:
        ...
        monkeypatch.setattr(site, "_host_cover", AsyncMock(return_value=""))
        monkeypatch.setattr(
            site, "_submit_upload", AsyncMock(return_value=True)
        )
        monkeypatch.setattr(
            site, "_finalize_upload", AsyncMock(return_value=True)
        )
        item = meta(tracker_status={"CATHODERAYTUBE": {}})
