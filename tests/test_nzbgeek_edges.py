...
def _config(*, limit: int = 10) -> dict[str, Any]:
    return {"TRACKERS": {"NZBGEEK": {"api_key": "test-key", "daily_api_hit_limit": limit}}, "USENET": {}}
...
def _response(status: int = 200, text: str = "") -> httpx.Response:
    return httpx.Response(status, text=text, request=httpx.Request("GET", "https://api.nzbgeek.info/api"))


@pytest.mark.asyncio
async def test_nzbgeek_name_query_and_parser(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    meta = _meta(tmp_path, scene_name="Scene.Release")
    monkeypatch.setattr(nzbgeek_module, "build_newznab_search_query", lambda _meta: "query")
    monkeypatch.setattr(nzbgeek_module, "parse_newznab_dupes", lambda text: [{"name": text}])

    assert await tracker.get_name(meta) == "Scene.Release"
    assert tracker.get_search_query(meta) == "query"
    assert tracker._parse_dupes_from_response("payload") == [{"name": "payload"}]


@pytest.mark.asyncio
async def test_nzbgeek_local_upload_cache_short_circuits(tmp_path: Path) -> None:
    tracker = _tracker()
    meta = _meta(tmp_path)
...
@pytest.mark.asyncio
async def test_nzbgeek_search_hit_limit_rejection(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(nzbgeek_module, "reserve_daily_api_hit", AsyncMock(return_value=(False, 10)))
    assert await _tracker().search_existing(_meta(tmp_path)) == []


@pytest.mark.asyncio
async def test_nzbgeek_search_success_and_empty_response(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(nzbgeek_module, "reserve_daily_api_hit", AsyncMock(return_value=(True, 1)))
    monkeypatch.setattr(tracker, "_parse_dupes_from_response", lambda text: [{"name": text}])
    monkeypatch.setattr(tracker, "_search_response", AsyncMock(return_value=_response(text="dupe")))
...
def test_nzbgeek_search_parameter_variants(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    monkeypatch.setattr(tracker, "get_search_query", lambda _meta: "fallback-query")

    tv = _meta(tmp_path, category="TV", tvdb_id=123, season_int=2, episode_int=4)
    assert tracker._search_params(tv) == {"cat": "5040", "t": "tvsearch", "tvdbid": "123", "season": "2", "ep": "4"}
    ...
    assert tracker._movie_search_params(movie) == {"t": "movie", "imdbid": "tt123"}
    assert tracker._movie_search_params(_meta(tmp_path, category="MOVIE", imdb_tt="")) == {"t": "movie", "q": "fallback-query"}
...
@pytest.mark.asyncio
async def test_nzbgeek_search_response_builds_api_params(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_nzbgeek_nfo_candidates_scene_and_missing(tmp_path: Path) -> None:
    tracker = _tracker()
    scene_root = tmp_path / "tmp" / "scene"
    scene_root.mkdir(parents=True)
    (scene_root / "release.nfo").write_bytes(b"nfo")
    scene = _meta(tmp_path, uuid="scene", scene=True)
    assert await tracker._get_nfo_file(scene) == ("release.nfo", b"nfo", "application/octet-stream")
...
@pytest.mark.asyncio
async def test_nzbgeek_upload_missing_source_and_debug(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    ...
    assert debug.tracker_status["NZBGEEK"]["status_message"].startswith("Debug mode")


@pytest.mark.asyncio
async def test_nzbgeek_upload_error_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    tracker = _tracker()
    meta = _meta(tmp_path, tracker_status={})
    files: dict[str, tuple[str, bytes, str]] = {}

    monkeypatch.setattr(tracker, "_submit", AsyncMock(side_effect=httpx.TimeoutException("timeout")))
    assert not await tracker._upload_with_error_handling(meta, files, None, meta.tracker_status.setdefault("NZBGEEK", {}))
    assert "timed out" in meta.tracker_status["NZBGEEK"]["status_message"]

    request_error = httpx.RequestError("offline", request=httpx.Request("POST", tracker.submit_url))
    tracker._submit = AsyncMock(side_effect=request_error)  # type: ignore[method-assign]
    assert not await tracker._upload_with_error_handling(meta, files, None, meta.tracker_status["NZBGEEK"])
    ...
    assert not await tracker._upload_with_error_handling(meta, files, None, meta.tracker_status["NZBGEEK"])
    assert "Unexpected error" in meta.tracker_status["NZBGEEK"]["status_message"]


@pytest.mark.asyncio
async def test_nzbgeek_process_upload_response_rejections(tmp_path: Path) -> None:
    tracker = _tracker()
    meta = _meta(tmp_path)
    status: dict[str, Any] = {}
    assert not await tracker._process_upload_response(meta, _response(status=500, text="error"), None, status)
    assert "HTTP 500" in status["status_message"]

    status = {}
    bad = _response(text='{"response":{"@attributes":{"API":"NO","REGISTER":"OK"}}}')
