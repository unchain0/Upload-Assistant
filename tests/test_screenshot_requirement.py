...
def test_configured_screen_minimum_not_reduced_avail_c9589b() -> None:
    ...
    assert (
        ...
        == "Minimum of 4 successfully hosted screenshots required, but only 1"
        " hosted screenshot(s) are available."
    )
...
def test_tracker_override_is_validated_instead_of_global_images() -> None:
    ...
    assert (
        ...
        == "Minimum of 4 successfully hosted screenshots required for TEST,"
        " but only 1 hosted screenshot(s) are available."
    )
...
def test_requirement_error_distinguishes_local_from_hosted_screenshots() -> (
    ...
    assert screenshot_requirement_error(meta, config, local_available=4) == (
        "4 local screenshot(s) available, 0 successfully hosted; minimum"
        " hosted required: 4."
    )
