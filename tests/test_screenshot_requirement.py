...
def test_configured_screen_minimum_not_reduced_avail_c9589b() -> None:
    ...
    assert (
        screenshot_requirement_error(meta, config)
        == "Minimum of 4 successfully hosted screenshots required, but only 1 hosted screenshot(s) are available."
    )


def test_configured_screenshot_minimum_allows_exact_count() -> None:
    meta = Meta(
        category="MOVIE", image_list=[_image(number) for number in range(4)]
    )
    config = {"DEFAULT": {"min_successful_image_uploads": "4"}}
...
def test_tracker_override_is_validated_instead_of_global_images() -> None:
    meta = Meta(
        category="TV", image_list=[_image(number) for number in range(4)]
    )
    ...
    assert (
        screenshot_requirement_error(meta, config, "TEST")
        == "Minimum of 4 successfully hosted screenshots required for TEST, but only 1 hosted screenshot(s) are available."
    )
...
def test_requirement_error_distinguishes_local_from_hosted_screenshots() -> (
    None
):
    meta = Meta(category="MOVIE", image_list=[])
    config = {"DEFAULT": {"min_successful_image_uploads": 4}}

    assert screenshot_requirement_error(meta, config, local_available=4) == (
        "4 local screenshot(s) available, 0 successfully hosted; minimum hosted required: 4."
    )
