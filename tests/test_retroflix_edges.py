...
def _response(
    ...
    *,
    ...
    text: str | None = None,
    method: str = "POST",
    url: str = "https://retroflix.club/api/upload",
) -> httpx.Response:
...
async def test_retroflix_upload_payload_media_and_disc(tmp_path: Path) -> None:
    ...
    (root / "MEDIAINFO.txt").write_text(
        "Width : 1 080 pixels", encoding="utf-8"
    )
    (root / "[RETROFLIX].torrent").write_bytes(b"torrent")
    item = _meta(
        tmp_path,
        image_list=[
            {"raw_url": "https://img.invalid/1.png"},
            {"raw_url": None},
            "bad",
        ],
    )
...
async def test_retroflix_debug_upload_truncates_file() -> None:
    ...
    item = _meta()
    assert await tracker._debug_upload(
        item, {"file": "abcdefghijklmnopqrstuvwxyz", "name": "Release"}
    )
    assert item.tracker_status["RETROFLIX"]["status_message"].startswith(
        "Debug mode"
    )
...
@pytest.mark.asyncio
async def test_retroflix_upload_release_transport_errors(
...
async def test_retroflix_created_upload_error_missing_and_success() -> None:
    ...
    assert (
        item.tracker_status["RETROFLIX"]["status_message"]
    ...
    assert (
        "missing key torrent.id"
        in item.tracker_status["RETROFLIX"]["status_message"]
    )

    tracker.common.create_torrent_ready_to_seed = AsyncMock()  # type: ignore[method-assign]
    success = _response(
        {"error": False, "torrent": {"id": 42}, "message": "ok"}, status=201
    )
...
async def test_retroflix_additional_checks_adult_and_age_paths() -> None:
    ...
    assert not await tracker.get_additional_checks(
        _meta(year=recent_year, release_date="")
    )
    assert await tracker.get_additional_checks(
        _meta(category="OTHER", year=2000, release_date="")
    )
...
def test_retroflix_latest_release_age_episode_paths() -> None:
    ...
    assert years == []
    assert (
        tracker._latest_episode_date(_meta(tvdb_episode_data={}), years)
        is None
    )
...
def test_retroflix_movie_and_date_policy_paths() -> None:
    ...
    future = (
        __import__("datetime").datetime.now(__import__("datetime").UTC).date()
    )
...
def test_retroflix_search_params_and_download_urls() -> None:
    ...
    assert (
        RetroFlix._search_params(_meta(imdb_id="tt999"))["imdbId"] == "tt999"
    )
    assert (
        RetroFlix._search_params(_meta(imdb_id=0, title="A: B, C's"))["search"]
        == "A B Cs"
    )
    ...
    entry = {
        ...
        "name": "Release",
        "size": 10,
        "url": "https://retroflix.club/browse/t/5",
    }
...
@pytest.mark.asyncio
async def test_retroflix_search_existing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Client:
                ...
                [
                    {
                        ...
                        "name": "Release",
                        "size": 100,
                        "url": "https://retroflix.club/browse/t/3",
                    }
                ],
                method="GET",
                url=RetroFlix.search_url,
            )

    monkeypatch.setattr(
        retroflix_module.httpx,
        "AsyncClient",
...
@pytest.mark.asyncio
async def test_retroflix_api_test_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
        tracker, "generate_new_api", AsyncMock(return_value=True)
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: Client(
            _response(
                ...
                status=200,
                method="GET",
                url="https://retroflix.club/api/test",
            )
        ),
    )
    ...
    request_error = httpx.RequestError(
        "offline",
        request=httpx.Request("GET", "https://retroflix.club/api/test"),
    )
    monkeypatch.setattr(
        retroflix_module.httpx,
        "AsyncClient",
        lambda *_args, **_kwargs: Client(request_error),
    )
    assert await tracker.api_test(_meta()) is None
    ...
    monkeypatch.setattr(
        retroflix_module.httpx,
        "AsyncClient",
        lambda *_args, **_kwargs: Client(RuntimeError("unexpected")),
    )
    assert await tracker.api_test(_meta()) is None


@pytest.mark.asyncio
async def test_retroflix_generate_new_api_paths(
    ...
    monkeypatch.setattr(
        tracker,
        "_login_response",
        AsyncMock(return_value=_response({}, status=401)),
    )
    ...
    monkeypatch.setattr(
        tracker,
        "_login_response",
        AsyncMock(return_value=_response({}, status=201)),
    )
    assert await tracker.generate_new_api(_meta(tmp_path)) is None
    ...
    monkeypatch.setattr(
        tracker,
        "_login_response",
        AsyncMock(return_value=_response({"token": "new-token"}, status=201)),
    )
    ...
    error = httpx.RequestError(
        "offline",
        request=httpx.Request("POST", "https://retroflix.club/api/login"),
    )
    monkeypatch.setattr(
        tracker, "_login_response", AsyncMock(side_effect=error)
    )
    assert await tracker.generate_new_api(_meta(tmp_path)) is None
...
@pytest.mark.asyncio
async def test_retroflix_save_new_api_success_missing_pattern_and_ioerror(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tracker = _tracker()
    ...
    path.write_text(
        "TRACKERS = {'RETROFLIX': {'api_key': 'old'}}", encoding="utf-8"
    )
    ...
    monkeypatch.setattr(
        tracker, "_read_config", AsyncMock(side_effect=OSError("broken"))
    )
