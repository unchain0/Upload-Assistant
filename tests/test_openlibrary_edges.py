...
def test_author_name_cache_success_personal_empty_404_and_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert asyncio.run(manager.get_author_name("/authors/OL1A", client, cache)) == "Cached Author"

    monkeypatch.setattr(openlibrary, "is_cache_miss", lambda _value: True)
    cache.value = object()
    _Client.queue = [_Response(200, {"name": "Named"})]
    assert asyncio.run(manager.get_author_name("/authors/OL2A", client, cache)) == "Named"
    _Client.queue = [_Response(200, {"personal_name": "Personal"})]
    assert asyncio.run(manager.get_author_name("OL3A", client, cache)) == "Personal"
...
def test_work_empty_cache_success_full_metadata_and_author_resolution(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = OpenLibraryManager()
    ...
    assert asyncio.run(manager.search_by_work_id("OL1W")) == {"title": "Cached"}
    ...
    assert len(result["subjects"] if "subjects" in result else result["keywords"]) == 9
    assert result["genres"] == result["keywords"]
    assert result["openlibrary"] == "OL1W"


def test_work_string_description_optional_fields_not_found_status_and_error(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = OpenLibraryManager()
    cache = _Cache(object())
    monkeypatch.setattr(openlibrary, "cache_for", lambda _base: cache)
    monkeypatch.setattr(openlibrary, "is_cache_miss", lambda _value: True)

    _Client.queue = [_Response(200, {"title": "Work", "description": "<b>Text</b>", "covers": [-1], "authors": [], "subjects": []})]
    result = asyncio.run(manager.search_by_work_id("OL2W"))
    assert result and result["title"] == "Work" and result["overview"] == "Text"
    assert "artwork_url" not in result and "author" not in result and "keywords" not in result
...
def test_isbn_empty_cache_work_success_and_publisher_year_merge(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = OpenLibraryManager()
    ...
    cache.value = object()
    monkeypatch.setattr(openlibrary, "is_cache_miss", lambda _value: True)
    monkeypatch.setattr(manager, "search_by_work_id", lambda *_args, **_kwargs: _async_value({"title": "Work"}))
    _Client.queue = [
        _Response(
            200,
            {
...
def test_isbn_work_existing_publisher_fallback_details_and_not_found(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = OpenLibraryManager()
    cache = _Cache(object())
    monkeypatch.setattr(openlibrary, "cache_for", lambda _base: cache)
    monkeypatch.setattr(openlibrary, "is_cache_miss", lambda _value: True)
    monkeypatch.setattr(manager, "search_by_work_id", lambda *_args, **_kwargs: _async_value({"title": "Work", "publisher": "Existing", "year": "2020"}))
    _Client.queue = [
        _Response(
            200,
            {
    ...
    result = asyncio.run(manager.search_by_isbn("9780306406157"))
    assert result and result["publisher"] == "Existing" and result["year"] == "2020"

    monkeypatch.setattr(manager, "search_by_work_id", lambda *_args, **_kwargs: _async_value(None))
    details = {
        "title": "Fallback",
        "subtitle": "Subtitle",
    ...
    _Client.queue = [_Response(200, {"ISBN:9780306406157": {"details": details, "thumbnail_url": "https://covers.invalid/id-S.jpg"}})]
    result = asyncio.run(manager.search_by_isbn("9780306406157"))
    assert result and result["title"] == "Fallback: Subtitle"
    assert result["author"] == "Author, Second" and result["publisher"] == "Publisher"
    assert result["year"] == "2023" and result["artwork_url"].endswith("-L.jpg")

    _Client.queue = [_Response(200, {"ISBN:9780306406157": {"details": {"works": []}}})]
    assert asyncio.run(manager.search_by_isbn("9780306406157")) is None
    assert cache.set_calls[-1][1]["negative"] is True


def test_isbn_empty_payload_status_json_and_network_errors(monkeypatch: pytest.MonkeyPatch) -> None:
    manager = OpenLibraryManager()
    cache = _Cache(object())
    monkeypatch.setattr(openlibrary, "cache_for", lambda _base: cache)
    monkeypatch.setattr(openlibrary, "is_cache_miss", lambda _value: True)
...
def test_add_year_and_metadata_details_all_branches() -> None:
    ...
    minimal = manager._metadata_from_book_details({}, {"title": "Book"}, "isbn")
    ...
    assert full["title"] == "Book: Sub" and full["author"] == "A" and full["publisher"] == "P"
    assert full["artwork_url"].endswith("-L.jpg") and full["search_year"] == 2022
