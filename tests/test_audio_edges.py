...
def test_language_normalization_and_equivalence(
    ...
    monkeypatch.setattr(
        audio.langcodes.Language,
        "get",
        lambda _value: (_ for _ in ()).throw(ValueError("bad")),
    )
    monkeypatch.setattr(
        audio.langcodes, "find", lambda _value: SimpleNamespace(language="xx")
    )
...
def test_channel_count_helpers_cover_all_modes() -> None:
    ...
    assert (
        audio.determine_channel_count(
            "6 channels", "L R C LFE Ls Rs", "", "AAC"
        )
        == "5.1"
    )
    assert (
        audio.determine_channel_count(
            "8 / 6", "L R C LFE Ls Rs Tfl Tfr", "Atmos", "E-AC-3"
        )
        == "5.1.2"
    )
...
def test_audio_v2_stream_order_defaults_silent_and_codec_matrix() -> None:
    ...
    value, chan, commentary = asyncio.run(
        audio._get_audio_v2(config, _mi(*tracks), meta, None)
    )
    ...
    cases = [
        (
            _audio_track(
                Format="DTS", Format_AdditionalFeatures="XLL", Channels=6
            ),
            "DTS-HD MA",
        ),
        (
            _audio_track(
                Format="DTS", Format_AdditionalFeatures="XLL X", Channels=8
            ),
            "DTS:X",
        ),
        (
            _audio_track(
                Format="MPEG Audio", Format_Profile="Layer 2", Channels=2
            ),
            "MP2",
        ),
        (
            _audio_track(
                Format="MPEG Audio", Format_Profile="Layer 3", Channels=2
            ),
            "MP3",
        ),
        (_audio_track(Format="AC-3", Channels=8), "DD+"),
        (
            _audio_track(Format="FLAC", Channels=2, Title="Auro3D main"),
            "FLAC 2.0 Auro3D",
        ),
    ]
    for track, expected in cases:
        result, _chan, _commentary = asyncio.run(
            audio._get_audio_v2(config, _mi(track), _meta(), None)
        )
...
def test_audio_v2_ids_invalid_order_pcm_explicit_and_unknown_codec() -> None:
    ...
    result, chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(*tracks), meta, None)
    )
    assert (
        result.startswith("AAC")
        and chan == "2.0"
        and meta.non_disc_has_pcm_audio_tracks
    )
    ...
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(*ids), _meta(), None)
    )
    ...
    result, chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(*unknown_ids), _meta(), None)
    )
    assert result == "UnknownCodec Unknown" and chan == "Unknown"


def test_audio_v2_dual_dubbed_commentary_and_language_bloat(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = {"DEFAULT": {}, "TRACKERS": {}}
    meta = _meta(dual_audio=True)
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(_audio_track()), meta, None)
    )
    ...
    result, _chan, commentary = asyncio.run(
        audio._get_audio_v2(config, _mi(*tracks), meta, None)
    )
    ...
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(
            config, _mi(_audio_track(Language="English")), dubbed, None
        )
    )
    assert result.startswith("Dubbed")


def test_audio_v2_bdinfo_plain_and_atmos_media_file(
    ...
    result, chan, _ = asyncio.run(
        audio._get_audio_v2(
            config, {}, _meta(is_disc="BDMV", type="DISC"), bdinfo
        )
    )
    ...
    (directory / "MediaInfo.json").write_text(
        json.dumps(payload), encoding="utf-8"
    )
    common = SimpleNamespace(
        get_bdmv_mediainfo=AsyncMock(return_value={"media": {}})
    )
    monkeypatch.setattr(audio, "Common", lambda _config: common)
    meta = _meta(
        base_dir=str(tmp_path), uuid="atmos", is_disc="BDMV", type="DISC"
    )
    bdinfo = {
        "audio": [
            {
    ...
    result, chan, _ = asyncio.run(
        audio._get_audio_v2(config, {}, meta, bdinfo)
    )
...
def test_bloated_check_registry_rules_and_language_display(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    audio.bloated_check(
        meta, ["fr-FR", "de"], is_eng_original_with_non_eng=True
    )
...
def test_dts_core_duplicate_all_decisions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        audio.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: True
    )
    audio.dts_core_additional_check(attended)

    monkeypatch.setattr(
        audio.cli_ui, "ask_yes_no", lambda *_args, **_kwargs: False
    )
    with pytest.raises(LossyDtsDuplicateError):
        audio.dts_core_additional_check(attended)
...
def test_remaining_audio_v2_and_bloat_branches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = {"DEFAULT": {}, "TRACKERS": {}}
    ...
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(*invalid_ids), _meta(), None)
    )
    ...
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(
            config, _mi(_audio_track(Language="zxx")), silent, None
        )
    )
    ...
    result, chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(broken), _meta(), None)
    )
    ...
    with pytest.raises(LossyDtsDuplicateError):
        asyncio.run(
            audio._get_audio_v2(config, duplicate.mediainfo, duplicate, None)
        )
    ...
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(
            config, multilingual, _meta(original_language="French"), None
        )
    )
    assert result
    monkeypatch.setattr(audio, "bloated_check", original_bloated_check)

    additional_dict = _audio_track(
        Format="AAC", Format_AdditionalFeatures={"bad": True}
    )
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(additional_dict), _meta(), None)
    )
    assert result.startswith("AAC")

    commercial_atmos = _audio_track(
        Format="MLP FBA", Format_Commercial="Dolby TrueHD Atmos", Channels=8
    )
    result, _chan, _ = asyncio.run(
        audio._get_audio_v2(config, _mi(commercial_atmos), _meta(), None)
    )
...
def test_bloat_registry_import_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import builtins
