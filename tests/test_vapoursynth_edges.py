...
class Clip:
    def __init__(self, *, width: int = 1920, height: int = 1080, frames: int = 20000, primaries: int = 1) -> None:
        self.width = width
        self.height = height
    ...
    def get_frame(self, _index: int) -> Any:
        return SimpleNamespace(props={"_Primaries": self._primaries, "_PictType": "I"})
...
def test_custom_frame_info_calls_frame_eval_and_text(monkeypatch: pytest.MonkeyPatch) -> None:
    clip = Clip(frames=42)
    core = FakeCore(clip)
...
def test_optimize_images_disabled_missing_old_python_and_levels(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(platform, "python_version_tuple", lambda: ("3", "6", "0"))
    vapoursynth.optimize_images(image, {"optimize_images": True})

    calls: list[tuple[Path, int]] = []
    fake_oxipng = SimpleNamespace(optimize=lambda path, level: calls.append((Path(path), level)))
    monkeypatch.setitem(sys.modules, "oxipng", fake_oxipng)
    monkeypatch.setattr(platform, "python_version_tuple", lambda: ("3", "14", "0"))
    vapoursynth.optimize_images(image, {"optimize_images": True})
    ...
    fake_oxipng.optimize = lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("optimize failed"))
    vapoursynth.optimize_images(image, {"optimize_images": True})


def test_vs_screengn_uses_existing_frames_and_m2ts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(vapoursynth, "ScreenGen", lambda clip, directory, suffix: screen_calls.append((clip, directory, suffix)))
    optimized: list[Path] = []
    monkeypatch.setattr(vapoursynth, "optimize_images", lambda image, _config: optimized.append(Path(image)))

    vapoursynth.vs_screengn("source.m2ts", num=2, dir=str(tmp_path), config={"optimize_images": True})
...
def test_vs_screengn_generates_frames_cache_and_missing_encode(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    source = Clip(frames=20_000, primaries=1)
    core = FakeCore(source)
    monkeypatch.setattr(vapoursynth, "core", core)
    (tmp_path / "screens.txt").write_text("-1\n", encoding="utf-8")
    generated = iter((1500, 2500))
    monkeypatch.setattr(vapoursynth.random, "randint", lambda *_args: next(generated))
    ...
    vapoursynth.vs_screengn("source.mkv", encode=str(tmp_path / "missing-encode.mkv"), num=2, dir=str(tmp_path))

    assert core.ffms_calls[0][0] == "source.mkv"
    assert (tmp_path / "screens.txt").read_text(encoding="utf-8") == "1500\n2500\n"
...
def test_vs_screengn_encode_resize_tonemap_and_screen_generation(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(vapoursynth, "DynamicTonemap", lambda clip, **_kwargs: tonemap_calls.append(clip) or clip)
    info_calls: list[str] = []
    monkeypatch.setattr(vapoursynth, "custom_frame_info", lambda clip, text: info_calls.append(text) or clip)
    screens: list[str] = []
    monkeypatch.setattr(vapoursynth, "ScreenGen", lambda _clip, _directory, suffix: screens.append(suffix))
    monkeypatch.setattr(vapoursynth, "optimize_images", lambda *_args: None)

    vapoursynth.vs_screengn("source.mkv", encode=str(encode_path), num=1, dir=str(tmp_path))
...
def test_vs_screengn_portrait_resize_and_index_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    core.ffms2.Source = lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("index failed"))
    with pytest.raises(RuntimeError, match="index failed"):
        vapoursynth.vs_screengn("broken.mkv", num=1, dir=str(tmp_path / "other"))
