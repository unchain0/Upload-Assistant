...
def test_generic_music_validator_all_branches() -> None:
    assert _codes(MusicValidator().validate(MusicRelease("/music"))) == {
        "no_audio"
    }
    ...
    assert (
        next(
            issue.level
            for issue in issues
            if issue.code == "inconsistent_album"
        )
        == ValidationLevel.ERROR
    )

    release = _release(
        _track("disc1.flac", disc=1), _track("disc2.flac", disc=2)
    )
    ...
    assert "inconsistent_artist" not in _codes(
        MusicValidator().validate(various)
    )
...
def test_orpheus_hybrid_single_physical_lineage_and_media_warnings() -> None:
    ...
    hybrid_issue = next(
        issue for issue in issues if issue.code == "hybrid_technical"
    )
    ...
    assert (
        next(
            issue.level for issue in issues if issue.code == "hybrid_technical"
        )
        == ValidationLevel.ERROR
    )
    ...
    assert "single_track" not in _codes(
        OrpheusMusicValidator().validate(official)
    )
    ...
    assert "missing_lineage" in _codes(
        OrpheusMusicValidator().validate(lineage)
    )
