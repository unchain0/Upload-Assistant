...
def test_blutopia_derived_dolby_vision_confirmation_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    declined = _meta(type="REMUX", hdr="DV HDR", unattended=False)
    monkeypatch.setattr(
        "src.integrations.trackers.UNIT3D.blutopia.cli_ui.ask_yes_no",
...
def test_blutopia_raw_only_group_confirmation_paths(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tracker = _tracker()
    monkeypatch.setattr(
    ...
    assert asyncio.run(
        tracker.get_additional_checks(
            _meta(type="ENCODE", tag="AOC", unattended=False)
        )
    )
...
def test_blutopia_name_uses_imdb_title_aka_and_derived_layer_marker() -> None:
    ...
    assert asyncio.run(tracker.get_name(meta)) == {
        "name": "Canonical AKA Alternate 2021 1080p DVP5/DVP8 WEB-DL H264-GROUP"
    }
