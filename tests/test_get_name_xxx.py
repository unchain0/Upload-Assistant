...
def test_xxx_name_preserves_release_and_only_replaces_dots():
    ...
    meta = Meta(
        category="XXX", basename_no_ext=release_name, tag="-P0RNL0V3RSD"
    )
...
def test_xxx_name_appends_a_tag_not_already_in_the_release_name():
    meta = Meta(
        category="XXX",
        basename_no_ext="OnlyFans.2026.Clip.XXX.MP4",
        tag="-CUSTOM",
    )

    _name_notag, name, _clean_name, _potential_missing = asyncio.run(
        NameManager({}).get_name(meta)
    )
...
def test_manual_name_override_is_used_without_automatic_tagging():
    meta = Meta(
        category="XXX",
        basename_no_ext="ignored",
        tag="-AUTO",
        manual_name="Custom.Release.Name",
    )
