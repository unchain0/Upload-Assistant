...
def release(
    tmp_path: Path, tracks: int = 1, duration: float = 60
) -> MusicRelease:
    value = MusicRelease(root=str(tmp_path))
    value.tracks = [
        AudioTrack(
            path=f"track-{index}.flac",
...
def test_cache_helpers_none_invalid_and_write_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    assert music_sources._music_cache_path("", "p", "k", "x") is None
    path = music_sources._music_cache_path(
        str(tmp_path), "p", "k", "secret query"
    )
...
def test_musicbrainz_find_all_cache_and_http_paths(tmp_path: Path) -> None:
    ...
    assert (
        asyncio.run(enricher._find_release("Artist", "Album", 1, "WEB", ""))[
            "id"
        ]
        == "memory"
    )
    ...
    cache = music_sources._music_cache_path(
        str(tmp_path), "musicbrainz", "release_search", "\x1f".join(key)
    )
    ...
    result = asyncio.run(
        enricher._find_release("Artist", "Album", 1, "WEB", "12345678")
    )
    ...
    Client.reset(
        httpx.RequestError(
            "offline", request=httpx.Request("GET", "https://musicbrainz.org")
        )
    )
    assert asyncio.run(enricher._find_release("Artist", "Error", 0)) is None


def test_musicbrainz_enrich_and_helper_edges(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    enricher = MusicBrainzEnricher(base_dir=str(tmp_path))
    ...
    monkeypatch.setattr(
        enricher, "_find_release", lambda *_args, **_kwargs: async_value(None)
    )
    ...
    monkeypatch.setattr(
        enricher,
        "_find_release",
        lambda *_args, **_kwargs: async_value(result),
    )
    asyncio.run(enricher.enrich(target))
    ...
    assert MusicBrainzEnricher._artists(
        {
            "artist-credit": [
                "bad",
                {"artist": "bad", "name": "Fallback"},
                {"artist": {"name": "One"}},
                {"artist": {"name": "One"}},
            ]
        }
...
def test_discogs_parse_and_enrich_release_master_missing(
    ...
    assert (
        target.external_ids["discogs_release"] == "1"
        and target.external_ids["discogs_master"] == "2"
    )
    target = release(tmp_path)
    asyncio.run(enricher.enrich(target, master_id="2"))
    assert (
        target.external_ids["discogs_master"] == "2"
        and target.external_ids["discogs_release"] == "3"
    )
    monkeypatch.setattr(enricher, "_get", lambda *_args: async_value(None))
    asyncio.run(
        enricher.enrich(release(tmp_path), release_id="404", master_id="405")
    )
...
def test_discogs_find_cache_remote_and_errors(tmp_path: Path) -> None:
    ...
    cache = music_sources._music_cache_path(
        str(tmp_path), "discogs", "release_search", key[1]
    )
    assert cache is not None
    ...
    payload = {
        "results": [
            {"id": 3, "title": "Artist - Album"},
            {"id": 4, "title": "Different - Album"},
            "bad",
        ]
    }
...
def test_discogs_get_cache_remote_invalid_and_error(tmp_path: Path) -> None:
    ...
    path = music_sources._music_cache_path(
        str(tmp_path), "discogs", "releases", "2"
    )
    ...
    Client.reset(
        httpx.RequestError(
            "offline", request=httpx.Request("GET", "https://discogs.com")
        )
    )
...
def test_remaining_musicbrainz_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    enricher = MusicBrainzEnricher(base_dir=str(tmp_path))
    target = release(tmp_path)
    ...
    assert (
        MusicBrainzEnricher._select_release(
            [{"title": "Different"}], "Album", 0
        )
        is None
    )
    ...
    monkeypatch.setattr(
        ...
        "_lock",
        Lock(
            lambda: MusicBrainzEnricher._cache.__setitem__(
                key, {"id": "inside"}
            )
        ),
    )
    assert asyncio.run(
        enricher._find_release("Artist", "Album", 1, "WEB", "")
    ) == {"id": "inside"}
    ...
    monkeypatch.setattr(
        music_sources,
        "_read_music_cache",
        lambda _path: async_value(next(reads)),
    )
...
def test_remaining_discogs_branches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    enricher = DiscogsEnricher(
        token="".join(("tok", "en")), base_dir=str(tmp_path)
    ...
    monkeypatch.setattr(
        ...
        "_lock",
        Lock(
            lambda: DiscogsEnricher._cache.__setitem__(
                key, {"results": [{"id": 10}]}
            )
        ),
    )
    ...
    monkeypatch.setattr(
        DiscogsEnricher,
        "_lock",
        Lock(lambda: DiscogsEnricher._cache.__setitem__(get_key, {"id": 20})),
    )
    monkeypatch.setattr(
        music_sources, "_read_music_cache", lambda _path: async_value(None)
    )
    ...
    assert not DiscogsEnricher._is_exact_release(
        {"title": "No Separator"}, "Artist", "Album"
    )
    target = release(tmp_path)
    ...
    assert DiscogsEnricher._genres({"genres": "bad", "styles": ["Rock"]}) == [
        "Rock"
    ]
...
def test_final_music_source_guard_branches() -> None:
    assert (
        MusicBrainzEnricher._select_release({"bad": True}, "Album", 0) is None
    )
